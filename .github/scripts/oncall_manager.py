...
def update_slack_usergroup(new_oncall_username, old_members_usernames):
    ...
    if not new_slack_id:
        print(
            f"Could not find Slack user ID for {new_oncall_username} ({new_email}), skipping Slack update"  # noqa: E501
        )
        return
    ...
    try:
        ...
        print(
            f"Updated Slack usergroup '{SLACK_USERGROUP_HANDLE}' to contain only {new_oncall_username}"  # noqa: E501
        )
...
def update_active_oncall_team(org, new_oncall):
    ...
    if new_oncall not in current_members:
        ...
        if resp.status_code == 200:
            ...
            print(
                f"Failed to add {new_oncall} to {ACTIVE_ONCALL_TEAM_SLUG}: {resp.status_code} {resp.text}"  # noqa: E501
            )
    ...
    for member in current_members:
        if member not in [new_oncall, 'svcnvidia-nemo-ci']:
            ...
            if resp.status_code == 204:
                ...
                print(
                    f"Failed to remove {member} from {ACTIVE_ONCALL_TEAM_SLUG}: {resp.status_code} {resp.text}"  # noqa: E501
                )
