...
def trigger_workflow(
    ...
    created_after = (
        datetime.fromtimestamp(started_at - 120)
        .astimezone()
    ...
    dispatch_url = (
        f"{api_url}/repos/{owner}/{repo}/actions/workflows/{workflow_path}/dispatches"
    )
...
def validate_workflow_result(
    completed: bool, conclusion: str | None, run_id: int
) -> None:
    """Fail unless the downstream workflow completed successfully."""
    if not completed:
        raise TimeoutError(
            f"Timed out waiting for downstream workflow run {run_id} to complete"
        )
