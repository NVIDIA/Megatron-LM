...
def create_app_jwt(app_id: str, private_key: str, now_ms: int | None = None) -> str:
    ...
    private_key_fd = os.memfd_create("github-app-private-key", os.MFD_CLOEXEC)
    try:
        os.write(private_key_fd, private_key.encode())
        os.lseek(private_key_fd, 0, os.SEEK_SET)
        signature = subprocess.run(
            ["openssl", "dgst", "-sha256", "-sign", f"/proc/self/fd/{private_key_fd}"],
            input=signing_input,
            check=True,
            capture_output=True,
