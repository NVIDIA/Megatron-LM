...
A separate 2×RTX PRO 6000 Blackwell instance was used for a supplementary test while the 8×RTX 5090 host was unreachable. Its existing Python had PyTorch 2.13/cu130 but no Transformer Engine, so both arms used the Megatron local GPT spec and sequential experts in FP32. No environment packages were changed. The variants were one-round returns (q=8192 for 2048 tokens); v11 fused the per-peer gathers, v12 also used the native `scatter_add_`, and v13 replaced the peer-order index with direct concatenation. All three passed isolated EP2 forward/backward cases; their first training losses matched native exactly and final losses differed by at most 5e-7 on the 2048-token shape. Each timing below is the slower rank's median over 20 complete optimizer steps after five warmups. These tests do not validate the original TE/5090 target.

| Local-spec GPT, 2 layers, H256, 2048 tokens, EP2 | Native step | Candidate step | Native / candidate speedup | Native / candidate allocated peak |
|---|---:|---:|---:|---:|
| v11 pilot | 17.170 ms | 18.908 ms | 0.908× | 594.21 / 594.27 MiB |
| v12 pair 0, native then candidate | 17.170 ms | 17.139 ms | 1.002× | 594.21 / 594.27 MiB |
