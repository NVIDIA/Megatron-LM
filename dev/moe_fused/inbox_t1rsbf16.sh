...
# t1rsbf16 is the tag,  the gates under test on top of the standing set.
set -uo pipefail
...
env $GATES MCORE_NVLS_RS_BF16=1 \
    TAG=t1rsbf16 OSL=1024 BS=256 NITERS=5 NWARMUP=2 \
    bash dev/moe_fused/run_e2e_cfg.sh 2>&1 | tee /tmp/t1rsbf16_raw.log

echo "================= RESULT t1rsbf16 ================="
echo "extra gates: "
grep -iE 'throughput=|throughput_tok_per_sec|avg_latency|TPOT|COHERENCE|coherent' /tmp/t1rsbf16_raw.log | tail -25
