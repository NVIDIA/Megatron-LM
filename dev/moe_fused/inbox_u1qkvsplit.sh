...
# u1qkvsplit is the tag,  the gates under test on top of the standing set.
set -uo pipefail
...
env $GATES MCORE_NVLS_RS_BF16=1 MCORE_QKV_SPLIT_VIEWS=1 \
    TAG=u1qkvsplit OSL=1024 BS=256 NITERS=5 NWARMUP=2 \
    bash dev/moe_fused/run_e2e_cfg.sh 2>&1 | tee /tmp/u1qkvsplit_raw.log

echo "================= RESULT u1qkvsplit ================="
echo "extra gates: "
grep -iE 'throughput=|throughput_tok_per_sec|avg_latency|TPOT|COHERENCE|coherent' /tmp/u1qkvsplit_raw.log | tail -25
