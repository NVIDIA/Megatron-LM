...
# y1pdl is the tag,  the gates under test on top of the standing set.
set -uo pipefail
...
env $GATES MCORE_NVLS_RS_BF16=1 MCORE_FLASHINFER_DECODE=1 MCORE_FLASHINFER_PDL=1 \
    TAG=y1pdl OSL=1024 BS=256 NITERS=5 NWARMUP=2 \
    bash dev/moe_fused/run_e2e_cfg.sh 2>&1 | tee /tmp/y1pdl_raw.log

echo "================= RESULT y1pdl ================="
echo "extra gates: "
grep -iE 'throughput=|throughput_tok_per_sec|avg_latency|TPOT|COHERENCE|coherent' /tmp/y1pdl_raw.log | tail -25
