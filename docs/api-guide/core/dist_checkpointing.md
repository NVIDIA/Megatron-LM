...
The gathered model entry records GTP copies separately from true DP/CP replicas,
using the live parameter's replica group even if caller metadata includes GTP.
The optimizer companion retains the DP coordinate but gives distinct physical GTP
shards a writer each; treating those shards as replicas would leave gaps in the
checkpoint's logical tensor coverage.
