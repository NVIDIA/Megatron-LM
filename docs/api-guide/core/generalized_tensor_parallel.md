...
**Output-layer fwd→bwd weight reuse.** The output layer (LM head) is the *tail* of chain 1 (its AG is the last forward gather) and the *head* of chain 2 (its dgrad is the first backward op, with no `prev` to overlap → a **synchronous** gather). Both gathers produce identical bytes — the dgrad `.t()` is applied at GEMM time, not by gathering something else — so the backward gather is served from the forward buffer instead. `classify_gtp_chains` sets `_retain_for_bwd` on `output_layer.weight`, excluding native-FP8 (chain 1 is rowwise-, chain 2 columnwise-scaled — different data); tied embeddings are registered under an `embedding...` name and so stay opted out, correctly, since their backward is a scatter-add with no gather to reuse. `_all_gather_weight` short-circuits when `not fwd and _retain_for_bwd`, returning the pinned forward buffers, so callers cannot tell a reuse from a collective and `_ag_ticket_bwd` is never reserved — which also saves that ticket's otherwise-persistent `[vocab, hidden]` buffer under CG. `reserve()` sets `pin_for_fwd` from the same flag, and `get()` registers the bound buffer in `GTPWeightCache._pinned_bufs`; `release()` checks the **buffer** rather than the slot, so no ticket can return it to a pool. Two halves make this safe. Slots created *before* the pin (an untied embedding, also `[vocab, hidden]`) may still share the buffer — that is deliberate and costs no extra allocation — and they are safe because tickets are minted in forward order, so every co-owner gathers earlier in forward and later in backward, never inside the retention window. Slots created *after* the pin are the ones that would gather inside the window, and they can never share it, because the pin keeps the buffer out of the pool. Tied-embedding models are not supported here: the shared weight is registered under an `embedding...` name and gathers twice per forward (lookup and output projection), so the retention would have to key off the second gather. Full-iteration CUDA graph is supported: the whole step is one capture session, so the recorded fwd AG rewrites the pinned fixed-address buffer on every replay, ordered before the dgrad read via the output layer's own forward GEMM on the main stream. Eval/no-grad is harmless — the buffer is simply overwritten in place by the next forward.
