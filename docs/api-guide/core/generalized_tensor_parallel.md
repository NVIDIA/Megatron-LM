...
- **Idempotent gather cache**: `GTPWeightCache.get(ticket)` keeps `slot.buf` set even after `release()`, so captured all-gathers replay against the same address. `clear()` drops buffers while keeping tickets valid, supporting CG re-capture with lazy re-allocation. Ungraphed RS outputs use the same cache; graphed RS outputs use the persistent backward arenas below.
- **Stable graph-owned backward storage**: padded wgrad inputs, asynchronous RS outputs, and FP32-RS scratch are discovered eagerly and allocated before capture in bounded fixed-address generations outside the CG memory pool. Symmetric RS inputs use a separate arena in the memory pool registered for their exact GTP or EGTP process group; other storage uses the default allocator. Capture only takes stable views; it does not allocate a per-weight ring or cache ticket.
...
- **Scratch lifetime.** Local CUDA graphs obtain the all-to-all and FP32-sum scratch from their
  persistent arena. Eager execution lets the work handle retain helper-allocated scratch. Either
  way, the storage remains live until the handle is waited — it is the *input* to the deferred
  FP32 sum.
...
- **CUDA-graph ownership.** Eager and ungraphed sends use the registered LIFO pool. Graphed sends
  are subviews of the persistent-buffer generation allocated through the exact group's
  `gtp_symm_pool_ctx`; they are plan-owned and never enter the LIFO release list. Dense GTP and
  expert GTP therefore cannot alias memory registered for the other process group.
...
Four storage paths cover different lifetimes and allocator domains:

- **`GTPWeightCache`** (AG and ungraphed RS outputs) — ticket-based, keyed on `(shape, dtype, fwd, expert_idx, reduce_scatter)`, plus a `("recompute", parity)` suffix for recompute-chain gathers. Same-shape buffers across layers are shared, **except between chain neighbours** — one-step-ahead keeps the predecessor and the current weight live at once, so `_ensure_no_shared_buffer_with` folds a parity bit into the key when the two would collide, at the cost of one extra buffer for the second of the pair. The caller names which chain to guard, because the chains disagree on who a weight's neighbour is: on the fwd chain the check is normally inert (neighbours are different roles, hence different shapes) and fires only when CG capture leaves two same-shaped weights adjacent — embedding + output_layer alone in the `UNGRAPHED` chain — whereas on a recompute chain same-shape adjacency is the norm. Tickets persist; buffers allocate lazily on first `get()`.
- **`GraphPersistentBufferPlan`** (graphed backward temporaries) — eager discovery records each runner's aligned capacity per allocation domain and dtype. Capture bump-allocates fixed views for padded wgrads, RS outputs, and FP32-RS scratch from bounded alternating arenas. Symmetric wgrad arenas are allocated from their exact group's registered pool; default and different-group requests cannot alias. A generation event protects every reused address through the full graph and side-stream tail.
- **`symmetric_wgrad_pool`** (eager registered RS inputs) — LIFO-recycles padded send buffers within one process-group domain. The explicit RS release list returns only these eager buffers; persistent graph views remain owned by `GraphPersistentBufferPlan`.
...
`bwd_completion_event` is recorded after Stage 2. The next graph therefore starts after the current graph's AG, RS, and `main_grad` finalization have completed. Because the RS input lifetime cannot extend into the next graph, no persistent cross-graph workspace is required.
...
*With cross-graph overlap.* The main stream may launch the next backward graph while the current graph's RS and `main_grad` finalization continue. Fixed-address persistent workspaces and replay-time events protect every graph-owned tensor whose lifetime crosses that boundary.

```text
time ------------------------------------------------------------------------------------>

runner i       wait ready[G0] -> wgrad_i writes G0 -> Stage 1 -> completion_i
RS stream                                      +------ RS_i(G0) ------> add_i
main stream                                                     +-> launch runner i-1
runner i-1                                                         wait ready[G1]
                                                                   wgrad_i-1 writes G1
RS stream                                                          +--- RS_i-1(G1) ---> add_i-1
main stream                                                                        +-> runner i-2
runner i-2                                                                            wait ready[G0]

             G0 and G1 are inferred and allocated before capture, outside the graph pool.
```
...
| Persistent cross-graph workspace | Not required | Inferred alternating generations |
| Additional persistent memory | None | Up to three maximum per-runner workspace footprints |
...
2. **Two-stage completion protocol.** Stage 1 calls `wait_async_comms(..., skip_rs=True)` and joins graph-owned AG streams before recording `bwd_completion_event`. Stage 2 uses the existing successor-cascade and capture-drain logic to finalize reduced wgrads and join the RS streams.
3. **Eager capacity discovery.** A graph-owned allocation calls `get_graph_persistent_buffer(shape, dtype, symmetric_group=...)` in both eager discovery and capture. Only registered RS send buffers provide a group; other requests use the default domain. Discovery records the aligned capacity required by each runner; capture bump-allocates stable views from that capacity. The calls cover padded RS inputs, asynchronous RS outputs, FP32-accumulation scratch, and repeated MTP uses without allocation names or an explicit prepare step.
4. **Bounded generation allocation.** `GraphPersistentBufferPlan` packs each buffer-owning runner's discovered capacity into up to three arenas per allocation-domain and dtype on the current GPU, outside the CUDA graph pool. Runners with no persistent allocation do not consume a generation. Non-adjacent runners reuse the maximum arena footprint assigned to their generation.
5. **Replay fencing.** Before replay, a runner waits for its arena generation. The graph tail publishes the generation's reuse event after graph-owned work has finished. `bwd_completion_event` still releases the next graph after Stage 1.
...
The persistent-buffer mechanism does not change the AG/RS launch order, two-stage drain, or gradient-finalization stream. It only replaces the former ring/cache ownership for graph-lived tensors and fences an address before that address is reused.

*Result and cost.* The persistent layout owns the padded RS input. The wgrad GEMM writes the logical prefix, the alignment tail remains zero (§3.7 covers why that permanent zero has to be excluded from `num_zeros`), and a producer that did not write directly into that view is copied before reduce-scatter. GTP workspace memory is bounded by up to three copies of the maximum per-runner footprint rather than one persistent buffer per layer. Capture validates the capacity consumed in every allocator domain. Per-generation events make reuse safe without constraining replay order.
...
- **A predictable invariant other systems can build on.** "Padding is always an exact structural zero, never written" is safe for any consumer that needs to distinguish real elements from padding — DCP's cross-topology reshard tolerance (§3.3) and the persistent fixed-address buffers (§3.6) both lean on it, and it's what lets `count_zeros_fp32` exclude these permanent zeros from `num_zeros` instead of miscounting them as converged-to-zero gradients.
...
- **A bookkeeping tax on every raw-buffer consumer.** Checkpointing, `num_zeros`, and the persistent wgrad buffers all have to explicitly account for padding rather than assume a shard's buffer is fully real.
...
|-----------|----------------|
| `test_gtp_basics.py` | Core GTP_remat shard/gather, cache ownership, persistent wgrad workspaces, DDP bucket alignment, and the `num_zeros` padding correction (§3.7). Also pins TE's recompute-phase flag as dtype-agnostic (true under BF16, not just FP8), which the forward-only readiness gate in §3.2 relies on. |
...
| `test_gtp_partial_cg.py` | Four-layer partial-CG loss and eager-vs-replay grad-norm parity with two-generation workspace reuse across independently replayed graphs (§3.6). |
