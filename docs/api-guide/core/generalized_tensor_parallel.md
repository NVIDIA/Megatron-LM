...
**Output-layer fwd→bwd weight reuse.** The output layer (LM head) is the *tail* of chain 1 (its AG is the last forward gather) and the *head* of chain 2 (its dgrad is the first backward op, with no `prev` to overlap → a **synchronous** gather). When the output layer is **BF16** (not native-FP8), chains 1 and 2 need the *identical* full weight (the dgrad `.t()` is applied at GEMM time, not a different gather), so the forward-gathered weight is **retained** (`_retained_fwd_weight`, set in `all_gather_and_prefetch` when `_reuse_fwd_weight_in_bwd` and not `_gtp_native_fp8`) and **reused** by `all_gather_and_prefetch_bwd`, dropping the redundant synchronous bwd all-gather. The opt-in flag is set for `output_layer.weight` in `classify_gtp_chains`. It auto-disables under native-FP8 output proj (chain 1 is rowwise-, chain 2 columnwise-scaled — different data). Full-iteration CUDA graph is supported: the whole step is one capture session, so the recorded fwd AG rewrites the pinned fixed-address buffer on every replay, ordered before the dgrad read via the output layer's own forward GEMM on the main stream (the Python retain/consume handshake runs only at capture time); skipping the bwd gather also means `_ag_ticket_bwd` is never reserved, saving its otherwise-persistent `[vocab, hidden]` buffer under CG. The retained buffer is **pinned** — `reserve()` sets `slot.pin` on that ticket so `release()` never returns it to the pool, guaranteeing no other same-shape gather (e.g. an untied embedding, also `[vocab, hidden]`) can pop and overwrite it before backward reads it. Eval/no-grad is harmless: the retain is only ever read in `all_gather_and_prefetch_bwd` (training), and is overwritten in place by the next forward.
