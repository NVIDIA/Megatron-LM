...
- **Split-QKV is layout-invariant.** Muon orthogonalizes a fused `linear_qkv` weight as separate **q**, **k** and **v** blocks, and *whether* it does must not depend on the parallel layout. Splitting and not splitting are **two different update rules**, not small perturbations of one: q is typically far larger than k and v, so orthogonalizing the fused matrix lets q dominate the spectrum the k and v rows are scaled by. Newton–Schulz is a nonlinear iteration, so the gap compounds over steps rather than averaging out — the same weight would train differently at TP1 than under GTP_remat.
  - **Decide from the owning layer's global QKV layout and the rows across TP/GTP shards**, never from this rank's shard. A shard-local divisibility test can disable splitting as soon as TP or GTP cuts a query group.
  - **Split after reconstruction**, never before — a row shard can cut q/k/v or individual head boundaries. Per-head, GTP-rematerialized, and fragmented TP layouts gather the required rows, exclude GTP padding, apply non-TP Newton–Schulz to the logical splits, and then restore the original shards.
  - **`auto` uses duplicated execution for reconstructed QKV layouts.** The general per-weight cost model cannot select a row-sharded mode without losing the logical split, so these paths explicitly use duplicated Newton–Schulz.
  - **Explicit distributed Newton–Schulz falls back with one warning** for those reconstructed layouts. Projection-level QKV splitting keeps the configured TP-aware path only when every TP rank already owns complete local query groups and GTP_remat is inactive.
...
| `test_gtp_muon_qkv.py` | Layout-invariant split-QKV (§1.6): the decision uses rows across shards, splitting runs after GTP/TP reconstruction, padding is excluded, and partial process-group collections resolve the same GTP group during tagging and optimization. |
