...
| Mamba/SSM Triton ops | `megatron/core/ssm/ops/common/determinism.py` | one fixed autotune config (the per-tile workspace reduced with an ordered `sum` lives in `mamba_ssm`'s copy of these helpers; Megatron's copy had no callers and was removed) | timing-based autotune |
