...
We used a Claude Code agent team to explore heterogeneous parallelism for colocated multimodal training in Megatron — a configuration space too large to sweep manually. The agents found optimization chains that deliver +12.7% to +14.0% TFLOPs/GPU at 8K and up to +41.8% at 64K with context parallelism, over each configuration's strongest homogeneous baseline. The rest of the article covers how we structured the team, the experiment loop and fairness rules, what the agents explored, where they failed, and which patterns are reusable.
