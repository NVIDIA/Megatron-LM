...
| **Packed rows (THD)** | ✅ **via `--sft` only** | `--sft --max-seqlen-per-dp-cp-rank 4096 --qsa-use-sparse-attention --eval-iters 1` | re-measured on this branch (8 iterations, 69.0 GiB peak allocated). `--pad-packed-seq-alignment`, which the `dev` recipe passed, does not exist on this base. The **sequence-packing schedulers** selected by `--use-varlen-dataset` are rejected by the memory ("use the `--sft` packed path instead"; it also needs `--calculate-per-token-loss` to reach that message). The SFT path still builds a validation loader, so `--eval-iters 0` fails with "no sample to consume". With `--mock-data` the rows are padded with EOD unless `--null-tokenizer-pad-id` names a real pad token |
