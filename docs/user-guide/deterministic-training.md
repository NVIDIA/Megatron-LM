...
The startup API explicitly sets `TRITON_CACHE_AUTOTUNING=0` when unset, preserving the pinned-config fallback even if an optional dependency such as vLLM sets a different import-time default. A launcher-supplied `1` is preserved and still requires `TRITON_CACHE_DIR`. The legacy environment-only helper leaves this variable unset.
