...
The bit-exact correctness tests are named `test_*_determinism.py` and live under `tests/unit_tests/core/models/gpt/`, `tests/unit_tests/core/models/hybrid/`, `tests/unit_tests/core/ssm/`, and `tests/unit_tests/core/transformer/`. The suite parametrizes over model presets (GPT-like, Llama-like, Hybrid/Mamba) × parallelism cells (TP, PP, VPP, EP, FSDP, and composites) and asserts that two runs of the same configuration produce bit-identical outputs and gradients. FP8 / FP4 recipes (`tensorwise`, `delayed`, `mxfp8`, `nvfp4`) are covered by `tests/unit_tests/core/models/gpt/test_fp8_determinism.py`; the Blackwell-only recipes are capability-skipped on Hopper.
