...
Supported offloading modules are `"attn_norm"`, `"qkv_linear"`, `"core_attn"`, `"attn_proj"`, `"mlp_norm"`, `"expert_fc1"`, `"moe_act"`, and `"layer_input"`. Module-level choices can be combined with fine-grained recomputation to free almost all activations for a transformer layer on the device. `"layer_input"` is a separate mode: it is only valid with uniform full recompute, cannot be combined with other offload modules, and offloads one checkpoint input per segment; `--recompute-num-layers N` controls the per-N-layer segment size.
...
# Choices: "attn_norm", "qkv_linear", "core_attn", "attn_proj", "mlp_norm",
#          "expert_fc1", "moe_act", "layer_input".
--offload-modules expert_fc1

# Offload one full-recompute checkpoint input per two-layer segment.
# layer_input is mutually exclusive with other offload modules.
--recompute-granularity full --recompute-method uniform --recompute-num-layers 2 \
  --offload-modules layer_input
```
