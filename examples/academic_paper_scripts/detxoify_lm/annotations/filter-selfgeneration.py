...
def main():
    ...
    print(
        f"Avg toxicity (nontoxic): {np.mean(scores[scores <= 0.5])} +- {np.std(scores[scores <= 0.5])}"  # noqa: E501
    )
    ...
    print(
        f"Avg toxicity (filtered): {np.mean(scores[nontoxic_indices])} +- {np.std(scores[nontoxic_indices])}"  # noqa: E501
    )
    print(
        f"Toxicity Range (filtered): {np.min(scores[nontoxic_indices])} ~ {np.max(scores[nontoxic_indices])}"  # noqa: E501
    )
