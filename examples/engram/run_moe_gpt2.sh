...
if [[ -n "${FULL_EVAL_SPLIT:-}" ]]; then
    : "${LOAD_PATH:?Set LOAD_PATH to the checkpoint for full evaluation}"
    EXPERIMENT_NAME="${EXPERIMENT_NAME}-full-${FULL_EVAL_SPLIT}"
fi
...
PRETRAIN_MODULE="${PRETRAIN_MODULE:-pretrain_hybrid}"
...
MODEL_ARGS=(
    ...
    --mtp-loss-scaling-factor-decay 0.15
    --mtp-loss-scaling-factor-decay-start "${MTP_DECAY_START:-$((TRAIN_ITERS - LR_WSD_DECAY_ITERS))}"
)
