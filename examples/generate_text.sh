...
launch_cmd="mpirun --tag-output --allow-run-as-root -np $procs -npernode 8 -hostfile /job/hostfile  -mca pml ob1 --mca btl ^openib -mca btl_tcp_if_exclude lo,docker0 -mca  coll_hcoll_enable 0  -x PATH -x LD_LIBRARY_PATH=/opt/hpcx/nccl_rdma_sharp_plugin/lib/:/mellanox/sharp/lib:/opt/msft/nccl-rdma-sharp-plugins-2.8.3/lib:/home/amawa/saemal/msccl/build/lib:$LD_LIBRARY_PATH -x UCX_IB_ENABLE_CUDA_AFFINITY=n -x NCCL_IB_PCI_RELAXED_ORDERING=1 -x UCX_IB_PCI_RELAXED_ORDERING=on -x UCX_NET_DEVICES=mlx5_0:1 -x UCX_TLS=rc -x NCCL_SOCKET_IFNAME=eth0 -x NCCL_DEBUG=WARN -x NCCL_NET_GDR_LEVEL=5 -x NCCL_TOPO_FILE=/opt/msft/topo.xml -x SNCCL_DEBUG_SUBSYS=INIT,ENV  -x SCCL_XML_FILES=/home/amawa/a2a-32.xml -x NCCL_NET_SHARED_BUFFERS=0 -x NCCL_ALGO=SCCL,RING,TREE  -x CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python"
