...
async def main(
    ...
    # 4. look at InferenceCoordinator.start() to see how we can route requests from
    #    users <-> data parallel group based on headers.
    ...
    if dist.get_rank() == 0:
        ...
        while True:
            ...
            if num_requests_added == num_requests_total:
                break
            # Relinquish control since there are no more requests to add at the moment.
            # This allows the engine to run.
        ...
        if args.output_path:
            ...
            for req in results:
                result_dict = {
                    ...
                    "latency": req.latency,  # InferenceClient sets this in the returned future.
                }
    ...
    # once the stop signal eventually makes its way to each GPU,
