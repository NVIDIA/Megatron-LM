...
from megatron.bridge.models.nemotron_omni.nemotron_omni_bridge import NemotronOmniLlavaBridge
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cuda-graph-scope", choices=("layer", "block", "none"), default="none")
...
def load_llava_model(args: argparse.Namespace) -> LLaVAModel:
    ...
    provider.initialize_model_parallel(seed=1234, seed_kwargs={"inference_rng_tracker": True})
    provider.finalize()
...
def build_prompt_tokens(tokenizer, user_prompt: str) -> list[int]:
    ...
    prompt = tokenizer.apply_chat_template(
        [{"role": "system", "content": "/no_think"}, {"role": "user", "content": content}],
        tokenize=False,
    ...
    payload = json.dumps(
        {
            ...
            "multi_modal_data": {args.media: base64.b64encode(media_bytes).decode("ascii")},
        }
    ).encode("utf-8")
...
def main() -> None:
    ...
    media_bytes = make_mock_mp4(args.video_num_frames) if args.media == "video" else make_mock_png()
    ...
    if args.api == "async":
        ...
        asyncio.run(run_completions(args, model, tokenizer, config, prompt_tokens, media_bytes))
    else:
