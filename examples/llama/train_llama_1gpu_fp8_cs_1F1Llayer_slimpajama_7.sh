...
# FP8 specific arguments - ENABLED FOR FP8 TRAINING
FP8_ARGS=(
    --fp8-format hybrid                    # Use hybrid fp8 format (recommended)
    --fp8-recipe tensorwise                   # Use current scaling recipe
    --fp8-amax-history-len 1024           # Amax history length for better scaling
    --fp8-amax-compute-algo max           # Use max algorithm for amax computation
)

# Optional FP8 arguments (uncomment as needed):
FP8_OPTIONAL_ARGS=(
    --first-last-layers-bf16             # Keep first/last layers in bf16
    --num-layers-at-start-in-bf16 1      # Number of start layers in bf16
    --num-layers-at-end-in-bf16 1        # Number of end layers in bf16
#     --fp8-param-gather                    # Use fp8 for parameter all-gather (requires distributed optimizer)
#     --no-fp8-wgrad                       # Keep weight gradients in higher precision
)
...
EVAL_AND_LOGGING_ARGS=(
    ...
    --wandb-exp-name "llama3x-small-slimpajama-fp8-cs-1f1f-7"
