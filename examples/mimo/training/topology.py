...
class ModuleGridSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.num_ranks % dense != 0:
            raise ValueError(
                "num_ranks "
                f"({self.num_ranks}) must be divisible by tp*gtp_remat*cp*pp ({dense})"
            )
        ...
        if self.num_ranks % expert != 0:
            raise ValueError(
                "num_ranks "
                f"({self.num_ranks}) must be divisible by "
...
def pg_collection_from_grid(
    ...
    pgc.expt_dp_gtp_remat = grid.get_pg(
        ["expt_gtp_remat", "expt_dp"], view=_EXPERT_VIEW
    )
