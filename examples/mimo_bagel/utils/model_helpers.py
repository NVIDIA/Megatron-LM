...
def get_pg_collection(model: Union[MegatronModule, list[MegatronModule]]) -> ProcessGroupCollection:
    ...
    ``reference_repo/Megatron-Bridge/src/megatron/bridge/training/utils/pg_utils.py``:
    prefers ``pg_collection`` attached to the wrapped model, falling back to the
    default MPU-based process groups when the attribute is absent.
    """
