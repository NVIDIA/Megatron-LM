...
def greedy_knapsack(item_sizes: List[int], samples: List, max_capacity: int) -> List:
    ...
    if sorted_item_sizes[-1] > max_capacity:
        raise ValueError(
            f"knapsack: A sample is larger {sorted_item_sizes[-1]} than the max_sequence_length {max_capacity}."  # noqa: E501
        )
...
class TaskEncoder(DefaultTaskEncoder[OCRSample, OCRSample, ImageTaskBatchPacked, dict]):
    ...
    def __init__(self):
        ...
        if self.dataloader_seq_length and self.packing_seq_length:
            assert self.dataloader_seq_length >= self.packing_seq_length, (
                "dataloader sequence length must be greater than or equal to the packing sequence length"  # noqa: E501
            )
    ...
    def _get_total_seq_length(self, input_ids, num_tiles):
        ...
        total_len = (
            len(input_ids) + total_num_tiles * self.num_image_embeddings_per_tile - total_num_images
        )
        return total_len
    ...
    def encode_sample(
        self, sample: Union[CaptioningSample, OCRSample, VQASample, SimilarityInterleavedSample]
    ):
        ...
        for idx, sample in enumerate(samples.samples):
            try:
                ...
                if current_length + encoded_sample.total_len > self.packing_seq_length:
                    print(
                        f"Encoding list of samples: stopped at {idx} samples to stick to {self.packing_seq_length}. Last sample key: {sample.__key__}"  # noqa: E501
                    )
                    break
                else:
    ...
    def encode_llava_sft(
        ...
        if (
            ...
            assert target_aspect_ratio is not None, (
                "Sample of type OfflineTargetAspectRatioSample needs to define the target aspect ratio."  # noqa: E501
            )
            ...
            while True:
                    ...
                    if isinstance(img, torch.Tensor) or isinstance(img, np.ndarray):
                        if len(img.shape) == 4:
                            assert img.shape[0] == 1, (
                                f"When len(img.shape) == 4, we expect the first dimension to be 1, but got img.shape: {img.shape} instead."  # noqa: E501
                            )
                            img = img[0]
                            use_tiling = False
                        to_pil = ToPILImage()
                ...
                if sum(num_tiles) * self.num_image_embeddings_per_tile > max_image_token_allowed:
                    if max_num_tiles in self.num_tiles_degradation_map:
                        ...
                        raise RuntimeError(
                            (
                                f"Tried to decrease the number of tiles {max_num_tiles} but it's not ",  # noqa: E501
        ...
        error_msg = f"Found {number_image_token} image tokens for len({num_tiles}) = {len(num_tiles)} image tiles in {conversation}."  # noqa: E501
        assert number_image_token == len(num_tiles), error_msg
        error_msg = f"Found sum({num_tiles}) = {np.sum(num_tiles)} tiles for {len(imgs)} images in {conversation}."  # noqa: E501
        assert np.sum(num_tiles) == len(imgs), error_msg

        # We need to ensure that there are at least some trainable tokens in the sample.
        assert self.target_has_trainable_tokens(input_ids, num_tiles, target), (
    ...
    def target_has_trainable_tokens(self, input_ids, num_tiles, target):
        ...
        expanded_target = self.replace_value_with_repetition(
            expanded_target,
            self.img_token_id,
            self.num_image_embeddings_per_tile * np.array(num_tiles),
            IGNORE_INDEX,
        )
    ...
    def encode_ocr_ref_prompt(self, sample: OCRSample) -> ImageTaskSample:
        ...
        if len(region) == 4:
            ...
            region = f"<quad>({region[0]},{region[1]}),({region[2]},{region[3]}),({region[4]},{region[5]}),({region[6]},{region[7]})</quad>"  # noqa: E501
    ...
    def bbox_coord_to_label(self, text, bbox):
        ...
        if len(bbox) == 4:
            ...
            label_str = f"<ref>{text}</ref><quad>({bbox[0]},{bbox[1]}),({bbox[2]},{bbox[3]}),({bbox[4]},{bbox[5]}),({bbox[6]},{bbox[7]})</quad>"  # noqa: E501
    ...
    def batch(
        ...
        # If the user hasn't defined a target dataloader sequence length, then use the max along the sample lengths.  # noqa: E501
    ...
    def pack_selected_samples(self, samples: List[ImageTaskSample]) -> List[ImageTaskSamplePacked]:
        ...
        for _, sample in enumerate(samples):
            ...
            # This should not happen. The select_samples_to_pack method should have already ensured that the samples fit.  # noqa: E501
            if current_length + sample_len > packing_seq_len:
                raise ValueError(
                    f"Packed sample exceeds the maximum sequence length of {packing_seq_len}: {samples}"  # noqa: E501
                )
