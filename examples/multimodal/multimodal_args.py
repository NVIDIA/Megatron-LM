...
def add_multimodal_extra_args(parser):
    ...
    group.add_argument("--allow-missing-sound-model-checkpoint", action="store_true", default=False)
    ...
    group.add_argument(
        ...
        choices=[
            ...
            "nemotron-h-5p5-reasoning-inference",
            "nemotron6-moe",
        ],
        required=True,
    ...
    group.add_argument(
        ...
        type=int,
        default=None,
        help="Length of class token. If not set, uses model-specific defaults (radio: 8, radio-g: 5, cradio-g: 8). FP8 overrides to 16.",
    )
    group.add_argument(
        "--packing-buffer-size",
        type=int,
    ...
    group.add_argument(
        ...
        type=int,
        default=0,
        help="Packing sequence length. Must be > 0 if using packing.",
    )
    group.add_argument(
        "--packing-knapsack-algorithm",
        type=str,
        default="greedy_knapsack",
        help="Knapsack algorithm to use for packing.",
    )
    group.add_argument(
        "--recompute-vision",
        action="store_true",
    ...
    group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable activation checkpointing in the sound model",
    )
    group.add_argument(
        "--use-loss-scaling",
        action="store_true",
        default=False,
        help="Scale loss based on conversation turn length (in tokens).",
    )
    group.add_argument(
        "--force-system-message",
        action="store_true",
        default=False,
        help="Force a specific system message",
    )
    ...
    group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use the MCore inference API",
    )
    group.add_argument(
        "--use-vision-backbone-fp8-arch",
        action="store_true",
        default=False,
        help="Use the FP8 arch in the vision backbone. This is used to load the FP8 checkpoint when running inference.",
    )
    group.add_argument(
        "--dynamic-resolution",
        action="store_true",
        default=False,
        help="Use input image dynamic resolution",
    )
    group.add_argument(
        "--dynamic-resolution-min-patches",
        type=int,
        default=0,
        help="Minimum number of patches per image for dynamic resolution",
    )
    group.add_argument(
        "--dynamic-resolution-max-patches",
        type=int,
        default=0,
        help="Maximum number of patches per image for dynamic resolution",
    )
    group.add_argument(
        "--dynamic-resolution-min-side",
        type=int,
        default=None,
        help="Minimum side length for dynamic resolution",
    )
    group.add_argument(
        "--match-tiling-dynamic-resolution",
        action="store_true",
        default=False,
        help="Use match-tiling dynamic resolution strategy that combines tiling logic with dynamic resolution processing",
    )
    group.add_argument(
        "--masked-tiling-dynamic-resolution",
        action="store_true",
        default=False,
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help="Token to use for image break tokens, must be added to --special-tokens as well",
    )
    group.add_argument(
        "--conv-merging",
        action="store_true",
        default=False,
        help="Use convolution merging which uses a convolution to merge tokens after the vision encoder",
    )
    group.add_argument("--allow-missing-conv-merge-checkpoint", action="store_true", default=False)
    group.add_argument(
        "--video-min-num-frames",
        type=int,
        default=8,
        help="Minimum number of frames to sample from the video as input to the model.",
    )
    group.add_argument(
        "--video-max-num-frames",
        type=int,
        default=32,
        help="Maximum number of frames to sample from the video as input to the model.",
    )
    group.add_argument(
        "--video-default-fps",
        type=int,
        default=2,
        help="Default frames per second to sample from the video as input to the model.",
    )
    group.add_argument(
        "--video-frame-temporal-jitter",
        action="store_true",
        default=False,
        help="Enable temporal jittering of the frames to sample from the video as input to the model.",
    )
    group.add_argument(
        "--video-target-img-size",
        type=int,
        default=None,
    ...
    group.add_argument(
        ...
        action="store_true",
        default=True,
        help="Optimize the broadcast of data.",
    )
    group.add_argument(
        "--recompute-vision-num-layers",
        type=int,
        default=0,
        help="Number of layers to recompute in the vision model.",
    )
    group.add_argument(
        "--recompute-granularity-vision",
        type=str,
        default=None,
        help="Granularity to recompute in the vision model.",
        choices=["full", "selective"],
    )
    group.add_argument(
        "--recompute-method-vision",
        type=str,
        default=None,
        choices=['uniform', 'block'],
        help="Method to recompute in the vision model.",
    )
    group.add_argument(
        "--recompute-vision-projection",
        action="store_true",
        default=False,
        help="Enable activation checkpointing in the vision projection layer.",
    )
    group.add_argument(
        "--recompute-sound-projection",
        action="store_true",
        default=False,
        help="Enable activation checkpointing in the sound projection layer.",
    )
    group.add_argument(
        "--allow-large-videos",
        action="store_true",
        default=False,
        help="Allow large videos to be loaded into the model.",
    )
    group.add_argument(
        "--efficient-video-sampling-variant",
        type=str,
        default=None,
        help="The EVS variant. Read docstring on EVSHelper",
    )
    group.add_argument(
        "--sound-target-rate",
        type=int,
    ...
    group.add_argument(
        "--sound-embedding-size", type=int, default=750, help="Size of the sound embedding."
    )
    group.add_argument(
        "--sound-clip-duration", type=int, default=30, help="Sound model clip duration in seconds."
    )
    group.add_argument(
        "--sound-min-duration",
        type=float,
    ...
    group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use the new dataloader path.",
    )
    group.add_argument(
        "--decoder-tp-comm-overlap",
        action="store_true",
        default=False,
        help="Enable tensor parallel communication overlap in the decoder.",
    )
    group.add_argument(
        "--freeze-vision-projection",
        action="store_true",
        default=False,
        help="Freeze the vision projection module.",
    )
    group.add_argument(
        "--freeze-sound-projection",
        action="store_true",
        default=False,
        help="Freeze the sound projection module.",
    )
    group.add_argument(
        "--relax-sender-check",
        action="store_true",
        default=False,
        help="Relax the sender check in the dataloader to allow other role than user and assistant.",
    )
    group.add_argument(
        "--relax-thinking-trace-check",
        action="store_true",
        default=False,
        help="Relax the checks in the dataloader which ensure the thinking trace is well formatted.",
    )
    group.add_argument(
        "--allow-cross-sample-attention",
        action="store_true",
        default=False,
        help="Allow cross sample attention when using sample packing.",
    )
    group.add_argument(
        "--only-keep-samples-with-img",
        action="store_true",
        default=False,
        help="Discard samples that do not have an image.",
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Log the gradient norms of the model components.",
    )
    group.add_argument(
        "--log-model-act-norms",
        action="store_true",
        default=False,
        help="Log the activation norms of the model components.",
    )
    group.add_argument(
        "--dynamic-resolution-no-truncate",
        action="store_true",
        default=False,
        help="Disable trunctation during dynamic resolution, and instead throw away the entire sample",
    )
    group.add_argument(
        "--video-aug-scale-frames-up",
        type=int,
        default=None,
    ...
    group.add_argument(
        "--video-aug-scale-resolution-only",
        action="store_true",
        default=False,
