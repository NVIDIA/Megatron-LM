...
from megatron.training import get_args, get_model, get_tokenizer, is_last_rank
...
def get_output_path(config, dp_rank):
    ...
    if ckpt_step is not None:
        return f"{config.output_path}-{config.task}-dprank={dp_rank}-partition={config.partition_id}-step={args.ckpt_step}.jsonl"  # noqa: E501
    else:
        return f"{config.output_path}-{config.task}-dprank={dp_rank}-partition={config.partition_id}.jsonl"  # noqa: E501
...
class VLMForwardStep(ForwardStep):
    ...
    def __call__(self, tokens, position_ids, attention_mask):
        ...
        if num_image_tokens > 0:
            # When there are image tokens and this stage only receives vision embeddings, adjust the recv buffer seq length to match the image embeddings sequence length.  # noqa: E501
            # If there are image tokens and this stage receives full embeddings, make sure we compensate for expansion of image tokens.  # noqa: E501
            # Note that this will set a recv_buffer_seq_length for the encoder stage, this length is irrelevant since that recv buffer is never allocated.  # noqa: E501
            ...
            # If this stage only receives vision embeddings and there are no image tokens we won't run the encoder and therefore shouldn't try to recv.  # noqa: E501
            recv_buffer_seq_length = 0

        # If the pipeline stage only has a vision encoder, then it only needs to run when there are image tokens  # noqa: E501
...
def get_conversation(task, question, metadata=None):
    ...
    if task == "captioning":
        conversation = [
            ...
            {
                "role": "user",
                "content": f"{IMAGE_TOKEN}\nGive a brief description of this image in one sentence.",  # noqa: E501
            },
        ]
        ...
        conversation = [
            ...
            {
                "role": "user",
                "content": f"{IMAGE_TOKEN}\n{question}\nAnswer the question using a single word, phrase, or number.",  # noqa: E501
            },
        ]
    elif task == "VQAv2":
        conversation = [
            ...
            {
                "role": "user",
                "content": f"{IMAGE_TOKEN}\n{question}\nAnswer the question using a single word or phrase.",  # noqa: E501
            },
        ]
    elif task == "ChartQA":
        conversation = [
