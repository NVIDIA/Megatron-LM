...
def test_model_provider_propagates_rope_fusion_to_vision(monkeypatch):
    ...
    def build_model(*, args, language_config, vision_config, **kwargs):
        captured["language_config"] = language_config
        captured["vision_config"] = vision_config
        return object()

    registry = {"vision_config_fn": lambda **kwargs: vision_config, "model_factory_fn": build_model}
    monkeypatch.setattr(training_entrypoint, "get_args", lambda: args)
    monkeypatch.setattr(
        training_entrypoint, "core_transformer_config_from_args", lambda args: language_config
    )
...
def test_vision_raw_mrope_freqs_match_legacy_materialized_rope():
    ...
    converted = mrope_freqs_to_rotary_emb(
        raw_freqs, [0, 16, 16], interleaved_mrope=False, rotary_interleaved=False
    )
...
def test_vision_fp32_wrapper_dispatches_raw_freqs_to_fused_mrope_thd(monkeypatch):
    ...
    monkeypatch.setattr(
        rope_utils, "get_fused_mrope_thd_unavailable_reason", lambda *args, **kwargs: None
    )
    ...
    out = _apply_rope_fp32_no_cp(t, freqs, config, cu_seqlens=cu_seqlens, max_seqlen=3)
...
def test_vision_fused_rope_matches_unfused_forward_backward_cuda():
    ...
    freqs = torch.randn(
        3, 1, total_tokens, half_rotary_dim, device="cuda", dtype=torch.float32, generator=generator
    )
    ...
    ref = _apply_rope_fp32_no_cp(
        t_ref, freqs, ref_config, cu_seqlens=cu_seqlens, max_seqlen=total_tokens
    )
    out = _apply_rope_fp32_no_cp(
        t_fused, freqs, fused_config, cu_seqlens=cu_seqlens, max_seqlen=total_tokens
    )
