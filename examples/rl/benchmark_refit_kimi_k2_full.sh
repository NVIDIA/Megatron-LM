...
#
#SBATCH --job-name=benchmark-refit-kimi-k2-full
#SBATCH --nodes=16
...
# FULL KIMI K2 PRODUCTION BENCHMARK
...
#
# Using 16 nodes (128 GPUs total)
# - Non-collocated: 64 source GPUs + 64 destination GPUs
# - Source TP=8, EP=8 (64 GPUs) -> Dest TP=4, EP=16 (64 GPUs)
#
# Strategy:
...
CONFIGS=(
    ...
    "8:8:4:16:non-collocated:nvshmem:TP8_EP8_to_TP4_EP16_64plus64_fullkimi_nvshmem_bf16"

    # Optional baseline: GLOO - For comparison (slower but reliable)
    # Uncomment to compare NVSHMEM vs GLOO performance
    # "8:8:4:16:non-collocated:gloo:TP8_EP8_to_TP4_EP16_64plus64_fullkimi_gloo_bf16"
)
...
NVSHMEM_PASSED=$(grep -q "TP8_EP8_to_TP4_EP16_64plus64_fullkimi_nvshmem_bf16" "$SUMMARY_FILE" && ! grep -A 3 "TP8_EP8_to_TP4_EP16_64plus64_fullkimi_nvshmem_bf16" "$SUMMARY_FILE" | grep -q "FAILED" && echo "yes" || echo "no")
GLOO_PASSED=$(grep -q "TP8_EP8_to_TP4_EP16_64plus64_fullkimi_gloo_bf16" "$SUMMARY_FILE" && ! grep -A 3 "TP8_EP8_to_TP4_EP16_64plus64_fullkimi_gloo_bf16" "$SUMMARY_FILE" | grep -q "FAILED" && echo "yes" || echo "no")
