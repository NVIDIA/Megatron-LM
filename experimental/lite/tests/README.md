...
With no arguments, the script discovers and runs the complete MLite-owned test set for the
visible hardware. It also accepts one or more test directories or test files:

~~~bash
experimental/lite/tests/run_tests.sh experimental/lite/tests/unit/runtime
experimental/lite/tests/run_tests.sh \
  experimental/lite/tests/unit/model/test_qwen_config_unit.py \
  experimental/lite/tests/smoke/primitive/test_parallel_topologies_smoke.py
~~~

Relative paths are interpreted from the directory where the script is invoked.
Targets must stay under unit/ or smoke/. The script accepts paths, not arbitrary
pytest options.
...
The entry recursively discovers normal pytest files below:

- experimental/lite/tests/unit/
- experimental/lite/tests/smoke/

unit/ contains focused tests organized by model, primitive, runtime, and
examples. smoke/ contains broader model and primitive validation. Execution
markers, rather than directory names, determine GPU count, environment, timeout,
and minimum architecture.

Adding a normal test does not require editing run_tests.sh or _test_harness/.
Stable roots, pytest arguments, and hardware profiles are defined once in
_test_harness/runner.py; tests and suites are never enumerated there. Every test
under unit/examples/ is marked optional: pytest still collects it, the
no-argument workflow filters it out, and an explicitly selected path includes it.
...
- pytest.mark.timeout(seconds=N) sets a positive per-suite timeout. Without it,
  the timeout is 600 seconds (10 minutes).
  Add an override only when documented runtime evidence shows that 600 seconds
  is insufficient.
- pytest.mark.optional excludes a test from the no-argument workflow. Explicit
  path selection includes optional tests.
  Optional suites may report skips for unavailable downstream dependencies; skips
  in all mandatory suites fail the workflow.
...
The harness sets NCCL_NVLS_ENABLE=0, matching Megatron unit-test CI, to avoid
node-dependent NVLS multicast allocations in tests that do not exercise NVLS.
