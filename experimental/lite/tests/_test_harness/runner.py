...
TEST_ROOTS = ("experimental/lite/tests/unit", "experimental/lite/tests/smoke")
PYTEST_ARGS = (
    "-o",
    "addopts=",
    "-q",
...
HARDWARE_PROFILES = {"standard": (8, "hopper"), "blackwell": (4, "blackwell")}
...
def resolve_targets(arguments: list[str], caller_cwd: Path) -> list[str]:
    ...
    for argument in arguments:
        ...
        if not any(resolved == root or root in resolved.parents for root in roots):
            raise SelectionError("test target is outside the MLite unit/smoke tree")
        if resolved.is_file():
...
def _configure_writable_caches(environment: dict[str, str], cache_root: Path) -> None:
    cache_paths = {
...
def collect_tests(
    targets: list[str], temporary_root: Path, environ: dict[str, str] | None = None
) -> list[TestCase]:
    plan_path = temporary_root / "collection-plan.json"
    environment = _sanitized_environment(environ)
    _configure_writable_caches(environment, temporary_root / "cache")
...
def select_cases(
    cases: list[TestCase], selection: HardwareSelection, explicit_targets: bool
) -> list[TestCase]:
...
def _suite_name(key: tuple[object, ...], cases: list[TestCase]) -> str:
    if (
        cases[0].gpus == 0
        and cases[0].timeout_seconds == DEFAULT_TIMEOUT_SECONDS
        and not cases[0].environment
        and not cases[0].optional
    ):
        return "cpu"
...
def build_suite_environment(
    ...
    if suite.allow_skips:
        base["MLITE_TEST_ALLOW_SKIPS"] = "1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="mlite-local-validation-") as temporary:
        ...
        if not suites:
            ...
            return _stop(selection.profile, "NOT_RUN", EXIT_EMPTY_PROFILE, reason)
        ...
        for suite in suites:
            ...
            if result.status != "PASS":
                return _stop(selection.profile, "FAIL", EXIT_FAILURE, results=results)
