...
def _capsule(rank: int, world_size: int):
    ...
    rows = torch.arange(_TOKENS, device="cuda").unsqueeze(1)
    slots = torch.arange(_TOPK, device="cuda").unsqueeze(0)
    indices = (
        rank * _LOCAL_EXPERTS + rows + slots * 3
    ).remainder(world_size * _LOCAL_EXPERTS)
...
def main() -> None:
    ...
    hidden, weights, indices = _capsule(rank, world_size)
    ...
    with set_current_vllm_config(VllmConfig()):
        moe = _build_moe(args.mode, group, rank, world_size)
    ...
    golden_file = (
        args.golden_dir
        / f"clean-normal-deepep-fp8-ep{world_size}-rank{rank}.pt"
    )
    if args.mode == "deepep":
