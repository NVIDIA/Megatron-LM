...
def _positions(values, device="cpu"):
    return torch.tensor([values], dtype=torch.long, device=device)


def test_rope_cache_returns_different_tables_for_different_positions() -> None:
    """Distinct positions must not share tables, whatever address they land on.

    The cache keyed on ``data_ptr`` plus ``_version``. The caching allocator
    reuses an address as soon as the previous tensor dies, and the replacement's
    ``_version`` starts at zero, so a second tensor of the same shape could hit
    tables built for the first one's positions. The entry now holds the position
    tensor; a hit is now honoured only when the entry's weak reference still
    resolves to the tensor being asked about.
    """
    ...
    cos_a, _ = build_compressed_rope_cos_sin(_positions([0, 1, 2, 3]), 4, 10000.0, **kw)
    cos_a = cos_a.clone()

    # Drop the first tensor and allocate an identically shaped one: the address
    # is a candidate for reuse exactly here.
    for _ in range(4):
        cos_b, _ = build_compressed_rope_cos_sin(_positions([10, 11, 12, 13]), 4, 10000.0, **kw)
    assert not torch.allclose(cos_a, cos_b), "different positions returned the same table"


def test_rope_cache_still_hits_for_the_same_positions() -> None:
    """Guard the guard: the fix must not turn the cache into a no-op.

    A key that never matches would also pass the test above, and would silently
    give back the per-layer rebuild the cache exists to remove.
    """
    from megatron.lite.primitive.modules.attention.csa import (
        _ROPE_CACHE,
        build_compressed_rope_cos_sin,
    ...
    first, _ = build_compressed_rope_cos_sin(positions, 4, 10000.0, **kw)
    second, _ = build_compressed_rope_cos_sin(positions, 4, 10000.0, **kw)
    assert first is second, "the same positions should reuse the cached table"
    assert len(_ROPE_CACHE) == 1
