...
**Owner:** shanmugamr
**Started:** 2026-05-11
**Last updated:** 2026-05-14 (Round 3 complete — added 2 hybrid Mamba tests (S9, S10) + 3 new product issues found. **Total this branch: 26 new dynamic-inference tests** + 1 drift fix + 12 issues found.)
...
| 12 | `--inference-dynamic-batching-prefix-caching: true` + bf16 Mamba states → CUDA illegal memory access | likely in `MambaSlotAllocator` or `_allocate_mamba_cache` path | High | When prefix caching is enabled with bf16 conv/ssm state dtypes on the hybrid 2B model, training crashes with `torch.AcceleratorError: CUDA error: an illegal memory access was encountered`. Isolated by retrying with fp32 dtypes — fp32 works, bf16 fails. The S10 test (bf16 dtypes WITHOUT prefix caching) works fine, and S9 (prefix caching WITH fp32) works fine. The combination is the bug. Filed as a separate engine-side defect. S9's test config is committed with fp32 dtypes to be reproducible; once #12 is fixed, can switch S9 to bf16 to exercise both axes. |
