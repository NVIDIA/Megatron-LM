...
class BlendedMegatronDatasetBuilder(object):
            ...
            for i in range(len(Split)):
                if split[i] is not None:
                    ...
                    blended_datasets[i] = self.build_generic_dataset(
                        ...
                        ),  # synchronize_ranks, default behavior to build on rank-0 first. Set to False if we are using --dataloader-fast-cache-load  # noqa: E501
                        megatron_datasets[i],
                        weights_i,
                        size_i,
                        self.config,
    ...
    def _build_megatron_datasets_parallel(
        ...
        # NOTE(asolergi-nv): Skip rank-0 first dataset building if we are using --dataloader-fast-cache-load  # noqa: E501
    ...
    def _build_megatron_dataset_splits(
        ...
        )  # NOTE(asolergi-nv): Set synchronize_ranks to False if we are using --dataloader-fast-cache-load  # noqa: E501
        ...
        for i, _split in enumerate(Split):
            if split[i] is None:
                ...
                if not (
                    isinstance(self.config, GPTDatasetConfig) and self.config.fast_cache_load
                ):  # NOTE(asolergi-nv): Skip indexed_indices building if we are using --dataloader-fast-cache-load  # noqa: E501
