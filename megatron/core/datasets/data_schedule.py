...
class DpBalancedScheduler(BasePackingScheduler):
    ...
    def run(
        ...
        new_data_iterator = create_data_iterator(new_samples, tp_group, config, vpp_needs_data)
...
def _get_scheduler_max_real_num_seqs(config) -> Optional[int]:
    """Return the real-sequence cap after reserving an optional dummy-tail slot."""
    ...
    if getattr(config, 'pad_packed_seq_alignment', None) is not None and getattr(
        ...
        if max_num_seqs < 2:
            raise ValueError(
                "thd_max_packed_sequences must be >= 2 when THD padding appends "
                "a dummy sequence."
            )
...
def get_batch_on_this_rank_for_sequence_packing(
    ...
    Args:
        ...
        pg_collection: The process group collection.
        config: Model config used for CP partitioning and optional THD
            packed-sequence padding. When None, CP partitioning defaults to
            zigzag and no padding is applied.
    Returns:
    ...
    if (
        ...
        if target_len is not None:
            ...
            # Fix the local width before slicing so later padding cannot shift
            # the origin assigned to each CP rank.
            assert alignment is not None
            total_rows = int(batch['cu_seqlens_padded'][-1].item())
            local_rows = (total_rows + cp_group.size() - 1) // cp_group.size()
            contiguous_cp_local_target_len = (
                (local_rows + alignment - 1) // alignment
            ) * alignment
    ...
    # Keep original boundaries for loss paths that must identify padding rows.
    # Attention kernels and THD partitioning consume the padded boundaries.
