...
class HybridCPDataLoaderWrapper:
    ...
    def get_global_seqlens(self, subsample_seqlens: torch.Tensor) -> List[int]:
        """Gather the sequence lengths of all subsamples from all DP ranks.
        ...
        # Find the max number of subsamples across all ranks and pad subsample_seqlens to max
        # length
    ...
    def _gid_to_src_rank(self, gid: int, offsets: List[int]) -> int:
        """Map a global ID to its source rank."""
    ...
    def reroute_samples_to_hdp_ranks(
        ...
        """Reroute the sub-samples to the correct rank after scheduling.
    ...
    def unpack_batch(self, batch):
        """Unpack the packed samples into a list of sub-samples.
