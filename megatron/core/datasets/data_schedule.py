...
def get_batch_on_this_rank_for_sequence_packing(
    ...
    if (
        ...
        and cp_group.size() > 1
        and getattr(config, 'pad_packed_seq_alignment', None) == 0
    ...
    if is_tp_rank_0 and (is_first_or_last_stage or mtp_on_this_rank):
        ...
        if config is not None and config.experimental_attention_variant == "dsv4_hybrid":
    ...
    if is_first_or_last_stage or mtp_on_this_rank:
        if is_tp_rank_0:
            if static_cp_target_len is not None:
                ...
                total_tokens = (batch['cu_seqlens_padded'][-1].to(torch.int32) // cp_world).reshape(
                    1
                )
        else:
