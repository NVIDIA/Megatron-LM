...
def prepare_state_dict_for_save(
    ...
    sharded_part, common_state_dict = save_preprocess(sharded_state_dict, validate_access_integrity, process_group=process_group)
    sharded_tensors = []
    sharded_objects = []
...
def recreate_state_dict_after_load(
    ...
    for k, v in loaded_tensors.items():
        loaded_tensors[k] = v.to(device=get_current_device())  # H2D
    if fully_parallel:
        distribution = (
