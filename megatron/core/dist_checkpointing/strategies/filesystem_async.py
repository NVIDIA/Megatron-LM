    ...
    **Note:** Step (3) can also be executed synchronously.
    ...
    def __init__(
        ...
        *args,
        ...
        source_checkpoint_dir: Optional[Union[str, os.PathLike]] = None,
        **kwargs,
    ):
        self.checkpoint_dir = path
        self.use_msc = use_msc
        self.source_checkpoint_dir = source_checkpoint_dir
    ...
    def finish(self, metadata: Metadata, results: List[List[WriteResult]]) -> None:
        ...
        """
        # Build storage_md from results once (used by both MSC and split metadata paths)
        storage_md = dict()
        ...
        if self.use_msc:
            ...
            if self.source_checkpoint_dir:
                ...
                if not os.path.exists(self.source_checkpoint_dir) or not os.path.exists(source_metadata_path):
                    ...
                    # Define the two independent I/O operations that release GIL
                    def copy_metadata():
                        copy_start = time()
                        shutil.copy2(source_metadata_path, dest_metadata_path)
                        copy_end = time()
                        logger.debug(f"Metadata copy time: {copy_end - copy_start:.6f}s")
                        return copy_end - copy_start
