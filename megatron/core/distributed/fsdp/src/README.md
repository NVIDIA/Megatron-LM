...
- `hfsdp_param_gather_overlap` pipelines HFSDP parameter gathers across its two communication domains. DP-Inner gathers are prefetched one FSDP unit ahead while DP-Outer gathers are prefetched two units ahead. The option is disabled by default and only applies when `outer_dp_sharding_strategy='optim'`.
