...
- `fsdp_zero_sm_allgather` enables NCCL zero-CTA all-gather for Megatron-FSDP parameter gathers. It requires `nccl_ub`, dedicated all-gather process groups, manual registration, and persistent double buffers. HSDP uses AG-only communicators matching its inner and outer parameter-gather groups. Parameter gather buffers are allocated from a dedicated symmetric NCCL memory pool; gradients, optimizer state, and main parameters remain in a separate non-symmetric pool because their rank-local layouts may differ.
    - **Only effective when using with Megatron-Core.**
    - Defaults to `False`.
...
- `megatron_fsdp_max_pool_double_buffer` uses a max-pooling algorithm to build persistent buffers that support hybrid or asymmetrical model architectures such as Nemotron. The pool contains two buffer groups by default. Set `megatron_fsdp_max_pool_buffer_count` above two only for schedules that intentionally keep more FSDP units live concurrently; each additional group increases persistent parameter and gradient buffer memory.
