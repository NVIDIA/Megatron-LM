    ...
    def reduce_partial_gradients(
        ...
        if self.main_grad.placements != self._accumulation_placements:
            ...
            if isinstance(self.main_grad.placements[reset_axis], Replicate):
                ...
                # to hold the accumulation, so re-allocate. Zero the fresh buffer only when
                # we accumulate into it (set_to_none=False); with set_to_none=True the
                # reduction below overwrites it (reduced into via out=, or copied), so
                # leaving it uninitialized is safe.
                self.main_grad = DBuffer(
                    mesh=self.mesh,
