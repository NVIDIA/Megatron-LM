...
class MegatronFSDP(torch.nn.Module):
    ...
    def __init__(
        ...
        if ddp_config is None:
            self.ddp_config = DistributedDataParallelConfig(
                # Defaults that may differ from Megatron-LM DDPConfig defaults.
    ...
    def _register_fsdp_hooks(self, root_module):
        ...
        def _register_post_backward_hook(
            ...
            once the current backward stage is done. None of the Tensors or Gradients
            in inp_tensors are modified, only ctx is modified.
            """
