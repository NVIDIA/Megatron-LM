...
class ParamAndGradBuffer:
    ...
    def __init__(
        ...
        if self.ddp_config.nccl_ub:
            ...
            if self.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True) is not None:
                ...
                self.ubr_groups.append(self.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True))
    ...
    def get_mem_alloc_context(self, groups=None, symmetric=True):
        ...
        if self.ddp_config.nccl_ub:
            ...
            if groups is None:
                ...
                if self.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True) is not None:
                    # All-gather group when overlapping
                    groups.append(self.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True))
    ...
    def all_gather_parameters(self, async_op: bool = True):
        ...
        if self.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True) is not None:
            # All-gather group when overlapping
            group = self.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True)
        else:
...
class AllGatherPipeline:
    ...
    def all_gather_params(
        ...
        for _, buckets in bucket_group_to_buckets.items():
            ...
            if self.buffer.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True) is not None:
                # All-gather group when overlapping
                dp_group = self.buffer.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True)
    ...
    def async_bucket_gather(self, bucket_id: int) -> None:
        ...
        if self.buffer.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True) is not None:
            # All-gather group when overlapping
            group = self.buffer.dist_index.get_fsdp_group(is_expert_parallel=False, all_gather=True)
        else:
