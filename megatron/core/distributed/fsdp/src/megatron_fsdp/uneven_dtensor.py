...
def _all_gather_int64(
    ...
    local_values = torch.tensor(
        values, dtype=torch.int64, device=_get_collective_device(group, device_type)
    )
    gathered_values = [torch.empty_like(local_values) for _ in range(dist.get_world_size(group))]
...
def _dtensor_contract(dtensor: DTensor, context: str | None = None) -> str:
    ...
    placements = tuple(_placement_contract(placement) for placement in dtensor.placements)
    return (
...
def _compute_chunk_metadata(
    ...
    def _update_offsets_and_cumulative_shape(
        ...
        global_shapes = _all_gather_int64(
            cumulative_shape, shard_group, device_type=device_mesh.device_type
        )
...
def gather_and_compute_chunk_metadata(
    ...
    chunk_metadata, expected_shape, actual_shape, gathered_shapes = _compute_chunk_metadata(dtensor)
    _raise_for_invalid_shard_composition(dtensor, expected_shape, actual_shape, gathered_shapes)
...
def update_uneven_dtensor_chunk_metadata(dtensor: DTensor, *, context: str | None = None) -> dict:
    """
...
def _validate_state_dict_contract(dtensors: list[tuple[list, str, DTensor]]):
    ...
    contract_entries = tuple(
        f"key_chain={key_path};{_dtensor_contract(dtensor)}" for _, key_path, dtensor in dtensors
    )
...
def _get_state_dict_dtensors(state_dict: dict) -> list[tuple[list, str, DTensor]]:
    ...
    return [
        (key_chain, repr(tuple(key_chain)), get_unflattened_state_dict(state_dict, key_chain))
...
def preprocess_state_dict_for_uneven_dtensor(state_dict: dict) -> dict:
    ...
    for _, key_path, dtensor in dtensors:
        chunk_metadata, expected_shape, actual_shape, gathered_shapes = _compute_chunk_metadata(
            dtensor
        )
