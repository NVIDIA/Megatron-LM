...
def get_sharding_strategy(ddp_config, is_expert_param: bool = False) -> str:
    ...
    Expert parameters follow `data_parallel_sharding_strategy_experts` when it is set,
    otherwise every parameter follows `data_parallel_sharding_strategy`.
    """
    experts_strategy = getattr(ddp_config, "data_parallel_sharding_strategy_experts", None)
