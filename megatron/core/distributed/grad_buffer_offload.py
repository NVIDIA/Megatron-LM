...
def _offload_single_buffer(
    buffer: _ParamAndGradBuffer, bucket_groups: List[_ParamAndGradBucketGroup], buffer_id: int
    ...
    for bucket in buffer.buckets:
        bucket_metadata.append(BucketMetadata(offset=bucket.offset, numel=bucket.grad_data.numel()))
    ...
    for param in buffer.params:
        if hasattr(param, 'main_grad') and param.main_grad is not None:
            ...
            param_grad_metadata[param] = ParamGradMetadata(
                start_index=start_index, end_index=end_index, shape=param.main_grad.shape
            )
...
def _onload_single_buffer(
    buffer: _ParamAndGradBuffer, offload_state: GradBufferOffloadState, zero_grad_data: bool = True
) -> None:
    ...
    buffer.grad_data = torch.empty(
        offload_state.numel, dtype=offload_state.dtype, device=offload_state.device
    )
    ...
    for param, metadata in offload_state.param_grad_metadata.items():
        param.main_grad = buffer.grad_data[metadata.start_index : metadata.end_index].view(
            metadata.shape
        )
