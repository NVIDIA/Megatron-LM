...
def group_params_for_buffers(
    params: List[torch.nn.Parameter], grad_reduce_in_fp32: bool, context_parallel_size: int = 1
    ...
    - excludes_cp_from_bucket: dense (non-expert) GTP-managed params whose weight-materialization
      group spans CP (the gtp_remat group folds CP in, see get_gtp_weight_remat_group). That
    ...
    Args:
        ...
        context_parallel_size: CP degree; gates excludes_cp_from_bucket. CP is folded into the
            gtp_remat group, so a GTP param's own reduce-scatter already summed it.
