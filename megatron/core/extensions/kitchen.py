...
class KitchenSpecProvider(BackendSpecProvider):
    ...
    def core_attention(self) -> type:
        ...
        if not self.use_kitchen_attention:
            if is_rank_0():
                logger.warning("KitchenSpecProvider: Using fallback (likely TE) as core attention.")
            return self.fallback.core_attention()

        if self.kitchen_attention_backend == "sdpa":
            if is_rank_0():
                logger.warning("KitchenSpecProvider: Using Kitchen SDPA as core attention.")
            return KitchenDotProductAttention
        elif self.kitchen_attention_backend == "fa":
            if is_rank_0():
                logger.warning("KitchenSpecProvider: Using Kitchen FA as core attention.")
