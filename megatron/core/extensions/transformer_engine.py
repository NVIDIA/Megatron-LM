...
if HAVE_TE and is_te_min_version("1.13.0"):
    ...
    class TEActivationOp:
        ...
        def __new__(cls, config: TransformerConfig):
            ...
            if layer_type is None:
                raise Exception(
                    'Only SwiGLU, SiTU-GLU, GEGLU, ReGLU, GELU, ReLU, SiLU are supported by '
...
if HAVE_TE and is_te_min_version("1.13.0"):
    ...
    def _make_te_ops_activation(
        ...
        if activation_func is situlu:
            assert config is not None, "SiTU-GLU requires the config for its beta parameters"
            kwargs.update(beta1=config.situ_glu_beta1, beta2=config.situ_glu_beta2)
...
class TEDotProductAttention(te.pytorch.DotProductAttention):
    ...
    def forward(
        ...
        if packed_seq_params is not None:
            ...
            # `local_cp_size` is authoritative: dynamic CP may hand us a
            # size-1 group for a CP-off sub-sample, which must still take the
            # CP-disabled path rather than TE's CP attention kernels.
            if packed_seq_params.local_cp_size is not None:
                if packed_seq_params.local_cp_size == 1:
                    ...
                    # Dynamic CP can enable CP at runtime on a model built with
                    # context_parallel_size == 1, where the constructor never
