    ...
    def get_fp8_recipe(config: TransformerConfig):
        ...
        if is_te_min_version("2.1.0"):
            if config.fp8_recipe == Fp8Recipe.delayed:
                ...
                fp8_recipe = block_scaling_recipe_cls(fp8_format=fp8_format)
                bwd_qparams = block_scaling_recipe_cls.fp8_quant_bwd_grad
                fp8_recipe.fp8_quant_bwd_grad = transformer_engine.common.recipe.QParams(
                    power_2_scale=bwd_qparams.power_2_scale,
                    amax_epsilon=1e-12,
                    random_hadamard_transform=bwd_qparams.random_hadamard_transform,
                    stochastic_rounding=bwd_qparams.stochastic_rounding,
                    fp4_2d_quantization=bwd_qparams.fp4_2d_quantization,
                )
