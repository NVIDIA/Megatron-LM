...
def _mla_rope_bwd_kernel(
    ...
    """
    Backward pass: inverse RoPE on the trailing emb_dim elements.
    ...
    if COPY_NOPE:
        nope_off = tl.arange(0, BLOCK_NOPE)[None, :]
        nope_mask = mask & (nope_off < nope_dim)
        tl.store(
            DO_OUT + head_off + nope_off,
            tl.load(DO_IN + head_off + nope_off, mask=nope_mask),
            mask=nope_mask,
        )
