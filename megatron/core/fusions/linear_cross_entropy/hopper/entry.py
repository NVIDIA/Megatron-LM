...
def _get_fwd_config() -> FwdConfig:
    """Lazy initialize forward configuration."""
    return FwdConfig()

@lru_cache(maxsize=1)
def _get_bwd_config() -> BwdConfig:
    """Lazy initialize backward configuration."""
    return BwdConfig()
...
def forward(
    ...
    if num_splits > 1:
        ...
        _max_packed = from_dlpack(_max, assumed_align=8)
        _accu_packed = from_dlpack(_accu, assumed_align=8)
    
    cuda_stream = cuda.CUstream(torch.cuda.current_stream().cuda_stream)
    
    use_multicast = True
    vocab_per_split_val = _get_fwd_config()._vocab_per_split
    key = f"vocab_size:{vocab_size}+dim:{dim}+dtype:{hidden_view.dtype}+multicast:{use_multicast}+vps:{vocab_per_split_val}"
    ...
    if _get_fwd_config()._fwd_mainloop_kernels.get(key) is None:
        fwd_mainloop_kernel = fwd_mainloop.FwdMainLoop(
            ...
            use_tma_multicast=use_multicast
        )
...
def backward(
    ...
    d_hidden = torch.zeros_like(global_hidden)
    ...
    if _backward_method == utils.BackwardMethodEnum.kDlogitsSplitN:
        ...
        _d_logits_T = torch.empty(
            (vocab_per_split, num_tokens),
            device=global_hidden.device,
            dtype=global_hidden.dtype,
        )
        _d_logits = _d_logits_T.T
