...
| `ServeConfig` | Dataclass for the HTTP frontend. In addition to host, port, parsers, logging, and replica settings, `default_top_p` / `default_top_k` control sampling defaults and `serving_mode=True` avoids returning prompt token IDs by default. |
