...
def _fused_fill_3_kernel(
    ...
    """Fill buf[start:start+count] = val for three buffers in a single launch."""
    pid = tl.program_id(0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < count
    idx = start + offsets
    tl.store(BUF_A_PTR + idx, val_a, mask=mask)
    tl.store(BUF_B_PTR + idx, val_b, mask=mask)
    tl.store(BUF_C_PTR + idx, val_c, mask=mask)
...
def fused_fill_3(
    ...
    """Fill three 1D tensors over [start, end) with respective values in one kernel."""
    count = end - start
    if count <= 0:
        return
    BLOCK_SIZE = 1024
    grid = (triton.cdiv(count, BLOCK_SIZE),)
    _fused_fill_3_kernel[grid](
        buf_a, buf_b, buf_c, val_a, val_b, val_c, start, count, BLOCK_SIZE=BLOCK_SIZE,
    )
