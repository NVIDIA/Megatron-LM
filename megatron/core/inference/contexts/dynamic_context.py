...
# MCORE_INFER_ATTN_PROF=1 to accumulate per-phase host wall time and print a
...
def _attn_prof_report() -> None:
    """Print the mean per-call host cost of each phase, then reset counters."""
    ...
    print(f"[ATTN_PROF rank={rank}] calls={n} us/call: {parts}", flush=True)
...
class DynamicInferenceContext(BaseInferenceContext):
    ...
    def _incr_attn_state_verify(self, snapshot: Dict) -> None:
        ...
        if self._incr_attn_state_verify_steps % 10 == 0:
            print(
                f"[INCR_ATTN_STATE verify] {self._incr_attn_state_verify_steps} "
                "incremental steps bit-identical to full recomputation",
                flush=True,
            )
