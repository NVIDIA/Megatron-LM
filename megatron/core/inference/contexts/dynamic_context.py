...
class DynamicInferenceContext(MTPContextMixin, BaseInferenceContext):
    ...
    def __init__(self, model_config: TransformerConfig, inference_config: InferenceConfig):
        ...
        if self.is_hybrid_model:
            ...
            # KDA (dev-only mixer) has no dynamic-inference recurrent-state contract yet, so
            # reject it up front rather than silently building a map that omits its layers.
