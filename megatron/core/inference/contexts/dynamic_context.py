...
class DynamicInferenceContext(BaseInferenceContext):
    ...
    def initialize_all_tensors(self) -> None:
        ...
        # batch_indices_decode is int64; async decode_write and all other fields are int32.
        if self.is_hybrid_model:
    ...
    def transfer_bookkeeping_to_gpu(
        ...
        *,
        ...
        By default, the coalesced H2D from the pinned `_cpu_bookkeeping_buf`
        blocks because that buffer is re-staged in place on the next step. Async
        scheduling may request an event-tracked non-blocking copy; its event must
        be synchronized before the pinned source buffer is reused.
        ...
        Args:
            include_token_to_input_ids (bool): Whether to copy CPU input token
                IDs into the GPU bookkeeping view.
            refresh_request_staging (bool): Whether to refresh request-level
                staging slots from the persistent CPU tensors.
        ...
        # Only event-tracked callers may leave the copy in flight; other callers
        # block before the pinned CPU source can be re-staged.
        non_blocking = record_done_event
        if include_token_to_input_ids:
            self.gpu_view._buf.copy_(self._cpu_bookkeeping_buf, non_blocking=non_blocking)
        else:
            input_ids_nbytes = self.max_tokens * self.token_to_input_ids.element_size()
            self.gpu_view._buf[input_ids_nbytes:].copy_(
                self._cpu_bookkeeping_buf[input_ids_nbytes:], non_blocking=non_blocking
            )
        done_event = None
