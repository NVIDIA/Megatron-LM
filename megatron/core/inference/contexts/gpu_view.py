...
class ContextGPUView:
    ...
    def __init__(
        ...
        #   int32 SSM fields (hybrid models only; omitted when
        #   max_ssm_chunks == 0).
        ...
        # SSM section, only present for hybrid models.
        #   ssm_batch_indices_decode    int64 (max_bs,)
        #   ssm_batch_indices_prefill   int32 (max_bs,)
        #   ssm_seq_idx                 int32 (1, max_tokens)
        #   ssm_cu_seqlens              int32 (max_bs + 1,)
        #   ssm_cu_chunk_seqlens        int32 (max_ssm_chunks + 1,)
        #   ssm_last_chunk_indices      int32 (max_bs,)
        #   ssm_seq_idx_for_varlen      int32 (max_ssm_chunks,)
        #   ssm_conv_seq_idx            int32 (max_tokens,)
        #   ssm_conv_seq_start          int32 (max_tokens,)
        # Bytes before the SSM section (needed to compute alignment padding).
        pre_ssm_bytes = (
            3 * tok_int64_bytes
            + 3 * tok_int32_bytes
            + 7 * req_4byte_bytes
        ...
        if max_ssm_chunks > 0:
            ...
            ssm_align_pad = (8 - pre_ssm_bytes % 8) % 8
            ssm_batch_indices_decode_bytes = max_bs * 8
            ssm_batch_indices_prefill_bytes = max_bs * 4
            ssm_seq_idx_bytes = max_tokens * 4
            ssm_cu_seqlens_bytes = (max_bs + 1) * 4
            ssm_cu_chunk_seqlens_bytes = (max_ssm_chunks + 1) * 4
            ssm_last_chunk_indices_bytes = max_bs * 4
            ssm_seq_idx_for_varlen_bytes = max_ssm_chunks * 4
            ssm_conv_seq_idx_bytes = max_tokens * 4
            ssm_conv_seq_start_bytes = max_tokens * 4
        else:
            ssm_align_pad = 0
            ssm_batch_indices_decode_bytes = 0
            ssm_batch_indices_prefill_bytes = 0
            ssm_seq_idx_bytes = 0
            ssm_cu_seqlens_bytes = 0
            ssm_cu_chunk_seqlens_bytes = 0
            ssm_last_chunk_indices_bytes = 0
            ssm_seq_idx_for_varlen_bytes = 0
            ssm_conv_seq_idx_bytes = 0
            ssm_conv_seq_start_bytes = 0

        total_bytes = (
            pre_ssm_bytes
            + ssm_align_pad
            + ssm_batch_indices_decode_bytes
            + ssm_batch_indices_prefill_bytes
            + ssm_seq_idx_bytes
            + ssm_cu_seqlens_bytes
            + ssm_cu_chunk_seqlens_bytes
            + ssm_last_chunk_indices_bytes
            + ssm_seq_idx_for_varlen_bytes
            + ssm_conv_seq_idx_bytes
            + ssm_conv_seq_start_bytes
        )
        ...
        if max_ssm_chunks > 0:
            off += ssm_align_pad
            self.ssm_batch_indices_decode = self._buf[
                off : off + ssm_batch_indices_decode_bytes
            ].view(torch.int64)
            off += ssm_batch_indices_decode_bytes
            self.ssm_batch_indices_prefill = self._buf[
                off : off + ssm_batch_indices_prefill_bytes
            ].view(torch.int32)
            off += ssm_batch_indices_prefill_bytes
            self.ssm_seq_idx = (
                self._buf[off : off + ssm_seq_idx_bytes].view(torch.int32).view(1, max_tokens)
            )
            off += ssm_seq_idx_bytes
            self.ssm_cu_seqlens = self._buf[off : off + ssm_cu_seqlens_bytes].view(torch.int32)
            off += ssm_cu_seqlens_bytes
            self.ssm_cu_chunk_seqlens = self._buf[off : off + ssm_cu_chunk_seqlens_bytes].view(
                torch.int32
            )
            ...
            off += ssm_conv_seq_start_bytes
        else:
            self.ssm_batch_indices_decode = None
            self.ssm_batch_indices_prefill = None
            self.ssm_seq_idx = None
            self.ssm_cu_seqlens = None
            self.ssm_cu_chunk_seqlens = None
            self.ssm_last_chunk_indices = None
            self.ssm_seq_idx_for_varlen = None
            self.ssm_conv_seq_idx = None
            self.ssm_conv_seq_start = None
