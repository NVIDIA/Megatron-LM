...
class PrefixCacheRegistry:
    ...
    Owns the host-side `hash -> block_id` mappings for KV cache blocks and Mamba state blocks,
    and exposes the match / register / evict primitives that drive prefix caching.

    The interface is deliberately torch-free: only Python `int` / `list` / `set` cross the boundary.

    Physical resources (the GPU `block_hashes` shadow and ref counts on the KV allocator,
    the slot pool and state tensors on the Mamba allocator) stay where they are;
    their owners call into this registry at register / evict time to keep the host view consistent
    ...
    def set_mamba_evict_callback(self, cb: Callable[[List[int]], None]) -> None:
        ...
        The callback receives the list of block IDs whose Mamba state should be released.
        Used by the Mamba allocator to free GPU slots; remote deployments leave this unset.
        """
    ...
    def match_kv_prefix(self, hashes: List[int]) -> Tuple[List[int], int]:
        ...
        Parent-chained hashing guarantees that if `hashes[N]` is cached, all earlier hashes are too.
        Backwards scan finds the longest cached prefix in one pass.

        Returns `(matched_block_ids, parent_hash)`
            `parent_hash` is the hash of the last matched block, `0` if no matches.
        """
        if not hashes:
            return [], 0
    ...
    def evict_kv(self, hashes: Iterable[int]) -> set:
        ...
        Cascades to the Mamba dict: a deregistered KV block can never retain valid Mamba state.
        Returns the set of hashes that were actually removed from the KV dict.
        """
        keys = set(hashes) - {-1}
        if not keys:
            return keys
    ...
    def match_mamba_farthest(self, hashes: List[int]) -> int:
        ...
        Mamba state is cumulative; only the farthest cached block needs to be restored.
        Backwards scan finds it in one pass. Returns 0 if nothing matches.
        """
        if not hashes:
            return 0
    ...
    def find_mamba_backoff(self, hashes: List[int], search_len: int) -> int:
        ...
        Used by `add_request` when the raw Mamba match overruns the chunk and we need to back off
        to a previous cached state. The normal `match_mamba_farthest` scans the whole list;
        this scans a bounded prefix.
        """
        if search_len <= 0:
            return 0
        d = self.mamba_hash_to_block_id
    ...
    def evict_mamba(self, hashes: Iterable[int]) -> set:
        ...
        Returns the set of hashes that were actually removed.
        Fires the Mamba-evict callback so the allocator can free GPU slots.
        """
