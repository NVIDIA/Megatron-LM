...
class DynamicInferenceEngine(AbstractEngine):
    ...
    def _cg_admission_gating_active(self) -> bool:
        ...
        """
        return (
            getattr(self, "cuda_graph_all_prefills", False)
    ...
    def _run_coroutine_sync(self, coro):
        ...
        # We're inside a running loop - run in a separate thread. Keep this
        # outside the probe's exception handler so coroutine failures propagate
        # instead of trying to execute an already-awaited coroutine again.
