    ...
    async def start_listening_to_data_parallel_coordinator(
        ...
        *,
        ...
        Args:
            ...
            coordinator_schedule_output_path (str | None): If set, the coordinator writes
                its per-request scheduling decisions as JSON to this path on shutdown.
                Used for debugging prefix-aware routing.
            loop (Optional[asyncio.AbstractEventLoop]): The event loop to attach the
                engine loop task to. Defaults to the current loop via `get_asyncio_loop`.
            listening_timeout (float): Seconds to yield to the event loop inside
                `schedule_requests` before draining ZMQ messages. 0 means a single
                `await asyncio.sleep(0)` yield. Defaults to 0.
            steps_before_listen (int): Drain ZMQ messages every Nth engine step when
                the engine has unfinished work. Throttles ZMQ polling during busy
                periods at the cost of increased latency for new requests. Defaults to
                1 (drain every step).
        ...
        try:
            while True:
                ...
                if self.state in (EngineState.RUNNING, EngineState.PAUSING):
                    ...
                    # World barrier only applies in coordinator mode. Standalone engines
                    # never receive these transient states through the normal path, but
                    # guard defensively in case something sets the state directly.
                    if cc is not None:
                        await cc.world_barrier()
