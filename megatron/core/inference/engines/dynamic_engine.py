...
class DynamicInferenceEngine(AbstractEngine):
    ...
    async def async_forward(self) -> Tuple[Dict, Dict, float]:
        ...
        # Generate tokens.
        nvtx_range_push(f"Prefill-{self.context.active_token_count}" if not is_decode_only else f"Decode-{self.context.active_token_count}")
        ...
        nvtx_range_pop(f"Prefill-{self.context.active_token_count}" if not is_decode_only else f"Decode-{self.context.active_token_count}")
