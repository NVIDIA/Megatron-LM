...
class InferenceFLOPsCalculator:
    ...
    def __init__(self, config: InferenceFLOPsConfig):
        ...
        in_proj_dim = 2 * d_inner + 2 * config.mamba_num_groups * config.mamba_state_dim + config.mamba_num_heads
        ...
        qkv_dim = config.num_attention_heads * config.kv_channels + 2 * config.num_query_groups * config.kv_channels
        q_proj_size = config.num_attention_heads * config.kv_channels
        self.f_attn_fixed_per_token = (
            2 * h * qkv_dim         # QKV projection
            + 2 * q_proj_size * h   # output projection
        )
        ...
        moe_ffn = config.moe_ffn_hidden_size if config.moe_ffn_hidden_size else config.ffn_hidden_size
        self.f_moe_per_token = (
            2 * h * config.num_experts                                       # router
            + 4 * h * moe_ffn * config.moe_router_topk * scale_factor       # routed experts fc1+fc2
            + 4 * h * config.moe_shared_expert_intermediate_size * scale_factor  # shared expert fc1+fc2
        )
        ...
        if prefill_tokens > 0:
            ...
            if num_prefill_reqs > 0:
                ...
                prefill_attn_quad = (
                    ...
                    * num_prefill_reqs
                    * 2 * self.config.num_attention_heads * self.config.kv_channels
                    * avg_prompt_len ** 2
                )
            else:
                prefill_attn_quad = 0
    ...
    def from_args(cls, args) -> "InferenceFLOPsCalculator":
        ...
        if getattr(args, 'hybrid_override_pattern', None):
            ...
            if parsed.main_pattern:
                for lt in parsed.main_pattern:
                    if lt in counts:
                        counts[lt] += 1
            num_attn, num_mamba, num_mlp, num_moe = counts['*'], counts['M'], counts['-'], counts['E']
        ...
        config = InferenceFLOPsConfig(
            ...
            moe_shared_expert_intermediate_size=getattr(args, 'moe_shared_expert_intermediate_size', 0) or 0,
