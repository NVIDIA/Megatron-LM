...
class DynamicInferenceRequest(InferenceRequest):
    ...
    def deserialize(cls, obj: dict) -> "DynamicInferenceRequest":
        ...
        """
        # Remove generated_tokens from obj since it's now a property computed from events
        # The tokens will be recovered from GENERATED_TOKEN events
        obj_copy = {k: v for k, v in obj.items() if k != "generated_tokens"}
        request = super().deserialize(obj_copy)
