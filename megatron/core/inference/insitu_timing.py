...
(see ``floor_ablation.py``, and the four failed attempts recorded in
EXPERIMENTS.md). Nsys is equally unavailable here -- it costs ~28% end-to-end, and
...
``cudaErrorInvalidValue``. Measured, not assumed: see
``dev/moe_fused/insitu_probe.py`` (plain events, fails) and ``insitu_probe3.py``
(external events, works). The flagged record must also target the *capturing*
stream, which ``torch.cuda.graph()`` picks itself unless told otherwise, so the
stream is read back at record time rather than assumed.
    ...
    for key, (st, en) in _events.items():
        try:
            ...
            if _first_error is None:
                ...
                print(f"[INSITU] first elapsed_time failure -- {_first_error}",
