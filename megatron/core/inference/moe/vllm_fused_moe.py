...
# Env-toggleable for A/B; the tuned path is the same kernel with different tile
# sizes, so it differs from the default only in fp32 accumulation order.
...
def _fused_moe_kernel(
    ...
    for pid in tl.range(pid_init, total_tiles, grid_size):
        ...
        if off_experts != -1:
            ...
            # SwiGLU fused on the fp32 accumulators: SiLU(gate) * up, matching
            # the standalone `bounded_silu_mul` kernel bit-for-bit (fp32 math,
            # single bf16 cast on the product). Removes that kernel and the 2N
