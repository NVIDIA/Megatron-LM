...
def _dump_entry_times():
    ...
    try:
        ...
        with open(os.path.join(d, f"entry_r{_rank()}.txt"), "w",
                  encoding="utf-8") as f:
...
def _report():
    ...
    lines.append(f"  block-graph GPU   median {g:7.3f} ms   "
    ...
    if len(_acc.period_ms) >= 8:
        ...
        lines.append(f"  step period (host) median {p:7.3f} ms   "
...
def install():
    ...
    for _ in range(_RING):
        _acc.events.append((torch.cuda.Event(enable_timing=True),
    ...
    _installed = True
    print(f"[STEP_GPU] installed (ring {_RING}, skip {_SKIP}, "
          f"report every {_REPORT_EVERY})", file=sys.stderr, flush=True)
