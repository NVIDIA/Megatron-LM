...
class AVLMTextGenerationController(TextGenerationController):
    ...
    def prep_inference_input(
        ...
        """
        assert len(active_requests) == 1, f"VLM inference currently only supports batch size 1, got {len(active_requests)}"

        request = list(active_requests.values())[0]

        assert isinstance(
            request, AVLMInferenceRequest
        ), f"Found inference request of type {type(request)}, expected AVLMInferenceRequest"
        ...
        inference_input = self.inference_wrapped_model.prep_inference_input(
            ...
            sound_clips=request.sound_clips,
            sound_length=request.sound_length,
            num_sound_embeddings=request.num_sound_embeddings,
        )
