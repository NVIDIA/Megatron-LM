...
class TextGenerationController:
    ...
    async def _generate_sync(
        ...
        """Synchronous generation logic using modular helper methods.
    ...
    async def async_generate_output_tokens_dynamic_batch(
        ...
        Dispatches to async or sync implementation based on context settings.
        ...
        # Dispatch to async or sync implementation
        if context.enable_async_scheduling:
            return await self._generate_async(active_sampling_map, skip_bookkeeping)
        else:
            return await self._generate_sync(active_sampling_map, skip_bookkeeping)
    ...
    async def _generate_async(
        ...
        # Check if any sampling params require log probs
        return_log_probs = False
        for sampling_params, mask in active_sampling_map:
            if sampling_params.return_log_probs:
                skip_prompt_log_probs_for_dynamic_inference = getattr(
        ...
        with torch.cuda.stream(metadata.execution_stream):
            with torch.inference_mode():
                ...
                # Sample from OLD logits using active_sampling_map
                vocab_size = inference_wrapper_config.padded_vocab_size
                new_sample, termination_id_tensor = self.sample_from_dynamic_logits(
                    last_token_logits, active_sampling_map, vocab_size=vocab_size
                )
        ...
        # termination_id_tensor is a tensor with termination IDs for each request
        terminated_mask = (new_sample == termination_id_tensor)
        ...
        # Calculate log probs if needed (checked earlier from active_sampling_map)
        log_probs = None
