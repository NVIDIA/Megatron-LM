...
class TextGenerationController:
    ...
    def _init_dynamic_sampling_tensors(self):
        ...
        self.skip_prompt_top_n_logprobs_cuda = torch.empty(
        ...
        **kwargs,
        ...
        if sampling_params.top_n_logprobs > 0:
            ...
            if logits is None:
                ...
                # If we skip prompt top_n_log_probs then we only append for generated tokens.
                # Otherwise we always append to the logprobs dict.
                if sampling_params.skip_prompt_top_n_logprobs:
                ...
                assert not sampling_params.skip_prompt_top_n_logprobs
    ...
    def _dynamic_step_sample_bookkeeping(
        ...
        *,
        ...
        self.skip_prompt_top_n_logprobs_cuda[:active_request_count] = request_metadata[
            :, request_metadata_labels["skip_prompt_top_n_logprobs"]
        ].to(dtype=torch.bool, copy=True, non_blocking=True)
    ...
    def _dynamic_step_top_n_logprobs_bookkeeping(self) -> bool:
        ...
        # Check if any request wants prompt top-n logprobs (top_n > 0 AND skip_prompt_top_n = False)
        # Create a copy to avoid modifying the original tensor with in-place operations
        to_check = (self.top_n_logprobs_cuda[:active_request_count] > 0).clone()
        to_check &= ~self.skip_prompt_top_n_logprobs_cuda[:active_request_count]

        assert not (
            to_check.any() and materialize_only_last_token_logits
        ), "Prompt top-n logprobs cannot be calculated if only last token logits are materialized. Set materialize_only_last_token_logits to False in DynamicInferenceContext or set skip_prompt_top_n_logprobs to True in SamplingParams."
    ...
    def _dynamic_step_calculate_top_n_logprobs(
        ...
        for req_idx in range(active_request_count):
            ...
            if top_n > 0:
                ...
                skip_prompt_top_n = bool(self.skip_prompt_top_n_logprobs_cuda[req_idx].item())

                # If skip_prompt_top_n_logprobs is True, only compute for last token
                if skip_prompt_top_n and request_log_probs.size(0) > 1:
    ...
    def generate_all_output_tokens_static_batch(
        ...
        with torch.inference_mode():
            ...
            while True:
                ...
                logits_for_top_n_prompt_logprobs = (
                    ...
                    and not sampling_params.skip_prompt_top_n_logprobs
                    else None
                )
        ...
        for idx, request in enumerate(active_requests.values()):
            ...
            if sampling_params.top_n_logprobs > 0:
                if not sampling_params.skip_prompt_top_n_logprobs:
                    assert (
