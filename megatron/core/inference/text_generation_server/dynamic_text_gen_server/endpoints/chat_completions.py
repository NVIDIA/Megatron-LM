        ...
        for record in batch_results:
            ...
            if parsers:
                message_text, metadata = apply_parsers(
                    message_text, tools=tools, parsers_list=parsers
                )
                # Normalize tool_calls to OpenAI shape for downstream clients.
                # The output keeps id/type/function.name/function.arguments (string).
                # If client did not request tools (plain chat), do not expose
                # parser-detected tool syntax as tool_calls.
                if "tool_calls" in metadata:
                    metadata["tool_calls"] = _normalize_tool_calls(metadata.get("tool_calls", []))
