...
from megatron.core.inference.text_generation_server.dynamic_text_gen_server.image_preprocessing import (  # noqa: E501,F401
...
def _print_resolved_args(title, args):
    ...
    print("---------------- VLM argument provenance ----------------", flush=True)
    for record in resolution:
        ...
        print(f"[vlm_arg_provenance] {json.dumps(payload, sort_keys=True)}", flush=True)
    print("------------ end of VLM argument provenance -------------", flush=True)
...
def _detect_vlm_from_checkpoint(args, user_passed_attrs=None):
    ...
    vlm_attrs = [
        'language_model_type', 'vision_model_type', 'vision_projection_type',
        'decoder_seq_length',
        ...
        'radio_disable_cpe',
        'spec', 'transformer_impl', 'is_hybrid_model',
        'hybrid_override_pattern', 'num_experts',
    ]
    resolution = []
