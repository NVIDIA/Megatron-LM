...
def get_backend(
    ...
    *,
    ...
    ``use_te_op_fuser``, ``fallback_to_eager_attn`` and ``use_flex_attention`` apply only
    to the Transformer Engine provider.
    """
...
def get_backend_from_config(
    ...
    return get_backend(
        ...
        # Dev-only eager-attention fallback; main's configs do not carry the field.
        fallback_to_eager_attn=getattr(config, "fallback_to_eager_attn", False),
        use_flex_attention=getattr(config, "use_flex_attention", False),
    )
