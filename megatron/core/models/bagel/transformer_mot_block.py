...
This module lives in examples/bagel/model and implements a Transformer block that
supports separate processing for understanding (und) and generation (gen) tokens,
following the MoT architecture. It aligns with megatron.core.transformer.transformer_block.
"""
...
class TransformerMoTBlock(GraphableMegatronModule, MegatronModule):
    ...
    def __init__(
        ...
        if getattr(config, 'overlap_moe_expert_parallel_comm', False):
            raise ValueError(
                "overlap_moe_expert_parallel_comm is not supported with MoTTransformerLayer. "
                "MoT calls both mlp and mlp_gen monolithically — the split-phase EP overlap "
                "pattern is absent. Set --no-overlap-moe-expert-parallel-comm."
            )
        ...
        # Propagate to config so MoTTransformerLayer picks it up via
        # getattr(config, 'freeze_und', False) during _build_layers().
        self.config.freeze_und = freeze_und
    ...
    def forward(
        ...
        **kwargs,
        ...
        All MoT metadata (und/gen token ranges) is carried by packed_seq_params.

        Args:
            hidden_states (Union[Tensor, WrappedTensor]): Compact input [Lund+Lgen, 1, h].
            attention_mask (Tensor): Attention mask / BlockMask.
            ...
            packed_seq_params (PackedSeqParams, optional): MoT metadata + CP shard info.
    ...
    def forward_train(
        ...
        hidden_states must be in compact form [Lund+Lgen, 1, h] with und tokens
        first. packed_seq_params carries all MoT metadata (Lund, Lgen, global index
        arrays for CP restore). Layers read Lund/Lgen directly from packed_seq_params.

        Args:
            hidden_states (Tensor): Compact input [Lund+Lgen, 1, h] (und first).
            attention_mask (Tensor): Attention mask / BlockMask.
    ...
    def _apply_final_layernorm_mot(
        ...
        When packed_seq_params.padded_und_seqlen is set, uses compact-mode slicing:
          und tokens: hidden_states[:Lund] → final_layernorm
          gen tokens: hidden_states[Lund:] → final_layernorm_gen
        Otherwise applies final_layernorm to the full tensor (non-MoT / inference fallback).

        Args:
            hidden_states (Tensor): Input hidden states.
            packed_seq_params: MoT metadata (padded_und_seqlen used for slicing).
    ...
    def _checkpointed_forward_train(
        ...
        Phase 2: index arrays removed — packed_seq_params carries all MoT metadata.
        ...
        def custom(start: int, end: int):
            ...
            # is captured via closure instead of being passed as a positional arg
        ...
        **kwargs,
        ...
        if self.final_layernorm is not None:
            if mode == "gen" and self.final_layernorm_gen is not None:
                hidden_states = apply_module(self.final_layernorm_gen)(hidden_states)
            else:
