...
def should_free_input(name, is_moe, config, num_local_experts):
    ...
    if not is_moe:
        return False
    enable_deepep = config.moe_token_dispatcher_type == "flex" and (
        config.moe_flex_dispatcher_backend in ("deepep", "deepepv2")
    )
    enable_hybridep = (
...
class PreProcessNode(ScheduleNode):
    ...
    def forward_impl(self):
        ...
        # Keep the model preprocess output above; the decoder-block input may be expanded
        # into mHC residual streams below. Only TransformerBlock implements the boundary
        # hook — HybridStack has no mHC support, so fall through unchanged there.
        preprocess_for_layer_schedule = getattr(
            self.model.decoder, "preprocess_for_layer_schedule", None
        )
        if preprocess_for_layer_schedule is not None:
            decoder_input = preprocess_for_layer_schedule(decoder_input)
...
class PostProcessNode(ScheduleNode):
    ...
    def forward_impl(self, hidden_states):
        ...
        # With MTP the decoder boundary is applied by the MTP pre-dispatch node before the
        # per-depth chunking, so re-applying it here would double-normalize the output.
        # A non-empty decoder applies it at its last layer node instead
        # (``finalize_decoder_layer_output``), so only empty shards need it here.
        if not self.model.config.mtp_num_layers and empty_decoder:
            # ``postprocess_for_layer_schedule`` applies the mHC output contraction plus the
            # final layer norm. Only TransformerBlock implements it; HybridStack has no mHC
            # support, so it falls back to applying its own final norm directly.
            postprocess_for_layer_schedule = getattr(
                self.model.decoder, "postprocess_for_layer_schedule", None
            )
            layer_norm = getattr(self.model.decoder, "final_norm", None) or getattr(
                self.model.decoder, "final_layernorm", None
            )
            if postprocess_for_layer_schedule is not None:
                hidden_states = postprocess_for_layer_schedule(hidden_states)
            elif layer_norm:
