...
def build_mtp_layer_callables(layer):
    ...
    def submodule_mtp_attn_forward(node, hidden_states):
        ...
        if node.is_first_layer:
            ...
            loss_mask = getattr(node.chunk_state, 'loss_mask', None)
            if loss_mask is None:
                # if loss_mask is not provided, use all ones as loss_mask
                loss_mask = torch.ones_like(node.chunk_state.labels_for_mtp)
            node.chunk_state.loss_mask = loss_mask
            node.chunk_state.hidden_states_main_model = node.detach(hidden_states)
    ...
    def submodule_mtp_postprocess_forward(node, hidden_states):
        ...
        if output_weight is None and node.chunk_state.model.share_embeddings_and_output_weights:
            ...
            node.chunk_state.output_weight = output_weight
        hidden_states, hidden_states_main_model, labels, loss_mask = layer._postprocess(
            labels=node.chunk_state.labels_for_mtp,
            loss_mask=node.chunk_state.loss_mask,
            hidden_states=hidden_states,
            hidden_states_main_model=node.chunk_state.hidden_states_main_model,
            output_layer=node.chunk_state.model.output_layer,
            output_weight=output_weight,
            runtime_gather_output=node.chunk_state.runtime_gather_output,
            compute_language_model_loss=node.chunk_state.model.compute_language_model_loss,
        )
        node.chunk_state.loss_mask = loss_mask
        node.chunk_state.labels_for_mtp = labels
        if node.is_last_layer:
            hidden_states_main_model = hidden_states_main_model.clone()
            return hidden_states_main_model
        else:
            node.chunk_state.hidden_states_main_model = node.detach(hidden_states_main_model)
            return hidden_states
