...
def get_mlp_module_spec_for_backend(
    ...
    if num_experts is None:
        ...
        # ``dense_grouped_gemm`` (dev) and ``use_grouped_gemm_for_dense_mlp`` (main) both
        # request the grouped-GEMM dense MLP fusion.
        module = backend_slot(
            backend,
            "mlp_module",
            lambda: MLP,
            grouped=bool(dense_grouped_gemm or use_grouped_gemm_for_dense_mlp),
        ).as_mlp_submodule
