...
def get_gpt_mtp_block_spec_for_backend(
    ...
    if isinstance(spec, TransformerBlockSubmodules):
        ...
        and issubclass(spec.module, TransformerLayer)
    ):
    ...
    # GPT MTP layers do not carry mHC residual streams (the MTP mHC contract is
    # HybridModel-only, see MultiTokenPredictionLayer); strip HC modules and
    # downgrade the layer class to plain TransformerLayer so the MTP layer spec
    # stays well-formed. Work on a shallow copy so the decoder spec is untouched.
