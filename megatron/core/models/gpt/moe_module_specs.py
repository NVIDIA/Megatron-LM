...
from megatron.core.models.backends import BackendSpecProvider, backend_slot, get_backend
...
def get_moe_module_spec_for_backend(
    ...
    router = backend_slot(backend, "moe_router", lambda: None)
...
def get_inference_optimized_moe_spec() -> MlpBuilder:
    ...
    activation_func = backend.activation_func()
    router = not_none(backend_slot(backend, "moe_router", lambda: None))
