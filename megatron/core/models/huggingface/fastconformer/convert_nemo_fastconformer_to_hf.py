...
from transformers.models.fastconformer.feature_extraction_fastconformer import (
...
class NeMoFastConformerConverter:
    ...
    def _extract_model_info(self) -> Dict[str, Any]:
        ...
        if hasattr(cfg, 'model') and cfg.model is not None:
            ...
            if hasattr(self.nemo_model, 'preprocessor') and hasattr(
                self.nemo_model.preprocessor, 'cfg'
            ):
    ...
    def _create_hf_config(self) -> FastConformerConfig:
        ...
        config_params = {
            ...
            'encoder_ffn_dim': getattr(encoder_cfg, 'd_model', 1024)
            * getattr(encoder_cfg, 'ff_expansion_factor', 4),
    ...
    def _map_encoder_weights(
        self, nemo_encoder_state_dict: Dict[str, torch.Tensor]
    ...
    def _convert_encoder(self, hf_config: FastConformerConfig) -> Tuple[FastConformerEncoder, int]:
        ...
        for param_name in hf_encoder_original_state_dict.keys():
            if param_name in hf_encoder_state_dict:
                if (
                    hf_encoder_original_state_dict[param_name].shape
                    == hf_encoder_state_dict[param_name].shape
                ):
        ...
        logger.info(
            f"Loaded {matched_params}/{len(hf_encoder_original_state_dict)} encoder parameters"
        )
    ...
    def verify_conversion(self) -> bool:
        ...
        # Create test input
        num_mel_bins = (
            self.model_info['preprocessor_cfg'].features
            if self.model_info['preprocessor_cfg']
            else 128
        )
        ...
        with torch.no_grad():
            ...
            nemo_encoded, _ = self.nemo_model.encoder(
                audio_signal=test_input.transpose(1, 2), length=lengths  # NeMo expects (B, C, T)
            )
        ...
        correlation = torch.corrcoef(torch.stack([nemo_encoded.flatten(), hf_encoded.flatten()]))[
            0, 1
        ].item()
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert NeMo FastConformer models to HuggingFace format"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output directory for HuggingFace model"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify conversion by comparing encoder outputs"
    )
    ...
    try:
        ...
        if args.verify:
            logger.info(
                f"Verification: {'✅ PASSED' if conversion_info.get('verification_passed') else '❌ FAILED'}"
            )
        logger.info("=" * 60)
        ...
        logger.info("\nUsage example:")
        logger.info(
            f"from transformers.models.fastconformer.modeling_fastconformer import FastConformerModel"
        )
