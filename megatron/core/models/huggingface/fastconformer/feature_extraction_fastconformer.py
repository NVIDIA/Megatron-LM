...
class FastConformerFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.win_length > self.n_fft:
            warnings.warn(
                f"win_length ({self.win_length}) > n_fft ({self.n_fft}), truncating window"
            )
    ...
    def get_window(self, win_length: int, window_type: str, device, dtype) -> "torch.Tensor":
        ...
        if window_type not in window_fns:
            raise ValueError(
                f"Unsupported window type: {window_type}. Supported: {list(window_fns.keys())}"
            )
    ...
    def normalize_mel_features(
        ...
        if normalize_type == "per_feature":
            ...
            variance = ((mel_masked - mean) ** 2 * mask).sum(dim=(1, 2)) / (total_valid - 1).clamp(
                min=1
            )
    ...
    def __call__(
        ...
        if raw_speech.dim() != 2:
            raise ValueError(
                f"Expected raw_speech to have shape (B, T), got shape {raw_speech.shape}"
            )
        ...
        if audio_lengths is None:
            ...
            audio_lengths = torch.full(
                (batch_audio.shape[0],), batch_audio.shape[1], dtype=torch.long
            )
        else:
        ...
        attention_mask = torch.arange(max_frames, device=target_device).unsqueeze(
            0
