...
class FastConformerRelPositionalEncoding(nn.Module):
    ...
    def forward(
        self, hidden_states: "torch.Tensor", cache_len: int = 0
...
class FastConformerAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = (
            self.linear_q(hidden_states)
            .view(batch_size, seq_len, self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
        key_states = (
            self.linear_k(hidden_states)
            .view(batch_size, seq_len, self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
        value_states = (
...
class FastConformerConvModule(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, pad_mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class FastConformerSubsamplingConv2D(nn.Module):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                pointwise_conv = nn.Conv2d(
                    self.conv_channels, self.conv_channels, kernel_size=1, bias=use_bias
                )
    ...
    def forward(
        self, input_features: torch.Tensor, lengths: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        lengths = calc_length(
            lengths,
            all_paddings=self.left_padding + self.right_padding,
...
class FastConformerEncoder(FastConformerPreTrainedModel):
    ...
    def forward(
        ...
        # Create masks
        pad_mask_valid = (
            torch.arange(max_audio_length, device=hidden_states.device)[None, :] < lengths[:, None]
        )
        ...
        for layer in self.layers:
            ...
            skip_the_layer = (
                True
                if self.training and (dropout_probability < self.config.encoder_layerdrop)
                else False
            )
...
class ParakeetCTC(FastConformerPreTrainedModel):
    ...
    def forward(
        ...
        return_dict = (
            return_dict if return_dict is not None else self.encoder_config.use_return_dict
        )
