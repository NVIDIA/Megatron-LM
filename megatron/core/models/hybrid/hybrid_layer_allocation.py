...
class Symbols(layer_utils.Symbols):
    """Symbols for different layer types and pattern separators.

    Main moved the shared symbol table into ``megatron/core/models/hybrid/layers/utils.py``
    and drives layer construction off ``Symbols.LAYER_CONFIG_MAP``. Dev additionally
    defines the KDA mixer symbol ``'K'``, which has no dedicated layer config type: KDA is
    a member of ``GDN_ATTENTION_VARIANTS`` and therefore shares ``GDNLayerConfig`` with
    GDN/GDN2, selected by ``experimental_attention_variant='kda'``. Subclassing keeps
    ``layers/utils.Symbols`` the single source of truth for the config-type mapping while
    exposing the union of both sides' symbols (including ``KDA`` and ``MLA_ATTENTION``)
    under ``hybrid_layer_allocation.Symbols``, which is what pattern-level callers import.
    """

    KDA = 'K'
    # Every symbol that may appear in a hybrid layer pattern: the symbols that own a layer
    # config type, plus KDA. ``layer_utils.Symbols.LAYER_CONFIG_MAP`` is deliberately left
    # alone so it stays the authority on symbol-to-config-type mapping.
    VALID_LAYERS = set(layer_utils.Symbols.LAYER_CONFIG_MAP) | {KDA}
    ...
    def name_sorted_valid_layer_symbols(cls) -> list[str]:
        ...
        name.
        """
        # ``dir()`` is sorted by name and walks the MRO, so inherited symbols are included
        # (``vars(cls)`` would only see this subclass's own ``KDA``).
        valid_layer_symbols = []
        for name in dir(cls):
            if name.startswith('_'):
                continue
            value = getattr(cls, name)
            if isinstance(value, str) and value in cls.VALID_LAYERS:
                valid_layer_symbols.append(value)
        return valid_layer_symbols


def _is_valid_symbol(layer_symbol: str, allow_pipe: bool = False) -> bool:
    """Return whether ``layer_symbol`` identifies a supported layer or allowed pipe.

    ``layer_utils.is_valid_symbol`` only knows the symbols that own a layer config type.
    This variant resolves against ``Symbols.VALID_LAYERS`` so the KDA symbol is accepted.

    Args:
        layer_symbol: Symbol to validate.
        allow_pipe: Whether to also accept the pipeline separator symbol.
    """
    return layer_symbol in Symbols.VALID_LAYERS or (allow_pipe and layer_symbol == Symbols.PIPE)


def _create_layer_config(config: TransformerConfig, layer_symbol: str) -> TransformerConfig:
    """Create a layer-specific config for ``layer_symbol``, including the KDA symbol.
    ...
    """
    if layer_symbol == Symbols.KDA:
        # KDA has no dedicated config type: it shares GDNLayerConfig with GDN/GDN2, and
        # ``experimental_attention_variant='kda'`` selects the Kimi Delta Attention mixer
        # exactly as ``'gdn2'`` selects the GDN2 mixer.
        return layer_utils.GDNLayerConfig.from_config(config)
    return layer_utils.create_layer_config(config, layer_symbol)


def _get_layer_symbol_from_config(layer_config: TransformerConfig) -> str:
    """Return the canonical symbol for a layer config, distinguishing KDA from GDN.
    ...
    if layer_symbol == Symbols.GDN and layer_config.experimental_attention_variant == "kda":
        return Symbols.KDA
    return layer_symbol
...
def _validate_pattern(pattern: str, allow_pipe: bool = False) -> None:
    ...
    """
    for char in pattern:
        if not _is_valid_symbol(char, allow_pipe=allow_pipe):
            raise ValueError(
                f"'{char}' is not a valid layer symbol. "
                f"Valid symbols are: {Symbols.name_sorted_valid_layer_symbols()}"
            )
...
def validate_segment_layers(segment: str, config: TransformerConfig) -> List[TransformerConfig]:
    ...
    """
    # `_validate_pattern` already rejects invalid symbols and standard Attention ('*') mixed
    # with any MLA-based attention (+/D/C/H/W), so no extra check is needed here.
