...
def _radio_h_spec(
    ...
    *,
    ...
    return EncoderSpec(
        name=name, model_type="radio", checkpoint_dir=checkpoint_dir,
        patch_dim=16, default_img_h=512, default_img_w=512,
        class_token_len=10, radio_force_eval_mode=True,
        dynamic_resolution=dynamic_resolution, pixel_shuffle=pixel_shuffle,
        use_tiling=use_tiling, max_num_tiles=max_num_tiles, use_thumbnail=use_thumbnail,
        radio_hf_resolution=radio_hf_resolution,
        num_layers=32, hidden_size=1280, num_attention_heads=16, ffn_hidden_size=5120,
        kv_channels=80, activation_func="fast_gelu",
        layernorm_epsilon=1e-6, qk_layernorm=False,
    )


REGISTRY: Dict[str, EncoderSpec] = {
    "clip": EncoderSpec(
        name="clip", patch_dim=16, default_img_h=512, default_img_w=512,
        num_layers=24, hidden_size=1024, num_attention_heads=16, ffn_hidden_size=4096,
        kv_channels=64, activation_func="quick_gelu",
    ),
    "siglip": EncoderSpec(
        name="siglip", patch_dim=16, default_img_h=512, default_img_w=512,
        num_layers=27, hidden_size=1152, num_attention_heads=16, ffn_hidden_size=4304,
        kv_channels=72, activation_func="fast_gelu",
        layernorm_epsilon=1e-6, qk_layernorm=False,
    ),
    "internvit": EncoderSpec(
        name="internvit", patch_dim=16, default_img_h=512, default_img_w=512,
        num_layers=45, hidden_size=3200,
        num_attention_heads=24, tp_round_up_heads=True,
        ffn_hidden_size=12800, activation_func="gelu",
        add_qkv_bias=False, normalization='RMSNorm', layernorm_epsilon=1e-6,
    ),
    "internvit300M": EncoderSpec(
        name="internvit300M", patch_dim=16, default_img_h=512, default_img_w=512,
        num_layers=24, hidden_size=1024, num_attention_heads=16, ffn_hidden_size=4096,
        kv_channels=64, activation_func="gelu",
        layernorm_epsilon=1e-6, qk_layernorm=False,
    ),
    "radio": _radio_h_spec(
        "radio",
    ...
    "radio-g": EncoderSpec(
        name="radio-g", patch_dim=16, default_img_h=512, default_img_w=512,
        num_layers=40, hidden_size=1536, num_attention_heads=24, ffn_hidden_size=4096,
        kv_channels=64, gated_linear_unit=True, activation_func="silu",
        layernorm_epsilon=1e-6, qk_layernorm=False,
    ),
    "cradio-g": EncoderSpec(
        name="cradio-g", patch_dim=16, default_img_h=512, default_img_w=512,
        class_token_len=10,
        num_layers=40, hidden_size=1536, num_attention_heads=24, ffn_hidden_size=6144,
        kv_channels=64, activation_func="fast_gelu",
        layernorm_epsilon=1e-6, qk_layernorm=False,
    ),
    # Pixtral-12B: SwiGLU, RMSNorm, Mistral-native interleaved 2D RoPE, no bias, no CLS
    "pixtral-vit": EncoderSpec(
        name="pixtral-vit", patch_dim=16, default_img_h=512, default_img_w=512,
        num_layers=24, hidden_size=1024, num_attention_heads=16, ffn_hidden_size=4096,
        kv_channels=64, gated_linear_unit=True, activation_func="silu",
        add_bias_linear=False, add_qkv_bias=False,
        normalization='RMSNorm', layernorm_epsilon=1e-5, rotary_interleaved=True,
    ),
    # Pixtral-Large (Mistral-Large-3-675B): 48L/1664h/8192ffn + 2×2 patch merger
    "pixtral-vit-large": EncoderSpec(
        name="pixtral-vit-large", patch_dim=14, default_img_h=1540, default_img_w=1540,
        has_native_spatial_merge=True, checkpoint_dir="pixtral_large",
        dynamic_resolution=True, conv_merging=True,
        dynamic_resolution_max_patches=12100, dynamic_resolution_max_side=1540,
        num_layers=48, hidden_size=1664, num_attention_heads=16, ffn_hidden_size=8192,
        kv_channels=104, gated_linear_unit=True, activation_func="silu",
        add_bias_linear=False, add_qkv_bias=False,
        normalization='RMSNorm', layernorm_epsilon=1e-5, rotary_interleaved=True,
    ),
    # Qwen3.5-MoE VL: GELU-tanh MLP, LayerNorm, 2D RoPE + learned pos, bias
    "qwen-vl": EncoderSpec(
        name="qwen-vl", patch_dim=16, default_img_h=768, default_img_w=768,
        has_native_spatial_merge=True, checkpoint_dir="qwen35vl_moe",
        dynamic_resolution=True, conv_merging=True,
        dynamic_resolution_max_patches=4096, dynamic_resolution_max_side=768,
        pixel_mean=_HALF_MEAN, pixel_std=_HALF_STD,
        num_layers=27, hidden_size=1152, num_attention_heads=16, ffn_hidden_size=4304,
        kv_channels=72, activation_func="gelu_tanh",
        layernorm_epsilon=1e-6,
    ),
    # Kimi-K2: GELU-tanh MLP, LayerNorm, interleaved 2D RoPE + learned pos, bias
    "kimi-vit": EncoderSpec(
        name="kimi-vit", patch_dim=14, default_img_h=896, default_img_w=896,
        has_native_spatial_merge=True, checkpoint_dir="kimi_k26",
        dynamic_resolution=True, conv_merging=True,
        dynamic_resolution_max_patches=8192, dynamic_resolution_max_side=896,
        pixel_mean=_HALF_MEAN, pixel_std=_HALF_STD,
        num_layers=27, hidden_size=1152, num_attention_heads=16, ffn_hidden_size=4304,
        kv_channels=72, activation_func="gelu_tanh",
        layernorm_epsilon=1e-5, rotary_interleaved=True,
    ),
}
...
def get_spec(vision_model_type: str) -> EncoderSpec:
    ...
    try:
        ...
        raise KeyError(
            f"Unknown vision_model_type {vision_model_type!r}. "
