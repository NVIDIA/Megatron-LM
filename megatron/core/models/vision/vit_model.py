...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_size: int, patch_dim: int, bias: bool = False):
        super().__init__()
        self.proj = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_dim, stride=patch_dim, bias=bias)
...
class Pixtral2DRotaryEmbedding(nn.Module):
    ...
    def forward(self, h_patches: int, w_patches: int, device: torch.device) -> torch.Tensor:
        ...
        ], dim=-1).reshape(-1, self.head_dim // 2)
...
class ViTModel(MegatronModule):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbedding(in_channels, hidden_size, patch_dim, bias=patch_embed_bias)
    ...
    def forward(
        ...
        if dynamic_resolution:
            ...
            patch_hw, seq_lens = _dynamic_patch_grid(imgs_sizes, self.patch_dim, pixel_values.device)
        ...
        if self.pos_emb_type == 'learned_absolute':
            ...
            if dynamic_resolution:
                rotary_pos_emb = _cat_rope(
                    [
                        self.rope(int(h), int(w), pixel_values.device)
                        for h, w in patch_hw.tolist()
                    ]
                )
            else:
...
class QwenVL2DRotaryEmbedding(nn.Module):
    ...
    def __init__(self, head_dim: int, max_patches_per_side: int, rope_theta: float = 10000.0):
        ...
        # freq_table: (max_patches_per_side, dim//2)
    ...
    def forward(self, row_ids: torch.Tensor, col_ids: torch.Tensor) -> torch.Tensor:
        """
        Args:
            row_ids: (N,) integer row indices of each patch
            col_ids: (N,) integer col indices of each patch
        Returns:
            freqs: (N, 1, 1, head_dim) — mcore rotary_pos_emb format (raw freqs, not cos/sin)
        """
        inv_freq = self.inv_freq.to(device=row_ids.device)
        max_pos = int(torch.maximum(row_ids.max(), col_ids.max()).item()) + 1
        positions = torch.arange(max_pos, device=row_ids.device, dtype=inv_freq.dtype)
        freq_table = torch.outer(positions, inv_freq)
...
class QwenPatchMerger(nn.Module):
    """Spatial 2×2 patch merger for Qwen VL.
...
class QwenPatchEmbedding(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_size: int, patch_dim: int, temporal_patch_size: int):
        super().__init__()
        self.proj = nn.Conv3d(
...
class QwenVLViTModel(MegatronModule):
    ...
    def __init__(
        ...
        self.patch_embed = QwenPatchEmbedding(in_channels, hidden_size, patch_dim, temporal_patch_size)
    ...
    def forward(
        ...
        dynamic_resolution = pixel_values.dim() == 3
        if dynamic_resolution:
            assert imgs_sizes is not None, "imgs_sizes is required for dynamic-resolution QwenVLViTModel"
            native_patch_order = (
            ...
            x = pixel_values.unfold(2, self.patch_dim, self.patch_dim).unfold(3, self.patch_dim, self.patch_dim)
            # x: (B, C, h_p, w_p, patch_dim, patch_dim)
            x = x.permute(0, 2, 3, 1, 4, 5).contiguous()                     # (B, h_p, w_p, C, p, p)
            ...
            x = self.patch_embed.proj(x.to(self.patch_embed.proj.weight.dtype))  # (N, hidden, 1, 1, 1)
...
class Kimi2DRotaryEmbedding(nn.Module):
    ...
    def __init__(self, head_dim: int, max_patches_per_side: int, rope_theta: float = 10000.0):
        ...
        inv_freq = 1.0 / (rope_theta ** (torch.arange(0, head_dim, 4).float() / head_dim))  # (head_dim//4,)
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self.head_dim = head_dim
        self.max_patches_per_side = max_patches_per_side
...
class KimiPatchMerger(nn.Module):
    """Spatial 2×2 mean-pooling patch merger for Kimi.

    For a static image (T=1): averages the temporal dimension (no-op) then
    groups 2×2 spatial patches. Output: (B, h_out * w_out, 4 * hidden).
    Matches tpool_patch_merger for T=1.
    """
...
class KimiViTModel(MegatronModule):
    ...
    5. Final LayerNorm
    6. 2×2 spatial mean-pool merger
    ...
    def forward(
        ...
        dynamic_resolution = pixel_values.dim() == 3
        if dynamic_resolution:
            assert imgs_sizes is not None, "imgs_sizes is required for dynamic-resolution KimiViTModel"
            patch_hw, seq_lens = _dynamic_patch_grid(imgs_sizes, self.patch_dim, device)
            x = self.patch_embed.forward_patches(pixel_values)
            chunks = torch.split(x, seq_lens.tolist(), dim=1)
        else:
        ...
        if dynamic_resolution:
            ...
            x = torch.cat(
                [
                    self.merger(chunk, int(h), int(w)).reshape(chunk.shape[0], -1, 4 * chunk.shape[-1])
                    for chunk, (h, w) in zip(chunks, patch_hw.tolist())
                ],
                dim=1,
            )
