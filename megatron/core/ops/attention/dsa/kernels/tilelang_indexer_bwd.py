...
INT32 = T.int32 if HAVE_TILELANG else None
...
def is_supported_indexer_bwd_head_count(heads: int) -> bool:
    """Return whether TileLang indexer backward supports this indexer head count."""
    return heads <= 64 and heads % 8 == 0
...
@tilelang_jit(pass_configs=pass_configs)
def tl_indexer_bwd_impl(  # pragma: no cover
    heads: int,
    dim: int,
    topk: int,
    block_I: int = 32,
        ...
        with T.Kernel(q_seq_len, threads=num_threads) as (bx):
            ...
            for bi_i in T.serial(num_blocks):
                ...
                for i, j in T.Parallel(block_I, heads):
                    d_weights_i[i, j] = grad[i] * (
                        T.max(logits[i, j], 0) if use_relu else logits[i, j]
                    )
...
def indexer_bwd_interface(  # pragma: no cover
    ...
    weights_kernel = weights.to(dtype=torch.float32).contiguous()
    ...
    return grad_q, grad_w, grad_k.to(index_k.dtype)
