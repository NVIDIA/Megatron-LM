...
def _build_megatron_fsdp_v2_muon_optimizer(
    ...
    Routes 2D matrix params to the single-root :class:`FullyShardV2Muon` and
    every other parameter to the standard FSDP Adam, combined via
    :class:`ChainedOptimizer`. Called only from ``get_megatron_optimizer`` when
    ``optimizer='muon'`` and Megatron-FSDP is enabled. The Muon/Adam split reuses
    the same ``_is_nonlinear_or_embedding`` config override as the non-FSDP path.
    """
    ...
    from .fully_shard_v2_muon import FullyShardV2MuonChainedOptimizer, FullyShardV2MuonOptimizer
    ...
    if len(optimizers) == 1:
        return optimizers[0]
    return FullyShardV2MuonChainedOptimizer(optimizers)
...
def get_megatron_optimizer(
    ...
    if ddp_config.use_megatron_fsdp:
        ...
        # Muon: route matrices -> FullyShardV2Muon and
        ...
        if config.optimizer not in ('adam', 'sgd'):
            raise ValueError(
                "Megatron-FSDP supports optimizer='muon' as its only emerging optimizer, "
