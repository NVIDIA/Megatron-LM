...
class OptimizerStateOffloader:
    ...
    PRIMARY_WEIGHT_KEY = 'master_param'

    def __init__(self, distrib_optimizer: "DistributedOptimizer"):
        """
        Args:
            distrib_optimizer: The DistributedOptimizer to offload states and primary weights from.
        """
        ...
        # Check if primary weights are stored in adam optimizer state
        self.optimizer_contains_primary_weights = self.adam_optimizer.master_weights
        ...
        # CPU buffers for mcore primary weights, matching the structure of source groups
        ...
        self._offloaded_mcore_primary_weights = False
        ...
        # Primary weights (shard_fp32_from_float16_groups) are available from the start.
    ...
    def _get_state_keys_to_offload(
        self, offload_optimizer_states: bool, offload_primary_weights: bool
    ) -> Tuple[str, ...]:
        ...
        if self._optimizer_states_initialized:
            ...
            if offload_primary_weights and self.optimizer_contains_primary_weights:
                keys.append(self.PRIMARY_WEIGHT_KEY)
        return tuple(keys)
    ...
    def _offload_states(
        ...
        offload_primary_weights: bool,
        use_pin_memory: bool = True,
    ):
        """Offload optimizer states and/or primary weights to CPU."""
        # Offload states from adam optimizer
        self._offloaded_state_keys = self._get_state_keys_to_offload(
            offload_optimizer_states, offload_primary_weights
        )
        ...
        # Offload mcore primary weights if not in optimizer state
        if offload_primary_weights and not self.optimizer_contains_primary_weights:
            self._offload_shard_groups(
                self.dist_optimizer.shard_fp32_from_float16_groups,
                self._shard_fp32_from_float16_cpu_buffers,
                use_pin_memory,
            )
            self._offloaded_mcore_primary_weights = True
    ...
    def _release_states(self):
        ...
        if self._offloaded_mcore_primary_weights:
    ...
    def _reload_states(self, is_allocate_stage: bool):
        """
        Reload optimizer states and/or primary weights from CPU to GPU.

        If is_allocate_stage is True, only allocate GPU memory for the states and primary weights,
        ...
        # Reload mcore primary weights if not in optimizer state
        if self._offloaded_mcore_primary_weights:
    ...
    def offload(self, offload_optimizer_states: bool = True, offload_primary_weights: bool = True):
        """
        Offload optimizer states and/or primary weights to CPU.
        ...
        Args:
            ...
            offload_primary_weights: Whether to offload primary weights.
        """
        ...
        with torch.cuda.stream(self._d2h_stream):
            self._offload_states(offload_optimizer_states, offload_primary_weights)

        self._offloaded = True

    def release_gpu_memory(self):
        """
        Release GPU memory for optimizer states and primary weights after D2H copy completes.

        This is separated from offload() to allow delayed GPU memory release,
        which is needed for mxfp8 + overlap_param_gather case where primary weights
