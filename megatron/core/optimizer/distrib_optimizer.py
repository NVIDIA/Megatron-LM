    ...
    def state_dict(self):
        ...
        if not HAVE_APEX_OR_TE:
            ...
            for group_idx, param_group in enumerate(inner_state_dict["param_groups"]):
                ...
                if not steps:
                    continue
                current_step = next(iter(steps))
                group_steps[group_idx] = current_step
                # Groups holding no local parameters report nothing and are skipped above,
                # so only groups that actually advanced a clock take part in this check.
                ...
                assert known_step == current_step, (
                    "Optimizer steps differ within the same DSA parameter bucket: "
                    f"is_dsa_indexer={bucket}, group={group_idx}, steps="
