...
class MegatronGradScaler(ABC):
    ...
    @property
    def inv_scale(self):
        """Return the inverse of the current scale value."""
        return self._scale.double().reciprocal().float()

    @abstractmethod
    def update(self, found_inf: bool):
        """Update the scale based on whether inf was found."""
...
class DynamicGradScaler(MegatronGradScaler):
    ...
    def __init__(
        ...
        Args:
            ...
            growth_factor (float): Factor to grow loss scale by
                if NaNs are not seen in `growth_interval`
                training iterations. Must be greater than 1.
            backoff_factor (float): Factor to decrease loss scale
                by if NaNs are seen in `hysteresis` consecutive
                training iterations. Must be between 0 and 1.
