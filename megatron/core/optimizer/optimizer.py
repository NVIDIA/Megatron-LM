...
class MixedPrecisionOptimizer(MegatronOptimizer):
    ...
    def step_with_ready_grads(self) -> bool:
        ...
        if not self.is_stub_optimizer:
            ...
            # param buffer, which only DistributedOptimizer owns. Non-DistOpt LayerWise base
            # optimizers (Float16, which in this tree also carries a
            # ``_copy_main_params_to_param_buffer`` shim) are tagged with
            # ``_layer_wise_non_distopt_child`` so they instead copy master -> model params
            # below; a ``hasattr`` probe cannot distinguish them here.
            if (
