...
def pad_sequence_for_thd(
    ...
    Notes:
        ...
        - With context parallelism, a dummy tail must be divisible by twice the
          effective CP size, matching TE's THD partitioning requirement.
    ...
    if has_dummy_padding_seq:
        ...
        if cp_size > 1:
            assert dummy_seq_len % (2 * cp_size) == 0, (
                ...
                f"2 * context_parallel_size ({2 * cp_size}). Choose an even CP-local "
                "padding target/alignment."
            )
