...
class BridgeCommunicator:
    ...
    def __init__(self, src_grid: HyperCommGrid, dest_grid: HyperCommGrid):
        ...
        self.src_tp_leaders, self.src_local_leader_rank = self.get_leader_rank(self.src_grid, is_src=True)
        self.dest_tp_leaders, self.dest_local_leader_rank = self.get_leader_rank(self.dest_grid, is_src=False)
        ...
        if is_src:
            ...
            for group in per_dp_replica_ranks:
                if self.current_rank in group:
                    assert local_leader_rank is None, "only one local leader rank is allowed per dp replica"
    ...
    def _create_activation_gather_scatter_pg(self, grid: HyperCommGrid, is_src: bool):
        ...
        if self.current_rank < grid.rank_offset or self.current_rank >= grid.rank_offset + grid.size:
            return [], None
    ...
    def send_forward(self, tensor_to_send: torch.Tensor):
        ...
        if not self.is_current_rank_in_grid(self.src_grid):
            raise ValueError(f"Rank {self.current_rank} is not in the source grid. Send forward is only allowed on src grid")
        ...
        if rank_info.role == 'SENDER':
            ...
            gathered_tensors = [torch.zeros_like(tensor_to_send) for _ in range(len(self.activation_gather_ranks))]
            dist.gather(tensor_to_send, gather_list=gathered_tensors, dst=self.src_local_leader_rank, group=self.activation_gather_pg)
            ...
            if num_sends > 0:
                ...
                for i, send_op in enumerate(rank_info.sends):
                    ...
                    print(f"rank {self.current_rank} sending tensor to dst rank {send_op.destination_rank} shape {tensor_split.shape} sum {tensor_split.sum()}")
            ...
            dist.gather(tensor_to_send, gather_list=None, dst=self.src_local_leader_rank, group=self.activation_gather_pg)
    ...
    def receive_forward(self, tensor_shape: Optional[Tuple[int, ...]] = None, 
        ...
        if not self.is_current_rank_in_grid(self.dest_grid):
            raise ValueError(f"Rank {self.current_rank} is not in the destination grid. Receive forward is only allowed on dest grid")
        
        rank_info = self.comm_map.get(self.current_rank)
        assert rank_info is not None, f"Rank {self.current_rank} is not in the comm map"
        
        if rank_info.role == 'RECEIVER':
            assert self.current_rank == self.dest_local_leader_rank, f"Rank {self.current_rank} is not the leader rank"
            # p2p call to receive the tensor

            received_tensors_list = []
            for each_receive_op in rank_info.receives:
                tensor_to_recv = torch.empty(tensor_shape, device=torch.cuda.current_device(), dtype=dtype)
                dist.recv(tensor_to_recv, src=each_receive_op.source_rank)
                print(f"rank {self.current_rank} received tensor from src rank {each_receive_op.source_rank} shape {tensor_to_recv.shape} sum {tensor_to_recv.sum()}")
                received_tensors_list.append(tensor_to_recv)
            
            # TODO: ykarnati - naive concat for now - FIXME
            aggregated_tensor = torch.cat(received_tensors_list, dim=0)
            print(f"rank {self.current_rank} aggregated tensor shape {aggregated_tensor.shape} sum {aggregated_tensor.sum()}")
            
            scatter_list = list(torch.chunk(aggregated_tensor, chunks=len(self.activation_scatter_ranks), dim=0))
            received_tensor = torch.empty_like(scatter_list[0])
            print('*'*100)
            print(f"rank {self.current_rank} scatter list shape {[x.shape for x in scatter_list]} doing the scatter in rank group {dist.get_process_group_ranks(self.activation_scatter_pg)}")
            print('*'*100)
            dist.scatter(received_tensor, scatter_list=scatter_list, src=self.dest_local_leader_rank, group=self.activation_scatter_pg)
            ...
            received_tensor = torch.empty(tuple(scatter_shape), device=torch.cuda.current_device(), dtype=dtype)
            print(f"rank {self.current_rank} doing the scatter with received tensor shape {received_tensor.shape} in rank group {dist.get_process_group_ranks(self.activation_scatter_pg)}")
            dist.scatter(received_tensor, scatter_list=None, src=self.dest_local_leader_rank, group=self.activation_scatter_pg)
            print(f"current rank {self.current_rank} received tensor shape {received_tensor.shape}")
...
if __name__ == "__main__":
    ...
    if bridge_communicator.is_current_rank_in_grid(bridge_communicator.src_grid):
        ...
        bridge_communicator.receive_forward(tensor_shape=(64, 256, 1024), dtype=random_hidden_state.dtype)
