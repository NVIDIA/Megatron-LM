...
class ChunkOffloadHandler(AsyncDoubleBufferGroupOffloadHandler):
            ...
            for tensor_tag, state in self._tensor_tag_to_state.items():
                ...
                if group_id == group_to_offload:
                    ...
                    if torch.distributed.get_rank() == DEBUG_RANK and DEBUG:
                        ...
                        print("hasattr(tensor_on_device, 'offloading_activation')", hasattr(tensor_on_device, 'offloading_activation'))
    ...
    def on_group_commit_forward(self, release_tensors):
        ...
        self._offloaded_group_per_layer = self._offloaded_group_index // self._num_layers
        if torch.distributed.get_rank() == DEBUG_RANK and DEBUG:
    ...
    def backward_sync(self):
        ...
        if self._offloaded_group_index % self._offloaded_group_per_layer == 0:
    ...
    def on_group_start_forward(self):
        self._offloaded_group_index = self._offloaded_group_index + 1
        self._tensor_count_current_group = 0
        self._groups_to_offload.append(self._offloaded_group_index)
...
class GroupStartFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, tensor, cpu_offload_handler, is_last_layer):
        # pylint: disable=missing-function-docstring
        ctx.cpu_offload_handler = cpu_offload_handler
        if torch.distributed.get_rank() == DEBUG_RANK and DEBUG:
            print("GroupStartFunction forward", is_last_layer)

        ctx.cpu_offload_handler.is_last_layer = is_last_layer
        cpu_offload_handler.on_group_start_forward()
        # return the identical tensor
        return tensor
...
def group_prefetch_offload_start(tensor, is_last_layer=False):
    cur_forward_chunk = PipelineOffloadManager.get_instance().cur_forward_chunk()
    return GroupStartFunction.apply(tensor, cur_forward_chunk, is_last_layer)
