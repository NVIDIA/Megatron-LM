...
def wrap_hybrid_cp_dataloader(
    ...
    def _reroute_samples_to_hdp_ranks(
        ...
        for key in data_keys:
            output_split_sizes, input_split_sizes, tensor_size = (
                (recv_counts, send_num_split, len(recv_lens_split))
                if key in ["original_seq_len"]
                else (recv_lens_split, send_lens_split, sum(recv_lens_split))
            )
            send_tensor = _pack_sample_by_key(key)
            recv_tensor = torch.empty(
                tensor_size, device=torch.cuda.current_device(), dtype=send_tensor.dtype
            )
...
class NaiveSequencePackingScheduler:
    """
