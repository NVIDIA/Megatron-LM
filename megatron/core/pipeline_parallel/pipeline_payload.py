...
class PipelinePayloadSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.metadata, tuple) or any(
            type(v) is not int for v in self.metadata
        ):
            raise ValueError(
                "Pipeline payload metadata must be a tuple of host integers"
            )
        self.schema
    ...
    def fingerprint(self) -> int:
        ...
        return (
            int.from_bytes(sha256(repr(value).encode("utf-8")).digest()[:8], "big") >> 1
        )
...
class PipelinePayloadPlan:
    ...
    def __post_init__(self) -> None:
        if len(self.incoming) != len(self.outgoing):
            raise ValueError(
                "Pipeline input/output plans must cover the same microbatches"
            )
...
# Factories and tensor_specs inspect host descriptors before the receive completes.
# The communicator waits for data before the model consumes the payload.
PipelinePayloadFactory = Callable[
    [tuple[torch.Tensor, ...], PipelinePayloadSpec], PipelinePayload
]
...
def backward_pipeline_payload(
    ...
    input_specs = (
        ...
        else tuple(s for s in input_payload.tensor_specs if s.present)
    )
    ...
    if isinstance(output, PipelinePayload):
        ...
        specs = tuple(
            s
            for s in (output.tensor_specs if state is None else state.specs)
            if s.present
        )
