...
class TypedP2PCommunicator(P2PCommunicator):
    ...
    def __init__(
        ...
        make_payload: PipelinePayloadFactory,
        *,
        payload_plan: PipelinePayloadPlan,
        ...
        if (
            ...
            raise ValueError(
                "Typed boundaries require eager PP without VPP, overlap, TP, CP, recompute or graphs"
            )
        self.make_payloads = [make_payload]
        ...
        self.payload_plans = [payload_plan]
        self._sent = [deque()]
        self._received = [deque()]
        self._send_microbatch = [0]
        self._recv_microbatch = [0]
    ...
    def _validate_plan_peers(self):
        ...
        ordered = [
            plans[rank][chunk] for chunk in range(count) for rank in range(len(plans))
        ]
    ...
    def _allocate_payload(self, descriptor, chunk_id, microbatch=0):
        ...
        if payload.descriptor != descriptor:
            raise ValueError(
                "Received schema or gradient slots disagree with the model payload"
            )
        if not self.forward_only:
            self._received[chunk_id].append(
                _MessageRecord(descriptor, microbatch, chunk_id)
            )
    ...
    def _validate_planned_output(self, output, chunk_id, microbatch):
        ...
        if (
            ...
            raise ValueError(
                "Outgoing pipeline payload disagrees with the prepared metadata"
            )
    ...
    def _exchange(
        ...
        *,
        ...
        if output is not None:
            ...
            microbatch = self._send_microbatch[0]
            self._validate_planned_output(output, 0, microbatch)
            record = _MessageRecord(descriptor, microbatch, 0)
            forward_sends = [
                torch.tensor(record.identity(), dtype=torch.int64, device=self.device),
                *output.tensors,
            ]
            self._send_microbatch[0] += 1
            if not self.forward_only:
                self._sent[0].append(record)
        if gradients is not None:
            record = self._received[0].popleft()
        ...
        if recv_backward:
            ...
            backward_message = PipelineGradientMessage(
                tensors, control, identity, indices
            )
            backward_receives = [control, *(tensors[i] for i in indices)]
        if recv_forward:
            microbatch = self._recv_microbatch[0]
            self._recv_microbatch[0] += 1
            descriptor = self._planned_payload(0, microbatch, outgoing=False)
            received, tensors = self._allocate_payload(descriptor, 0, microbatch)
            identity = _MessageRecord(descriptor, microbatch, 0).identity()
    ...
    def recv_forward(
        self, tensor_shapes: object, is_first_stage: bool
    ...
    def send_forward(
        self, output_tensors: PipelinePayload, is_last_stage: bool
    ) -> None:
    ...
    def recv_backward(
        ...
        if not is_last_stage:
            return self._exchange(
                recv_backward=True, backward_recv_chunk_id=recv_chunk_id
            )[1]

    def send_backward(
        self, input_tensor_grads: PipelineGradients, is_first_stage: bool
    ) -> None:
    ...
    def finish(self) -> None:
        ...
        if self.payload_plans is not None:
            for chunk, plan in enumerate(self.payload_plans):
                ...
                ) or self._recv_microbatch[chunk] != sum(
                    s is not None for s in plan.incoming
                ):
                    raise RuntimeError(
                        "Typed pipeline schedule did not consume its payload plan"
                    )
