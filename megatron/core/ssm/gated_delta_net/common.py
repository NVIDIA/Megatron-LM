...
class _GDNBase(MegatronModule):
    ...
    def _prepare_input_for_gated_delta_rule(
        ...
        *gate_feats: tuple[torch.Tensor],
        ...
        # use_qk_l2norm_in_kernel=self.use_qk_l2norm (see GatedDeltaNet.forward()).
        # The kernel saves only rstd for backward and recomputes normalized q/k,
        # avoiding a materialized normalized query_key activation. Both the FLA
        # kernel and the torch deterministic path use eps=1e-6, matching the
        # previous explicit l2norm() default -> numerically lossless.
