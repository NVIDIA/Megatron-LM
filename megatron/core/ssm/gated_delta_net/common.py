    ...
    def _prepare_input_for_gated_delta_rule(
        ...
        *gate_feats: tuple[torch.Tensor],
        ...
        if self.use_qk_l2norm:
            ...
            # Measured on one node in one session: with the Triton kernel the deviation
            # appeared in 7 of 7 repeats (~1e-4, growing to 1.6e-02 over 20 iterations); with
            # the torch implementation, 0 of 14 across two nodes. The deviation reaches the
            # scan through q and k while v and beta stay bit-identical, which is the
            # signature of this step alone.
            query_key = query_key.contiguous()
            if self.config.deterministic_mode:
                qk32 = query_key.float()
                query_key = (
                    qk32 * torch.rsqrt(qk32.pow(2).sum(-1, keepdim=True).clamp(min=1e-12))
                ).to(query_key.dtype)
            else:
