...
class Symbols:
    ...
    PIPE = '|'
    MTP_SEPARATOR = "/"
    VALID_LAYERS = {MAMBA, ATTENTION, DS_ATTENTION, MLP, MOE}
...
def _validate_pattern(pattern: str, pattern_name: str, allow_pipe: bool = False) -> None:
    ...
    # Disallow Attention+DSA hybridity.
    if Symbols.ATTENTION in pattern and Symbols.DS_ATTENTION in pattern:
        raise ValueError("Not supported to have both Attention and DSA in one model")
