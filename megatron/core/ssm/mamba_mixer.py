...
class MambaMixer(MegatronModule):
        ...
        if in_inference_mode:
            if inference_context.is_dynamic_batching():
                ...
                if inference_context.seqlen_offset > 0:
                    ...
                    out, out_bias, _, _ = self.decode(hidden_states, conv_state, ssm_state)
                    return out, out_bias
    ...
    def decode(
        ...
        Args:
            ...
            conv_state (torch.Tensor): Conv state
            ssm_state (torch.Tensor): SSM state

        Returns:
            A 4-tuple of (output, bias, updated conv state, updated SSM state)
        """
    ...
    def ssm_prefill(self, zxBCdt: torch.Tensor) -> torch.Tensor:
    ...
    def ssm_decode(self, zxBCdt: torch.Tensor) -> torch.Tensor:
