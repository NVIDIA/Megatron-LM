...
def _y_scatter_kernel(
    src_ptr,  # (ws_chunks * L, H, P) scratch, token-major within a chunk
    ...
    c = slot // L
    i = slot % L
    token = tl.load(ws_token_base_ptr + c) + i
    if (token < tl.load(ws_valid_lo_ptr + c)) | (token >= tl.load(ws_valid_hi_ptr + c)):
...
def scatter_y_ragged(
    ...
    out: torch.Tensor,
    ws_token_base: torch.Tensor,
    ws_valid_lo: torch.Tensor,
    ws_valid_hi: torch.Tensor,
    kernel_chunk_size: int,
) -> None:
    """Copy real tokens from the expanded Y scratch into the caller's output.

    Args:
        y_scratch: ``(ws_chunks * L, H, P)`` kernel output, contiguous rows.
        out: ``(T, H, P)`` caller output, contiguous rows.
        ws_token_base: ``(ws_chunks,)`` first token of each workspace chunk.
        ws_valid_lo/hi: ``(ws_chunks,)`` real-token window of the owning sequence.
        kernel_chunk_size: ``L``.
    """
