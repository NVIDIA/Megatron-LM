...
should subclass this mixin and implement the two ``_ssm_*`` hooks rather than
...
class SSMDynamicInferenceMixin:
    ...
    def _ssm_prefill(
        ...
        ``seq_idx``, etc.) from ``context.mamba_metadata`` and use them to
    ...
    def ssm_dynamic_inference(
        ...
        conv_state, ssm_state = context.mamba_states_cache(
