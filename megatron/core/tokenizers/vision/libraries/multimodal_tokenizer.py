...
# These templates must remain byte-for-byte compatible with the model tokenizer configs.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if prompt_format == "mistral":
            ...
            self._prompt_config = PromptConfig(
                assistant_prefix_len=0,  # Not used.
                pad_token_id=tokenizer.convert_tokens_to_ids("<unk>"),
                custom_chat_template=None,
                has_bos=False,
                has_system_role=True,
        ...
        **kwargs,
        ...
        if self._prompt_format == "nemotron-h-reasoning":
            ...
            for i in range(1, len(idx)):
                if i % 2 == 0:
                    # Do not mask <SPECIAL_11>; it also terminates the previous
                    # assistant message.
            ...
            return tokens, target
        elif self._prompt_format == "nemotron-h-5p5-reasoning":
            idx = np.where(tokens == 11)[0]
            target[: idx[1]] = IGNORE_INDEX  # system prompt + initial user prompt
            ...
            if len(idx_system) > 0:
                ...
                # Skip the first system message because it is already masked above.
                for sys_pos in idx_system[1:]:
            ...
            target = np.full_like(tokens, IGNORE_INDEX)
            # Find the [10, 1503, 19464] pattern that starts assistant messages.
            ...
            if train_only_on_last_assistant_turn and has_nonempty_thinking_trace:
                # User messages start with [10, 3263]; tool-call user messages
                # start with [10, 3263, 1010, 16].
                ...
                if len(user_start_positions) > 0:
                    ...
                    is_tool_call[valid_tool_check] = (tokens[tool_check_positions + 2] == 1010) & (
