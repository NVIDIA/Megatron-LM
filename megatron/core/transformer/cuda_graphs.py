...
class TECudaGraphHelper:
    ...
    def __init__(
        ...
        # Constructor placement is intentional: initialize parent paths before the first real
        # PP/VPP warmup step. create_cudagraphs() repeats this immediately before capture so a
        # retry cannot encounter lazy NCCL setup.
            ...
            try:
                if self._reuse_parent_cp_transport:
                    self._warmup_dynamic_cp_communicators(
                        self._get_dynamic_cp_capture_contexts(), self.dp_cp_group
                    )
                else:
    ...
    def _should_use_dynamic_microbatch_slots(self) -> bool:
        """Whether to capture bounded physical graph slots for varying microbatch counts."""
    ...
    def create_cudagraphs(self):
        ...
        if getattr(self.config, 'cuda_graph_granularity', 'layer') == 'chunk' and getattr(
