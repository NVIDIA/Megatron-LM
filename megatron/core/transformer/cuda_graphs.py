...
def _get_local_dsa_metric_tracker_size(model=None):
    ...
    if model is None:
        # Legacy callers without a model can only infer the potential writer range from
        # storage populated by the eager step. Explicit models must not inherit storage from
        # an unrelated model that previously ran in this process.
...
def _prepare_dsa_metric_tracker_for_capture(model=None, pp_group=None):
    ...
    size_was_negotiated_from_model = model is not None
    if size_was_negotiated_from_model and torch.distributed.is_initialized():
    ...
    if size_was_negotiated_from_model and local_required_size == 0:
    ...
    if not size_was_negotiated_from_model and torch.distributed.is_initialized():
        size_tensor = torch.tensor(
            [local_required_size], device=torch.cuda.current_device(), dtype=torch.long
        )
        torch.distributed.all_reduce(size_tensor, op=torch.distributed.ReduceOp.MAX, group=pp_group)
...
class TECudaGraphHelper:
    ...
    def create_cudagraphs(self):
        ...
        if self._captures_dsa_metric_tracker and not self._graphs_created:
