...
class TECudaGraphHelper:
    ...
    def __init__(
        ...
        if self._should_share_dynamic_cp_pool():
            self._warm_dynamic_cp_parent_communicators(self._get_dynamic_cp_capture_contexts())
    ...
    def _should_warm_dynamic_cp_parent_p2p(self) -> bool:
        """Whether captured attention needs the parent P2P path initialized."""
    ...
    def _should_warm_dynamic_cp_parent_collective(self) -> bool:
        """Whether captured router reductions need the parent collective initialized."""
        graph_modules = getattr(self.config, 'cuda_graph_modules', ())
    ...
    @staticmethod
    def _warm_dynamic_cp_parent_p2p(capture_contexts, parent_group):
        """Initialize both ring directions on the parent P2P communicator."""
        ...
        for cp_size, cp_group in capture_contexts:
            ...
            # TE sends clockwise in forward and counter-clockwise in backward. Warm both paths
            # with TE's even-send/odd-recv ordering before NCCL operations enter graph capture.
            for direction in (1, -1):
                ...
                requests = torch.distributed.batch_isend_irecv(
                    [
                        torch.distributed.P2POp(op, tensor, peer, parent_group)
    ...
    @staticmethod
    def _warm_dynamic_cp_parent_collective(parent_group):
        """Initialize the parent communicator used by captured router reductions."""
    ...
    def _warm_dynamic_cp_parent_communicators(self, capture_contexts):
        """Prepare parent communication paths before CUDA graph capture."""
        if self._should_warm_dynamic_cp_parent_collective():
            self._warm_dynamic_cp_parent_collective(self.dp_cp_group)
        if self._should_warm_dynamic_cp_parent_p2p():
            self._warm_dynamic_cp_parent_p2p(capture_contexts, self.dp_cp_group)
    ...
    def create_cudagraphs(self):
        ...
        try:
            ...
            self._warm_dynamic_cp_parent_communicators(capture_contexts)
