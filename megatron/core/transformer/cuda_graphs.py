...
def _get_gtp_persistent_buffer_max_inflight(gtp_backward_runners):
    """Choose the number of live persistent arenas from the local graph scope."""
    local_graph_modules = set()
    for runner in gtp_backward_runners:
        config = getattr(runner.base_module, "config", None)
        local_graph_modules.update(getattr(config, "cuda_graph_modules", ()) or ())
    # The local implementation treats moe_router + moe_preprocess as one graph scope.
    local_graph_modules.discard(CudaGraphModule.moe_preprocess)
    num_local_graph_modules = max(1, len(local_graph_modules))
    # max_inflight controls overlap, not correctness: one arena generation is enough for
    # one configured local-CG module scope, while two covers normal multi-module scopes.
    # Larger values only allow more graphs to overlap if possible and increase persistent memory.
    return min(2, num_local_graph_modules)
...
class _CudagraphGlobalRecord:
    ...
    def _create_cudagraphs(cls):
        ...
        if gtp_active:
            ...
            plan = GraphPersistentBufferPlan.create(
                [runner.persistent_buffer_state for runner in gtp_backward_runners],
                max_inflight=_get_gtp_persistent_buffer_max_inflight(gtp_backward_runners),
            )
            CudaGraphManager.persistent_buffer_plan = plan
        ...
        for g_idx, g in progress_bar:
            ...
            if graph_type == 'fwd':
                ...
                state = runner.persistent_buffer_state if runner.gtp_remat else None
                capture_context = (
                    use_graph_persistent_buffer_state(state) if state is not None else nullcontext()
                )
...
def delete_cuda_graphs():
    ...
    for record in [
        ...
        *_CudagraphGlobalRecord.cudagraph_inference_record,
        ...
        runner.mempool = None
        runner.persistent_buffer_state.clear()
    ...
    if CudaGraphManager.persistent_buffer_plan is not None:
        CudaGraphManager.persistent_buffer_plan.clear()
    CudaGraphManager.persistent_buffer_plan = None
    set_graph_persistent_buffer_discovery(None)
...
class _CudagraphRecordNode(torch.autograd.Function):
    ...
    def backward(ctx, grads):
        ...
        if not runner.bwd_graph_recorded:
            if runner.gtp_remat:
                runner.persistent_buffer_state.start_discovery()
                set_graph_persistent_buffer_discovery(runner.persistent_buffer_state)
                # Record nodes switch discovery state as backward crosses runner boundaries. If
                # the last record node is a GTP runner, no later node clears its state. Clear any
                # residual state after the complete autograd backward, before gradient
                # finalization or a later backward can issue unrelated requests.
                torch.autograd.Variable._execution_engine.queue_callback(
                    lambda: set_graph_persistent_buffer_discovery(None)
                )
            else:
                set_graph_persistent_buffer_discovery(None)
...
class _CudagraphReplayNode(torch.autograd.Function):
    ...
    def backward(ctx, *grads):
        ...
        if runner.use_stream:
            runner.stream.wait_stream(torch.cuda.current_stream())
            if state is not None and state.needs_replay_fence:
                state.wait_for_reuse(runner.stream)
            with torch.cuda.stream(runner.stream):
                runner.bwd_graph.replay()
                if state is not None and state.needs_replay_fence:
                    state.mark_reusable_after(runner.stream)
...
class _CudaGraphRunner(torch.nn.Module):
    ...
    def __init__(
        ...
        self.persistent_buffer_state = GraphPersistentBufferState()
