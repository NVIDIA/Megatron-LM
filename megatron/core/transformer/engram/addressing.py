...
def find_next_prime(start: int, seen_primes: set) -> int:
    """Find the smallest prime above start that is absent from seen_primes."""
    candidate = start + 1
    while True:
...
class NgramHashMapping(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        tokenizer_vocab_size = len(self.compressed_tokenizer)
        if pad_id is None:
            self.pad_id = None
        else:
        ...
        half_bound = max(1, int(max_long // tokenizer_vocab_size) // 2)
        for layer_id in self.layer_ids:
            # The NumPy generator and layer seed define persistent hash multipliers.
    ...
    def _resolve_hash_moduli_by_layer(self) -> Dict[int, list]:
    ...
    def forward(self, input_ids: Tensor, layer_id: int) -> Tensor:
        """Compress raw tokens and return layer-specific n-gram addresses."""
        return self.forward_compressed(self.compressed_tokenizer(input_ids), layer_id)

    def forward_compressed(self, compressed_input_ids: Tensor, layer_id: int) -> Tensor:
        """Hash already-compressed token IDs into table addresses."""
