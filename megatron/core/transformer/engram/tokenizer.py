...
_LOOKUP_CACHE = WeakKeyDictionary()


def get_engram_tokenizer_pad_id(tokenizer, configured_pad_id: Optional[int] = None) -> int:
...
def _unwrap_engram_tokenizer(tokenizer):
...
def build_engram_tokenizer_lookup(tokenizer) -> torch.Tensor:
...
class CompressedTokenizer(nn.Module):
    ...
    def __init__(self, lookup_table: Union[Tensor, Sequence[int]]):
        super().__init__()
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        """Map valid raw token IDs while preserving negative padding sentinels."""
