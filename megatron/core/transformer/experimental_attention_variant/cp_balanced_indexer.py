...
def _use_zigzag(multi_seq, cp_size, l_local, use_fused, config):
    ...
    if os.environ.get("MCORE_DSA_CP_BAL_DEBUG") == "1" and not getattr(
        ...
        print(
            f"[zz-gate] verdict={verdict} multi_seq={multi_seq} "
            f"S={cp_size * l_local} use_fused={use_fused} "
            f"pad={getattr(config, 'pad_packed_seq_alignment', None)!r} "
            f"env={os.environ.get('MCORE_DSA_CP_BAL_PACK_SCOPE')!r}",
            file=sys.stderr,
            flush=True,
        )
    return verdict
