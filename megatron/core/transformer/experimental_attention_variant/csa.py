    ...
    * ``ratio == 128``: window + 128x compressed, attend to all (compressor built only)
    ...
    def _forward_fused_sbhd(
        ...
        if has_indexer_compressed and self.training and torch.is_grad_enabled():
            ...
            topk_indices_compressed, _ = indexer_topk(
                q_indexer,
                k_indexer,
                weights_indexer,
                self.indexer.index_topk,
            ...
            compress_topk_idxs = torch.where(
                topk_indices_compressed >= 0, topk_indices_compressed + offset, -1
            )
            flat_idxs, flat_topk_length = build_flat_topk_idxs(
