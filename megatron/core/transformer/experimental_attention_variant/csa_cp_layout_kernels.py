...
if _CUTE_AVAILABLE:
            ...
            for seq in range(n_seq):
                ...
                if local_seq_start < local_seq_end:
                    ...
                    if slot >= running and slot < running + visible_group_count:
                        ...
                        valid[slot] = group_end - 1 >= range_start and group_end - 1 < range_end
                    running = running + visible_group_count

        if row < n_seq + 1 and tidx == 1:
            upto = row
            running = 0
            for seq in range(n_seq):
                if seq < upto:
                    ...
                    if local_seq_start < local_seq_end:
                        ...
                        if visible_group_count < 0:
                            visible_group_count = 0
                        running = running + visible_group_count * ratio
            cu_compact[row] = running
    ...
    def _thd_full_kv_pack_kernel(
        ...
        if out_row < total_work:
            source_kind = cutlass.Int32(-1)
            source_index = cutlass.Int32(0)
            resolve_row = tidx == 0 or tidx == 32

            if resolve_row and chunk_count != 0:
                        ...
                        if out_row >= offset and out_row < offset + seq_total:
                            ...
                            if inner < window_len:
                                ...
                                if use_seq_major_map != 0:
                                    ...
                                    # Slow fallback for no-indexer paths: linear scan is
                                    # intentional and not expected to be the hot training path.
            ...
            source_kind = cute.arch.shuffle_sync(source_kind, 0, mask=-1, mask_and_clamp=31)
            source_index = cute.arch.shuffle_sync(source_index, 0, mask=-1, mask_and_clamp=31)
            ...
            while vec_col < vec_cols:
                ...
                if direction == 0:
                    if cutlass.const_expr(kv_full.element_type.width == 16 and row_width % 4 == 0):
                        dst_offset = out_row * row_width + vec_col * vec_elems
                        value = cutlass.Int64(0)
                        if source_kind == 0:
                            src_offset = source_index * row_width + vec_col * vec_elems
                            src_ptr = cute.recast_ptr(
                                kv_local.iterator + src_offset, dtype=cutlass.Int64
                            )
                            value = cute.arch.load(
                                src_ptr.llvm_ptr,
                                cutlass.Int64,
                            ...
                            src_offset = source_index * row_width + vec_col * vec_elems
                            src_ptr = cute.recast_ptr(
                                boundary_kv.iterator + src_offset, dtype=cutlass.Int64
                            )
                            value = cute.arch.load(
                                src_ptr.llvm_ptr,
                                cutlass.Int64,
    ...
    def _repack_compressed_kv_init_rank_map_kernel(
        rank_major_by_seq_major: cute.Tensor, seq_major_rows: cutlass.Int32
    ):
        tidx, _, _ = cute.arch.thread_idx()
        bidx, _, _ = cute.arch.block_idx()
        linear = bidx * 128 + tidx
    ...
    def _repack_compressed_kv_rank_map_kernel(
        ...
        if rank_row < rank_major_rows and valid_rank_major[rank_row] == 1:
            ...
            if seq_major >= 0 and seq_major < seq_major_rows:
                ...
                cute.arch.atomic_max(rank_map_ptr.llvm_ptr, rank_row, sem="relaxed", scope="gpu")
    ...
    # Kernel contract:
    #   Same inputs as _build_attention_indices_kernel, but emits compressed-first
    #   indexer-loss order.
...
class ThdCompressedRope(torch.autograd.Function):
    ...
    def forward(
        ...
        """Apply compressed-row RoPE and save metadata for backward."""
    ...
    @staticmethod
    def backward(ctx, grad_output: torch.Tensor):
        """Apply the adjoint compressed-row RoPE transform to gradients."""
...
class CompressorInputCompact(torch.autograd.Function):
    ...
    def _forward_kernel(
        ...
        total_work = max(compact_len, c_cap, cu_seqlens.shape[0])
        _run_compiled_launch(
...
class ThdFullKvPack(torch.autograd.Function):
    ...
    def forward(
        ...
        total_work = int(kv_full_capacity)
        use_seq_major_map = (
            int(chunk_count) == 0
            and ratio > 1
        ...
        _run_compiled_launch(
            ...
            key_arg_indices=(5, 8, 9, 11, 14, 15),
            static_arg_indices=(5, 8, 9, 11, 14, 15),
        )
