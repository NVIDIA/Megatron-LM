...
def _compute_dense_attn_lse_thd(
    q_attn_thd: Tensor,
    k_attn_thd: Tensor,
    softmax_scale: float,
    qhead_per_kv_head: int,
    ...
    """Compute full-KV attention LSE for flattened THD sequences."""
    if q_attn_thd.ndim != 3 or k_attn_thd.ndim != 3:
        raise RuntimeError(
            "THD attention Q/K must be 3-D, got "
            f"q.ndim={q_attn_thd.ndim} and k.ndim={k_attn_thd.ndim}."
        )
    if cu_seqlens_q.ndim != 1 or cu_seqlens_kv.ndim != 1:
        raise RuntimeError("THD cu_seqlens_q/kv must be 1-D.")
    if cu_seqlens_q.shape != cu_seqlens_kv.shape:
        raise RuntimeError(
            f"THD cu_seqlens shapes must match, got {tuple(cu_seqlens_q.shape)} "
            f"and {tuple(cu_seqlens_kv.shape)}."
        )
    if q_causal_offsets is not None and (
        q_causal_offsets.ndim != 1 or q_causal_offsets.numel() != cu_seqlens_q.numel() - 1
    ):
        raise RuntimeError(
            "q_causal_offsets must have one entry per THD segment, got "
            f"shape={tuple(q_causal_offsets.shape)} for {cu_seqlens_q.numel() - 1} segments."
        )

    total_q, num_heads, head_dim = q_attn_thd.shape
    total_k, num_kv_heads, k_head_dim = k_attn_thd.shape
    if head_dim != k_head_dim:
        raise RuntimeError(
            f"attention Q/K head dimensions must match, got {head_dim} and {k_head_dim}."
        )
    if num_heads != num_kv_heads * qhead_per_kv_head:
        raise RuntimeError(
            "query-head count must equal key-value heads times qhead_per_kv_head, "
            f"got h_q={num_heads}, h_kv={num_kv_heads}, "
            f"qhead_per_kv_head={qhead_per_kv_head}."
        )
    if total_q == 0:
        return torch.empty((0, num_heads), device=q_attn_thd.device, dtype=torch.float32)
    if max_seqlen_kv <= 0 or total_k == 0:
        return torch.full(
            (total_q, num_heads), float("-inf"), device=q_attn_thd.device, dtype=torch.float32
        )

    row_batch_ids = batch_of_row(cu_seqlens_q, total_q=total_q)
    row_positions = torch.arange(total_q, device=q_attn_thd.device, dtype=torch.int64)
    row_positions = row_positions - cu_seqlens_q[row_batch_ids].to(torch.int64)
    if q_causal_offsets is not None:
        row_positions = row_positions + q_causal_offsets[row_batch_ids].to(torch.int64)
    kv_starts = cu_seqlens_kv[row_batch_ids].to(torch.int64)
    kv_lengths = (cu_seqlens_kv[1:] - cu_seqlens_kv[:-1])[row_batch_ids].to(torch.int64)
    valid_k_per_row = torch.minimum((row_positions + 1) // ratio, kv_lengths)
    key_positions = torch.arange(max_seqlen_kv, device=q_attn_thd.device, dtype=torch.int64)

    lse = torch.empty((total_q, num_heads), device=q_attn_thd.device, dtype=torch.float32)
    # Advanced indexing materializes one K vector per query row. Bound both
    # that buffer and the score tensor, rather than accounting for scores only.
    bytes_per_row = max_seqlen_kv * (head_dim + qhead_per_kv_head) * 4
    chunk_rows = min(total_q, max(1, _DENSE_ATTN_LSE_CHUNK_MAX_BYTES // max(1, bytes_per_row)))

    for kv_head in range(num_kv_heads):
        head_start = kv_head * qhead_per_kv_head
        head_end = head_start + qhead_per_kv_head
        for q_start in range(0, total_q, chunk_rows):
            q_end = min(q_start + chunk_rows, total_q)
            global_k = kv_starts[q_start:q_end].unsqueeze(1) + key_positions.unsqueeze(0)
            global_k = global_k.clamp(max=total_k - 1)
            k_group = k_attn_thd[global_k, kv_head, :].float()
            q_group = q_attn_thd[q_start:q_end, head_start:head_end, :].float()
            scores = torch.einsum("qhd,qkd->qhk", q_group, k_group) * softmax_scale
            valid = key_positions.unsqueeze(0) < valid_k_per_row[q_start:q_end].unsqueeze(1)
...
def _compute_dense_attn_lse(
    ...
    *,
    ...
    """Compute full-KV per-query-head attention LSE for BSHD or THD."""
    if (cu_seqlens_q is None) != (cu_seqlens_kv is None):
        raise ValueError(
            "cu_seqlens_q and cu_seqlens_kv must both be provided for THD, or "
            "both None for BSHD."
    ...
    if cu_seqlens_q is not None:
        if max_seqlen_kv is None:
            raise ValueError("max_seqlen_kv is required for THD dense attention LSE.")
        return _compute_dense_attn_lse_thd(
            q_attn_bshd,
            k_attn_bshd,
            softmax_scale,
            qhead_per_kv_head,
    ...
    b, sq, num_heads, _ = q_attn_bshd.shape
    kb, sk, num_kv_heads, _ = k_attn_bshd.shape
    if kb != b:
    ...
    seq_lens = ((q_positions + 1) // ratio).clamp(max=sk)
    score_bytes_per_row = b * qhead_per_kv_head * sk * torch.finfo(torch.float32).bits // 8
    chunk_rows = min(sq, max(1, _DENSE_ATTN_LSE_CHUNK_MAX_BYTES // max(1, score_bytes_per_row)))
    ...
    * **Dense loss** (``sparse_loss=False``, the default) — KL is
    ...
    def forward(
        ...
        if sparse_loss:
            ...
            dense_lse = _compute_dense_attn_lse(
                q_attn_det,
