...
class DSv4HybridAttention(Attention):
    ...
    def __init__(
        ...
        # Hybrid C/H/W symbols provide a fixed ratio through their ModuleSpec. The array-driven
        # D path falls back to the global per-layer ratio list.
    ...
    def forward(
        ...
        *,
        ...
        if packed_seq_params is not None and packed_seq_params.local_cp_size is not None:
            raise ValueError(
                "DSv4HybridAttention does not support per-microbatch context-parallel groups."
            )
            ...
            if self.config.apply_rope_fusion:
                ...
                if packed_seq and cp_size > 1:
                    ...
                    if packed_seq_params is None:
                        # Keep direct SBHD forward calls with shorter sequences aligned to
                        # their inputs. THD reuses the max-length table per packed segment.
