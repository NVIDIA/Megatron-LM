    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if ctx.needs_input_grad[1]:
            if (
                ...
                if (
                    getattr(linear, "fuse_wgrad_accumulation", False)
                    and getattr(weight, "main_grad", None) is not None
                ):
                    # Trigger the normal parameter hook without adding the gradient twice.
                    zero_dummy = getattr(weight, "zero_out_wgrad", False)
                    if get_dummy_wgrad is not None:
                        grad_weight = get_dummy_wgrad(
                            list(weight.shape), weight.dtype, zero=zero_dummy
                        )
                    elif zero_dummy:
