...
def compute_dsa_indexer_loss(
    ...
    Args:
        ...
        causal_mask_override: Optional [b, sq, sk] additive mask (with -inf for invalid
            positions and 0 for valid). When provided, used instead of the standard
            upper-triangular causal mask. Used by CSA where compressed KV positions do
            not follow the dense causal pattern.
    ...
    # Rows where every KV position is masked produce NaN from softmax(all -inf). Detect
    # and zero out their logits before softmax, then mask their contributions out.
...
def bwd_fused_indexer_loss_naive(
    ...
    # Detect rows where every KV is masked (early CSA query positions); zero those rows
    # before softmax to avoid NaN, then mask out the corresponding gradients afterwards.
    row_valid = (causal_mask > float('-inf')).any(dim=-1)
    del causal_mask
    if row_valid.dim() == 1:
        attn_row_mask = row_valid.view(1, 1, sq, 1)
