...
class FusedDSAIndexerLoss(torch.autograd.Function):
    ...
    Supports both SBHD (default) and THD packed-sequence layouts. THD is
    selected by passing ``cu_seqlens_q`` (and the corresponding
    ``cu_seqlens_compressed_idx`` + ``ratio``); those args follow the
    SBHD-only ``calculate_per_token_loss`` so existing SBHD callers remain
    source-compatible. The THD branch loops over segments and delegates each
    one to the SBHD naive helpers with ``b=1``.

    The ``varlen_starts`` / ``varlen_ends`` / ``key_positions`` /
    ``query_valid_rows`` arguments (from PR #5246) drive the SBHD
    packed/varlen masking path via ``dsa_masking`` and are appended after the
    THD arguments; ``use_relu`` selects DeepSeek V3.2 indexer scoring.
    """
    ...
    def forward(
        ...
        # Save for backward (recomputation strategy). ``mask`` is SBHD only;
        # THD rebuilds per-segment masks in the backward.
    ...
    def forward(
        ...
        def compute_indexer_loss_with_reference_path():
            ...
            return FusedDSAIndexerLoss.apply(
                ...
                None,  # cu_seqlens_q (SBHD path)
                None,  # cu_seqlens_compressed_idx
                None,  # ratio
                varlen_starts,
                varlen_ends,
                key_positions,
