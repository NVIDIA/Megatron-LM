...
# RoPE settings the indexer needs are not fields on TransformerConfig: the CLI already declares
# --rotary-base and friends by hand, and adding fields would make ArgumentGroupFactory generate a
# second flag for each. megatron/training maps args onto the config for real runs (see
# _apply_dsa_rope_config_from_args); these defaults cover configs built without it, such as tests.
_DSA_ROPE_DEFAULTS = {
    "rope_type": "rope",
    "rotary_base": 10000.0,
    "rotary_percent": 1.0,
...
def _dsa_rope_setting(config: TransformerConfig, name: str):
    """Read a RoPE setting the indexer needs, falling back to the model-wide default."""
    return getattr(config, name, _DSA_ROPE_DEFAULTS[name])
...
class SimplifiedDSGQAIndexerSubmodules:
    """Submodules of the simplified DSA indexer: its Q and optional learned-K projections."""
...
class DSGQAAttentionSubmodules:
    """Submodules of a DSA-over-GQA core attention: the indexer and the dense fallback."""
...
class SimplifiedDSGQAIndexer(MegatronModule):
    ...
    def __init__(
        ...
        self.index_rotary_dim = int(
            self.index_head_dim * _dsa_rope_setting(config, "rotary_percent")
        )
        ...
        if self.index_rotary_dim > 0:
            if _dsa_rope_setting(config, "rope_type") == 'rope':
                self.rotary_pos_emb = RotaryEmbedding(
                    self.index_rotary_dim,
                    rotary_percent=1.0,
                    rotary_interleaved=config.rotary_interleaved,
                    seq_len_interpolation_factor=_dsa_rope_setting(
                        config, "rotary_seq_len_interpolation_factor"
                    ),
                    rotary_base=_dsa_rope_setting(config, "rotary_base"),
                    rope_scaling=_dsa_rope_setting(config, "use_rope_scaling"),
                    rope_scaling_factor=_dsa_rope_setting(config, "rope_scaling_factor"),
                    use_cpu_initialization=config.use_cpu_initialization,
                    cp_group=self.pg_collection.cp,
                )
            elif _dsa_rope_setting(config, "rope_type") == 'yarn':
...
class _DSAZeroParamDependency(torch.autograd.Function):
    ...
    def forward(ctx, output: torch.Tensor, *params: torch.Tensor) -> torch.Tensor:
        """Pass the output through, recording the parameter shapes backward must produce."""
        ctx.param_metadata = tuple((param.shape, param.dtype, param.device) for param in params)
        return output

    @staticmethod
    def backward(ctx, grad_output: torch.Tensor):
        """Return the incoming gradient unchanged, plus a zero gradient per indexer parameter."""
        zero_grads = tuple(
...
class DSGQACoreAttention(MegatronModule):
    ...
    def forward(
        ...
        """Run DSA over the grouped-query Q/K/V, returning attention output and indexer loss."""
    ...
    def _forward_min_memory(
        ...
        if attn_mask_type != AttnMaskType.causal:
            raise NotImplementedError(
                f"dsa_min_memory_backend='{dsa_min_memory_backend}' only supports causal "
                "fixed-length batches."
            )
        if query.size(0) != key.size(0) or key.size(0) != hidden_states.size(0):
            raise NotImplementedError(
                f"dsa_min_memory_backend='{dsa_min_memory_backend}' requires full-sequence self "
                "attention."
            )
        if skip_dsa:
        ...
        if getattr(self.config, "dsa_sparse_attention_use_gather", False):
            raise NotImplementedError(
                f"dsa_min_memory_backend='{dsa_min_memory_backend}' bypasses the reference gather "
                "backend; "
        ...
        if not simplified_indexer and not getattr(self.config, "dsa_indexer_use_hadamard", False):
            raise NotImplementedError(
                f"dsa_min_memory_backend='{dsa_min_memory_backend}' requires "
                "dsa_indexer_use_hadamard."
            )
        if (
        ...
        if dense_warmup:
            ...
            if not self.training:
                raise NotImplementedError(
                    f"dsa_min_memory_backend='{dsa_min_memory_backend}' currently supports "
                    "training only."
                )
            ...
            if indexer_loss_coeff <= 0:
                raise NotImplementedError(
                    f"dsa_min_memory_backend='{dsa_min_memory_backend}' expects "
        ...
        if not self.training:
            raise NotImplementedError(
                f"dsa_min_memory_backend='{dsa_min_memory_backend}' currently supports training "
                "only."
            )
        ...
        if not train_main_only and configured_indexer_loss_coeff <= 0:
            raise NotImplementedError(
                f"dsa_min_memory_backend='{dsa_min_memory_backend}' expects dsa_indexer_loss_coeff "
                "> 0 "
