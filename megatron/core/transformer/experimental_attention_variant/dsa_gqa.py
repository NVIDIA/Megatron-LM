...
class DSGQACoreAttention(MegatronModule):
    ...
    def _forward_min_memory(
        ...
        if attn_mask_type != AttnMaskType.causal:
            raise NotImplementedError(
                f"dsa_kernel_backend='{dsa_kernel_backend}' only supports causal "
        ...
        if dense_warmup:
            ...
            if not self.training:
                raise NotImplementedError(
                    f"dsa_kernel_backend='{dsa_kernel_backend}' currently supports "
                    "training only."
                )
            ...
            if indexer_loss_coeff <= 0:
                raise NotImplementedError(
                    f"dsa_kernel_backend='{dsa_kernel_backend}' expects "
