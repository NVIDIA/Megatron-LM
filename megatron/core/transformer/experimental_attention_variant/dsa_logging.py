...
class DSAIndexerLossLoggingHelper:
    ...
    def reduce_loss_in_tracker(
        ...
        final_avg_group: Optional[torch.distributed.ProcessGroup] = None,
    ):
        ...
        Args:
            pg_collection: Backward-compatible process-group collection. Explicit groups
                below take precedence when supplied.
            num_layers: Total number of decoder layers; required to lazily initialize the
                tracker on ranks where no indexer layer ran.
            hybrid_context_parallel: Whether Hybrid CP scheduled variable-sized local CP
                groups. This path reconstructs the nominal static-CP normalization over the
                stable full data-distribution group.
            configured_cp_size: Configured/static CP width. Hybrid CP accumulates raw local
            ...
            final_avg_group: Full language-model data-distribution group (DP x CP x GTP-remat)
                used for the final metric average. Defaults to the collection's full group,
                then its DP x CP group, then its DP group for backward compatibility.
        """
        tracker = DSAIndexerLossLoggingHelper.tracker
        if pg_collection is not None:
            if pp_group is None:
                pp_group = pg_collection.pp
            if final_avg_group is None:
                final_avg_group = (
                    getattr(pg_collection, "dp_cp_gtp_remat", None)
        ...
        if hybrid_context_parallel and (configured_cp_size is None or configured_cp_size < 1):
            raise ValueError("configured_cp_size must be positive for Hybrid CP logging.")
        ...
        if final_avg_group is None:
            # Main's default includes CP and GTP-remat, covering every rank that owns distinct
            # input data. Including CP is harmless after static CP normalization because each
            # rank in a CP group holds the same reduced value.
            final_avg_group = parallel_state.get_data_parallel_group(with_context_parallel=True)

        if hybrid_context_parallel:
            if torch.distributed.is_initialized():
                final_avg_size = get_pg_size(final_avg_group)
                if final_avg_size % configured_cp_size != 0:
                    raise ValueError(
                        "Hybrid CP metric group size must be divisible by configured_cp_size."
                    )
            torch.distributed.all_reduce(values, group=pp_group)
            values.mul_(configured_cp_size)
            torch.distributed.all_reduce(
                values, group=final_avg_group, op=torch.distributed.ReduceOp.AVG
            )
        else:
    ...
    def track_indexer_metrics(
        ...
        Args:
            ...
            hybrid_context_parallel: Whether Hybrid CP used variable-sized local CP groups.
            configured_cp_size: Configured/static CP width used to normalize Hybrid-CP
                raw local token sums.
            pp_group: Pipeline-parallel group that owns the language model.
            final_avg_group: Full data-distribution group used for the final average.
        """
...
def initialize_dsa_metric_tracker(
    ...
    Returns:
        ...
        language-model PP group contains no DSA/CSA metric writers. A zero result leaves the
        process-global tracker uninitialized so a later model lifecycle can use another PP
        group without inheriting stale provenance.
    """
