...
def _get_bwd_kernel(
    ...
    return _get_cached_kernel(
        ...
        _tilelang_sparse_mla_bwd_cache_lock,
        key,
        lambda: bwd(H, D, D_tail, topk, kv_group, sm_scale, max_block_h=max_block_h),
    )
...
def _get_postprocess_kernel(D: int, D_tail: int, kv_group: int):
    return _get_cached_kernel(
        ...
        _tilelang_sparse_mla_bwd_cache_lock,
        (D, D_tail, kv_group),
        lambda: postprocess(D, D_tail, kv_group),
    )
...
@tilelang_jit(out_idx=[-2], pass_configs=_SPARSE_MLA_BWD_PASS_CONFIGS)
def bwd(  # pragma: no cover
    H,
    D,
    D_tail,
    topk,
...
def _sparse_mla_delta_batched(o, do):  # pragma: no cover
    """Compute Delta = sum(O * dO) with safe sequence padding for TileLang tiles."""
    require_tilelang()
    ...
    seq_len_padded = _round_up(S, _SPARSE_MLA_BWD_BLOCK_SIZE)
    if seq_len_padded != S:
        o_padded = torch.zeros((B, seq_len_padded, H, D), dtype=o.dtype, device=o.device)
        o_padded[:, :S].copy_(o)
        o = o_padded

        do_padded = torch.zeros((B, seq_len_padded, H, D), dtype=do.dtype, device=do.device)
        do_padded[:, :S].copy_(do)
        do = do_padded

    preprocess_kernel = _get_preprocess_kernel(H, D)
    return preprocess_kernel(o, do)[:, :S].contiguous()


def sparse_mla_delta(o, do):  # pragma: no cover
    """Compute Delta = sum(O * dO) per sequence row and head."""
    squeeze_batch = o.ndim == 3
    if squeeze_batch:
        o = o.unsqueeze(0)
        do = do.unsqueeze(0)
    delta = _sparse_mla_delta_batched(o, do)
    if squeeze_batch:
        delta = delta.squeeze(0)
    return delta


def sparse_mla_bwd(q, kv, o, do, indices, lse, sm_scale=None, delta=None):  # pragma: no cover
    ...
    assert q.ndim == 4 and kv.ndim == 4 and do.ndim == 4 and indices.ndim == 4 and lse.ndim == 3
