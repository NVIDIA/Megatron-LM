...
class TEGroupedMLP(MegatronModule):
    ...
    def forward(
        ...
        if self.config.fp8 and not self.config.moe_enable_hybridep:
        ...
        if fc1_expert_dispatch_event is not None:
            fc1_expert_dispatch_event.wait(torch.cuda.current_stream())
        ...
        if fc2_expert_dispatch_event is not None:
            fc2_expert_dispatch_event.wait(torch.cuda.current_stream())
        if self.activation_recompute:
            self.activation_checkpoint = tensor_parallel.CheckpointWithoutOutput()
            intermediate_parallel = self.activation_checkpoint.checkpoint(
