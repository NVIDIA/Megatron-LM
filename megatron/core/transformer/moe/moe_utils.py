...
def get_default_pg_collection() -> ProcessGroupCollection:
    ...
    # EGTP_remat-excluded expert-DP groups used to stamp expert-weight replica_ids. Must not be
    # left None (get_pg_rank(None)==0 -> duplicate-writer collision at checkpoint save).
    pg_collection.expt_dp_no_egtp_remat = parallel_state.get_expert_data_parallel_group(
        no_gtp_remat=True, check_initialized=False
    )
    pg_collection.intra_expt_dp_no_egtp_remat = parallel_state.get_expert_data_parallel_group(
        no_gtp_remat=True, partial_expert_data_parallel=True, check_initialized=False
    )
