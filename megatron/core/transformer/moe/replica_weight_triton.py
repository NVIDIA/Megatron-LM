...
"""Triton weight-push kernel for direct intra-node replica weight movement.

Only virtual weights occupy PyTorch native symmetric memory. Source weights remain
in parameter or GTP-gather storage: each owner pushes them directly into the
destination virtual slots of its peers. No activation transport is involved.

The push is pure wire movement, so it is bound by NVLink bandwidth rather than by
arithmetic, and within the reserved SM budget only the TMA unit can saturate the
link. ``tl.make_tensor_descriptor`` reaches that bandwidth from a peer base
address resolved at runtime, and Triton's loop pipeliner supplies the multi-stage
global-to-shared-to-global schedule that the transfer needs.
"""
...
# flat byte stream is addressed as ``[rows, _ROW_BYTES]``. Every transport offset
# is a multiple of the tile size and therefore already row aligned.
_ROW_BYTES = tl.constexpr(256)
...
def _push_projection(
    ...
    """
    ROWS: tl.constexpr = MEMBER_BYTES // _ROW_BYTES
    TILE_ROWS: tl.constexpr = TILE_BYTES // _ROW_BYTES
...
def _tile_bytes(limit: int, *component_bytes: int) -> int:
    ...
    if tile % _ROW_BYTES.value:
        raise ValueError(
            f"Replica weight components must share a {_ROW_BYTES.value}-byte aligned "
            f"transport tile, got {component_bytes} yielding {tile}."
        )
    return tile


def _validate_push_shape(world_size: int, num_local_experts: int, num_sms: int) -> None:
...
def compile_replica_weight_push(
    *,
    ...
    """
    _validate_push_shape(world_size, num_local_experts, num_sms)
