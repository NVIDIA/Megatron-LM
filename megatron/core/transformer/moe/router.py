...
class TopKRouter(Router):
    ...
    def _apply_seq_aux_loss(
        ...
        probs = self.attach_and_log_load_balancing_loss(
            ...
            avg_group=aux_loss_groups.metric_avg_group,
            needs_dp_avg=aux_loss_groups.metric_needs_dp_avg,
            # local_num_tokens / total_num_tokens are per-sequence (bsz was folded into the
            # expert dim above); * bsz recovers the micro-batch total, else per-token-loss
            # scaling keeps a 1/MBS factor.
    ...
    def apply_z_loss(self, logits, padding_mask: Optional[torch.Tensor] = None):
        ...
        if self.config.moe_z_loss_coeff is not None and self.training and torch.is_grad_enabled():
            ...
            get_moe_metrics_tracker().record(
                # z_loss_mean / mtp_loss_scale rather than z_loss / moe_z_loss_coeff:
                # moe_z_loss_coeff is only bound in the non-per-token-loss branch
                # above, so dividing by it would raise NameError when
                # --calculate-per-token-loss is set.
                "z_loss",
