...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def __init__(
        ...
        self.fixed_expert_capacity = self.config.moe_pad_expert_input_to_capacity
        self.capacity = None
    ...
    def preprocess(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        if not is_real_cuda_device_available() and drop_without_pad:
            # Dropped tokens are masked-out at core/transformer/moe/moe_layer.py:topk_softmax_with_capacity.
            # Mask value equals num_experts (max(expert_ids) + 1), so we need to drop the last position from
            # num_local_tokens_per_expert tensor, as it contains the sum of masked-out tokens, which
            # is not needed for further calculations.
            num_local_tokens_per_expert = num_local_tokens_per_expert[: self.num_experts]
        ...
        if self.ep_size > 1 or self.tp_size > 1:
            ...
            # ===================================================
            # num_global_tokens_per_expert: [self.ep_size, num_experts]
            # num_global_tokens_per_local_expert: [self.ep_size, num_local_experts]
    ...
    def token_permutation(
        ...
        permutated_local_input_tokens, self.reversed_local_input_permutation_mapping = permute(
            ...
            padded_mode=self.fixed_expert_capacity,
        )
