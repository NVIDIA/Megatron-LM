...
class NVLSAllGatherVDispatcher(InferenceAllGatherDispatcherBase):
    ...
    def allocate_buffers(
        ...
        rsv_dtype = torch.bfloat16 if _NVLS_RS_BF16 else torch.float32
    ...
    def token_dispatch(self, hidden_states, probs):
        ...
        if (
            self.__class__._real_token_count_tensor is not None
            and not getattr(self.routing_map, "_mcore_padding_masked", False)
            and not (_ablate.ABLATE_ROUTE_MASK and _ablate.hit("route_mask"))
        ):
