...
def get_mtp_layer_spec_for_backend(
    ...
    if enable_hyper_connections:
        # mHC mode: separate per-stream e_proj/h_proj instead of concatenated eh_proj.
...
class MultiTokenPredictionLayer(MegatronModule):
    ...
    def __init__(
        ...
        if self.mhc_enabled:
            ...
            # e_proj: [h] -> [h], applied to embedding and broadcast across streams.
            # h_proj: [h] -> [h], applied per-stream to hidden states.
    ...
    def _concat_embeddings(self, hidden_states: torch.Tensor, decoder_input: torch.Tensor):
        ...
        In standard MTP mode, ``hidden_states`` is single-stream ``[s, b, h]``.
        In mHC + MTP mode, ``hidden_states`` is multi-stream ``[s, b, n*h]`` and
        the embedding is mixed in per-stream using separate ``e_proj`` and
        ``h_proj`` projections (instead of the concatenated ``eh_proj``).
        """
    ...
    def _proj_and_transformer_layer(
        ...
        # In mHC mode the output of the inner transformer layer is still
        # multi-stream. The caller (MultiTokenPredictionBlock.forward) is
        # responsible for contracting it once per depth so the contracted
        # representation can be fed to the shared LM head, while the
        # multi-stream representation continues to flow to the next MTP depth.
    ...
    def _postprocess(self, hidden_states: torch.Tensor):
        ...
        With mHC enabled, ``hidden_states`` is the multi-stream tensor
        ``[s, b, n*h]`` and is first contracted to single-stream ``[s, b, h]``
        via the learned per-depth output contraction. Without mHC this is just
    ...
    def forward(
        ...
        Args:
            ...
            mhc_multistream (Tensor, optional): When mHC is enabled, the pre-contraction
                multi-stream decoder output ``[s, b, n*h]`` used as the input to MTP depths.
            attention_mask (Tensor): Boolean tensor of shape [1, 1, s, s] for masking
                self-attention.
        ...
        if mhc_multistream is not None:
            # mHC mode: MTP depths consume the multi-stream tensor; the
            # contracted tensor is only used to populate the per-depth loss
            # list once each depth's _postprocess produces it.
        ...
        for iteration in range(self.config.mtp_num_layers):
            ...
            if mhc_multistream is not None:
                # Keep the multi-stream tensor flowing to the next depth, but
                # contract here so the per-depth output enters the loss list as
                # single-stream [s, b, h].
