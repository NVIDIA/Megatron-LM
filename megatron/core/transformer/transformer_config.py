...
class TransformerConfig(ModelParallelConfig):
    ...
    experimental_attention_variant: Optional[
        Literal["gated_delta_net", "dsa", "msa"]
    ] = None
    """Type of attention variant to use. Currently support gated_delta_net, dsa, and msa."""
    ...
    """Block size for MSA block-level selection."""

    msa_num_selected_blocks: int = 16
    """Number of top-k KV blocks to select per GQA group in MSA."""

    msa_indexer_head_dim: int = 64
    """Dimension of the MSA indexer Q/K projections."""

    msa_loss_coeff: float = 0.0
    """Coefficient for the MSA KL divergence loss. Set to 0 to disable indexer loss."""

    msa_warmup: bool = False
    """If True, run full attention during indexer warmup (both branches)."""
    ...
    def __post_init__(self):
        ...
        if self.experimental_attention_variant == "gated_delta_net":
            ...
            pass
        elif self.experimental_attention_variant == "msa":
            assert not self.multi_latent_attention, (
                "MSA is not compatible with multi_latent_attention; use standard GQA instead."
            )
        ...
        if self.experimental_attention_variant == "msa":
            assert self.context_parallel_size == 1, (
                "Currently context parallelism is not supported by MSAAttention!"
            )
