...
class TransformerConfig(ModelParallelConfig):
    ...
    ``gdn``, ``gdn2``, and ``kda`` select Gated DeltaNet-family implementations.
    ...
    """Deprecated alias for experimental_attention_variant."""
    ...
    """Deprecated alias for selecting the DSA kernel backend."""
    ...
    "mhc" use output-discarding checkpointing, "core_attn", "mlp", "moe",
    "shared_experts", and "gdn" use normal checkpointing.
    """
    ...
    """Context-parallel execution mode for GDN-family linear-attention layers."""
    ...
    def __post_init__(self):
        ...
        if self.linear_cp_layout not in ("zigzag", "contiguous"):
            raise ValueError(f"Unsupported cp_partition_mode: {self.linear_cp_layout}")

        if self.linear_cp_layout == "contiguous" and (
            self.context_parallel_size > 1 or self.hybrid_context_parallel
        ):
        ...
        if self.context_parallel_size > 1:
            if self.linear_cp_layout == "contiguous":
                if (
            ...
            elif self.linear_cp_layout == "zigzag":
