...
class TransformerConfig(ModelParallelConfig):
    ...
    """Type of attention variant to use. Currently support gdn, gdn2, dsa and dsv4_hybrid.
    ...
    Enable this when local token counts can differ across ranks, for example with dynamically
    packed THD inputs. When disabled, the caller must guarantee equal token counts across the
    HybridEP communication group, for example by padding THD inputs to a fixed maximum before
    dispatch. CUDA Graph inputs should be statically padded upstream and leave this option
    disabled.
    """
    ...
    def __post_init__(self):
        ...
        if self.gdn_pre_gated_delta_rule_fusion and not is_gated_delta_net_variant(
            self.experimental_attention_variant
        ):
            raise ValueError(
        ...
        if self.recompute_granularity == "selective":
            ...
            if "gdn" in self.recompute_modules and (
