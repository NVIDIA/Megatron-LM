...
class FlextronModelManager:
    ...
    def _setup_param_loss_func(self):
        ...
        self.all_param, self.active_param = torch.tensor(get_num_parameters(
    ...
    def _setup_memory_loss_func(self):
        ...
        self.total_memory = get_memory_footprint(
            hybrid_pattern=self.model.hybrid_layer_pattern,
            mamba_num_heads=self.config.mamba_num_heads,
            mamba_d_head=self.config.mamba_head_dim,
            mamba_d_state=self.config.mamba_state_dim,
        ...
        ).float().to(torch.cuda.current_device())
    ...
    def budget_loss_func(self, flextron_kwargs, budget_item=0, original_model=True):
        ...
        dtype, device = flextron_kwargs['router_mlp'][0].dtype, flextron_kwargs['router_mlp'][0].device

        flex_mamba_num_head = flextron_kwargs['router_mamba'][0] @ torch.tensor(self.config.mamba_int_list, dtype=dtype, device=device)
        flex_hidden_size = flextron_kwargs['router_emb'][0] @ torch.tensor(self.config.emb_int_list, dtype=dtype, device=device)
        flex_ffn_hidden_size = flextron_kwargs['router_mlp'][0] @ torch.tensor(self.config.mlp_int_list, dtype=dtype, device=device)
        flex_moe_expert = flextron_kwargs['router_moe_expert'][0] @ torch.tensor(self.config.moe_expert_int_list, dtype=dtype, device=device)
        num_attention_heads = flextron_kwargs['router_head'][0] @ torch.tensor(self.config.head_int_list, dtype=dtype, device=device)
        ...
        if self.config.add_skipping:
            ...
            mamba_idxs = [i for i, char in enumerate(self.model.hybrid_layer_pattern) if char == 'M']
        ...
        if budget_item == 1.0:
            if self.config.flex_hetero_moe_expert:
                ...
                mse_loss_moe_expert = F.mse_loss(flextron_kwargs['router_moe_expert'][0], label_moe_expert)
            else:
                label_moe_expert = torch.zeros_like(flextron_kwargs['router_moe_expert'][0])
                label_moe_expert[0] = 1.0
                mse_loss_moe_expert = F.mse_loss(flextron_kwargs['router_moe_expert'][0], label_moe_expert)
    ...
    def process_router_output(self, budget_item):
        ...
        (router_mlp, router_skip, router_emb, router_mamba, router_head, router_moe_expert) = self.router(budget_item)
    ...
    def flextron_forward(
        ...
        **flextron_kwargs
        ...
        if getattr(self.config, 'override_selected_budget', None) is not None:
            assert self.config.is_flex_eval, "Override selected budget should only be set in flex eval mode"
        ...
        if (hasattr(self, '_flextron_manager') and 
            self._flextron_manager is not None and 
            self._flextron_manager.router is not None):
        ...
        if labels is not None:
            ...
            if (hasattr(self, '_flextron_manager') and 
                self._flextron_manager is not None and 
                self._flextron_manager.router is not None and
                getattr(self.config, 'flextron', False) and 
                not getattr(self.config, 'is_flex_eval', False)):
