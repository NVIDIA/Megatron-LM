...
def get_num_parameters(        
        ...
        if flex_hetero_ffn or flex_hetero_moe_expert:
            ...
            for i in range(len(num_experts)):
                ...
                linear_fc1 =  ffn_hidden_size[i] * (hidden_size * num_experts[i] + shared_expert_intermediate_size)
                linear_fc2 = ffn_hidden_size[i] * (hidden_size * num_experts[i] + shared_expert_intermediate_size)
                linear_fc1_active = ffn_hidden_size[i] * (hidden_size * moe_router_topk + shared_expert_intermediate_size)
                linear_fc2_active = ffn_hidden_size[i] * (hidden_size * moe_router_topk + shared_expert_intermediate_size)
            ...
            linear_fc1_active = ffn_hidden_size * (hidden_size * moe_router_topk + shared_expert_intermediate_size)
            linear_fc2_active = ffn_hidden_size * (hidden_size * moe_router_topk + shared_expert_intermediate_size)
        ...
        for i, c in enumerate(hybrid_pattern):
            ...
            if c == 'M': 
                ...
                pass
            else: raise RuntimeError(f'Unknown layer type: {c}')

        return embedding + all_params + final_layernorm + output_layer, embedding + active_params + final_layernorm + output_layer
...
def get_kv_cache_size(
    ...
    if flex_hetero_head:
        ...
        for c in hybrid_pattern:
            if c == '*':
                current_heads = num_attention_heads[head_idx] if flex_hetero_head else num_attention_heads
                
                kv_cache_size_per_layer = 2.0 * mem_batch_size *mem_infer_seq_len * num_query_groups * current_heads * kv_channels / current_heads.detach().item()
                kv_cache_size += kv_cache_size_per_layer
                head_idx += 1
        
    else:
        num_attention_layers = hybrid_pattern.count('*')
        divider = num_attention_heads.detach().item() if isinstance(num_attention_heads, torch.Tensor) else num_attention_heads
        kv_cache_size = (2.0 
                    * mem_batch_size
                    * mem_infer_seq_len
                    * num_query_groups
        ...
        if flex_hetero_mamba:
            ...
            for c in hybrid_pattern:
                if c == 'M':
                    ...
                    ssm_cache_size += mem_batch_size * current_mamba_num_heads * mamba_d_head * mamba_d_state
                    mamba_idx += 1

        else:
            num_mamba_layers = hybrid_pattern.count('M')
            ssm_cache_size = (mem_batch_size
                        * mamba_num_heads
                        * mamba_d_head
                        * mamba_d_state
                        * num_mamba_layers)
...
def get_max_buffer_size(
        ...
        if isinstance(moe_num_experts, int) or moe_num_experts.shape[0] == 1:
            moe_num_experts = torch.tensor([moe_num_experts] * hybrid_pattern.count('E')).to(torch.cuda.current_device()).float()

        if isinstance(ffn_hidden_size, int) or ffn_hidden_size.shape[0] == 1:    
            ffn_hidden_size = torch.tensor([ffn_hidden_size] * hybrid_pattern.count('E')).to(torch.cuda.current_device()).float()
...
def get_memory_footprint(
        ...
        mem_params = memory_config.bpe_params * get_num_parameters(
