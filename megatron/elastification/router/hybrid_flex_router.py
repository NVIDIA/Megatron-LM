...
class FlextronRouter(MegatronModule):
    def __init__(self, config: TransformerConfig):
        ...
        self.grad_accumulation_steps = get_current_global_batch_size() // (
            get_micro_batch_size() * self.dp_size
        )
    ...
    def add_router_for_mlp(self):
        ...
        if self.config.flex_hetero_ffn:
            ...
            gate_mlp_layer_list = [
                ...
                nn.LeakyReLU(0.1),
                self._create_linear_layer(
                    self.n_dim, len(mlp_list) * num_mlp, bias=False, is_first_layer=False
                ),
            ]
            ...
            gate_mlp_layer_list = [
                ...
                nn.LeakyReLU(0.1),
                self._create_linear_layer(
                    self.n_dim, len(mlp_list), bias=False, is_first_layer=False
                ),
            ]
    ...
    def add_router_for_moe_expert(self):
        ...
        if self.config.flex_hetero_moe_expert:
            ...
            gate_moe_expert_layer_list = [
                ...
                self._create_linear_layer(
                    self.n_dim,
                    len(moe_expert_list) * num_moe_expert,
                    bias=False,
            ...
            gate_moe_expert_layer_list = [
                ...
                nn.LeakyReLU(0.1),
                self._create_linear_layer(
                    self.n_dim, len(moe_expert_list), bias=False, is_first_layer=False
                ),
            ]
    ...
    def add_router_for_skipping(self):
        ...
        gate_skip_mlp_layer_list = [
            ...
            nn.LeakyReLU(0.1),
            self._create_linear_layer(
                self.n_dim, self.output_dim, bias=False, is_first_layer=False
            ),
        ]
    ...
    def add_router_for_mamba(self):
        ...
        if self.config.flex_hetero_mamba:
            ...
            gate_mamba_layer_list = [
                ...
                nn.LeakyReLU(0.1),
                self._create_linear_layer(
                    self.n_dim, len(mamba_list) * num_mamba, bias=False, is_first_layer=False
                ),
            ]
            # Set bias for the last layer
            if (
                hasattr(gate_mamba_layer_list[-1], 'bias')
                and gate_mamba_layer_list[-1].bias is not None
            ):
                last_layer_bias = [0.00 for _ in range(len(mamba_list))]
                last_layer_bias[-1] = 1.00
                gate_mamba_layer_list[-1].bias.data = torch.tensor(last_layer_bias).repeat(
                    num_mamba
                )
        else:
            gate_mamba_layer_list = [
                ...
                nn.LeakyReLU(0.1),
                self._create_linear_layer(
                    self.n_dim, len(mamba_list), bias=False, is_first_layer=False
                ),
            ]
        ...
        if self.config.flex_hetero_mamba:
            ...
            if self.config.normalize_router_logits:
                router_mamba_logits = (
                    scale
                    * router_mamba_logits
                    / router_mamba_logits.std(dim=1, keepdim=True).clamp(min=1e-6)
                )
            else:
            ...
            return (
                router_mamba_logits,
                [self.config.mamba_int_list[i] for i in choices_mamba.flatten().tolist()],
            )
        else:
            if self.config.normalize_router_logits:
                if len(self.config.mamba_int_list) > 1:
                    router_mamba_logits = (
                        scale
                        * router_mamba_logits
                        / router_mamba_logits.std(dim=0, keepdim=True).clamp(min=1e-6)
                    )
            else:
                router_mamba_logits = scale * router_mamba_logits
            router_mamba_logits = self._dp_gumbel_softmax(
        ...
        if self.config.flex_hetero_moe_expert:
            ...
            if self.config.normalize_router_logits:
                router_moe_expert_logits = (
                    scale
                    * router_moe_expert_logits
                    / router_moe_expert_logits.std(dim=1, keepdim=True).clamp(min=1e-6)
                )
            else:
            ...
            return (
                router_moe_expert_logits,
                [self.config.moe_expert_int_list[i] for i in choices_moe_expert.flatten().tolist()],
            )
        else:
            if self.config.normalize_router_logits:
                router_moe_expert_logits = (
    ...
    def add_scaler_schedule(self):
        ...
        if (
            self.config.linear_scaler_start is not None
            and self.config.linear_scaler_end is not None
        ):
    ...
    def forward(self, budget):
        ...
        if budget in self.budget_map.keys():
            ...
            one_hot_1 = torch.nn.functional.one_hot(
                idx1.to(torch.long), num_classes=num_classes
            ).to(device=device, dtype=dtype)
            one_hot_2 = torch.nn.functional.one_hot(
