...
and materializes them on-demand via async all-gather. The implementation lives in
``megatron.experimental.gtp.generalized_tensor_parallelism`` and depends on
TransformerEngine's FP8 / MXFP8 / NVFP4 primitives. Importing this package will
raise ``ImportError`` (with a helpful message) if TransformerEngine is missing or
too old.
"""
