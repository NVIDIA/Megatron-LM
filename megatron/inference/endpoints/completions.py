...
class MegatronCompletions(Resource):
    def __init__(self, engine, args, mcore_engine_func=run_mcore_engine, return_generated_text=False):
        ...
        self.return_generated_text = return_generated_text
    ...
    def post(self):
        ...
        local_kwargs = {
            "prompts": prompts,
            "tokens_to_generate": int(req.get("max_tokens", getattr(self.args, "num_tokens_to_generate", 512))),
            "temperature": float(req.get("temperature", getattr(self.args, "temperature", 1.0))),
            "top_k_sampling": int(req.get("top_k", getattr(self.args, "top_k", 0))),
            "top_p_sampling": float(req.get("top_p", getattr(self.args, "top_p", 1.0))),
            "return_topk_logprobs": int(req.get("logprobs", getattr(self.args, "return_log_probs", 0))),
            "echo": bool(req.get("echo", False)),
            "random_seed": int(req.get("seed", getattr(self.args, "seed", -1))),
        ...
        with LOCK:
            ...
            response_dict = self.mcore_engine_func(
                self.engine,
                prompts,
                temperature,
                top_k,
            ...
            result.append(response_dict.get("generated_text", None))
        ...
        results = []
        for batch_idx, (prompt_plus_generation, prompt, generated_text) in enumerate(
            zip(prompts_plus_generations, prompts, generated_text_list)
        ):
