...
def get_indexed_dataset_(data_prefix, dataset_type):
    ...
    print_rank_0(
        '    number of documents: {}'.format(indexed_dataset.document_indices.shape[0] - 1)
    )
...
def get_samples_mapping(
    ...
    """Get a list that maps a sample index to a starting sentence index, end sentence index, and length"""  # noqa: E501
    ...
    if torch.distributed.get_rank() == 0 and not os.path.isfile(indexmap_filename):
        ...
        print_rank_0(
            ' > elasped time to build and save samples mapping (seconds): {:4f}'.format(
    ...
    assert counts[0].item() == (
        torch.distributed.get_world_size() // mpu.get_tensor_model_parallel_group().size()
    )
