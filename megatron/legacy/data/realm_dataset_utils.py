...
class BlockSampleData(object):
    ...
    :param end_idx: for last sentence of the block
        (may be partially truncated in sample construction)
    :param doc_idx: the index of the document from which the block
        comes in the original indexed dataset
    ...
    def as_array(self):
        return np.array([self.start_idx, self.end_idx, self.doc_idx, self.block_idx]).astype(
            np.int64
        )
...
def get_block_samples_mapping(
    ...
    if mpu.get_data_parallel_rank() == 0 and not os.path.isfile(indexmap_filename):
        ...
        print_rank_0(
            ' > elapsed time to build and save samples mapping (seconds): {:4f}'.format(
