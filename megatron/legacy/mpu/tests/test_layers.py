...
def parallel_self_attention(
    ...
    rank = mpu.get_tensor_model_parallel_rank()
    mpu.destroy_model_parallel()
    return rank, hidden_size, tensor_model_parallel_size, loss, attention_layer, identity_layer
...
def test_parallel_self_attention(tensor_model_parallel_size):
    ...
    my_lin_grad_list = torch.split(
        attention_layer_1.query_key_value.weight.grad, hidden_size // tensor_model_parallel_size, 0
    ...
    torch.distributed.barrier()
    print(
        '   weight gradient error on global rank {}: {}'.format(torch.distributed.get_rank(), error)
    )
    assert error < 5.0e-6
    ...
    torch.distributed.barrier()
    print(
        '   input gradient error on global rank {}: {}'.format(torch.distributed.get_rank(), error)
    )
    assert error < 5.0e-6
...
def parallel_transformer(
    ...
    transformer_layer = mpu.BertParallelTransformerLayer(
        hidden_size, intermediate_size, num_att_heads, 0.0, 0.0, torch.nn.functional.relu, 1.0e-5
    ).cuda()
    ...
    rank = mpu.get_tensor_model_parallel_rank()
    mpu.destroy_model_parallel()
    return rank, hidden_size, tensor_model_parallel_size, loss, transformer_layer, identity_layer
...
def test_parallel_transformer_layer(tensor_model_parallel_size):
    ...
    (
        ...
        1, num_att_heads_per_partition, hidden_size_per_att_head, batch_size, sequence_length
    )
