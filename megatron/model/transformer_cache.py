...
class ParallelAttentionWithCache(MegatronModule):
    ...
    def __init__(self, config, layer_number,
                 ...
                 attn_mask_type=AttnMaskType.padding):
        super(ParallelAttentionWithCache, self).__init__()
        args = get_args()
        self.layer_number = max(1, layer_number)
    ...
    def create_attn_cache(self, batch_size, max_seq_len, device='cuda', dtype=torch.float32):
        args = get_args()
        # create cache for K, V
        # shape = [seq, bsz, ng, hn]
        k = torch.zeros(max_seq_len, batch_size, self.num_query_groups_per_partition, self.hidden_size_per_attention_head, device=device, dtype=dtype)
        v = torch.zeros(max_seq_len, batch_size, self.num_query_groups_per_partition, self.hidden_size_per_attention_head, device=device, dtype=dtype)
        return k, v
...
class ParallelTransformerLayerWithCache(MegatronModule):
    ...
    def __init__(self, config,
        ...
        super(ParallelTransformerLayerWithCache, self).__init__()
        ...
        # Self attention.
        self.self_attention = ParallelAttentionWithCache(
            config,
            layer_number,
            attention_type=AttnType.self_attn,
        ...
        if self.layer_type in (LayerType.decoder,
            ...
            self.inter_attention = ParallelAttentionWithCache(
                config,
                layer_number,
        ...
        if layer_type == LayerType.retro_decoder_with_retriever:
            self.retriever = ParallelTransformerWithCache(
                config=config,
...
class ParallelTransformerWithCache(MegatronModule):
    """Transformer class."""
        ...
        def build_layer(layer_number):
            if args.transformer_impl == 'local':
                current_layer_type = _get_layer_type(
                    ...
                    layer_number)
                return ParallelTransformerLayerWithCache(
                    config,
                    layer_number,
