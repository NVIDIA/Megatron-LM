...
class RolloutPipeline:
    ...
    def __init__(
        ...
        self.gate = _SubmissionGate(
            capacity=max(1, round(parallel_generation_tasks * self.gran_policy.units_per_batch())),
    ...
    async def stage_assemble(self) -> None:
        ...
        try:
            while True:
                ...
                self._output_enqueued_at[(group.batch_id, group.index_in_batch)] = time.monotonic()
    ...
    async def stage_consume(self) -> AsyncIterator[RolloutGroup]:
        ...
        consume = {"G": self._consume_completion_order, "B": self._consume_batch_order}[
