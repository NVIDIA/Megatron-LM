...
class WeightedMultiTask(
    ...
    def _quantized_counts(self, total: int) -> list[int]:
        ...
        while 0 in counts:
            zero = counts.index(0)
            donor = max(
                (i for i in range(num_envs) if counts[i] >= 2), key=lambda i: counts[i] - exact[i]
            )
            counts[zero] += 1
            counts[donor] -= 1
        return counts
    ...
    def rollout_allocations(self, num_groups: int) -> list[EnvAllocation]:
        ...
        if any(abs(count - target) > 1e-9 for count, target in zip(counts, exact)):
            logger.warning(
                ...
                ", ".join(
                    ...
                    for eid, weight, count in zip(env_ids, self._rollout_weights, counts)
                ),
            )
        logger.info(
        ...
        return [
            ...
            for agent, env_id, count in zip(self._rollout_agents, env_ids, counts)
        ]
