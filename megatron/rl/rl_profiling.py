...
# profiler-only (already logged by base Megatron, or fine-grained profiler detail).
# Import RL_LOGGABLE_TIMER_NAMES in training.py instead of hardcoding the list.
...
RL_NONLOGGABLE_TIMER_NAMES = [
    # Already logged by base Megatron (not added to timers_to_log)
    "forward-backward",
    "optimizer",
...
RL_TIMER_NAMES = RL_LOGGABLE_TIMER_NAMES + RL_NONLOGGABLE_TIMER_NAMES
...
class RLProfiler:
    ...
    def log_iteration(
        ...
        Token throughput metrics (tokens_per_sec, actual_tokens_per_sec,
        packing_efficiency) are read from RLRuntimeState, which is populated
        by the observability metrics code in training_log().
        ...
        # Read token throughput metrics from RLRuntimeState (populated by
