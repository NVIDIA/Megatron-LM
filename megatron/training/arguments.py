...
def _add_inference_args(parser):
    ...
    group.add_argument('--inference-dynamic-batching-prefix-caching-evict-policy',
                       type=str, default='ref_zero',
                       choices=['ref_zero', 'lru'],
                       dest='inference_dynamic_batching_prefix_caching_evict_policy',
                       help='Eviction policy for prefix caching blocks. '
                       '"lru": blocks with ref_count==0 stay cached and are '
                       'evicted via LRU only when space is needed. '
                       '"ref_zero" (default): immediately returns blocks to '
                       'the free pool when ref_count hits 0.')
