...
def parse_and_validate_args(
    extra_args_provider=None, ignore_unknown_args=False, args_defaults={}
):
...
def _normalize_cuda_graph_modules_args(args):
    ...
    normalized_scopes, deprecated_scopes, used_full_scope = (
        normalize_cuda_graph_modules(args.cuda_graph_modules)
    )
    validate_deprecated_cuda_graph_modules_migration_inputs(
        deprecated_scopes,
...
def validate_args(args, defaults={}):
    ...
    if (
        args.step_batch_size_schedule is not None
        and is_global_batch_size_explicitly_specified
    ):
        raise ValueError(
    ...
    if (
        ...
        print_rank_0(
            f"Converted legacy MTP pattern to unified: {args.hybrid_layer_pattern}"
        )
    ...
    if args.hybrid_layer_pattern is not None:
        ...
        if (
            hybrid_pipeline_segments == 1
            and args.transformer_pipeline_model_parallel_size > 1
        ):
    ...
    args.mamba_inference_conv_states_dtype = map_dtype(
        args.mamba_inference_conv_states_dtype
    )
    args.mamba_inference_ssm_states_dtype = map_dtype(
        args.mamba_inference_ssm_states_dtype
    )

    args.megatron_fsdp_main_params_dtype = map_dtype(
        args.megatron_fsdp_main_params_dtype
    )
    ...
    if args.use_megatron_fsdp:
        ...
        if (
            ...
            in ["optim_grads_params", "optim_grads"]
        ...
        if args.nccl_ub:
            ...
            warn_rank_0(
                "FSDP manual registration is enabled by default when --nccl-ub is enabled!"
            )

        if (
            args.init_model_with_meta_device
            and args.data_parallel_sharding_strategy == "no_shard"
        ):
            raise ValueError(
    ...
    if args.fsdp_manual_registration:
        ...
        assert args.nccl_ub, (
            "FSDP manual registration is only supported with --nccl-ub argument."
        )
    ...
    if args.logits_save_dir is not None:
        assert args.logits_save_top_k is not None, (
    ...
    assert not (args.cuda_graph_impl == "full_iteration" and args.cuda_graph_modules), (
...
def _add_experimental_attention_variant_args(parser):
    ...
    group.add_argument(
        ...
        type=int,
        default=128,
        help="Block size for MSA block-level KV selection.",
    )
    group.add_argument(
        "--msa-num-selected-blocks",
        type=int,
        default=16,
        help="Number of top-k KV blocks to select per GQA group in MSA.",
    )
    group.add_argument(
        "--msa-indexer-head-dim",
        type=int,
        default=64,
        help="Dimension of the MSA indexer Q/K projections.",
    )
    group.add_argument(
        "--msa-loss-coeff",
        type=float,
        default=0.0,
        help="Coefficient for the MSA KL divergence loss. Set to 0 to disable indexer loss.",
    )
    group.add_argument(
        "--msa-warmup",
        action="store_true",
        default=False,
        help="If set, run full attention during indexer warmup (both branches).",
    )
    return parser
