...
def _add_data_args(parser):
    ...
    group.add_argument('--dataloader-fast-cache-load', action='store_true',
                       help='Option to use the fast cache path when building the datasets. Requires all the dataset caches to be built and stored in --data-cache-path.')
    group.add_argument('--path-to-sequences-per-dataset-json', default=None,
                       help='Path to a json file with the sequences per dataset.')
    group.add_argument('--defer-memmap', action='store_true',
                       help='Defer the mmap of the dataset indexes until the first access.')
