...
def _validate_cyclic_dataloader_resume(args, checkpoint_args, release):
    """Reject cyclic dataloader resumes whose sampler state cannot be remapped safely."""
    ...
    if checkpoint_dp > 0 and run_dp > 0 and checkpoint_dp != run_dp:
        raise RuntimeError(
            'Cannot resume a cyclic dataloader with a different data-parallel size '
            f'({checkpoint_dp} from the checkpoint vs. {run_dp} for this run).'
        )
