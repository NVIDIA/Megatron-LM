...
efficiently executed on a running model.  We especially need to be careful
about the size and timing of computation and communication, as well as ensure
that we don't keep large raw tensors around beyond their normal lifespan.

Each metric specifies a set of `source_kinds` that it cares about, such as
`params` or `router_diagnostics`.  During model execution, we will check
...
At the end of the step, all active metrics will be given a chance to reduce
across ranks.  First, all the prepared tensors will be supplied to
`metric.groups()`, which will divide them into aggregation groups.  For
example, for a global L2 norm, all prepared tensors should go into one group;
for separate per-tensor L2 norms, each prepared tensor should get its own
group; for per-layer L2 norms, prepared tensors should be in groups with others
of their own rank.  Each tensor comes with metadata including the module name
to make such distinctions.

Importantly, `metric.groups()` doesn't need to place each tensor in exactly one
group.  For example, you might return all three of the above groupings from the
same metric.  This is how you can share the same prepare step across multiple
groupings.

Next, we will call `metric.start()` with each group.  This function should not
call any collectives internally, but it may return either a final
`MetricResult` or a `CollectiveRequestSet` which specifies a dimension to
AllGather or AllReduce across.

Each tensor is given an abstract represenation of the ways it is sharded.  For
...
might only exist on one rank, which represented with `Owned(rank=...)`. Metrics
...
"logical" manner: ignore other replicas, use the same reduction function across
shards and other grouped tensors.  Other metrics need the full power: for
example, a metric to measure replica drift has special requirements for
reducing across `Replica` dimensions.

The executor runs the collectives that were requested with `metric.start()`
(batching any compatible collectives).  Then it will call `metric.resume()`
with those results, and this function again can return `CollectiveRequestSet`
or a final `MetricResult`.  This continues in a loop until every metric has
...
| `groups()`           | Group prepared values into aggregation keys.                                    |
| `start()`            | Start reducing across ranks.                                                    |
...
- While no metric functions should call collectives directly, it's still
  important that the returned CollectiveRequestSets match (including tensor
  sizes) across ranks, or else your job will hang.

- Most functions are written to accept and return lists; this lets you use
  batched kernels if they are available, which can significantly improve
  performance.
