...
process groups to `TensorMetricExecutor`. Metrics perform local work and explicitly request
...
class TensorMetric(ABC):
    ...
    A caller may use `accepts` to avoid constructing observations that cannot contribute.
    The executor applies the same site predicate before calling `prepare` while observed
    tensors are available. Callers may accumulate prepared contributions from several observation
    points before passing them to the batch-native `start`. Selection, aggregation, and
    ...
    def accepts(self, site: MetricSite) -> bool:
        ...
        a `MetricTensor`. The executor invokes it again before `prepare`. The default
        accepts every site when `source_kinds` is empty and otherwise accepts declared source
    ...
    def prepare(self, values: Sequence[MetricTensor]) -> list[MetricTensor]:
        ...
        supplies detached inputs whose sites have passed `accepts` and detaches returned
...
_REDUCTION_OP_KEYS = {
    torch.distributed.ReduceOp.SUM: 0,
    torch.distributed.ReduceOp.AVG: 1,
    torch.distributed.ReduceOp.PRODUCT: 2,
    torch.distributed.ReduceOp.MIN: 3,
    torch.distributed.ReduceOp.MAX: 4,
    torch.distributed.ReduceOp.BAND: 5,
    torch.distributed.ReduceOp.BOR: 6,
    torch.distributed.ReduceOp.BXOR: 7,
}
...
class LogicalReductionMetric(TensorMetric):
    ...
    `finalize`. Ranks without an `Owned` value must contribute the identity for
    `reduction_op`.
    Existing `Replica` axes require no communication.

    `prepare` creates the contributions. Each prepared contribution is reduced along all of
    ...
    The default `combine_contributions` applies `reduction_op` locally for associative
    PyTorch reductions with direct tensor equivalents. A metric may override it for different
    cross-tensor semantics. More complex layouts should override `start`. Subclasses
    may override `contribution_batch` to use fused or multi-tensor kernels.
    """
    ...
    def contribution(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Fixed-shaped contribution that can be combined with `reduction_op`.
        """
    ...
    def prepare(self, values: Sequence[MetricTensor]) -> list[MetricTensor]:
        ...
        Raises:
            ValueError: If `contribution_batch` returns the wrong number of contributions.
        """
        if not values:
            return []
...
class TensorMetricExecutor:
    ...
    def prepare(self, metric: TensorMetric, values: Sequence[MetricTensor]) -> list[MetricTensor]:
        ...
        available. Its outputs may be accumulated and passed together to `start` later. A
        value is selected only when every contributing site passes `TensorMetric.accepts`.
    ...
    def start(self, metric: TensorMetric, values: Sequence[MetricTensor]) -> list[MetricStep]:
        ...
        Args:
            ...
            values: Prepared contributions returned by `prepare`.

        Returns:
            Results and explicit collective stages, which may be passed to `complete`
            later.
    ...
    def complete(
        ...
        Together with `prepare`, this separation lets short-lived tensors be compacted when
        ...
        Args:
            ...
            initial_steps: Stages and results previously returned by `TensorMetric.start`.
    ...
    def _execute_requests(self, requests: Sequence[CollectiveRequest]) -> list[MetricTensor]:
        ...
        for _, compatible_group in groupby(
            ...
            if len(packed_values) != len(entries):
                raise RuntimeError(
                    "Collective execution did not return one value for every metric request."
                )
        ...
        for value in completed_values:
            if value is None:
                raise RuntimeError("A tensor metric collective request did not complete.")
            resolved_values.append(value)
        return resolved_values

    def _compatibility_key(self, request: CollectiveRequest) -> tuple:
        tensor = request.value.tensor
        # Fixed-width numeric fields make mixed collective kinds safe to sort without relying on
        # tuple comparison short-circuiting before heterogeneous field types.
        if isinstance(request.collective, AllReduce):
            try:
                reduction_key = _REDUCTION_OP_KEYS[request.collective.op]
            except KeyError as error:
                raise ValueError(
                    "Tensor metrics support only standard PyTorch all-reduce operations."
                ) from error
            collective_key = (0, reduction_key, 0, 0)
        else:
            dim = request.collective.dim
            collective_key = (1, 0, int(dim is not None), 0 if dim is None else int(dim))
        return (
            request.axis,
            collective_key,
            str(tensor.device),
