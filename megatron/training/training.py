...
from megatron.core.optimizer import get_megatron_optimizer, OptimizerConfig, ParamKey
...
def train_step(
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        # Rollout Routing Replay (R3): inject rollout-phase routing indices before the
        # forward+backward pass so the router replays inference expert-selection instead
        # of recomputing top-k.  This reduces training-inference KL divergence and
        # prevents RL training collapse for MoE models.
        _r3_active = (
            getattr(args, 'moe_enable_routing_replay', False)
            and routing_indices is not None
            and len(RouterReplay.global_router_replay_instances) > 0
        )
        if _r3_active:
            RouterReplay.set_rollout_routing_data(routing_indices, activate_replay=True)
        ...
        if _r3_active:
            # Clear routing replay state after the step so normal training steps
            # (without R3) are not affected.
