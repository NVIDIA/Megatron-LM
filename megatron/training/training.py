...
def _num_multimodal_extra_floating_point_operations(args, batch_size):
    """Estimate theoretical FLOPs that sit outside the decoder stack.

    ``args.is_hybrid_model`` is still reserved for decoder-internal layer
    structure (for example hybrid Transformer/Mamba stacks). Multimodal extras
    such as a vision encoder are accounted for orthogonally on top of the
    decoder FLOPs so that these two notions remain compatible.

    This estimate mirrors the existing decoder-side FLOPs accounting style:
    count the dominant GEMMs/attention terms and ignore comparatively small
    elementwise ops like LayerNorm, GELU, RoPE table lookups, and residual
    adds.
    """
    ...
    if vision_flops_variant == "qwen35_vl_v2":
        ...
        temporal_patch_size = args.vision_temporal_patch_size
        ...
        kv_channels = getattr(
            args, "vision_kv_channels", hidden_size // num_attention_heads
        )
        in_channels = args.vision_in_channels
        out_hidden_size = args.vision_out_hidden_size

        grid_h = image_size // patch_size
        grid_w = image_size // patch_size
        frame_seq_len = grid_h * grid_w
        total_patches = temporal_patch_size * frame_seq_len
        merged_tokens = (
            temporal_patch_size
        ...
        ffn_expansion_factor = 2  # GELU MLP: h->ffn and ffn->h

        query_projection_size = kv_channels * num_attention_heads
        key_projection_size = kv_channels * num_attention_heads
        value_projection_size = kv_channels * num_attention_heads
        ...
        # multimodal_dev vision attention runs in THD packed mode, where each
        # temporal frame is an independent sequence of length H*W.
        vision_self_attn_term = (
            forward_backward_expansion_factor
            * fma_expansion_factor
            * (
                hidden_size
                * (
                ...
                + query_projection_size * frame_seq_len * 2
    ...
    required_fields = (
        ...
        "vision_spatial_merge_size",
        "vision_in_channels",
        "vision_merger_intermediate_size",
        "vision_merger_output_size",
        "image_size",
    )
    if any(getattr(args, field, None) is None for field in required_fields):
        return 0
    ...
    in_channels = args.vision_in_channels
    merger_intermediate_size = args.vision_merger_intermediate_size
    merger_output_size = args.vision_merger_output_size

    # Qwen3.5-VL inputs use a square spatial grid and a fixed temporal patch
    # count per sample.
    grid_h = image_size // patch_size
    grid_w = image_size // patch_size
    total_patches = temporal_patch_size * grid_h * grid_w
    merged_tokens = (
        temporal_patch_size
        * (grid_h // spatial_merge_size)
        * (grid_w // spatial_merge_size)
    ...
    # Vision attention is bidirectional, so the QK^T and AV terms use the full
    # sequence length instead of the causal seq_len / 2 used by the decoder.
    vision_self_attn_term = (
        forward_backward_expansion_factor
        * fma_expansion_factor
        * (
            ...
            * (
            ...
            + query_projection_size * total_patches * 2
            + query_projection_size * hidden_size
        )
    )
    vision_transformer_flops = (
    ...
    patch_merger_flops = (
        ...
        * merged_tokens
        * (
            merge_dim * merger_intermediate_size
            + merger_intermediate_size * merger_output_size
        )
    )
...
def num_floating_point_operations(args, batch_size):
    ...
    if is_hybrid_model(args):
        ...
        # This branch models decoder-internal hybrid layers only.
        ...
        # Compute decoder FLOPs for a standard Transformer stack.
        total_floating_point_operations = transformer_flops()

    return (
        total_floating_point_operations
        + _num_multimodal_extra_floating_point_operations(args, batch_size)
    )
