...
def num_floating_point_operations(
    ...
    def hybrid_flops(
        ...
        # The GDP stack spec swaps every Mamba layer for a Gated Delta Product layer.
        mamba_flops = (
    ...
    if is_hybrid_model(args):
        ...
        total_floating_point_operations = hybrid_flops(
            ...
            # Matches TransformerConfig.gdp_num_householder; only read when the
            # selected stack spec actually swaps Mamba layers for GDP layers.
            gdp_num_householder=getattr(args, "gdp_num_householder", 3),
...
def _get_indexer_logging_layer_counts(args) -> tuple[int, int | None]:
    ...
    # Matches TransformerConfig.csa_dense_mode; entry points that never enable CSA
    # (and host-only logging tests) build args without the flag.
    return tracker_layers, 0 if getattr(args, "csa_dense_mode", False) else indexer_layers
...
def training_log(
    ...
    batch_size = (
        args.micro_batch_size
        * args.data_parallel_size
        # Match arguments.py / common_utils.py: legacy entry points and host-only tests
        # build args without the GTP knob, which defaults to a single remat shard.
    ...
    if (
        ...
        if args.csa_compress_ratios is not None and is_hybrid_model(args):
            # Hybrid DSv4/CSA stacks derive their tracker slots from the layer pattern
            # (not num_layers + mtp_num_layers) and repeat each inner-pattern indexer
            # once per unshared prediction depth. Main's helper already does both,
            # plus the --csa-dense-mode override.
            tracked_layers, num_indexer_layers = _get_indexer_logging_layer_counts(args)
        elif args.csa_compress_ratios is not None:
            # Standard stacks index the ratio list by executed logical layer number, so a
            # repeated MTP layer reuses ratio N+1 at every depth and unused tail entries
            # must not dilute the reported loss.
            compress_ratios = [
            ...
            if getattr(args, "csa_dense_mode", False):
                # Dense mode runs every CSA layer without an indexer, so no indexer
                # loss is produced even though the ratios still select C layers.
...
def train(
    ...
    # Legacy hybrid-CP wrapper only for the deprecated ``--hybrid-context-parallel`` flag and
    # only when dev's packing schedulers are not in play: ``train_step`` wraps the iterator via
    # ``wrap_data_iterator`` whenever ``config.sequence_packing_scheduler`` is set, and
    # ModelParallelConfig.__post_init__ forces that to ``default_dynamic_cp`` for dynamic CP
    # (and for the deprecated alias it migrates to). Gating on the scheduler the same way
    # ``build_pretraining_data_loader`` does avoids wrapping the iterator twice.
    ...
    while not _finished_training(iteration):
        ...
        # Drain the vision-encoder stats the same way. Returns ``(None, None, None)``
        # when vision FLOPs accounting is off or the model has no vision encoder.
        (
            vision_total_tokens_in_batch,
            vision_seqlen_squared_sum_in_batch,
            vision_merged_tokens_in_batch,
