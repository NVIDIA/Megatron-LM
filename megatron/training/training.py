    ...
    def transformer_flops():
        ...
        if is_linear_attention_variant(args.experimental_attention_variant):
            ...
            if args.experimental_attention_variant == "gated_delta_net":
                ...
                raise ValueError(
                    f"Invalid experimental_attention_variant: {args.experimental_attention_variant}"
                )
        else:
...
def preprocess_common_state_dict(common_state_dict):
    ...
    # Remove rank and local rank from state dict if it exists, since they are expected to be different  # noqa: E501
...
def pretrain(
    ...
    # Initialize program_start_global with a fallback value in case set_startup_timestamps() wasn't called  # noqa: E501
    ...
    if program_start is not None and main_entry is not None and pretrain_entry is not None:
        ...
        startup_timers = {
            ...
            - program_start_global,  # Local program start timestamp vs the global earliest program start timestamp  # noqa: E501
            ...
            - program_start_global,  # Local megatron init vs the global earliest program start timestamp  # noqa: E501
        }
    ...
    if args.perform_rl_step:
        if (
            ...
            # mempool so we can prefetch weights to CPU when idle while keeping CUDA-graph-safe pointers.  # noqa: E501
        ...
        if args.rl_offload_inference_model_weights_when_idle and inference_model is None:
            raise ValueError(
                "--rl-offload-inference-model-weights-when-idle requires a separate inference model. "  # noqa: E501
                "This flag is only useful when doing refit since the weights are shared with the training model."  # noqa: E501
            )
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        ...
        for vp_stage in range(len(model)):
            ...
            assert "vp_stage" in dataset_provider_parameters, (
                "vp_stage must be a kwarg in train_valid_test_dataset_provider when using virtual pipeline parallelism"  # noqa: E501
            )
    ...
    # Track if training is enabled. Can only be done once args.do_train is assigned after dataloader is built.  # noqa: E501
...
def get_model(
    ...
    # Materialize tensors on meta device (GPU allocation) if not using FSDP2 and not using Megatron FSDP.  # noqa: E501
...
def setup_model_and_optimizer(model_provider_func, model_type, checkpointing_context=None):
    ...
    if args.skip_train:
        ...
        if 'muon' not in config.optimizer:
            # If the user is asking for a non-zero embedding init std, skip weight decay for embeddings  # noqa: E501
...
def training_log(
    ...
    # On first iteration, log stats but don't reset accumulators so normal interval stats remain accurate.  # noqa: E501
    ...
    # learning rate will be None on ranks without trainable params, so we must gather across mp ranks  # noqa: E501
    ...
    if iteration % args.log_interval == 0 or is_first_iteration:
        ...
        if report_memory_flag:
            ...
            if iteration > (loaded_iteration + 1):
                # Make sure the memory after the second iteration is reported to include optimizer state memory.  # noqa: E501
...
def train(
    ...
    # IMPORTANT FIX: For RL training, reinitialize the microbatch calculator with the correct configuration  # noqa: E501
    if getattr(args, 'perform_rl_step', False):
    ...
    def get_e2e_base_metrics():
        ...
        return {
            ...
            'total_flops_since_current_train_start': num_floating_point_operations_since_current_train_start,  # noqa: E501
        ...
        if getattr(args, 'perform_rl_step', True):
            ...
            for valid_d in valid_ds:
                if args.skip_train or args.full_validation:
                    ...
                    if args.multiple_validation_sets:
                        # TODO(bnorick): for multiple validation sets without full validation, args.consumed_valid_samples is not  # noqa: E501
    ...
    if valid_dataloaders is not None:
        ...
        if args.multiple_validation_sets:
            if valid_dataloaders[0] is None:
                ...
                print(
                    f"[VALID DATA LOADER LENGTHS] , ".join(
                        f"{idx}: {len(dl)}" for idx, dl in enumerate(valid_dataloaders)
                    )
                )
