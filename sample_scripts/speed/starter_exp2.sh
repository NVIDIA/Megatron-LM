...
OUTPUT_LOG_PATH="${OUTPUT_LOG_PATH}/Exp2/${SLURM_JOB_ID}" # TODO I used job id to be a unique identifier for each run, you can change it with a unique value
SCRIPT_DIRECTORY="/N/slate/jindjia/LLM/Megatron-SpeedTest/sample_scripts/speed/Exp2" # TODO abslute path for Exp2 scripts

NUM_NODES_LIST=(4 8 16 )  # TODO Number of Nodes you will use to test on it. (Need to equal or smaller than your allocation)
SCRIPT_LIST=("2_7B_Baseline.sh" "2_7B_QGrad.sh" "2_7B_QWeightdiff.sh" "2_7B_QWG.sh" )  # TODO specify the training script you plan to run

# TODO Set master node and node list (will use ssh to connect each node)
...
for num_nodes in "${NUM_NODES_LIST[@]}"; do
    ...
    for script_name in "${SCRIPT_LIST[@]}"; do
        ...
        for node in $selected_nodes; do
            ...
            execute_script $node $full_script_path $OUTPUT_LOG_PATH $rank $run_in_background
            ((rank++))
        done
    done
done
