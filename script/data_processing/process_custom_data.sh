...
"""
自定义数据处理脚本
用于处理任意格式的数据集，支持多种配置选项
"""

# 设置默认参数
INPUT_PATH=${1:-"./dataset/custom/**/*.json"}
OUTPUT_PREFIX=${2:-"./dataset/custom_processed"}
WORKERS=${3:-16}
PARTITIONS=${4:-4}
...
OVERWRITE=${9:-"false"}
JSON_KEYS=${10:-"text"}
TOKENIZER_VOCAB_FILE=${11:-""}
TOKENIZER_MERGE_FILE=${12:-""}
...
# 检查分词器模型
if [ ! -d "$TOKENIZER_MODEL" ] && [ -z "$TOKENIZER_VOCAB_FILE" ]; then
    echo "错误: 分词器模型目录不存在: $TOKENIZER_MODEL"
    echo "或者请提供vocab文件和merge文件"
    exit 1
fi
...
# 添加分词器相关参数
if [ -n "$TOKENIZER_VOCAB_FILE" ] && [ -n "$TOKENIZER_MERGE_FILE" ]; then
    CMD="$CMD --tokenizer-vocab-file $TOKENIZER_VOCAB_FILE"
    CMD="$CMD --tokenizer-merge-file $TOKENIZER_MERGE_FILE"
else
    CMD="$CMD --tokenizer-model $TOKENIZER_MODEL"
fi

# 添加JSON键参数
if [ "$JSON_KEYS" != "text" ]; then
    CMD="$CMD --json-keys $JSON_KEYS"
fi
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "${OUTPUT_PREFIX}.bin" ]; then
        echo ""
        echo "文件统计:"
        echo "  .bin文件大小: $(du -h "${OUTPUT_PREFIX}.bin" | cut -f1)"
        echo "  .idx文件大小: $(du -h "${OUTPUT_PREFIX}.idx" | cut -f1)"
        
        # 尝试获取文档数量
        if [ -f "${OUTPUT_PREFIX}.idx" ]; then
            DOC_COUNT=$(wc -l < "${OUTPUT_PREFIX}.idx")
            echo "  文档数量: $DOC_COUNT"
        fi
    fi
