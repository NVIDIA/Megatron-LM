...
# Training script for deepseek2_lite on wikipedia with FA_linear_hifp8 quantization
# FA QK + Linear quantization with hifp8
...
QUANT_TYPE="FA_linear_hifp8"
EXPERIMENT_NAME="wikipedia_FA_linear_hifp8"
...
parse_arguments() {
    ...
    if [[ "fp8" == "fp8" ]]; then
        ...
        if [[ "hifp8" != "None" ]]; then
            cmd+=("--linear-quantization" "hifp8")
        fi
        if [[ "hifp8" != "None" ]]; then
            cmd+=("--attention-quantization" "hifp8")
        fi
    fi
    ...
    if [[ "$dry_run" == true ]]; then
        ...
        echo "FA QK + Linear quantization with hifp8"
        exec "${cmd[@]}"
    fi
}
