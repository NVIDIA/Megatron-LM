...
# Training script for deepseek2_lite on wikipedia with FA_linear_mxfp4 quantization
# FA QK + Linear quantization with mxfp4
...
QUANT_TYPE="FA_linear_mxfp4"
EXPERIMENT_NAME="wikipedia_FA_linear_mxfp4"
...
parse_arguments() {
    ...
    if [[ "fp8" == "fp8" ]]; then
        ...
        if [[ "mxfp4" != "None" ]]; then
            cmd+=("--linear-quantization" "mxfp4")
        fi
        if [[ "mxfp4" != "None" ]]; then
            cmd+=("--attention-quantization" "mxfp4")
        fi
    fi
    ...
    if [[ "$dry_run" == true ]]; then
        ...
        echo "FA QK + Linear quantization with mxfp4"
        exec "${cmd[@]}"
    fi
}
