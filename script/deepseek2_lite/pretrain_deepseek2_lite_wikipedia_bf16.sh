...
# Script: pretrain_deepseek2_lite_wikipedia_bf16.sh
...
CHECKPOINT_PATH=${1:-"checkpoints/deepseek2_lite/pretrain_deepseek2_lite_wikipedia_bf16"}
TENSORBOARD_LOGS_PATH=${2:-"tensorboard_logs/deepseek2_lite_bf16"}
...
bash examples/deepseek2_lite/train_deepseek2_lite_h100_fp8.sh \
    ...
    "$DATA_ARG" \
    "$DTYPE" \
    2>&1 | tee "${HOST_TENSORBOARD_LOGS_PATH}/training_pretrain_deepseek2_lite_wikipedia_bf16_$(date +'%y-%m-%d_%H-%M-%S').log"
