...
# Training script for llama31-8b on wikipedia with FA QK mxfp8 quantization
# Only quantizes QK in Flash Attention, keeps other parts in bf16
...
QUANT_TYPE="FA_QK_mxfp8"
EXPERIMENT_NAME="wikipedia_FA_QK_mxfp8"
...
This script only quantizes QK in Flash Attention, keeping other parts in bf16.
...
parse_arguments() {
    ...
    local cmd=(
        ...
        # FA QK quantization arguments - only quantize QK in Flash Attention
        "--fp8-format" "mxfp8"
        "--fp8-amax-history-len" "1024"
        "--fp8-amax-compute-algo" "max"
        "--attention-quantization" "mxfp8"
        # Note: No --linear-quantization flag, so linear layers remain in bf16
    )
...
main() {
    ...
    echo "This configuration only quantizes QK in Flash Attention, keeping linear layers in bf16"
    parse_arguments "$@"
}
