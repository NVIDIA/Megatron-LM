...
# Script: pretrain_llama31-8b_wikipedia_linear_mxfp4.sh
...
CHECKPOINT_PATH=${1:-"checkpoints/llama31-8b/pretrain_llama31-8b_wikipedia_linear_mxfp4"}
...
bash examples/llama/train_llama3_8b_h100_fp8.sh \
    ...
    "$DATA_ARG" \
    "$DTYPE" \
    2>&1 | tee "${HOST_TENSORBOARD_LOGS_PATH}/training_pretrain_llama31-8b_wikipedia_linear_mxfp4_$(date +'%y-%m-%d_%H-%M-%S').log"
