...
# =============================================================================
# Training Script for LLAMA31-8B - Updated with new pattern
# Script: pretrain_llama31-8b_wikipedia_linear_mxfp8.sh
...
CHECKPOINT_PATH=${1:-"checkpoints/llama31-8b/pretrain_llama31-8b_wikipedia_linear_mxfp8"}
TENSORBOARD_LOGS_PATH=${2:-"tensorboard_logs/llama31-8b_mxfp8"}
TOKENIZER_ARG=${3:-"model/llama31-8b"}
...
sed -i "s/^\([[:space:]]*custom_quant_type[[:space:]]*=[[:space:]]*\)'[^']*'/'mxfp8'/" \
...
bash examples/llama/train_llama3_8b_h100_fp8.sh \
    ...
    "$DATA_ARG" \
    "$DTYPE" \
    2>&1 | tee "${HOST_TENSORBOARD_LOGS_PATH}/training_pretrain_llama31-8b_wikipedia_linear_mxfp8_$(date +'%y-%m-%d_%H-%M-%S').log"
