...
# Script: pretrain_llama32-1b_dolma_FA_hifp8.sh
...
CHECKPOINT_PATH=${1:-"checkpoints/llama32_1b/pretrain_llama32-1b_dolma_FA_hifp8"}
...
bash examples/llama/train_llama32_1b_h100_fp8.sh \
    ...
    "$DATA_ARG" \
    "$DTYPE" \
    2>&1 | tee "${HOST_TENSORBOARD_LOGS_PATH}/training_pretrain_llama32-1b_dolma_FA_hifp8_$(date +'%y-%m-%d_%H-%M-%S').log"
