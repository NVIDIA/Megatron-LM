...
`tests/functional_tests/test_cases/{package_path}/{test_case}/model_config.yaml`, where
`package_path` is `core/models/gpt` for GPT and `core/transformer/moe` for MoE.

1-node test cases use the `_1node` suffix:
`tests/functional_tests/test_cases/{package_path}/{test_case}_1node/model_config.yaml`
...
```bash
# Trivial copy
mkdir -p tests/functional_tests/test_cases/{package_path}/{test_case}_1node
cp tests/functional_tests/test_cases/{package_path}/{test_case}/model_config.yaml \
   tests/functional_tests/test_cases/{package_path}/{test_case}_1node/model_config.yaml
