...
shared helpers, `correctness/` tests, and `kernels/` tests together. Keep other
shared test infrastructure and tests for code outside `megatron/` as flat
files at the unit-test root.
...
6. If the change adds or modifies a GPU kernel (Triton, `jit_fuser` /
   ...
   `tests/unit_tests/core/determinism/kernels/` and register it in
   `tests/unit_tests/core/determinism/kernels/manifest.py`. The `linting` CI job
