...
- [ ] Mechanism confirmed by a LAUNCH COUNT change, not only by a bucket time —
      counts are sequence-length invariant, per-bucket times are not
- [ ] If two traces are compared: buckets the change cannot touch held still. If they
      moved, the windows sit at different average KV length and no time is comparable
...
- [ ] DTYPE checked, not just length: does any buffer carry a tensor narrower than
      itself? A collective buffer wider than its payload doubles wire bytes and adds
      a cast per layer (this was worth 2.5%)
