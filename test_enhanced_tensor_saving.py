...
"""
测试增强版tensor保存功能
验证forward和backward的输入输出tensor保存
"""
...
def test_enhanced_tensor_saving():
    """测试增强版tensor保存功能"""
    print("=== 测试增强版Tensor保存功能 ===")
    ...
    results = save_attention_tensors(
        ...
        metadata={
            ...
            "test": "attention_forward_input"
        }
    )
    print(f"保存输入tensor: {len([r for r in results.values() if r is not None])} 个文件")
    ...
    output_result = save_tensor(
        ...
        metadata={
            ...
            "seq_len": seq_len,
            "hidden_size": hidden_size,
            "test": "attention_forward_output"
        }
    )
    print(f"保存输出tensor: {output_result}")
    ...
    results = save_linear_tensors(
        ...
        metadata={
            ...
            "output_size": output_size,
            "input_size": input_size,
            "test": "linear_backward_input"
        }
    )
    print(f"保存backward输入tensor: {len([r for r in results.values() if r is not None])} 个文件")
    ...
    output_result = save_tensor(
        ...
        metadata={
            ...
            "input_size": input_size,
            "output_size": output_size,
            "test": "linear_backward_output"
        }
    )
    print(f"保存backward输出tensor: {output_result}")
    ...
    for quant_type in quant_types:
        ...
        print(f"  保存结果: {result is not None}")
...
def analyze_saved_tensors():
    ...
    # 查找保存的tensor文件
    tensor_dir = os.environ.get('TENSOR_SAVE_DIR', './enhanced_tensor_logs')
    ...
    for file_path in tensor_files:
        try:
            ...
            if 'input' in metadata['tensor_name']:
                input_count += 1
            if 'output' in metadata['tensor_name']:
                output_count += 1
    ...
    print(f"\n统计结果:")
    print(f"  输入tensor: {input_count} 个")
    print(f"  输出tensor: {output_count} 个")
    print(f"  Forward操作: {forward_count} 个")
    print(f"  Backward操作: {backward_count} 个")
    print(f"  Attention层: {attention_count} 个")
    print(f"  Linear层: {linear_count} 个")
    
    # 显示前几个文件的信息
    print(f"\n前5个文件信息:")
    for i, file_path in enumerate(tensor_files[:5]):
        try:
            data = torch.load(file_path, map_location='cpu')
            metadata = data['metadata']
...
def main():
    """主函数"""
    print("开始测试增强版Tensor保存功能...")
    
    try:
        # 运行测试
        test_enhanced_tensor_saving()
