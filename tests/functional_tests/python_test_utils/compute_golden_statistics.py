...
def format_summary(stats: Dict[str, Any], tolerances: Dict[str, Dict[str, float]]) -> str:
    ...
    for metric_name in sorted(stats.keys()):
        ...
        if tol:
            lines.append(f"  Max observed relative variance: {tol.get('max_observed_relative_variance', 'N/A'):.4%}")
            lines.append(f"  Recommended relative tolerance: {tol.get('relative_tolerance', 'N/A'):.2%}")
            lines.append(f"  Recommended absolute tolerance: {tol.get('absolute_tolerance', 'N/A'):.2e}")
        ...
        if example_steps:
            ...
            for step in example_steps:
                s = values[step]
                lines.append(f"    Step {step}: mean={s['mean']:.6g}, std={s['std']:.6g}, "
...
def main():
    ...
    if args.results_dir:
        ...
        if not result_files:
            ...
            logger.info("The script looks for .out files and parses them to find the result JSON paths.")
            logger.info("Each .out file should contain: 'This test wrote results into /opt/megatron-lm/runs/<uuid>'")
            sys.exit(1)
    ...
    if args.recommend_tolerances or args.verbose:
        ...
        training_metrics = ["lm-loss", "lm loss", "iteration-time", "mem-allocated-bytes", "mem-max-allocated-bytes"]
        # Inference test metrics
        inference_metrics = ["latency", "step_count", "prompt_logprob_mean", "generated_logprob_mean"]
        ...
        for metric_name in training_metrics + inference_metrics:
            if metric_name in tolerances:
                ...
                print(f"{var_name}_RELATIVE_TOLERANCE = "
                      f"{tol['relative_tolerance']}  # {tol['relative_tolerance']:.2%}")
                print(f"{var_name}_ABSOLUTE_TOLERANCE = "
                      f"{tol['absolute_tolerance']:.2e}")
