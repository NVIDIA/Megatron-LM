...
#
# Arguments:
#   test_script  - Repository-relative path to a functional test script (required)
#   num_jobs     - Number of jobs to submit (default: 10)
#   partition    - Slurm partition to use (default: interactive)
#
# Examples:
#   ./tests/functional_tests/run_batch_ci_tests.sh tests/functional_tests/core/transformer/moe/my_test.sh
#   ./tests/functional_tests/run_batch_ci_tests.sh tests/functional_tests/core/models/gpt/my_test.sh 5
#   ./tests/functional_tests/run_batch_ci_tests.sh tests/functional_tests/core/models/bert/my_test.sh 10 batch_block1
#
# To list available test scripts:
#   ./tests/functional_tests/run_batch_ci_tests.sh --list
#   ./tests/functional_tests/run_batch_ci_tests.sh --list core/transformer/moe
#   ./tests/functional_tests/run_batch_ci_tests.sh --list core/models/gpt
#

set -e

TESTS_DIR="tests/functional_tests"

# Function to list available test scripts
list_tests() {
    local filter="${1:-}"
    echo "Available test scripts in $TESTS_DIR/:"
    echo
    if [ -n "$filter" ]; then
        # List tests in specific subdirectory
        if [ -d "$TESTS_DIR/$filter" ]; then
            find "$TESTS_DIR/$filter" -name "*.sh" -type f | sort
        else
            echo "No $TESTS_DIR/$filter directory found."
            echo "Available subdirectories:"
            find "$TESTS_DIR" -mindepth 1 -type d | sort | sed "s|^$TESTS_DIR/|  |"
            exit 1
        fi
    else
...
# e.g., "tests/functional_tests/core/models/gpt/my_test.sh" -> "my_test"
