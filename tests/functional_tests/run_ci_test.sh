...
run_training_phase() {
    ...
    bash $ROOT_DIR/tests/functional_tests/_run_training.sh || TRAINING_EXIT_CODE=$?
...
for i in $(seq 1 $N_REPEAT); do
    ...
    if [[ "$NODE_RANK" -eq 0 && (${SKIP_PYTEST:-0} != 1 || "$TEST_TYPE" == "release") ]]; then
        ...
        if [[ "$MODE" == "pretraining" ]]; then
            ...
            uv run --no-sync python $ROOT_DIR/tests/functional_tests/get_test_results_from_tensorboard_logs.py \
    ...
    if [[ "$NODE_RANK" -eq 0 ]]; then
        ...
        if [[ "$MODE" == "pretraining" && ("$TRAINING_EXIT_CODE" -eq 0 || "$TEST_TYPE" == "release") ]]; then
            if [[ "$TEST_TYPE" == "checkpoint-consistency" ]]; then
                ...
                uv run --no-sync python $ROOT_DIR/tests/functional_tests/test_optimizer_grads_match.py "${ITER_CHECKPOINT_DIRS[@]}"
            else
                uv run --no-sync pytest -s -o log_cli=true --log-cli-level=info $ROOT_DIR/tests/functional_tests/test_pretraining_regular_pipeline.py \
                ...
                if [[ "$TEST_TYPE" == "ckpt-resume" || "$TEST_TYPE" == "frozen-resume" ]]; then
                    ...
                    uv run --no-sync pytest -s -o log_cli=true --log-cli-level=info $ROOT_DIR/tests/functional_tests/test_pretraining_resume_checkpoint_pipeline.py \
        ...
        if [[ "$MODE" == "inference" && ("$TRAINING_EXIT_CODE" -eq 0 || "$TEST_TYPE" == "release") ]]; then
            if [[ "$TEST_TYPE" == "frozen-start" ]]; then
                uv run --no-sync pytest -s -o log_cli=true --log-cli-level=info $ROOT_DIR/tests/functional_tests/test_inference_regular_pipeline.py \
                    --golden-values-path $GOLDEN_VALUES_PATH \
        ...
        if [[ "$MODE" == "rl" && ("$TRAINING_EXIT_CODE" -eq 0 || "$TEST_TYPE" == "release") ]]; then
            if [[ "$TEST_TYPE" == "frozen-start" ]]; then
                ...
                uv run --no-sync python $ROOT_DIR/tests/functional_tests/get_test_results_from_tensorboard_logs.py \
                    ...
                    --output-path ${OUTPUT_PATH}/$(basename $GOLDEN_VALUES_PATH) \
                    "${EXTRACT_ARGS[@]}"
                uv run --no-sync pytest -s -o log_cli=true --log-cli-level=info $ROOT_DIR/tests/functional_tests/test_grpo_training_loop.py \
                    --golden-values-path $GOLDEN_VALUES_PATH \
