...
def test_ScatterToModelParallelRegion():
    ...
    expected_output = torch.cat((torch.ones(8) * 0, torch.ones(8) * 1, torch.ones(8) * 2, torch.ones(8) * 3)).cuda()
    if Utils.rank >= 4:
...
def test_ScatterToSequenceParallelRegion():
    ...
    expected_output = torch.concat((torch.ones(4) * 0, torch.ones(4) * 1, torch.ones(4) * 2, torch.ones(4) * 3)).cuda()
    if Utils.rank >= 4:
        expected_output = expected_output + 4
...
def test_GatherFromSequenceParallelRegion():
    ...
    input_data = torch.vstack((torch.ones(4) * 0, torch.ones(4) * 1, torch.ones(4) * 2, torch.ones(4) * 3)).cuda()
