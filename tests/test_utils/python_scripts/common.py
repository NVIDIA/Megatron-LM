...
def flatten_workload(
    ...
    for product in products:
        ...
        workload["spec"] = {
            k: v for k, v in workload["spec"].items() if k not in product.keys()
        }
...
def set_build_dependency(
    workload_manifests: List[dotdict],
) -> List[dotdict]:
...
def filter_by_test_case(
    workload_manifests: List[dotdict], test_case: str
) -> Optional[dotdict]:
...
def filter_by_environment(
    workload_manifests: List[dotdict], environment: str
) -> List[dotdict]:
...
def filter_by_platform(
    workload_manifests: List[dotdict], platform: str
) -> List[dotdict]:
    workload_manifests = list(
        workload_manifest
        for workload_manifest in workload_manifests
...
def filter_by_tag(workload_manifests: List[dotdict], tag: str) -> List[dotdict]:
    ...
    workload_manifests = list(
        workload_manifest
        for workload_manifest in workload_manifests
        if hasattr(dotdict(**workload_manifest["spec"]), "tag") and workload_manifest["spec"]["tag"] == tag
    )
...
def filter_by_test_cases(
    workload_manifests: List[dotdict], test_cases: str
) -> List[dotdict]:
    """Returns a workload with matching name. Raises an error if there no or more than a single workload."""
    workload_manifests = list(
        workload_manifest
...
def load_workloads(
    ...
    if workloads and environment:
        workloads = filter_by_environment(
            workload_manifests=workloads, environment=environment
        )
    ...
    if workloads and test_cases != "all":
        workloads = filter_by_test_cases(
            workload_manifests=workloads, test_cases=test_cases
        )

    if workloads and test_case:
        workloads = [
            filter_by_test_case(workload_manifests=workloads, test_case=test_case)
        ]
    ...
    for workload in list(workloads):
        for build_workload in build_workloads:
            if (
                ...
                container_image = (
                    container_image or build_workload.spec["source"]["image"]
                )
                build_workload.spec["source"]["image"] = (
                    f"{container_image}:{container_tag}"
                )
