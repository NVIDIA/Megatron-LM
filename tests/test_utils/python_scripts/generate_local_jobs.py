...
@click.command()
@click.option(
    "--model", required=False, type=str, help="Filters all tests by matching model"
)
@click.option(
    "--scope", required=False, type=str, help="Filters all tests by matching scope"
)
@click.option(
    "--test-case",
    required=False,
    type=str,
    help="Returns a single test-case with matching name.",
)
@click.option(
    "--environment",
    required=True,
...
def main(
    ...
    for workload in workloads:
        ...
        with open(file_path, "w", encoding="utf-8") as fh:
            fh.write(
                f"export ENABLE_LIGHTWEIGHT_MODE={str(enable_lightweight_mode).lower()}\n"
            )
