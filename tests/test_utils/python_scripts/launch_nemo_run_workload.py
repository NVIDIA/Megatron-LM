...
def main(scope, model, test_case, environment, platform):
    ...
    inline_script = run.Script(
        inline=workload.spec["script"]
    )

    artifacts = ["{host_path}:{mount_path}".format(mount_path=mount_path, host_path=str(pathlib.Path("/root") / host_path)) for mount_path, host_path in workload.spec["artifacts"].items()]
    ...
    executor = run.DockerExecutor(
        ...
        ipc_mode="host",
        shm_size="30g",
        env_vars={"PYTHONUNBUFFERED": "1", "OUTPUT_PATH": os.getcwd(), "ENABLE_LIGHTWEIGHT_MODE": "true"},
