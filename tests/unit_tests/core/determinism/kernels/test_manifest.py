...
from tests.unit_tests.core.determinism.kernels import manifest
...
def test_registered_test_imports_kernel(entry):
    ...
    for test in entry.tests:
        if not test.startswith("tests/unit_tests/core/determinism/kernels/") or not sources:
            continue
...
def test_every_kernel_bearing_file_is_registered():
    ...
    assert not unregistered, (
        ...
        "test, or an exempt_reason, in tests/unit_tests/core/determinism/kernels/manifest.py):\n  "
        + "\n  ".join(unregistered)
    )
