...
def test_sft_batch(tp_size, pp_size, cp_size, seq_length):
    ...
    # Main moved get_batch from a fixed tuple to ContextParallelBatch; padding_mask and
    # packed_seq_params are no longer positional members of BATCH_KEYS, so read them through
    # main's accessors (same pattern as pretrain_hybrid.forward_step) to keep dev's assertions.
