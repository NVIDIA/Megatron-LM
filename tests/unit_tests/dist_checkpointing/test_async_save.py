    ...
    def test_retry_on_transient_error_with_eventual_success(self, tmp_path_dist_ckpt, caplog):
        ...
        def mock_write_item_fail_twice(*args, **kwargs):
            ...
            if call_count['count'] <= 2:
                raise IOError(f"Transient error on attempt {call_count['count']}")
        ...
        write_bucket = (
            ...
            'storage_key',
            ([mock.MagicMock()], []),  # (bytes_data, tensor_data)
        )
        ...
        assert 'IOError: Transient error on attempt 1' in warning_logs[0].message
