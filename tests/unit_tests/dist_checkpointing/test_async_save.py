...
class TestHasNvrxAsyncSupport:
    ...
    def test_get_async_strategy_rejects_old_nvrx(self):
        """Explicit NVRx async still fails clearly when the installed version is too old."""
        with mock.patch(
            'megatron.core.dist_checkpointing.strategies.torch.is_nvrx_min_version',
            return_value=False,
        ):
            with pytest.raises(ModuleNotFoundError, match="minimum version"):
