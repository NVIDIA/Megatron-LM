...
class TestLayerWiseOptimizer:
    ...
    def test_layer_wise_optimizer_resharding(
        self, tmp_path_dist_ckpt, src_tp_pp, dest_tp_pp
    ):
        ...
        with TempNamedDir(
            ...
            load_sharded_sd = optimizer_B.sharded_state_dict(
                model_sharded_sd, is_loading=True
            )
    ...
    def test_layer_wise_optimizer_with_moe(self, tmp_path_dist_ckpt, tp_pp_ep):
        ...
        with TempNamedDir(
            tmp_path_dist_ckpt / 'test_layer_wise_moe', sync=True
        ) as ckpt_dir:
            ...
            load_sharded_sd = optimizer_new.sharded_state_dict(
                model_sharded_sd, is_loading=True
            )
            state_dict = load(load_sharded_sd, ckpt_dir)
...
class TestLayerWiseOptimizerIntegration:
    ...
    @pytest.mark.parametrize('bf16', [True, False])
    def test_layer_wise_with_save_checkpoint(
        self, tmp_path_dist_ckpt, tp_pp, bf16
    ):
