    ...
    **config_kwargs,
    ...
    if is_moe:
        layer_spec = get_gpt_decoder_block_spec(
            transformer_config, use_transformer_engine=HAVE_TE, vp_stage=vp_stage
        )
    else:
        layer_spec = gpt_te_spec(multi_latent_attention=is_mla) if HAVE_TE \
            else gpt_local_spec(multi_latent_attention=is_mla)
    this_model = GPTModel(
