...
def test_save_and_load_checkpoint_pp(tmp_path_dist_ckpt):
    src_layer_spec_fn = gpt_te_spec if HAVE_TE else gpt_local_spec
    dst_layer_spec_fn = gpt_te_spec if HAVE_TE else gpt_local_spec
    use_fpsl = False
    load_order = 'tp-dp-pp'
...
def test_save_and_load_checkpoint_vpp(
    ...
    iteration = 123
    layer_spec_fn = get_gpt_decoder_block_spec if is_moe else gpt_te_spec if HAVE_TE else gpt_local_spec
