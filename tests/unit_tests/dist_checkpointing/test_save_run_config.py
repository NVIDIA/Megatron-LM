...
def test_save_checkpoint_skips_run_config_for_non_torch_dist(
    init_model_parallel, create_args, tmp_path_dist_ckpt
):
    """No run_config.yaml is written for checkpoint formats other than torch_dist."""
    args = create_args
    ...
    run_config_data = {"_target_": "dummy.Config"}

    with TempNamedDir(
        tmp_path_dist_ckpt / "test_save_checkpoint_skips_run_config", sync=True
    ) as save_dir:
        args.save = save_dir
        args.save_tokenizer_assets = False
        set_args(args)
        ...
        assert not os.path.exists(run_config_filename)
