...
def setup_model_and_optimizer(
    ...
    def _init_states(optimizer):
        # Hybrid LayerWise + DistOpt nests a ChainedOptimizer inside the top-level one; recurse so
        # the Muon Float16 sub-optimizers get seeded. Optimizers without ``init_state_fn`` (DistOpt,
        # seeded elsewhere) are skipped.
...
def setup_moe_model_and_optimizer(
    ...
    with mock.patch('megatron.training.training.get_args', new=lambda: mock_args):
        init_basic_mock_args(mock_args, tp, pp, bf16=bf16)
        mock_args.ddp_bucket_size = ddp_bucket_size
        # ``resolve_ddp_bucket_size`` discards an explicit bucket_size unless overlap_grad_reduce
        # is on. Turn it on (no backward is run here) so ddp_bucket_size splits the sibling DistOpt
        # buffer into small buckets -> some DP rank owns an all-padding shard (empty-bucket-synth).
