...
def _reference_orthogonalize(opt, grad):
    ...
    try:
        torch.set_float32_matmul_precision(opt._fp32_matmul_prec)
...
def test_dist_muon_matches_reference(strategy, nesterov, mesh_case, shapes):
    ...
    step_weight_decays = (0.0, 0.02, 0.005)
    for step, live_weight_decay in enumerate(step_weight_decays):
        opt.param_groups[0]["weight_decay"] = live_weight_decay
        ...
        for i, s in enumerate(shapes):
            ...
            if live_weight_decay != 0.0:
                ref_w[i].mul_(1.0 - lr * live_weight_decay)
