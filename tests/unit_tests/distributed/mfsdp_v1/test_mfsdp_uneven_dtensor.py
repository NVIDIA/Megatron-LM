...
def test_uneven_dtensor_checkpoint_round_trip(distributed_setup, tmp_path_dist_ckpt):
    ...
    source_state_dict = preprocess_state_dict_for_uneven_dtensor(
        {"weight": source_dtensor}
    )
    ...
    target_state_dict = preprocess_state_dict_for_uneven_dtensor(
        {"weight": target_dtensor}
    )
