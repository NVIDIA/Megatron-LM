...
class TestMegatronFSDPE2E:
    ...
    @staticmethod
    def _assert_meta_initialized_tied_embeddings_match(model):
        """Check both optimizer-facing and first compute weights across PP endpoints."""
        embd_group = mpu.get_embedding_group()
        if get_pg_size(
            embd_group
        ) <= 1 or torch.distributed.get_rank() not in torch.distributed.get_process_group_ranks(
            embd_group
        ):
            return
        ...
        assert model_module.share_embeddings_and_output_weights
        main_weight = model_module.shared_embedding_or_output_weight().detach()
        assert isinstance(main_weight, DTensor)

        def assert_group_equal(tensor):
            gathered = [torch.empty_like(tensor) for _ in range(get_pg_size(embd_group))]
            torch.distributed.all_gather(gathered, tensor, group=embd_group)
            for peer_tensor in gathered[1:]:
                assert_close(gathered[0], peer_tensor, rtol=0, atol=0)

        assert_group_equal(uneven_dtensor_to_full_tensor(main_weight))

        model_chunk.module.unshard_parameters_for_forward_only()
        compute_weight = model_module.shared_embedding_or_output_weight().detach().clone()
        assert not isinstance(compute_weight, DTensor)
        assert_group_equal(compute_weight)
        model_chunk.module._release_unsharded_parameter_groups()
    ...
    def _run_training(cls, use_mfsdp_v2, case, initial_parameters=None):
        ...
        try:
            ...
            overlap_case = case["model_config"].get("overlap_moe_expert_parallel_comm", False)
            runner = None
            if use_mfsdp_v2 and overlap_case:
                contexts = [model_chunk.module.context for model_chunk in model]
                assert all(context is contexts[0] for context in contexts)
                runner = contexts[0].runner
                if model_parallel_config.get("pipeline_model_parallel_size", 1) > 1:
                    cls._assert_meta_initialized_tied_embeddings_match(model)
            ...
            if runner is not None:
                ...
                assert runner._replayed_occurrences > 0
                assert runner._divergences == 0
                assert runner._complete_trace_calls == cls.NUM_STEPS
