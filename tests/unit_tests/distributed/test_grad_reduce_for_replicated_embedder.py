...
def test_allreduce_conditional_embedding_grads():
    ...
    model = [torch.nn.Linear(10, 10, bias=True).to(device=get_current_device()) for _ in range(2)]
    ...
    expect_weight_grad = torch.ones([10, 10]).to(device=get_current_device()) * expect_value

    for i, chunk in enumerate(model):
        expect_bias_grad = torch.ones([10]).to(device=get_current_device()) * (pp_rank * 10.0 + i)
