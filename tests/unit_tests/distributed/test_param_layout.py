...
class TestGroupParamsForBuffers:
    ...
    def test_gtp_managed_excluded_from_bucket_under_cp(self):
        """Dense GTP param whose materialization group folds CP in -> excludes_cp_from_bucket.

        The flag is stamped at GTP wrap time from the group the weight is actually sharded
        over, so its own reduce-scatter already summed CP and the ordinary bucket collective
        must skip it -- expressed as a distinct BufferKey.
        """
        gtp_param = _make_param_with_attrs(
            (100,), is_gtp_weight_remat=True, excludes_cp_from_bucket=True
        )
    ...
    def test_gtp_managed_not_excluded_without_cp(self):
        """Same GTP param, but its group does not fold CP in (no stamp) -> no-op flag."""
    ...
    def test_expert_gtp_managed_not_excluded_under_cp(self):
        ...
        expert_gtp_param = _make_param_with_attrs(
            (100,), is_gtp_weight_remat=True, allreduce=False, excludes_cp_from_bucket=True
        )
        result = group_params_for_buffers([expert_gtp_param], grad_reduce_in_fp32=True)
    ...
    def test_non_gtp_param_not_excluded_under_cp(self):
        """Ordinary (non-GTP) params carry no stamp, so they are never flagged."""
        plain_param = _make_params((100,))[0]
        result = group_params_for_buffers([plain_param], grad_reduce_in_fp32=True)
    ...
    def test_gtp_managed_and_plain_params_split_under_cp(self):
        ...
        plain_param = _make_params((100,))[0]
        result = group_params_for_buffers([gtp_param, plain_param], grad_reduce_in_fp32=True)
