...
class TestFusedProjRmsComputeHKeepFp32:
    ...
    def test_public_entry_point_native_branch(self, monkeypatch, backend):
        """The public op must run natively for the forced and the auto-fallback policy.
        ...
        outs = fused_mod.fused_proj_rms_compute_h(
            x, w, one, one, one, bias, n, 1e-6, backend=backend
        )
        for t in outs:
            assert torch.isfinite(t).all()
