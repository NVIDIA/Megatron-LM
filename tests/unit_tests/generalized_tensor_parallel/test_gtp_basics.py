...
Most multi-GPU tests require four ranks; the fused-layout regression also runs on eight.
"""
...
def _worker_gated_fused_microbatches(rank, world_size, port):
    ...
    dtype = torch.bfloat16
    gtp_remat_group = dist.new_group(list(range(world_size)))
    assert gtp_remat_group.size() == world_size
    ...
    # Both GTP4 and GTP8 divide the logical rows without padding; every rank owns a
    # distinct shard and participates in the forward gathers and backward reduction.
    assert out_f % world_size == 0
    shard_rows = out_f // world_size
    logicals = []
    for layer in layers:
        assert tuple(layer.weight.shape) == (shard_rows, in_f)
...
class TestGTPGatedFusedLayout:
    def test_gated_fused_layout_every_microbatch(self):
        ...
        _worker_gated_fused_microbatches(dist.get_rank(), world_size, None)
