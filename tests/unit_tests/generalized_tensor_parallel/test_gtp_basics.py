...
class TestGTPGraphWgradRing:
    ...
    def test_eager_and_graph_rings_coexist_and_reset(self, monkeypatch, eager_first):
        ...
        old_slots = [p._gtp_wgrad_ring_slot for p in bound]
        assert len(gtp_cuda_graphs._WGRAD_RINGS) == 3
        assert eager_view.data_ptr() not in [slot.tensor.data_ptr() for slot in old_slots[:-1]]
        gtp_module.initialize_graph_wgrad_rings()
        for param, slot in zip(bound, old_slots):
            assert param._gtp_wgrad_ring_slot is slot
        assert eager.get_wgrad_tensor().data_ptr() == eager_view.data_ptr()

        torch.cuda.synchronize()
        gtp_module.reset_gtp_state()
        assert not gtp_cuda_graphs._WGRAD_RINGS
        assert all(not hasattr(p, "_gtp_wgrad_ring_slot") for p in bound)
        assert all(not hasattr(p, "_gtp_wgrad_ring_view") for p in bound)
        gtp_module.initialize_graph_wgrad_rings()
        eager.get_wgrad_tensor(persistent=True)
        for param, old_slot in zip(bound, old_slots):
            assert param._gtp_wgrad_ring_slot is not old_slot
    ...
    def test_eager_ring_isolates_logical_shape_with_same_padded_shape(self, monkeypatch):
        ...
        assert grad.data_ptr() != longer._gtp_wgrad_ring_view.data_ptr()
    ...
    def test_partial_cg_wgrad_ring_ownership(self, monkeypatch):
        ...
        slot_1 = weights[1]._gtp_wgrad_ring_slot
        slot_2 = weights[2]._gtp_wgrad_ring_slot
        slot_3 = weights[3]._gtp_wgrad_ring_slot
        assert slot_1 is not slot_2
        assert slot_1 is slot_3
        assert len(gtp_cuda_graphs._WGRAD_RINGS) == 2
    ...
    def test_native_fp8_style_param_uses_wgrad_ring(self, monkeypatch):
        ...
        assert second._gtp_wgrad_ring_slot.tensor.shape == (6, 4)
        assert len(gtp_cuda_graphs._WGRAD_RINGS) == 1
    ...
    def test_rs_input_copies_into_ring(self, monkeypatch):
        ...
        assert rs_input is weights[1]._gtp_wgrad_ring_slot.tensor
