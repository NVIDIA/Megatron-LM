...
_WORLD_SIZES = [4, 8]
_PADDED_GROUPS = 7
_PADDED_ROWS = _PADDED_GROUPS * _GROUP
_PAD_ALIGNMENT = 16
_MLA_HEAD_SPLITS = [192, 64, 192, 64]
_MLA_ROWS = sum(_MLA_HEAD_SPLITS)
_MLA_GTP_SIZE = 8
_ATOL = 1e-4
_RTOL = 1e-4
...
def _make_muon(
    pg_collection, tp_mode="duplicated", *, split_qkv_per_head=False, qkv_split_shapes=_SPLIT
):
...
def _full_weight(rows=_ROWS):
    torch.manual_seed(0)
    weight = torch.randn(rows, _COLS, dtype=torch.float32, device="cuda")
    torch.distributed.broadcast(weight, src=0)
    return weight


def _tag_fragmented_qkv(tensor, *, num_groups=_GROUPS, pad_length=0):
    ...
    tensor.qkv_split_shapes_global = _SPLIT * num_groups
    tensor.qkv_split_groups_are_complete = False
    tensor.qkv_gtp_pad_length = pad_length
    tensor.pad_length = pad_length
...
def _reference_split_orth(opt, weight, tp_group):
    ...
    assert weight.shape[0] % _GROUP == 0
    num_groups = weight.shape[0] // _GROUP
...
def _padded_local_qkv_shard(weight, pg_collection):
    """Pad the global fused weight exactly as GTP does, then tag its local row shard."""
    gtp_size = torch.distributed.get_world_size(group=pg_collection.gtp_remat)
    gtp_rank = torch.distributed.get_rank(group=pg_collection.gtp_remat)
    pad_length = (-weight.shape[0]) % (_PAD_ALIGNMENT * gtp_size)
    assert pad_length > 0, "test shape must exercise real GTP alignment padding"
    padded = torch.nn.functional.pad(weight, (0, 0, 0, pad_length))
    shard_rows = padded.shape[0] // gtp_size
    shard = padded[gtp_rank * shard_rows : (gtp_rank + 1) * shard_rows].clone()
    return (
        _tag_fragmented_qkv(shard, num_groups=_PADDED_GROUPS, pad_length=pad_length),
        gtp_rank,
        shard_rows,
        pad_length,
    )
...
def _unequal_mla_local_shard(weight, pg_collection):
    """Tag one GTP8 shard of an unequal MLA per-head layout as production does."""
    gtp_size = torch.distributed.get_world_size(group=pg_collection.gtp_remat)
    gtp_rank = torch.distributed.get_rank(group=pg_collection.gtp_remat)
    assert gtp_size == _MLA_GTP_SIZE
    shard_rows = _MLA_ROWS // gtp_size
    local_start = gtp_rank * shard_rows
    local_split_shapes, local_heads_are_complete = _localize_qkv_split_shapes(
        _MLA_HEAD_SPLITS, local_start=local_start, local_rows=shard_rows
    )
    shard = weight[local_start : local_start + shard_rows].clone()
    shard.is_qkv = True
    shard.is_gtp_weight_remat = True
    shard.qkv_split_shapes = local_split_shapes
    shard.qkv_split_shapes_global = _MLA_HEAD_SPLITS
    shard.qkv_split_heads_are_complete = local_heads_are_complete
    shard.qkv_gtp_pad_length = 0
    shard.partition_dim = 0
    return shard, gtp_rank, shard_rows, local_heads_are_complete


def _reference_per_head_orth(opt, weight, tp_group, split_shapes):
    result = torch.empty_like(weight)
    offset = 0
    for rows in split_shapes:
        result[offset : offset + rows] = opt.scaled_orthogonalize_fn(
            weight[offset : offset + rows].clone(), tp_group, None
        )
        offset += rows
    return result
...
def _worker_padded_split_after_gather(rank, world_size, port):
    """The reconstructed QKV path removes padding before NS and restores zero rows."""
    _init_model_parallel(world_size)
    try:
        pg_collection = ProcessGroupCollection.use_mpu_process_groups()
        opt = _make_muon(pg_collection)
        weight = _full_weight(_PADDED_ROWS)
        expected = _reference_split_orth(opt, weight, pg_collection.tp)
        local, gtp_rank, shard_rows, pad_length = _padded_local_qkv_shard(
            weight, pg_collection
        )

        actual = opt.orthogonalize(local, local.clone())

        padded_expected = torch.nn.functional.pad(expected, (0, 0, 0, pad_length))
        expected_local = padded_expected[
            gtp_rank * shard_rows : (gtp_rank + 1) * shard_rows
        ]
        torch.testing.assert_close(actual, expected_local, atol=_ATOL, rtol=_RTOL)
    finally:
        ps.destroy_model_parallel()
...
def _worker_unequal_mla_heads_gather_consistently(rank, world_size, port):
    """Every GTP rank gathers even when two local shards contain complete MLA slices."""
    _init_model_parallel(world_size)
    try:
        pg_collection = ProcessGroupCollection.use_mpu_process_groups()
        opt = _make_muon(pg_collection, split_qkv_per_head=True, qkv_split_shapes=_MLA_HEAD_SPLITS)
        weight = _full_weight(_MLA_ROWS)
        expected = _reference_per_head_orth(opt, weight, pg_collection.tp, _MLA_HEAD_SPLITS)
        local, gtp_rank, shard_rows, local_heads_are_complete = _unequal_mla_local_shard(
            weight, pg_collection
        )

        # [192, 64, 192, 64] / GTP8 gives 64 rows per rank. Ranks 3 and 7
        # contain a complete 64-row RoPE slice; the other ranks cut a 192-row
        # content slice. This mixed local metadata must not control all-gather.
        assert local_heads_are_complete == (gtp_rank in (3, 7))
        actual = opt.orthogonalize(local, local.clone())
...
class TestGTPMuonQKVSplit:
    ...
    def test_unequal_mla_heads_gather_consistently(self):
        _requires_multi_gpu(_MLA_GTP_SIZE)
        _run_distributed(_worker_unequal_mla_heads_gather_consistently, _MLA_GTP_SIZE)
