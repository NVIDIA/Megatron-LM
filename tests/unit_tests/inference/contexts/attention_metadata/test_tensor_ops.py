...
def test_tensor_merge_basic(device, merge_params):
    params = merge_params
    ...
    tensor_merge(tensor_a, tensor_b, output_triton, pos_on_device, check_bounds=True)

    assert torch.equal(output_ref, output_triton)
    assert torch.equal(output_ref[:5], tensor_a[:5])
    assert torch.equal(output_ref[5:17], tensor_b[:12])
...
def test_tensor_merge_pos_zero(device, merge_params):
    ...
    tensor_merge(
        tensor_a, tensor_b, output_tensor, torch.tensor([0], device=device), check_bounds=True
    )
...
def test_tensor_merge_small(device):
    ...
    tensor_merge(tensor_a, tensor_b, output_tensor, torch.tensor([2], device=device))
