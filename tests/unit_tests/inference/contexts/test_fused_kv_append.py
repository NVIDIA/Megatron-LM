...
class TestFusedKVAppend:
    ...
    def test_single_token(self, cache_params):
        ...
        # Verify - M_2NCHD has layout [2, N, C, H, D]
        cache_content = cache.get_content()
        assert torch.allclose(cache_content[0, 3, 42, :, :], key[0, 0, :, :], rtol=1e-3, atol=1e-3)
        assert torch.allclose(cache_content[1, 3, 42, :, :], value[0, 0, :, :], rtol=1e-3, atol=1e-3)
