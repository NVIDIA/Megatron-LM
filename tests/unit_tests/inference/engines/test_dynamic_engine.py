...
class TestDynamicInferenceEngine(DynamicInferenceEngineTestBase):
    ...
    def test_async_scheduling_decode_only_cuda_graph_e2e(self, model_provider: str) -> None:
        ...
        assert (
            async_env.engine.controller._async_chained_decode_graph_launch_count > 0
        ), async_env.engine.controller._async_disable_reason
        assert (
            sum(count > 0 for count in async_env.engine.controller._async_sample_slot_launch_counts)
            > 1
        )
    ...
    def test_async_scheduling_top_k_sampling_gpt_cuda_graph_fallback_e2e(self) -> None:
        """Top-k sampling uses async forward overlap without replaying the greedy decode graph."""
        common_kwargs = dict(
            num_requests=4,
            min_prompt_length=4,
            max_prompt_length=4,
    ...
    def test_async_scheduling_prompt_logprobs_resume_decode_gpt_e2e(self) -> None:
        """Prompt logprobs are collected in prefill, then decode resumes async overlap."""
        common_kwargs = dict(
            num_requests=4,
            min_prompt_length=4,
            max_prompt_length=4,
        ...
        assert controller._async_decode_graph_launch_count == 0
        assert [request.generated_tokens for request in async_env.requests] == serial_tokens
        for request, expected_prompt_log_probs, expected_generated_log_probs in zip(
            async_env.requests, serial_prompt_log_probs, serial_generated_log_probs
        ):
            assert len(request.prompt_log_probs) == len(request.prompt_tokens) - 1
            assert request.prompt_log_probs == pytest.approx(expected_prompt_log_probs)
            assert len(request.generated_log_probs) == len(request.generated_tokens)
            assert request.generated_log_probs == pytest.approx(expected_generated_log_probs)
    ...
    def test_async_scheduling_generated_top_n_logprobs_gpt_cuda_graph_fallback_e2e(self) -> None:
        ...
        serial_tokens = [request.generated_tokens for request in serial_env.requests]
        serial_log_probs = [request.generated_log_probs for request in serial_env.requests]
        serial_top_n = [request.generated_top_n_logprobs for request in serial_env.requests]
        ...
        assert controller._async_decode_graph_launch_count == 0
        assert [request.generated_tokens for request in async_env.requests] == serial_tokens
        for request, expected_log_probs, expected_top_n in zip(
            async_env.requests, serial_log_probs, serial_top_n
        ):
            assert request.prompt_top_n_logprobs is None or len(request.prompt_top_n_logprobs) == 0
            assert len(request.generated_log_probs) == len(request.generated_tokens)
            assert request.generated_log_probs == pytest.approx(expected_log_probs)
            assert request.generated_top_n_logprobs == expected_top_n
    ...
    def test_async_scheduling_allows_prefix_caching_decode_only_cuda_graph_e2e(self) -> None:
        """Prefix caching config does not block async scheduling on decode-only steps."""
        skip_if_mamba_sequence_packing_not_available("hybrid")
        common_kwargs = dict(
            num_requests=4,
            min_prompt_length=4,
    ...
    def test_async_scheduling_staggered_add_cuda_graph_e2e(self) -> None:
        """Async scheduling stays correct when new requests are admitted between decode steps."""
        common_kwargs = dict(
            num_requests=4,
            min_prompt_length=4,
            max_prompt_length=4,
    ...
    def test_async_lifecycle_controller_disable_reason_matrix(self, monkeypatch) -> None:
        ...
        with torch.inference_mode():
            mtp_context.active_request_metadata["skip_prompt_log_probs"][0] = True
            mtp_context.active_request_metadata["return_log_probs"][0] = False
            mtp_context.active_request_metadata["top_n_logprobs"][0] = 3
        assert mtp_controller._async_scheduling_disabled_reason(allow_mtp=True) is None
        with torch.inference_mode():
            mtp_context.active_request_metadata["skip_prompt_log_probs"][0] = False
        assert mtp_controller._async_scheduling_disabled_reason(allow_mtp=True) is None
    ...
    def test_ep_dummy_async_handoff_launches_mirror_forward(self, monkeypatch) -> None:
        ...
        token_ids = torch.zeros((1, 1), device=torch.cuda.current_device(), dtype=torch.long)
    ...
    @pytest.mark.internal
    def test_ep_dummy_async_handoff_does_not_block_step_completion(self, monkeypatch) -> None:
        """Dummy async handoff must not synchronize before EP STEP_COMPLETE."""
        env = self._build_test_env(
            DynamicEngineTestConfig(
                num_requests=0,
                min_prompt_length=4,
        ...
        class UnexpectedSyncStream:
            def synchronize(self):
                calls.append(("unexpected-sync",))
                raise AssertionError("dummy async handoff must not synchronize")
        ...
        monkeypatch.setattr(controller, "_dummy_serial_mtp_forward", lambda: calls.append(("mtp",)))
        monkeypatch.setattr(
            controller,
        ...
        monkeypatch.setattr(env.engine.context, "reset", lambda: calls.append(("reset",)))
        monkeypatch.setattr(torch.cuda, "current_stream", lambda: UnexpectedSyncStream())
    ...
    def test_ep_dummy_forward_skips_base_forward_when_real_ranks_reuse_pending_forward(
        ...
        assert calls == [("begin", False, False), ("mtp",), ("handoff",), ("reset",)]
    ...
    def test_ep_global_row_map_defers_handoff_until_after_sampling(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            context, "release_deferred_async_kv_blocks", lambda: calls.append(("release",))
        )
        monkeypatch.setattr(
            controller,
        ...
        monkeypatch.setattr(
            controller, "_should_collect_dynamic_sampling_bookkeeping", lambda **kwargs: False
        )
        monkeypatch.setattr(
            controller,
    ...
    def test_ep_dummy_async_handoff_skip_does_not_forward(self, monkeypatch) -> None:
        ...
        assert calls == [("handoff", False, True), ("disable", "ep async handoff skipped")]

    @pytest.mark.internal
    def test_ep_fallback_async_forward_votes_handoff_without_async_graph(self, monkeypatch) -> None:
        ...
        assert calls == [("eligible", None), ("prepare",), ("handoff", True, True)]
