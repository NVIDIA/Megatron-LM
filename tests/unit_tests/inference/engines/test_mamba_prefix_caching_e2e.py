...
class TestMambaPrefixCachingE2E:
    ...
    def _assert_step(self, step, reqs_by_group, alloc, step_prefill, num_groups):
        ...
        if step == 1:
            for g in range(G):
                ...
                assert r[0].precomputed_block_hashes[0] in alloc.mamba_hash_to_block_id
            ...
            for g in range(G):
                ...
                assert r[1].precomputed_block_hashes[2] in alloc.mamba_hash_to_block_id
            ...
            for g in range(G):
                ...
                assert r[2].precomputed_block_hashes[1] in alloc.mamba_hash_to_block_id
                assert r[2].precomputed_block_hashes[2] in alloc.mamba_hash_to_block_id
                assert r[4].precomputed_block_hashes[3] in alloc.mamba_hash_to_block_id
            ...
            for g in range(G):
                ...
                assert r[3].precomputed_block_hashes[2] in alloc.mamba_hash_to_block_id
                h0 = r[0].precomputed_block_hashes[0]
                h1 = r[1].precomputed_block_hashes[1]
    ...
    def test_mamba_lru_eviction_e2e(self):
        ...
        assert h_E0 in alloc.mamba_hash_to_block_id and h_E0 in alloc.kv_hash_to_block_id
        assert len(alloc.mamba_hash_to_block_id) == 1 and alloc.total_avail == 1

        # F: disjoint prefix, forces eviction of E's cached block
        req_F = _run_one(1, prompts[1])
        assert req_F.precomputed_block_hashes[0] in alloc.mamba_hash_to_block_id
        assert h_E0 not in alloc.kv_hash_to_block_id and h_E0 not in alloc.mamba_hash_to_block_id
        assert len(alloc.mamba_hash_to_block_id) == 1
