...
class TestSpeculativeDecodingBatchDimensions:
    ...
    def test_ep_mixed_decode_prefill_with_speculative_tokens(self, num_cuda_graphs):
        ...
        if rank % 2 == 0:
            ...
            token_count = decode_reqs * (num_speculative_tokens + 1)
            real = BD(
                token_count=token_count, prefill_req_count=0, decode_req_count=decode_reqs
            )
        else:
        ...
        if result is not None:
            max_token_count = max(
                4 * (num_speculative_tokens + 1),
                2 * (num_speculative_tokens + 1) + 8,
            )
            assert result.token_count >= max_token_count, (
                f"Matched graph token_count {result.token_count} < "
                f"required {max_token_count}"
            )
