...
def test_submit_goes_to_prefill_then_prefill_done_picks_decode_round_robin():
    ...
    assert r.route_prefill_done(0) == "d0"
    assert r.route_prefill_done(1) == "d1"
    assert r.route_prefill_done(2) == "d0"
...
def test_routing_prefers_lower_load_and_round_robins_ties():
    ...
    assert r.route_prefill_done(0, {"d0": (3,), "d1": (1,)}.get) == "d1"
...
def test_forget_clears_request_state():
    ...
    assert r.requests_involving("p0") == [7]
    r.forget(7)
    assert r.requests_involving("p0") == []
...
def test_remove_engine_drops_from_pool():
    ...
    assert all(r.route_prefill_done(i) == "d1" for i in range(3))
