...
# --------------------------------------------------------------------------- #
#  TestMtpKvCacheExpertParallel (EP = 2, MTP KV cache ON)
# --------------------------------------------------------------------------- #

# The MTP KV cache changes the per-step MTP forward COUNT: real ranks run one commit-pass
# forward before the depth loop and one extra append after it (D+2 total), and the idle EP
# rank must mirror both the count and the graph/eager mode. A mismatch does not raise -- the
# MoE all-to-all simply blocks forever -- so these tests assert by COMPLETING. They need a
# real EP process group, live MoE layers, and production CUDA-graph warmup; a single-process
# fake cannot observe the failure at all.
#
# `TestMTPCudaGraphExpertParallel` above leaves `mtp_use_repeated_layer` at its default, so
# `enable_mtp_kv_cache` is False there and none of its tests touch this path.

_KV_STATE_COMBOS = list(itertools.combinations_with_replacement(ALL_STATES, _EP_SIZE))


@pytest.mark.internal
class TestMtpKvCacheExpertParallel:
    """Expert-parallel coverage for the MTP KV cache, with real collectives."""
    ...
    def teardown_method(self):
        # CUDA-graph replay is async at the CPU level; sync then barrier so no rank races
        # into the next test's collectives while a peer is still executing this one.
        torch.cuda.synchronize()
        dist.barrier()
        delete_cuda_graphs()
    ...
    def _build_model(self):
        ...
        `mtp_use_repeated_layer=True` is what makes `enable_mtp_kv_cache` reachable: a
        per-depth head has no single `mtp.layers[0]` to seed the draft KV through.
        """
        model_parallel_cuda_manual_seed(123, inference_rng_tracker=True, force_reset_rng=True)
        config = TransformerConfig(
            num_layers=self.NUM_LAYERS,
    ...
    def _build_engine(self, *, num_cuda_graphs=-1, enable_prefix_caching=False, max_requests=16):
        """Full `DynamicInferenceEngine` so MTP graph warmup matches production exactly.

        `DynamicInferenceEngine.__init__` runs `create_cuda_graphs()`, which captures both the
        cache-free `("mtp", n, depth)` graphs and the KV-aware `("mtp_kv", n, None)` graphs that
        the real draft loop replays. Building the engine (rather than hand-driving the
        controller) is what makes the capture/replay key agreement a real assertion.
        """
        delete_cuda_graphs()
        model = self._build_model()
        context = DynamicInferenceContext(
        ...
        assert context.enable_mtp_kv_cache, (
            "MTP KV cache is OFF for this config, so these EP tests would pass vacuously; "
            "check the mtp_use_repeated_layer / mtp_num_layers / speculative-token gates"
        )
        wrapped = GPTInferenceWrapper(model, context)
        wrapped.model_is_pipeline_parallel = False
        ctrl = TextGenerationController(inference_wrapped_model=wrapped, tokenizer=mock.Mock())
        InferenceMode.set_active()
        engine = DynamicInferenceEngine(ctrl, context)
        return engine

    def _add_requests(self, engine, count, prompt_length, offset=0):
        for i in range(count):
            prompt = (
                torch.arange(prompt_length, dtype=torch.int64, device="cuda") + offset + i
            ) % (self.VOCAB_SIZE - 1)
            engine.add_request(
                request_id=offset + i,
                prompt=prompt,
                sampling_params=SamplingParams(num_tokens_to_generate=4, top_k=1),
            )

    @staticmethod
    def _assert_ep_agrees(value, label):
        """All-reduce min/max over the EP group; they must be equal."""
        ep_group = parallel_state.get_expert_model_parallel_group()
        t = torch.tensor([int(value)], device='cuda', dtype=torch.int32)
        ...
        assert (
            lo.item() == hi.item()
        ), f"{label} disagrees across EP ranks: min={lo.item()}, max={hi.item()}"
        return lo.item()

    def _drive_step(self, engine, is_idle):
        """Mirror the engine run loop's own branch: real step, or the EP dummy forward.

        See `DynamicInferenceEngine`'s loop -- a rank with no schedulable work must still
        participate in every EP collective, so it runs `controller.dummy_forward()` (which
        calls `_run_dummy_serial_mtp_forward`) instead of sleeping.
        """
        if is_idle:
            engine.controller.dummy_forward()
            engine.context.step_count += 1
            return None
    ...
    @pytest.mark.internal
    @torch.inference_mode()
    def test_kv_cache_gate_is_on_for_this_model(self):
        """Guard: if the gate flips off, every other test in this class becomes vacuous."""
        engine = self._build_engine()
        context = engine.context

        assert context.enable_mtp_kv_cache is True
        # One extra attention slot is reserved for the draft KV.
        assert context.mtp_kv_layer_slot == self.NUM_LAYERS
        assert context.num_attention_layers == self.NUM_LAYERS + 1
        self._assert_ep_agrees(context.enable_mtp_kv_cache, "enable_mtp_kv_cache")

    @pytest.mark.parametrize(
        "rank_states", _KV_STATE_COMBOS, ids=[",".join(s) for s in _KV_STATE_COMBOS]
    )
    @pytest.mark.internal
    @torch.inference_mode()
    def test_idle_and_active_ranks_complete_a_step(self, rank_states):
        """Cross-product of per-rank load, including idle ranks running the dummy MTP path.

        With the KV cache on, an active rank issues D+2 MTP forwards and an idle rank must
        issue D+2 as well. If the counts or the graph/eager modes diverge, the MoE all-to-all
        deadlocks and this test times out rather than failing an assertion.
        """
        ep_rank = parallel_state.get_expert_model_parallel_rank()
        my_state = rank_states[ep_rank]
        ...
        engine = self._build_engine()
        if not is_idle:
            dims = _STATE_DIMS[my_state]
            count = max(1, dims.prefill_req_count + dims.decode_req_count)
            self._add_requests(engine, count, prompt_length=8, offset=ep_rank * 100)

        self._drive_step(engine, is_idle)

        torch.cuda.synchronize()
        dist.barrier()

    @pytest.mark.parametrize("num_idle_ranks", list(range(_EP_SIZE)))
    @pytest.mark.internal
    @torch.inference_mode()
    def test_multi_step_decode_with_idle_ranks(self, num_idle_ranks):
        """Several steps, so the commit pass runs against real accepted/rejected drafts.

        Step 1 is a prefill (the commit pass seeds prompt positions); later steps are decodes
        whose commit pass refreshes the accepted-draft positions. Both shapes must keep the EP
        forward count matched against the idle ranks.
        """
        ep_rank = parallel_state.get_expert_model_parallel_rank()
        is_idle = ep_rank < num_idle_ranks

        engine = self._build_engine()
        if not is_idle:
            self._add_requests(engine, 2, prompt_length=16, offset=ep_rank * 100)
    ...
    @pytest.mark.internal
    @torch.inference_mode()
    def test_ep_ranks_agree_on_the_mtp_padded_count(self):
        """Both paths key their graph decision off the EP-synced padded request count."""
        ep_rank = parallel_state.get_expert_model_parallel_rank()
        is_idle = ep_rank % 2 == 0

        engine = self._build_engine()
        if not is_idle:
            self._add_requests(engine, 2, prompt_length=8, offset=ep_rank * 100)

        self._drive_step(engine, is_idle)

        # `_mtp_resolved_padded_count` is the signal the draft loop and the dummy path both
        # use to choose graphed vs eager; None means the main step was eager.
        resolved = engine.controller._mtp_resolved_padded_count
        self._assert_ep_agrees(resolved is not None, "MTP graphed decision")
        if resolved is not None:
            self._assert_ep_agrees(resolved, "MTP padded request count")
    ...
    @pytest.mark.internal
    @torch.inference_mode()
    def test_idle_and_active_ranks_complete_an_eager_step(self):
        """Same parity contract with CUDA graphs disabled entirely."""
        ep_rank = parallel_state.get_expert_model_parallel_rank()
        is_idle = ep_rank % 2 == 0

        engine = self._build_engine(num_cuda_graphs=None)
        assert not engine.context.using_cuda_graph_this_step()
        if not is_idle:
            self._add_requests(engine, 2, prompt_length=8, offset=ep_rank * 100)
    ...
    @pytest.mark.internal
    @torch.inference_mode()
    def test_prefix_caching_with_idle_ranks(self):
        """A prefix-cache hit changes the commit pass's append COUNT but not the forward count.

        On a hit the request skips the shared prefix, so its seeded run is `q-1` starting at
        `off` rather than `q` starting at `off-1`. That must not change how many MTP forwards
        the rank issues, or it desyncs from the idle peers.
        """
        ep_rank = parallel_state.get_expert_model_parallel_rank()
        is_idle = ep_rank % 2 == 0

        engine = self._build_engine(enable_prefix_caching=True)
        # Two identical prompts: the second reuses the first's blocks once it is cached.
        if not is_idle:
            shared = torch.arange(32, dtype=torch.int64, device="cuda") % (self.VOCAB_SIZE - 1)
            for i in range(2):
                engine.add_request(
                    request_id=ep_rank * 100 + i,
                    prompt=shared.clone(),
                    sampling_params=SamplingParams(num_tokens_to_generate=4, top_k=1),
                )
