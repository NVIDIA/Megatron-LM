...
class TestInferenceWandbLogging:
    ...
    def test_get_kvcache_utilization_stats_with_requests(self):
        ...
        assert stats_after['allocated_blocks'] + stats_after['block_count_avail'] + 1 == dynamic_context.block_allocator.block_count_total
        ...
        expected_blocks_needed = (context_length + dynamic_context.block_size_tokens - 1) // dynamic_context.block_size_tokens
    ...
    def test_kvcache_utilization_stats_types(self):
        ...
        # All integer fields
        int_fields = [
            'total_blocks', 'allocated_blocks', 'active_unique_blocks',
            'active_request_count', 'paused_request_count', 'gtd_block_count',
            'block_count_avail', 'num_non_gtd_blocks', 'active_token_count',
            'total_request_count', 'max_requests'
        ]
        
        for field in int_fields:
            assert isinstance(stats[field], int), f"{field} should be int but is {type(stats[field])}"
        
        # All float fields
        float_fields = ['allocated_utilization', 'active_utilization']
        for field in float_fields:
            assert isinstance(stats[field], float), f"{field} should be float but is {type(stats[field])}"
