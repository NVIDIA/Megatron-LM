...
class TestTextGenerationController:
    ...
    @pytest.mark.parametrize("return_prompt_top_n_log_probs", [True, False])
    @pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
    def test_logprobs_and_topn_consistency(
        self, num_tokens_to_generate, return_prompt_top_n_log_probs, dtype
    ):
        """
        ...
        for rid, p in enumerate(prompts):
            ...
            active_reqs[str(rid)] = InferenceRequest(
                ...
                sampling_params=SamplingParams(
                    ...
                    top_n_logprobs=5,
                    return_prompt_top_n_log_probs=return_prompt_top_n_log_probs,
                ),
                arrival_time=time.time(),
                status=Status.ACTIVE_BUT_NOT_GENERATING_TOKENS,
            )
        ...
        for request_id, request in completed.items():
            ...
            assert (not return_prompt_top_n_log_probs and prompt_top_n_logprobs is None) or (
                return_prompt_top_n_log_probs
    ...
    def test_zero_tokens_generated_batch_vs_single(self):
        ...
        for rid, p in enumerate(prompts):
            ...
            sampling_params = SamplingParams(
                ...
                return_log_probs=True,
                top_n_logprobs=5,
                return_prompt_top_n_log_probs=True,
            )
    ...
    def test_dynamic_top_n_logprobs_calculation(
        self, return_prompt_top_n_log_probs: bool, materialize_only_last_token_logits: bool
    ):
        """
        Test the _dynamic_step_calculate_top_n_logprobs function directly.
        Verifies:
        1. top_n_logprobs are computed for all requests
        2. return_prompt_top_n_log_probs controls computation for prompt tokens
        ...
        request_metadata[:, request_metadata_labels["return_prompt_top_n_log_probs"]] = float(
            return_prompt_top_n_log_probs
        )
        ...
        if materialize_only_last_token_logits:
            ...
            for req_idx in range(batch_size):
                ...
                if return_prompt_top_n_log_probs:
                    ...
                    assert (
                        len(top_n_list) == 1
                    ), f"Request {req_idx}: expected 1 token when return_prompt_top_n_log_probs=False, got {len(top_n_list)}"
