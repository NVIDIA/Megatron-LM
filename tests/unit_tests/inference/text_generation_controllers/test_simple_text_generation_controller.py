...
class TestTextGenerationController:
    ...
    def test_logprobs_and_topn_consistency(
        self, num_tokens_to_generate, skip_prompt_top_n_logprobs, dtype
    ):
        """
        ...
        for request_id, request in completed.items():
            ...
            assert (skip_prompt_top_n_logprobs and prompt_top_n_logprobs is None) or (
                not skip_prompt_top_n_logprobs
    ...
    def test_zero_tokens_generated_batch_vs_single(self):
        ...
        for rid, p in enumerate(prompts):
            ...
            sampling_params = SamplingParams(
                ...
                return_log_probs=True,
                top_n_logprobs=5,
                skip_prompt_top_n_logprobs=False,
            )
    ...
    def test_dynamic_top_n_logprobs_calculation(
        self, skip_prompt_top_n_logprobs: bool, materialize_only_last_token_logits: bool
    ):
        """
        Test the _dynamic_step_calculate_top_n_logprobs function directly.
        Verifies:
        1. top_n_logprobs are computed for all requests
        2. skip_prompt_top_n_logprobs controls computation for prompt tokens
        ...
        request_metadata[:, request_metadata_labels["skip_prompt_top_n_logprobs"]] = float(
            skip_prompt_top_n_logprobs
        )
        ...
        if materialize_only_last_token_logits:
            ...
            for req_idx in range(batch_size):
                ...
                if not skip_prompt_top_n_logprobs:
                    ...
                    assert (
                        len(top_n_list) == 1
                    ), f"Request {req_idx}: expected 1 token when skip_prompt_top_n_logprobs=True, got {len(top_n_list)}"
