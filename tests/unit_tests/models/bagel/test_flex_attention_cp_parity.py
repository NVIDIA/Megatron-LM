...
def _build_local_qkv(
    q_full, k_full, v_full, u, g, nh, hd, cp_size, rank, noisy_padding=False, seed=0
):
    """
...
def test_cp1_smoke(cfg):
    ...
    fa = FlexAttention(
        config=config, layer_number=1, attn_mask_type=None, attention_type="self", pg_collection=pgc
    ).to(device)
...
def run_parity_test(u, g, nh, hd, tp_group, cp_group, seed=42):
    ...
    (
        ...
        local_und_idx,
        local_gen_idx,
    ) = _build_local_qkv(q_full, k_full, v_full, u, g, nh, hd, cp_size, rank, seed=seed)

    psp_cp1 = MoTPackedSeqParams(
        qkv_format="thd",
        packed_und_token_indexes=und_idx_full,
        packed_gen_token_indexes=gen_idx_full,
        local_und_token_indexes=und_idx_full,
        local_gen_token_indexes=gen_idx_full,
        padded_und_seqlen=u,
        padded_gen_seqlen=g,
    )
    ...
    fa_cp1 = FlexAttention(
        config=config,
        layer_number=1,
        attn_mask_type=None,
        attention_type="self",
        pg_collection=pgc_cp1,
    ).to(device)
    fa_cpN = FlexAttention(
        config=config,
        layer_number=1,
        attn_mask_type=None,
        attention_type="self",
        pg_collection=pgc_cpN,
    ).to(device)
    ...
    print(
        f"  [cp={cp_size} parity fwd  rank={rank:2d}] PASS  "
    ...
    print(
        f"  [cp={cp_size} parity grad rank={rank:2d}] PASS  "
...
def run_padding_test(u, g, nh, hd, tp_group, cp_group, seed=77):
    ...
    (
        ...
        q_full, k_full, v_full, u, g, nh, hd, cp_size, rank, noisy_padding=False, seed=seed
    )
    ...
    print(
        f"  [cp={cp_size} pad/A fwd   rank={rank:2d}] PASS  "
    ...
    (q_noisy, k_noisy, v_noisy, *_) = _build_local_qkv(
        q_full, k_full, v_full, u, g, nh, hd, cp_size, rank, noisy_padding=True, seed=seed
    )
    with torch.no_grad():
    ...
    print(
        f"  [cp={cp_size} pad/C grad  rank={rank:2d}] PASS  "
...
def run_mot_vs_nonmot_test(u, g, nh, hd, tp_group, cp_group, seed=42):
    ...
    gen_err = (gen_got - gen_ref).abs().max().item() if actual_lgen > 0 else 0.0
    print(
        f"  [cp={cp_size} mot_vs_nonmot rank={rank:2d}] PASS  "
...
def main():
    ...
    assert world_size in CONFIGS, (
        f"No config for world_size={world_size}. " f"Supported: {sorted(CONFIGS.keys())}"
    )
    ...
    if rank == 0:
        ...
        print(
            f"  Parity test    : U={cfg.u_par}  G={cfg.g_par}"
            f"  (Lund={math.ceil(cfg.u_par/world_size)}"
            f"  Lgen={math.ceil(cfg.g_par/world_size)})"
        )
        print(
            f"  Padding test   : U={cfg.u_pad}  G={cfg.g_pad}"
            f"  (Lund={math.ceil(cfg.u_pad/world_size)}"
            f"  Lgen={math.ceil(cfg.g_pad/world_size)})"
        )
        print(
            f"  und-only test  : U={cfg.u_und}  G=0" f"  (Lund={math.ceil(cfg.u_und/world_size)})"
        )
        print(
            f"  gen-only test  : U=0   G={cfg.u_gen}" f"  (Lgen={math.ceil(cfg.u_gen/world_size)})"
        )
        print(
            f"  mot vs nonmot  : U={cfg.u_par}  G={cfg.g_par}"
            f"  (Lund={math.ceil(cfg.u_par/world_size)}"
            f"  Lgen={math.ceil(cfg.g_par/world_size)})"
        )
        print(f"{'='*60}")
