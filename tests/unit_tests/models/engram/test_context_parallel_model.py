...
def _run(model):
    ...
    # already reduced and normalized its owner-local FP32 gradients.
    losses.mean().backward()
    gradients = {}
    for name, parameter in model.named_parameters():
        assert parameter_gradient(parameter) is not None, name
        if getattr(getattr(parameter, 'engram_table_metadata', None), 'row_parallel', False):
            gradients[name] = parameter.main_grad.float().cpu()
        else:
