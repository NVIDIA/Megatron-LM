...
_BATCH_SIZE = 2
_NUM_GPT_LAYERS = 4
_MAMBA_PATTERN = "D-D-D-D-"  # len=8 = 2 * _NUM_GPT_LAYERS

# MoE variant: first 2 GPT layers dense, last 2 MoE → 8 Mamba layers
_MOE_MAMBA_PATTERN = "D-D-DEDE"  # 2 dense (D-) + 2 MoE (DE)
...
def _build_gpt_model(
    config: MLATransformerConfig, pre_process: bool = True, post_process: bool = True
) -> GPTModel:
...
def _remap_gpt_to_mamba_state_dict(
    ...
    for key, value in gpt_sd.items():
        ...
        assert (
            0 <= layer_n < num_local_gpt_layers
...
def _forward_logprobs_pp2(model: torch.nn.Module, tokens: torch.Tensor) -> Optional[torch.Tensor]:
    ...
    *same order* on all ranks to avoid deadlocks.
    ...
    if pp_rank == 0:
        ...
        hidden_buf = torch.empty(
            seq_len, batch_size, hidden_size, dtype=torch.bfloat16, device=tokens.device
        )
...
def _compare_against_golden_values(
    logprobs: torch.Tensor, golden_logprobs: torch.Tensor, abs_tol: float = 1e-3
):
    """Assert logprobs match the golden values JSON within *abs_tol*."""
    golden_lp = golden_logprobs[0].float().tolist()
    actual_lp = logprobs[0].float().tolist()
    assert len(actual_lp) == len(
        golden_lp
...
_GOLDEN_BASE = Path(__file__).parent.parent.parent / (
    "functional_tests/test_cases/core/models/hybrid"
)
...
class TestDSAGPTMambaEquivalence:
    ...
    def _skip_if_insufficient_gpus(self, tp: int, pp: int) -> None:
        ...
        if world_size < required:
            pytest.skip(
                f"Test tp={tp} pp={pp} requires {required} GPU(s), " f"but WORLD_SIZE={world_size}"
            )
    ...
    def test_dsa_logprobs_match(self, tp: int, pp: int) -> None:
        ...
        # ---- Build GPTModel ----
        gpt_config = _make_dsa_config(num_layers=_NUM_GPT_LAYERS, tp=tp, pp=pp)
    ...
    def test_record_and_compare_golden_values(self, tp: int, pp: int) -> None:
        ...
        gpt_config = _make_dsa_config(num_layers=_NUM_GPT_LAYERS, tp=1, pp=1)
...
class TestDSAMoEGPTMambaEquivalence:
    ...
    maps to 8 Mamba layers with pattern "D-D-DEDE":
      GPT layer 0 (dense) → Mamba layers 0 (D) + 1 (-)
      GPT layer 1 (dense) → Mamba layers 2 (D) + 3 (-)
      GPT layer 2 (MoE)   → Mamba layers 4 (D) + 5 (E)
      GPT layer 3 (MoE)   → Mamba layers 6 (D) + 7 (E)
    """
    ...
    def _skip_if_insufficient_gpus(self, tp: int, pp: int) -> None:
        ...
        if world_size < required:
            pytest.skip(
                f"Test tp={tp} pp={pp} requires {required} GPU(s), but WORLD_SIZE={world_size}"
            )
    ...
    def test_dsa_moe_logprobs_match(self, tp: int, pp: int) -> None:
        ...
        gpt_config = _make_dsa_moe_config(num_layers=_NUM_GPT_LAYERS, tp=tp, pp=pp)
        gpt_model = _build_gpt_model(gpt_config, pre_process=pre_process, post_process=post_process)
        num_local_gpt_layers = len(gpt_model.decoder.layers)
        gpt_sd = gpt_model.state_dict()
    ...
    def test_moe_record_and_compare_golden_values(self, tp: int, pp: int) -> None:
        ...
        gpt_config = _make_dsa_moe_config(num_layers=_NUM_GPT_LAYERS, tp=1, pp=1)
        gpt_model = _build_gpt_model(gpt_config)
