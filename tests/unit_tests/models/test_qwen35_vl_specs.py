...
def _rope_config(**overrides):
    """Minimal config carrying every attribute the RoPE dispatcher reads eagerly."""
...
def test_apply_rope_fp32_forwards_bshd_options(monkeypatch):
    ...
    output = specs._apply_rope_fp32(
        ...
        mscale=0.75,
        # The dispatcher only checks non-None here; the unfused BSHD path never uses it.
        cp_group=object(),
...
def test_apply_rope_fp32_defaults_mla_interleaving_from_config(monkeypatch, config_value):
    ...
    specs._apply_rope_fp32(
        tensor, freqs, _rope_config(multi_latent_attention=config_value), cp_group=object()
    )
...
def test_apply_rope_fp32_forwards_thd_options(monkeypatch):
    ...
    # The THD dispatcher calls cp_group.size() while classifying the freqs layout.
    cp_group = SimpleNamespace(size=lambda: 1)
    expected = torch.randn_like(tensor, dtype=torch.float32)
