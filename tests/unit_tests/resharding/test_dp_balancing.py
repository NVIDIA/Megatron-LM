...
class TestDPBalancing:
    ...
    def _create_metadata(self, rank, tp_group, dp_group, tp_local, ep_local=None, ep_group=None):
        ...
        return ParameterMetadata(
            ...
            tensor_parallel_local_rank=tp_local,
            expert_parallel_local_rank=ep_local,
            is_tp=True,
            partition_dim=0,
        )
    ...
    def test_dp_balancing_basic(self):
        ...
        src_meta_list = [
            self._create_metadata(rank=0, tp_group=[0, 1], dp_group=[0, 2], tp_local=0),
            self._create_metadata(rank=1, tp_group=[0, 1], dp_group=[1, 3], tp_local=1),
            self._create_metadata(rank=2, tp_group=[2, 3], dp_group=[0, 2], tp_local=0),
            self._create_metadata(rank=3, tp_group=[2, 3], dp_group=[1, 3], tp_local=1),
        ]

        # Destination metadata (TP=1, DP=4)
        dst_meta = self._create_metadata(rank=0, tp_group=[0], dp_group=[0, 1, 2, 3], tp_local=0)
    ...
    def test_dp_balancing_skips_when_same_tp_size(self):
        """Test that TP filtering happens when TP sizes match (non-collocated mode)."""
        # Setup: TP=2, DP=2, World=4 (non-collocated, same config on both sides)
        # This simulates non-collocated mode where src and dst have same TP/EP sizes
        ...
        # Destination with TP=2 (same as source), TP local rank = 0
        dst_meta = self._create_metadata(rank=4, tp_group=[4, 5], dp_group=[4, 5], tp_local=0)

        # When TP sizes match, should filter to same TP local rank
        selected = select_src_metadata_balanced(src_meta_list, dst_meta, dst_rank=4)

        # Should select from TP-local-0 ranks only (0 or 2)
        assert selected.tensor_parallel_local_rank == 0
        assert selected.owner_rank in [0, 2]
    ...
    def test_dp_balancing_distribution(self):
        ...
        src_meta_list = [
            self._create_metadata(rank=0, tp_group=[0, 1], dp_group=[0, 2, 4, 6], tp_local=0),
            self._create_metadata(rank=1, tp_group=[0, 1], dp_group=[1, 3, 5, 7], tp_local=1),
            self._create_metadata(rank=2, tp_group=[2, 3], dp_group=[0, 2, 4, 6], tp_local=0),
            self._create_metadata(rank=3, tp_group=[2, 3], dp_group=[1, 3, 5, 7], tp_local=1),
            self._create_metadata(rank=4, tp_group=[4, 5], dp_group=[0, 2, 4, 6], tp_local=0),
            self._create_metadata(rank=5, tp_group=[4, 5], dp_group=[1, 3, 5, 7], tp_local=1),
            self._create_metadata(rank=6, tp_group=[6, 7], dp_group=[0, 2, 4, 6], tp_local=0),
            self._create_metadata(rank=7, tp_group=[6, 7], dp_group=[1, 3, 5, 7], tp_local=1),
        ]

        dst_meta = self._create_metadata(rank=0, tp_group=[0], dp_group=list(range(8)), tp_local=0)
    ...
    def test_dp_balancing_with_ep(self):
        ...
        # When EP sizes match, should filter by EP local rank too

        src_meta_list = [
            # TP group [0,1], EP local 0, DP group [0,4]
            self._create_metadata(
                rank=0, tp_group=[0, 1], dp_group=[0, 4], tp_local=0, ep_local=0, ep_group=[0, 1]
            ),
            # TP group [0,1], EP local 1, DP group [2,6]
            self._create_metadata(
                rank=2, tp_group=[2, 3], dp_group=[2, 6], tp_local=0, ep_local=0, ep_group=[2, 3]
            ),
            # Add more for completeness
            self._create_metadata(
                rank=4, tp_group=[4, 5], dp_group=[0, 4], tp_local=0, ep_local=1, ep_group=[4, 5]
            ),
            self._create_metadata(
                rank=6, tp_group=[6, 7], dp_group=[2, 6], tp_local=0, ep_local=1, ep_group=[6, 7]
            ),
        ]

        # Destination with same EP size, EP local rank = 0
        dst_meta = self._create_metadata(
            rank=8, tp_group=[8, 9], dp_group=[8, 9], tp_local=0, ep_local=0, ep_group=[8, 9]
        )
        ...
        # Should select from EP-local-0 ranks only (0 or 2)
        assert selected.expert_parallel_local_rank == 0
        assert selected.owner_rank in [0, 2]
    ...
    def test_dp_balancing_single_dp_group(self):
        ...
        src_meta_list = [
            self._create_metadata(rank=0, tp_group=[0, 1], dp_group=[0, 1], tp_local=0),
            self._create_metadata(rank=1, tp_group=[0, 1], dp_group=[0, 1], tp_local=1),
        ]

        dst_meta = self._create_metadata(rank=0, tp_group=[0], dp_group=[0], tp_local=0)
    ...
    def test_tp_size_mismatch_no_filter(self):
        """Test that TP filtering is skipped when TP sizes differ (resharding mode)."""
        # Setup: TP=4 -> TP=2 (resharding)

        src_meta_list = [
            self._create_metadata(rank=0, tp_group=[0, 1, 2, 3], dp_group=[0, 4], tp_local=0),
            self._create_metadata(rank=1, tp_group=[0, 1, 2, 3], dp_group=[1, 5], tp_local=1),
            self._create_metadata(rank=2, tp_group=[0, 1, 2, 3], dp_group=[2, 6], tp_local=2),
            self._create_metadata(rank=3, tp_group=[0, 1, 2, 3], dp_group=[3, 7], tp_local=3),
            self._create_metadata(rank=4, tp_group=[4, 5, 6, 7], dp_group=[0, 4], tp_local=0),
            self._create_metadata(rank=5, tp_group=[4, 5, 6, 7], dp_group=[1, 5], tp_local=1),
            self._create_metadata(rank=6, tp_group=[4, 5, 6, 7], dp_group=[2, 6], tp_local=2),
            self._create_metadata(rank=7, tp_group=[4, 5, 6, 7], dp_group=[3, 7], tp_local=3),
        ]

        # Destination with TP=2 (different from source TP=4)
        dst_meta = self._create_metadata(rank=8, tp_group=[8, 9], dp_group=[8, 9], tp_local=0)

        # Should NOT filter by TP local rank (sizes differ)
        # Should only do DP balancing
        selected = select_src_metadata_balanced(src_meta_list, dst_meta, dst_rank=8)
    ...
    def test_load_distribution_across_parameters(self):
        ...
        src_meta_list = [
            self._create_metadata(rank=0, tp_group=[0], dp_group=[0, 1, 2, 3], tp_local=0),
            self._create_metadata(rank=1, tp_group=[1], dp_group=[0, 1, 2, 3], tp_local=0),
            self._create_metadata(rank=2, tp_group=[2], dp_group=[0, 1, 2, 3], tp_local=0),
            self._create_metadata(rank=3, tp_group=[3], dp_group=[0, 1, 2, 3], tp_local=0),
        ]

        dst_meta = self._create_metadata(
            rank=0, tp_group=[0], dp_group=[0, 1, 2, 3, 4, 5, 6, 7], tp_local=0
        )
    ...
    def test_local_copy_preference_collocated(self):
        ...
        # Source metadata from all ranks (TP local 0 only, after TP filtering)
        src_meta_list = [
            self._create_metadata(rank=0, tp_group=[0, 1], dp_group=[0, 2, 4, 6], tp_local=0),
            self._create_metadata(rank=2, tp_group=[2, 3], dp_group=[0, 2, 4, 6], tp_local=0),
            self._create_metadata(rank=4, tp_group=[4, 5], dp_group=[0, 2, 4, 6], tp_local=0),
        ...
        dst_meta = self._create_metadata(rank=0, tp_group=[0, 1], dp_group=[0, 2, 4, 6], tp_local=0)
        ...
        # Try rank 4
        dst_meta = self._create_metadata(rank=4, tp_group=[4, 5], dp_group=[0, 2, 4, 6], tp_local=0)
        selected = select_src_metadata_balanced(src_meta_list, dst_meta, dst_rank=4)
