...
class TestMFUTracking:
    ...
    def test_inference_flops_e2e(
        ...
        args = SimpleNamespace(
            hidden_size=256, padded_vocab_size=1024, num_attention_heads=8,
            num_query_groups=4, kv_channels=32, ffn_hidden_size=512, num_layers=8,
            swiglu=True, hybrid_override_pattern="M*M*-EMM*",
            num_experts=4, moe_router_topk=2, moe_ffn_hidden_size=512,
            moe_shared_expert_intermediate_size=256, mamba_num_heads=4,
            mamba_head_dim=64, mamba_state_dim=128, mamba_num_groups=8,
            mamba_d_conv=4, inference_dynamic_batching_block_size=256,
        )
        ...
        result = calc.compute_step_flops(
            ...
            total_tokens=total_tokens, active_blocks=active_blocks,
            active_reqs=active_reqs, num_prefill_reqs=num_prefill_reqs,
        )
        ...
        if prefill_tokens > 0 and num_prefill_reqs > 0:
            r2 = calc.compute_step_flops(
                decode_tokens=0, prefill_tokens=prefill_tokens * 2,
                total_tokens=prefill_tokens * 2, active_blocks=active_blocks,
                active_reqs=active_reqs, num_prefill_reqs=num_prefill_reqs,
            )
