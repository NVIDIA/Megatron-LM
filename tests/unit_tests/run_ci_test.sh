...
for i in $(seq $UNIT_TEST_REPEAT); do
    ...
    CMD=$(echo uv run --frozen --active python -m torch.distributed.run ${DISTRIBUTED_ARGS[@]} \
        -m coverage run \
    ...
    if [[ "$TAG" == "latest" ]]; then
        CMD=$(echo uv run --frozen --active python -m torch.distributed.run ${DISTRIBUTED_ARGS[@]} -m pytest \
            -xvs \
            --experimental \
             ${IGNORE_ARGS[@]} \
