...
"""Unit tests for GatedDeltaNet with cached prefix and chunked pipeline model parallel."""
...
class TestGatedDeltaNetWithCachedPrefix:
    """Test GatedDeltaNet with different chunked_pipeline_model_parallel_splits values."""
    ...
    def test_chunked_pp_splits_consistency(self):
        ...
        This test compares the output of GatedDeltaNet with chunked_pp_splits=1 (baseline)
        ...
        micro_batch_size = 2
        hidden_size = 2048
        linear_num_key_heads = 16  # Must be divisible by tp_size
        linear_num_value_heads = 32  # Must be a multiple of linear_num_key_heads
        seed = 42
        ...
        layer_spec = get_gated_delta_net_module_spec(config_baseline)
        attn_layer_spec = layer_spec.submodules
        ...
        attention_baseline = GatedDeltaNet(
            config_baseline,
            attn_layer_spec,
            layer_number=1,
        ...
        hidden_states_baseline = hidden_states.clone().requires_grad_()
        ...
        attention_chunked = GatedDeltaNet(
            config_chunked,
            attn_layer_spec,
            layer_number=1,
            attn_mask_type=AttnMaskType.causal,
