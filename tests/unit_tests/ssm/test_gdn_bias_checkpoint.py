...
def _state(module, **metadata):
    return module.sharded_state_dict(
        metadata={'dp_cp_group': module.pg_collection.dp_cp, **metadata}
    )
...
@pytest.mark.parametrize(
    'legacy_tp_size,error',
    [(1, 'must first be loaded at their saved TP size')]
    + [(value, 'must be a positive integer') for value in (None, 0, -1, True, '2')],
)
def test_gdn_legacy_bias_rejects_invalid_metadata(gdn_groups, legacy_tp_size, error):
