...
def _run(model, hidden, dy, metadata, *, staged=False):
    model.zero_grad(set_to_none=True)
    x = hidden.detach().clone().requires_grad_()
    if staged:
        assert model.supports_two_stage_attention()
        norm_out = model.forward_pre_attn_and_core_attn(x, None, packed_seq_params=metadata)
        out, _ = model.forward_post_core_attn(norm_out)
    else:
...
def test_post_fusion_module_parity(model_parallel, packed, recompute, pre_fusion):
    ...
    assert model.supports_two_stage_attention() == (not recompute)
    _assert_close(actual, expected, grads, reference_grads)


@pytest.mark.parametrize("packed", [False, True])
@pytest.mark.parametrize("pre_fusion", [False, True])
@pytest.mark.parametrize("post_fusion", [False, True])
def test_split_forward_matches_monolithic(model_parallel, packed, pre_fusion, post_fusion):
    model = _model(pre_fusion)
    model.config.gdn_gated_output_norm_fusion = post_fusion
    hidden, dy, metadata = _inputs(packed)
    expected, reference_grads = _run(model, hidden, dy, metadata)
    actual, grads = _run(model, hidden, dy, metadata, staged=True)
