...
class TestGDPPackedSequence:
    ...
    def test_split_compute_interface_matches_forward(self, seq_lens):
        ...
        with torch.no_grad():
            ...
            split_output = mixer.output_proj(
                mixer.input_proj_ssm(hidden_full, packed_seq_params=psp)
            )

        forward_output = (
            forward_output[0] if isinstance(forward_output, tuple) else forward_output
        )
