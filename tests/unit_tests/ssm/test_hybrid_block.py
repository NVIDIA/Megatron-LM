...
class TestFusedLayerValidation:
    ...
    def _call(self, fused_symbols: str):
        ...
        # The layer attributes are only read for valid fusion groups; the
    ...
    def _capture_fused_spec(self, fused_symbols: str, submodules, monkeypatch):
        from megatron.core.models.hybrid import hybrid_layer_fusion

        captured = {}

        def fake_build_module(spec, **kwargs):
            captured["spec"] = spec
            captured["kwargs"] = kwargs
            return "fused-layer-sentinel"

        monkeypatch.setattr(hybrid_layer_fusion, "build_module", fake_build_module)
        result = hybrid_layer_fusion.build_fused_layer(
            fused_symbols,
        ...
        assert result == "fused-layer-sentinel"
        return captured["spec"], captured["kwargs"]

    def test_fusion_walks_attention_and_mlp_layer_specs(self, monkeypatch):
        from megatron.core.models.hybrid.hybrid_block import HybridStackSubmodules
        ...
        submodules = HybridStackSubmodules(
            attention_layer=ModuleSpec(
                ...
                submodules=TransformerLayerSubmodules(
                    ...
                    self_attention="attn-primitive",
                    self_attn_bda="attn-bda",
                    pre_mlp_layernorm="unused-attn-mlp-norm",
                    sharded_state_dict_keys_map={
                        "input_layernorm.": "self_attention.linear_qkv.layer_norm_",
                        "pre_mlp_layernorm.": "unused.",
                    },
                ),
            ),
            mlp_layer=ModuleSpec(
                module=TransformerLayer,
                submodules=TransformerLayerSubmodules(
                    input_layernorm="unused-mlp-attn-norm",
                    pre_mlp_layernorm="mlp-norm",
                    mlp="mlp-primitive",
                    mlp_bda="mlp-bda",
                    sharded_state_dict_keys_map={
                        "input_layernorm.": "unused.",
        ...
        fused_spec, kwargs = self._capture_fused_spec(
            Symbols.ATTENTION + Symbols.MLP, submodules, monkeypatch
        )
        fused_submodules = fused_spec.submodules

        assert fused_submodules.input_layernorm == "attn-norm"
        assert fused_submodules.self_attention == "attn-primitive"
        assert fused_submodules.self_attn_bda == "attn-bda"
        assert fused_submodules.pre_mlp_layernorm == "mlp-norm"
        assert fused_submodules.mlp == "mlp-primitive"
        assert fused_submodules.mlp_bda == "mlp-bda"
        assert fused_submodules.sharded_state_dict_keys_map == {
            "input_layernorm.": "self_attention.linear_qkv.layer_norm_",
            "pre_mlp_layernorm.": "mlp.linear_fc1.layer_norm_",
        }
        assert kwargs["pp_layer_offset"] == 2

    def test_fusion_gets_dsa_and_moe_norms_from_layer_specs(self, monkeypatch):
        from megatron.core.models.hybrid.hybrid_block import HybridStackSubmodules
        from megatron.core.transformer.spec_utils import ModuleSpec
        from megatron.core.transformer.transformer_layer import (
            TransformerLayer,
        ...
        submodules = HybridStackSubmodules(
            dsa_layer=ModuleSpec(
                module=TransformerLayer,
                submodules=TransformerLayerSubmodules(
                    input_layernorm="dsa-norm",
                    self_attention="dsa-primitive",
                    self_attn_bda="dsa-bda",
                ),
            ),
            moe_layer=ModuleSpec(
                module=TransformerLayer,
                submodules=TransformerLayerSubmodules(
                    pre_mlp_layernorm="moe-norm", mlp="moe-primitive", mlp_bda="moe-bda"
                ),
            ),
        )

        fused_spec, _ = self._capture_fused_spec(
            Symbols.DS_ATTENTION + Symbols.MOE, submodules, monkeypatch
        )
        fused_submodules = fused_spec.submodules

        assert fused_submodules.input_layernorm == "dsa-norm"
        assert fused_submodules.self_attention == "dsa-primitive"
        assert fused_submodules.self_attn_bda == "dsa-bda"
        assert fused_submodules.pre_mlp_layernorm == "moe-norm"
        assert fused_submodules.mlp == "moe-primitive"
        assert fused_submodules.mlp_bda == "moe-bda"

    def test_fusion_adapts_mamba_layer_spec_for_modelopt_shape(self, monkeypatch):
        from megatron.core.models.hybrid.hybrid_block import HybridStackSubmodules
        ...
        mixer_submodules = object()
        mamba_mixer_spec = ModuleSpec(
            module=MambaMixer, params={"param-sentinel": True}, submodules=mixer_submodules
        )
        submodules = HybridStackSubmodules(
            mamba_layer=ModuleSpec(
                ...
                submodules=MambaLayerSubmodules(
                    norm="mamba-norm",
                    mixer=mamba_mixer_spec,
                    mamba_bda="mamba-bda",
                    sharded_state_dict_keys_map={"norm.": "mixer.in_proj.layer_norm_"},
                ),
            ),
            mlp_layer=ModuleSpec(
                module=TransformerLayer,
        ...
        fused_spec, _ = self._capture_fused_spec(
            Symbols.MAMBA + Symbols.MLP, submodules, monkeypatch
        )
        fused_submodules = fused_spec.submodules

        assert fused_submodules.input_layernorm == "mamba-norm"
        # We copy `mamba_mixer_spec` when modifying it to adapt it to `TransformerLayer`, so it
        # cannot be the same object.
        assert fused_submodules.self_attention is not mamba_mixer_spec
        assert fused_submodules.self_attention.module is MambaMixerForTransformerLayer
        assert mamba_mixer_spec.module is MambaMixer
        assert fused_submodules.self_attention.params == {"param-sentinel": True}
        assert fused_submodules.self_attention.submodules is mixer_submodules
        assert fused_submodules.self_attn_bda == "mamba-bda"
        assert fused_submodules.sharded_state_dict_keys_map == {
            "input_layernorm.": "self_attention.in_proj.layer_norm_",
            "pre_mlp_layernorm.": "mlp.linear_fc1.layer_norm_",
        }

    def test_modelopt_hybrid_spec_supports_fused_layer_extraction(self, monkeypatch):
        from megatron.core.models.hybrid.hybrid_layer_fusion import MambaMixerForTransformerLayer
        ...
        submodules = get_hybrid_stack_modelopt_spec(remap_te_layernorm=True).submodules
        ...
        assert fused_submodules.input_layernorm is Norm
        assert fused_submodules.self_attention.module is MambaMixerForTransformerLayer
        assert fused_submodules.pre_mlp_layernorm is Norm
        assert fused_submodules.sharded_state_dict_keys_map == {
            "input_layernorm.": "self_attention.in_proj.layer_norm_",
            "pre_mlp_layernorm.": "mlp.linear_fc1.layer_norm_",
        }
...
class TestCanonicalShardedStateDict:
    ...
    def test_fused_mamba_external_norm_maps_to_standalone_norm(self):
        # ModelOpt local Mamba specs keep the Mamba norm outside the mixer.
        # In a fused TransformerLayer that norm lives at `input_layernorm.*`,
        # but the canonical stand-alone Mamba path is `norm.*`.
        fused = self._fake_layer(
            [
        ...
        assert self._keys(self._run(stub)) == {
            ...
            "layers.0.mixer.in_proj.weight",
            "layers.0.mamba_bda.extra_state",
            "layers.1.mlp.linear_fc1.weight",
        }
