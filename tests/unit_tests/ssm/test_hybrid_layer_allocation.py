...
def expected_layer_counts(nonzero=None):
    """Build an exact count map while keeping zero-count symbols explicit."""
...
class TestGetHybridLayerCounts:
    ...
    def test_all_layer_types(self):
        ...
        assert get_hybrid_layer_counts("MGDCHW+-E") == expected_layer_counts(
            {'D': 1, 'C': 1, 'H': 1, 'W': 1, 'G': 1, 'M': 1, '+': 1, '-': 1, 'E': 1}
        )
    ...
    def test_with_mtp(self):
        ...
        assert get_hybrid_layer_counts("M*M*/MM/MM") == expected_layer_counts({'*': 2, 'M': 6})
    ...
    def test_mtp_with_attention(self):
        ...
        assert get_hybrid_layer_counts("MMMM/*M/*M/*M") == expected_layer_counts({'*': 3, 'M': 7})
    ...
    def test_gdn_hybrid_pattern(self):
        # GDN + Mamba + Attention
        assert get_hybrid_layer_counts("G*GM*") == expected_layer_counts({'*': 2, 'G': 2, 'M': 1})
