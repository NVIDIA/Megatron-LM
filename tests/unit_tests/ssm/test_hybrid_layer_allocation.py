...
# KDA has no dedicated layer config type: it shares GDNLayerConfig with GDN/GDN2 and is
# selected by experimental_attention_variant='kda'. It is therefore a valid pattern symbol
# without being a key of Symbols.LAYER_CONFIG_MAP.
_EXPECTED_LAYER_CONFIG_CLASSES_BY_SYMBOL = {
    **_EXPECTED_LAYER_CONFIG_CLASSES,
    Symbols.KDA: GDNLayerConfig,
}
...
def _assert_layer_config_types(layer_config_list, pattern: str) -> None:
    assert [type(config) for config in layer_config_list] == [
        _EXPECTED_LAYER_CONFIG_CLASSES_BY_SYMBOL[layer_symbol] for layer_symbol in pattern
    ]
...
class TestValidateSegmentLayers:
    ...
    def test_window_symbol(self):
        ...
        _assert_layer_config_types(validate_segment_layers("WEWE", self.config), "WEWE")
        assert get_hybrid_layer_counts("WEWE")["W"] == 2
        # 'W' coexists with the other MLA-family attentions (C/H/D)...
        validate_segment_layers("WEDECEHE", self.config)
        # ...but not with standard '*' attention.
        with pytest.raises(ValueError):
            validate_segment_layers("MWM*-", self.config)

    def test_kda_symbol(self):
        """'K' (Kimi Delta Attention) is a valid pattern symbol backed by GDNLayerConfig."""
        assert Symbols.KDA == "K"
        assert Symbols.KDA in Symbols.VALID_LAYERS
        # KDA has no dedicated layer config type; it reuses GDNLayerConfig like GDN/GDN2.
        assert Symbols.KDA not in Symbols.LAYER_CONFIG_MAP
        _assert_layer_config_types(validate_segment_layers("K+K", self.config), "K+K")
        assert get_hybrid_layer_counts("K-KEKE+E")["K"] == 3
...
class TestGetLayerMapsFromLayerTypeList:
    ...
    def test_mixed_dsa_and_mla(self):
        """DSA and MLA layers maintain separate local indices (both use decoupled RoPE)."""
        maps = get_layer_maps_from_layer_type_list(
