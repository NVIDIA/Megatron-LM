...
class TestGetLayerMapsFromLayerTypeList:
    ...
    def test_standard_layer_types(self):
        ...
        assert len(maps) == 7
        attention_map, mamba_map, multi_decay_map, mlp_map, moe_map = operator.itemgetter(
            Symbols.ATTENTION, Symbols.MAMBA, Symbols.MULTI_DECAY, Symbols.MLP, Symbols.MOE
        )(maps)
        assert attention_map == {0: 0}
        assert mamba_map == {1: 0}
        assert multi_decay_map == {2: 0}
        assert mlp_map == {3: 0}
        assert moe_map == {4: 0}
