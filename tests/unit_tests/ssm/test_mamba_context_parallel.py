...
class TestMambaContextParallel:
    ...
    def test_runtime_cp_views_do_not_mutate_base_helper(self):
        """Sequential CP1/2/4 geometry resolution remains forward-local."""
        ...
        groups = {size: FakeGroup(size) for size in (1, 2, 4)}
        helper = MambaContextParallel(
            ...
            ngroups_local_tp=4,
            d_state=8,
            conv1d_weight_cp1=torch.randn(128, 1, 4),
            conv1d_bias_cp1=torch.randn(128),
            conv1d_padding=3,
        ...
        for cp_size in (1, 2, 4, 2, 1):
            runtime_helper = helper.for_context_parallel_group(groups[cp_size])
            assert runtime_helper.cp_group is groups[cp_size]
            assert runtime_helper.cp_size == cp_size
            assert runtime_helper.nheads_local_tpcp == 4 // cp_size
            assert helper.cp_group is groups[1]
    ...
    def test_runtime_cp_group_forward_backward_matches_static_group(self):
        ...
        try:
            ...
            singleton_group = parallel_state.get_dynamic_data_context_parallel_groups(group_size=1)
            ...
            dynamic, dynamic_tensors = build(singleton_group)
            runtime_dynamic = dynamic.for_context_parallel_group(runtime_group)
            ...
            dynamic_result = run(runtime_dynamic, dynamic_tensors)

            assert dynamic.cp_group is singleton_group
            assert dynamic.cp_size == 1
