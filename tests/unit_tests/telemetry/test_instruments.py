...
pytest.importorskip(
    "opentelemetry.sdk",
    reason="opentelemetry-sdk is an optional dependency (install megatron-core[otel])",
)

from nemo.lens.instruments.inference import _INFERENCE_INSTRUMENTS, record_inference_metrics
from nemo.lens.instruments.training import _TRAINING_INSTRUMENTS, record_training_metrics
...
class TestRecordInferenceMetrics:
    ...
    def test_genai_attributes_present(self, meter_and_reader):
        meter, reader = meter_and_reader
        record_inference_metrics(meter, request_duration_s=1.0, model='llama', input_tokens=10)
        metrics_data = reader.get_metrics_data()
        for sm in metrics_data.resource_metrics:
            for scope in sm.scope_metrics:
                for metric in scope.metrics:
