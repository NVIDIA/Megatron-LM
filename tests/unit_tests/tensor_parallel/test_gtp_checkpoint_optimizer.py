...
def _loaded_tree(tree, saved=None):
    """Replace leaves independently of the factory's merge implementation."""

    def load(leaf):
        if isinstance(leaf, LocalNonpersistentObject):
            return leaf.unwrap()
        assert isinstance(leaf, ShardedTensor)
        leaf.validate_metadata_integrity()
        assert leaf.flattened_range is None and not leaf.data.requires_grad
        if saved is None:
            return leaf.data.clone()
        index = tuple(
            slice(start, start + size)
            for start, size in zip(
                leaf.global_offset, (1,) * leaf.prepend_axis_num + leaf.local_shape
            )
        )
        return saved[leaf.key][index].reshape(leaf.local_shape).clone()

    return dict_list_map_outplace(load, tree)
...
def test_gtp_optimizer_factory_preserves_source_and_keys(optimizer_groups, offsets):
    ...
    assert companion.key == factory.key and companion.for_optimizer() is companion
    ...
    state = replace(companion, key=f'optimizer.state.exp_avg.{companion.key}', data=moment)
    ...
    expected_keys = {f'optimizer.state.exp_avg.{companion.key}.{name}' for name in _NAMES}
...
def test_gtp_optimizer_factory_handles_flat_ranges(optimizer_groups, start, stop):
    ...
    assert all(part.replica_id == (0, 0, 0) for part in parts['chunks'])
...
def test_gtp_swiglu_optimizer_layout(optimizer_groups, singleton, flat_range):
    ...
    # dev's apply_swiglu_sharded_factory has no tp_group/dp_group (main added them for FSDP2
    # DP-sharded params, which this factory is not).
    factory = apply_swiglu_sharded_factory(original, (), singleton_local_shards=singleton)
    ...
    state = replace(factory.for_optimizer(), key=key, data=moment, flattened_range=flat_range)
