...
def create_checkpoint(load_path, ckpt_format):
    ...
    if ckpt_format == "torch":
        ...
        torch.distributed.checkpoint.save(state_dict, checkpoint_id=ckpt_dir, process_group=get_default_process_group())
...
def test_load_base_checkpoint(init_model_parallel, create_args, ckpt_format, tmp_path_dist_ckpt):
    ...
    with TempNamedDir(tmp_path_dist_ckpt / "test_load_base_checkpoint", 
                      sync=True,
                      process_group=get_default_process_group()) as load_dir:
...
def test_save_checkpoint(init_model_parallel, create_args, tmp_path_dist_ckpt, ckpt_format):
    ...
    with TempNamedDir(tmp_path_dist_ckpt / "test_save_checkpoint", 
                      sync=True,
                      process_group=get_default_process_group()) as save_dir:
        args.save = save_dir
        set_args(args)
...
def test_load_checkpoint(
    ...
    with TempNamedDir(tmp_path_dist_ckpt / "test_load_checkpoint", 
                      sync=True,
                      process_group=get_default_process_group()) as ckpt_dir:
        args.load = ckpt_dir
        args.save = ckpt_dir
        set_args(args)
...
def test_dist_checkpoint_versioning(init_model_parallel, tmp_path_dist_ckpt, create_ckpt_load_args):
    ...
    with TempNamedDir(
        tmp_path_dist_ckpt / "test_dist_checkpoint_versioning", 
        sync=True,
