...
def test_load_checkpoint_override_opt_param_scheduler(
    ...
    with TempNamedDir(
        ...
        assert loaded_iter == iteration
        assert loaded_flops == num_floating_point_operations_so_far
        assert new_optimizer.param_groups[0]["max_lr"] == 1.0
        assert new_optimizer.param_groups[0]["min_lr"] == 0.1
        assert new_optimizer.param_groups[1]["max_lr"] == 0.5
        assert new_optimizer.param_groups[1]["min_lr"] == 0.05
