...
class TestFP8Param:
    ...
    def _run_test_helper(
        ...
        **kwargs,
        ...
        # Main's call is a superset of dev's (same two kwargs plus `use_cudagraphable_rng`).
        ...
        for i in range(num_steps):
            ...
            # Main's extra `forward_pre_hook_enabled` term pairs with the first-iteration hook
            # disable above: while the pre-hooks are off there is no param-gather to repopulate,
            # so the copy must be skipped (mirrors training.py's identical guard).
