...
from megatron.core.device_utils import get_current_device, get_distributed_backend, get_distributed_init_method, get_local_device_count, get_xla_model
...
class TestHyperCommGrid:
    ...
    def test_create_pg_with_options(self, mock_new_subgroups):
        ...
        grid = HyperCommGrid([2, 4], ["tp", "dp"], backend=get_distributed_backend())
        ...
        args, kwargs = mock_new_subgroups.call_args
        assert kwargs["backend"] == get_distributed_backend()
...
class TestHyperCommGridIntegration:
    ...
    def test_real_distributed_basic_functionality(self):
        ...
        grid = HyperCommGrid(shape, dim_names, backend=get_distributed_backend())

        assert grid.size == world_size
        assert grid.shape == shape
        assert grid.dim_names == dim_names
        assert grid.backend == get_distributed_backend()
    ...
    def test_real_distributed_process_group_creation(self):
        ...
        grid = HyperCommGrid([2, 2, 2], ["tp", "cp", "dp"], backend=get_distributed_backend())
    ...
    def test_real_distributed_all_reduce(self):
        ...
        all_reduce(tensor, group=tp_pg)
        assert tensor.cpu().item() == expected_sum
    ...
    def test_real_distributed_different_world_sizes(self):
        ...
        if world_size > 1:
            ...
            all_reduce(tensor, group=first_dim_pg)
            assert tensor.cpu().item() == expected_sum
