    ...
    def _run_steps(self, args, model, optimizer, n):
        ...
        for _ in range(n):
            ...
            if args.overlap_param_gather:
                # The deferred gather must still be pending: a forced sync that forgets to re-arm
                # leaves param_gather_dispatched=True, turning the pre-hook into a silent no-op.
                # ``all`` (not ``any``) so a dropped expert_parallel_bucket_groups can't be masked.
