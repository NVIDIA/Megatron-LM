...
compact LayerWise layout (``--layer-wise-param-layout decoupled``, the default).

Construction mirrors tests/unit_tests/test_muon_mxfp8_fp8_param_gather.py
(PR #4987), but on the decouple path, which has its own FP8-aware whole-param
all-gather (the ``padded`` and ``legacy`` layouts gather through DDP bucket
groups instead): blockwise (Hopper) needs no ``reuse_grad_buf``; mxfp8
(Blackwell) does. Under deterministic kernels, fp8_param_gather ON must match OFF
bitwise on per-step loss, forward output, per-param ``main_grad``, fp32 master,
and (bf16) model param. Covers ``overlap_grad_reduce`` + ``overlap_param_gather``
both ON and OFF.
"""
...
class TestMuonDecoupleFP8ParamGather:
    ...
    def _create_args(
        ...
        # DECOUPLED compact LayerWise layout.
        args.layer_wise_param_layout = 'decoupled'
