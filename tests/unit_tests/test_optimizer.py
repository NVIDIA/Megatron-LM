...
def test_distrib_optimizer_store_param_remainders_range_check():
    """Test that loading dp_zero checkpoint with out-of-range values raises ValueError.

    This catches the case where a checkpoint saved with store_param_remainders=False
    (containing full fp32 master weights) is loaded into a session with
    ...
    # Save via dp_zero, then corrupt values to simulate store_param_remainders=False checkpoint
    saved_state = distrib_optim.get_parameter_state_dp_zero()
    if saved_state is not None:
        for gbuf_idx in saved_state:
            if isinstance(gbuf_idx, int):
                for dtype, world_tensors in saved_state[gbuf_idx].items():
                    if "param" in world_tensors:
                        # Set values outside int16 range to simulate wrong checkpoint
                        world_tensors["param"].fill_(100000.0)
