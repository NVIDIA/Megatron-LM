...
def test_sharded_optimizer_steps_reject_divergence_within_a_bucket():
    """Two backbone groups must never carry different clocks.

    Upstream asserted a single clock across the whole optimizer, which caught any accidental
    desynchronization. Checking only within a group would catch almost none of it, since the
    parameters of one group always step together. The bucket is the level where the invariant
    is both true and useful.
    """
    state_dict = {
        'state': {
            0: {'step': torch.tensor(123.0), 'exp_avg': torch.ones(2)},
            1: {'step': torch.tensor(456.0), 'exp_avg': torch.ones(3)},
        },
        'param_groups': [
            {'params': [0], 'is_dsa_indexer': False},
            {'params': [1], 'is_dsa_indexer': False, 'wd_mult': 0.0},
        ],
    }

    with pytest.raises(ValueError, match="within one parameter bucket"):
        MegatronOptimizer._move_per_param_steps_to_param_groups(state_dict)


def test_sharded_optimizer_steps_ignore_groups_holding_no_local_parameters():
    """An empty group never advanced a clock, so it cannot contradict one that did."""
    state_dict = {
        'state': {0: {'step': torch.tensor(123.0), 'exp_avg': torch.ones(2)}},
        'param_groups': [
            {'params': [0], 'is_dsa_indexer': False},
            {'params': [], 'is_dsa_indexer': False},
        ],
    }
    ...
    assert state_dict['param_groups'][0]['step'].item() == 123.0
    assert 'step' not in state_dict['param_groups'][1]
