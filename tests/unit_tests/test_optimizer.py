...
def test_precision_aware_optimizer(
    ...
    baseline_ddp_config = DistributedDataParallelConfig(use_distributed_optimizer=xm is None)
    ...
    baseline_optimizer_config = OptimizerConfig(
        ...
        lr=0.01,
        bf16=True,
        use_distributed_optimizer=xm is None,
    ...
    ddp_config = DistributedDataParallelConfig(use_distributed_optimizer=xm is None)
...
def test_optimizer_reload_model_params():
    ...
    optimizer_config = OptimizerConfig(optimizer='adam', bf16=True, use_distributed_optimizer=xm is None)
    optim = get_megatron_optimizer(optimizer_config, [model])
...
def test_get_megatron_optimizer_with_custom_process_groups(world_size, tp_size, cp_size, dp_size):
    ...
    input_tensor = torch.randn(32, 100, device=get_current_device(), requires_grad=True)
