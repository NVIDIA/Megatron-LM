...
def test_te_thd_partition_helper_delegates_to_transformer_engine(monkeypatch):
    from megatron.core.extensions import transformer_engine as te_extension

    calls = []
    expected = torch.tensor([3, 1], dtype=torch.int64)

    def _partition(cu_seqlens, total_tokens, cp_size, cp_rank):
        calls.append((cu_seqlens, total_tokens, cp_size, cp_rank))
        return expected

    monkeypatch.setattr(te_extension, "is_te_min_version", lambda _version: True)
    monkeypatch.setitem(
        sys.modules,
        "transformer_engine_torch",
        SimpleNamespace(thd_get_partitioned_indices=_partition),
    )
    cu_seqlens = torch.tensor([0, 2, 6], dtype=torch.int32)

    actual = te_extension.get_thd_partitioned_indices(cu_seqlens, 6, 2, 1)

    assert actual is expected
    assert calls == [(cu_seqlens, 6, 2, 1)]
...
@pytest.mark.parametrize(
    "rank, expected_tokens, expected_padding_mask",
    [
        (2, torch.arange(8, 12, dtype=torch.int64), torch.zeros(4, dtype=torch.bool)),
        (3, torch.zeros(4, dtype=torch.int64), torch.ones(4, dtype=torch.bool)),
    ],
)
def test_dsv4_thd_cp_slice_uses_static_partition_total(
    ...
    expected_labels = expected_tokens + 100 if rank == 2 else expected_tokens
    ...
    assert torch.equal(batch["loss_mask"], expected_loss_mask)
    assert torch.equal(batch["padding_mask"], expected_padding_mask)
...
def test_dsv4_thd_contiguous_cp_slice_rejects_invalid_static_totals():
    from megatron.core.datasets.data_schedule_utils import get_cp_slice_for_thd
    ...
    with pytest.raises(RuntimeError, match="smaller than tokens length"):
        get_cp_slice_for_thd(
            batch,
            _MockCPGroup(size=4, rank=0),
            keys=("tokens",),
            cp_partition_mode="contiguous",
...
def test_dp_balanced_scheduler_reserves_dummy_tail_capacity():
    config = SimpleNamespace(
        thd_max_packed_sequences=3,
        pad_packed_seq_alignment="max",
        pad_packed_seq_by_appending_dummy_seq=True,
    )
    assert _get_scheduler_max_real_num_seqs(config) == 2
    ...
    assert scheduler.get_groups_and_subsamples(
        [(0, 1), (1, 1), (2, 1), (3, 1)]
    ) == [[[0, 1]], [[2, 3]]]


def test_get_batch_applies_static_thd_padding_from_config():
    Utils.initialize_model_parallel(1, 1)
    try:
        device = torch.device("cuda", torch.cuda.current_device())
        tokens = torch.arange(8, dtype=torch.int64, device=device)
        ...
        tokens, *_, packed_seq_params, padding_mask = (
            get_batch_on_this_rank_for_sequence_packing(
                iter([batch]),
        ...
        assert packed_seq_params.cu_seqlens_q.shape == (5,)
        assert packed_seq_params.cu_seqlens_q.tolist() == [0, 3, 5, 16, 16]
        assert packed_seq_params.cu_seqlens_q_padded.tolist() == [0, 4, 8, 16, 16]
        assert padding_mask.shape == (1, 16)
        assert padding_mask[0, 8:].all()
    finally:
        Utils.destroy_model_parallel()
...
def test_wrap_dataloader(tp, pp, cp, vpp, scheduler_type, mtp_vpp, monkeypatch):
    ...
    try:
        ...
        (
            ...
            data_iterator,
            config,
            num_micro_batches_old,
            _scheduler_pg_collection(),
        )
