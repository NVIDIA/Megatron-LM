...
class TestSaveResidualDgradRawMomentsByLayer:
    def test_creates_jsonl_with_loss_scale(self, tmp_path):
        save_residual_dgrad_raw_moments_by_layer(
            str(tmp_path),
            iteration=100,
        ...
        filepath = (
            tmp_path / "training_stats" / "residual_dgrad_raw_moments_by_layer" / "rank3.jsonl"
        )
        records = _read_records(filepath)
        assert records == [
            {
                "iter": 100,
                "consumed_train_samples": 8192,
                "stat": "residual_dgrad_raw_moments_by_layer",
                "values": {
                    "decoder.layers.0/output0": {
                        "count": 2.0,
                        "sum_1": 1.5,
