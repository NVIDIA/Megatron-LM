...
def test_layer_residual_observation_skips_checkpoint_no_grad_forward():
    ...
    with capture_tensor_observations(
        ...
        observe_layer_residuals(layer, torch.tensor([3.0]), torch.tensor([4.0]))

    assert len(observed) == 1
    torch.testing.assert_close(observed[0][3], torch.tensor([3.0]))
