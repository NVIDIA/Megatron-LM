...
def load_and_verify_saved_tensors():
    ...
    # 查找保存的tensor文件
    tensor_files = glob.glob('./enhanced_tensor_logs/*.pt')
...
def test_tensor_saver_environment_variables():
    """测试环境变量设置"""
    # 设置环境变量
    os.environ['TENSOR_SAVE_DIR'] = './enhanced_tensor_logs'
    os.environ['TENSOR_SAVE_ENABLED'] = 'true'
    ...
    # 验证环境变量设置
    assert os.environ.get('TENSOR_SAVE_DIR') == './enhanced_tensor_logs'
    assert os.environ.get('TENSOR_SAVE_ENABLED') == 'true'


def test_tensor_saver_different_quant_types():
    """测试不同量化类型的tensor保存"""
    quant_types = ["hifp8", "mxfp8", "mxfp4", "bf16"]
    
    for quant_type in quant_types:
        # 创建测试tensor
        test_tensor = torch.randn(2, 3, dtype=torch.float32)
        
        # 测试保存
        from megatron.core.tensor_saver import save_tensor
        filepath = save_tensor(
            tensor=test_tensor,
        ...
        # 验证保存成功
        assert filepath is not None or not os.environ.get('TENSOR_SAVE_ENABLED', 'false').lower() == 'true'
