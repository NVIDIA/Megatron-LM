...
def test_missing_distributed_gtp_source_uses_rank_local_logical_shape():
    ...
    views = _optimizer_tensor_views(
        parameter_names, optimizer, _pg_collection(dp_size=2, gtp_size=2, gtp_rank=1)
    )
...
@pytest.mark.parametrize(
    ...
    ((False, "gtp", {"gtp_size": 2}), (True, "expert_gtp", {"expert_gtp_size": 2})),
)
...
def test_model_parallel_relations_keep_padded_gtp_as_dim_zero_shard(is_expert):
    ...
    pg_collection = _pg_collection(
        gtp_size=1 if is_expert else 2, expert_gtp_size=2 if is_expert else 1
    )
...
def test_observer_reduces_gtp_parameter_and_wgrad_shards_end_to_end(is_expert):
    ...
    model_parameter = torch.nn.Parameter(
        torch.tensor([[first_row, 2 * first_row], [second_row, 2 * second_row]], device=device)
    )
    ...
    logical_row_square_sum = sum(float(index + 1) ** 2 for index in range(logical_row_count))
