...
def _pg_collection(
    ...
    return SimpleNamespace(
        ...
        gtp_remat=_ProcessGroup(gtp_size, gtp_rank),
        expt_gtp_remat=_ProcessGroup(expert_gtp_size, expert_gtp_rank),
...
def _fp32_optimizer(*model_parameters):
    optimizer = object.__new__(FP32Optimizer)
    optimizer.is_stub_optimizer = False
    optimizer.optimizer = SimpleNamespace(param_groups=[{"params": list(model_parameters)}])
    return optimizer
...
@pytest.mark.parametrize(("gtp_rank", "expected_rows"), ((0, 2), (1, 1), (2, 0), (3, 0)))
def test_optimizer_source_trims_rank_local_gtp_logical_prefix(is_expert, gtp_rank, expected_rows):
    model_parameter = torch.nn.Parameter(torch.zeros(2, 2, dtype=torch.bfloat16))
    main_parameter = torch.nn.Parameter(torch.tensor([[1.0, 2.0], [1000.0, 1000.0]]))
    model_parameter.main_grad = torch.tensor([[3.0, 4.0], [2000.0, 2000.0]])
    model_parameter.allreduce = not is_expert
    model_parameter.is_gtp_weight_remat = True
    model_parameter.pad_length = 5
    pg_collection = _pg_collection(
        gtp_size=1 if is_expert else 4,
        gtp_rank=0 if is_expert else gtp_rank,
        expert_gtp_size=4 if is_expert else 1,
        expert_gtp_rank=gtp_rank if is_expert else 0,
    )

    views = _local_optimizer_tensor_views(
        _float16_optimizer(model_parameter, main_parameter), pg_collection
    )

    assert tuple(views[0].parameter.shape) == (expected_rows, 2)
    assert tuple(views[0].wgrad.shape) == (expected_rows, 2)
    torch.testing.assert_close(views[0].parameter, main_parameter[:expected_rows])
    torch.testing.assert_close(views[0].wgrad, model_parameter.main_grad[:expected_rows])


@pytest.mark.parametrize(
    ("start", "end", "expected_values", "expected_placement"),
    (
        (0, 2, [10.0, 20.0], FlatShard((1, 2), 0, 2)),
        (1, 3, [20.0], FlatShard((1, 2), 1, 2)),
        (2, 4, [], FlatShard((1, 2), 2, 2)),
    ),
)
def test_distributed_optimizer_source_clips_flat_shards_to_gtp_logical_prefix(
    start, end, expected_values, expected_placement
):
    physical_parameter = torch.tensor([[10.0, 20.0], [1000.0, 1000.0]])
    physical_wgrad = torch.tensor([[30.0, 40.0], [2000.0, 2000.0]])
    model_parameter = torch.nn.Parameter(physical_parameter.clone())
    model_parameter.main_grad = physical_wgrad
    model_parameter.is_gtp_weight_remat = True
    model_parameter.pad_length = 1
    main_parameter = torch.nn.Parameter(physical_parameter.reshape(-1)[start:end].clone())
    optimizer = _distributed_optimizer(model_parameter, main_parameter, start, end)

    views = _local_optimizer_tensor_views(
        optimizer, _pg_collection(dp_size=2, gtp_size=2, gtp_rank=1)
    )

    assert views[0].storage_relations[0].placement == expected_placement
    torch.testing.assert_close(views[0].parameter, torch.tensor(expected_values))
    expected_wgrad = physical_wgrad.reshape(-1)[start : min(end, 2)]
    torch.testing.assert_close(views[0].wgrad, expected_wgrad)
...
def test_missing_distributed_gtp_source_uses_rank_local_logical_shape():
    local_parameter = torch.nn.Parameter(torch.tensor([1.0, 2.0]))
    missing_parameter = torch.nn.Parameter(torch.ones(2, 2))
    missing_parameter.is_gtp_weight_remat = True
    missing_parameter.pad_length = 1
    optimizer = _distributed_optimizer(
        local_parameter, torch.nn.Parameter(torch.tensor([1.0, 2.0])), 0, 2
    )
    parameter_names = CanonicalParameterNameMap(
    ...
    views = _optimizer_tensor_views(
        parameter_names,
        optimizer,
        _pg_collection(dp_size=2, gtp_size=2, gtp_rank=1),
    )

    assert views[0].model_parameter is missing_parameter
    assert views[0].storage_relations[0].placement == FlatShard((1, 2), 0, 0)
...
def test_optimizer_metric_tensors_compose_model_and_storage_placements():
    ...
    assert [relation.axis for relation in values[0].rank_relations] == [
        "tp",
        "ep",
        "gtp",
        "dp",
...
@pytest.mark.parametrize(
    ("is_expert", "axis", "group_size_argument"),
    (
        (False, "gtp", {"gtp_size": 2}),
        (True, "expert_gtp", {"expert_gtp_size": 2}),
    ),
)
def test_model_parallel_relations_describe_gtp_shards_and_replicas(
    is_expert, axis, group_size_argument
):
    sharded = torch.nn.Parameter(torch.ones(2, 2))
    sharded.allreduce = not is_expert
    sharded.is_gtp_weight_remat = True
    sharded.pad_length = 0
    replicated = torch.nn.Parameter(torch.ones(2))
    replicated.allreduce = not is_expert
    pg_collection = _pg_collection(**group_size_argument)

    sharded_relations = _model_parallel_relations(sharded, pg_collection)
    replicated_relations = _model_parallel_relations(replicated, pg_collection)

    sharded_relation = next(relation for relation in sharded_relations if relation.axis == axis)
    replicated_relation = next(
        relation for relation in replicated_relations if relation.axis == axis
    )
    assert sharded_relation.placement == Shard(0)
    assert replicated_relation.placement == Replica()


@pytest.mark.parametrize("is_expert", (False, True))
def test_model_parallel_relations_keep_padded_gtp_as_dim_zero_shard(is_expert):
    parameter = torch.nn.Parameter(torch.ones(2, 2))
    parameter.allreduce = not is_expert
    parameter.is_gtp_weight_remat = True
    parameter.pad_length = 1
    pg_collection = _pg_collection(
        gtp_size=1 if is_expert else 2,
        expert_gtp_size=2 if is_expert else 1,
    )

    relations = _model_parallel_relations(parameter, pg_collection)

    axis = "expert_gtp" if is_expert else "gtp"
    relation = next(relation for relation in relations if relation.axis == axis)
    assert relation.placement == Shard(0)


def test_optimizer_manifest_records_rank_local_gtp_logical_shape():
    parameter = torch.nn.Parameter(torch.ones(2, 2))
    parameter.is_gtp_weight_remat = True
    parameter.pad_length = 1
    parameter_names = CanonicalParameterNameMap(_named_model(weight=parameter))
    pg_collection = _pg_collection(gtp_size=2, gtp_rank=1)

    manifest = _build_optimizer_parameter_manifest(
        parameter_names, _fp32_optimizer(parameter), pg_collection
    )

    assert manifest[0].logical_shape == (1, 2)
...
@pytest.mark.parametrize("is_expert", (False, True))
def test_observer_reduces_gtp_parameter_and_wgrad_shards_end_to_end(is_expert):
    launched_world_size = int(os.environ.get("WORLD_SIZE", "1"))
    if launched_world_size < 2:
        pytest.skip("This test requires at least two distributed ranks.")
    if not torch.distributed.is_initialized():
    ...
    rank = torch.distributed.get_rank()
    world_size = torch.distributed.get_world_size()
    first_row = float(2 * rank + 1)
    second_row = float(2 * rank + 2)
    model_parameter = torch.nn.Parameter(
        torch.tensor(
            [[first_row, 2 * first_row], [second_row, 2 * second_row]], device=device
        )
    )
    model_parameter.main_grad = torch.tensor(
        [[3 * first_row, 4 * first_row], [3 * second_row, 4 * second_row]], device=device
    )
    model_parameter.allreduce = not is_expert
    model_parameter.is_gtp_weight_remat = True
    model_parameter.pad_length = world_size - 1
    replicated_parameter = torch.nn.Parameter(torch.tensor([4.0], device=device))
    replicated_parameter.main_grad = torch.tensor([3.0], device=device)
    replicated_parameter.allreduce = not is_expert
    model = torch.nn.Module()
    model.decoder = torch.nn.Module()
    model.decoder.layers = torch.nn.ModuleList([torch.nn.Module()])
    model.decoder.layers[0].register_parameter("weight", model_parameter)
    model.decoder.layers[0].register_parameter("bias", replicated_parameter)

    world = torch.distributed.group.WORLD
    pg_collection = _pg_collection()
    if is_expert:
        pg_collection.expt_gtp_remat = world
    else:
        pg_collection.gtp_remat = world
    captured = {}
    observer = build_tensor_metric_observer(
        [
            "global-param-l2:1",
    ...
    observer(
        model=[model],
        optimizer=_fp32_optimizer(model_parameter, replicated_parameter),
        iteration=0,
        pg_collection=pg_collection,
    )

    logical_row_count = world_size + 1
    logical_row_sum = sum(float(index + 1) for index in range(logical_row_count))
    logical_row_square_sum = sum(
        float(index + 1) ** 2 for index in range(logical_row_count)
    )
    expected_parameter = (5 * logical_row_square_sum + 4.0**2) ** 0.5
    expected_mean_row = (5**0.5 * logical_row_sum + 4.0) / (logical_row_count + 1)
    expected_mean_column = (3 * logical_row_square_sum**0.5 + 4.0) / 3
    expected_wgrad = (25 * logical_row_square_sum + 3.0**2) ** 0.5
    for metric_name, expected in (
        ("global-param-l2", expected_parameter),
        ("layer-param-l2", expected_parameter),
        ("global-param-mean-row-l2", expected_mean_row),
        ("global-param-mean-column-l2", expected_mean_column),
