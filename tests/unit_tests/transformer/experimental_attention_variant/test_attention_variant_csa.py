...
class TestCompressorThd:
    ...
    @pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
    def test_thd_projection_disables_fp8(self, compress_ratio, monkeypatch):
        """THD projections use the same high-precision path as SBHD projections."""
        compressor = self._make_compressor(compress_ratio)
        tracker = _DisabledContextTracker()
        calls = []
        ...
        x = torch.randn(seq_len, 1, self.config.hidden_size, dtype=torch.bfloat16, device='cuda')
        compressor(x, packed_seq_params=_make_packed_seq_params_thd([seq_len]))
