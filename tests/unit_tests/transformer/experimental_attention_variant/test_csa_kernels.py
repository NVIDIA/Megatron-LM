...
class TestKLLossFromDenseScores:
    ...
    def test_ratio_masked_positions_do_not_produce_infinite_loss(self):
        ...
        loss = _kl_loss_from_dense_scores(
            attn_score, attn_l1norm, index_score, index_lse, loss_coeff=1.0
        )
...
class TestRealKernelFusedIndexerSparseAttn:
    ...
    def test_real_fused_dense_loss_matches_reference(self, reset_lazy_kernel_state):
        ...
        # ``fused_indexer_sparse_attn`` in ``csa_kernels.py``. Mirror
...
class TestThdPureHelpers:
    ...
    * ``_compute_dense_attn_lse`` — THD/segmented-BSHD numerical parity.
    """
    ...
    def test_dense_attn_lse_thd_matches_segmented_bshd(self):
        """Packed multi-segment LSE matches independent BSHD segments."""
        torch.manual_seed(123)
        q_lens = [5, 7]
        kv_lens = [2, 3]
        num_heads = 4
        ...
        head_dim = 8
        ratio = 2
        qhead_per_kv_head = num_heads // num_kv_heads

        q_thd = torch.randn(sum(q_lens), num_heads, head_dim)
        k_thd = torch.randn(sum(kv_lens), num_kv_heads, head_dim)
        cu_q = _make_cu_seqlens(q_lens)
        cu_kv = _make_cu_seqlens(kv_lens)
        actual = dk._compute_dense_attn_lse(
            q_thd,
            k_thd,
        ...
        for q_len, kv_len in zip(q_lens, kv_lens):
            expected.append(
                dk._compute_dense_attn_lse(
                    q_thd[q_start : q_start + q_len].unsqueeze(0),
                    k_thd[kv_start : kv_start + kv_len].unsqueeze(0),
                    softmax_scale=head_dim**-0.5,
                    qhead_per_kv_head=qhead_per_kv_head,
                    ratio=ratio,
                ).squeeze(0)
    ...
    def test_dense_attn_lse_thd_honors_q_causal_offsets(self):
        """Local CP rows use their sequence-relative global query positions."""
        torch.manual_seed(321)
        q_lens = [2, 3]
        kv_lens = [3, 3]
        q_causal_offsets = torch.tensor([4, 2], dtype=torch.int32)
        num_heads = 4
        num_kv_heads = 2
        head_dim = 8
        ratio = 2
        ...
        actual = dk._compute_dense_attn_lse(
            ...
            cu_seqlens_kv=_make_cu_seqlens(kv_lens),
            max_seqlen_kv=max(kv_lens),
        ...
        q_start = 0
        kv_start = 0
        for segment, (q_len, kv_len) in enumerate(zip(q_lens, kv_lens)):
            for q_position in range(q_len):
                visible_k = min((int(q_causal_offsets[segment]) + q_position + 1) // ratio, kv_len)
                row_lse = []
                for head in range(num_heads):
                    kv_head = head // qhead_per_kv_head
                    scores = (
                        q_thd[q_start + q_position, head]
                        * k_thd[kv_start : kv_start + visible_k, kv_head]
                    ).sum(dim=-1)
                    row_lse.append(torch.logsumexp(scores * softmax_scale, dim=-1))
                expected.append(torch.stack(row_lse))
            q_start += q_len
            kv_start += kv_len
...
class TestFusedIndexerSparseAttnFromTopk:
    """Focused contract tests for the CP caller-supplied-top-k autograd path."""

    @pytest.mark.parametrize("sparse_loss", [False, True], ids=["dense", "sparse"])
    def test_offsets_padding_mask_and_backward(self, sparse_loss, monkeypatch):
        total_q, num_heads, head_dim = 3, 2, 4
        total_kv, total_comp = 6, 4
        index_heads, index_dim, topk = 2, 3, 2

        query = torch.randn(total_q, num_heads, head_dim, requires_grad=True)
        kv_full = torch.randn(total_kv, head_dim, requires_grad=True)
        attn_sink = torch.randn(num_heads, requires_grad=True)
        topk_idxs = torch.tensor([[0, 1], [2, 3], [4, 5]], dtype=torch.int32)
        q_indexer = torch.randn(total_q, index_heads, index_dim, requires_grad=True)
        k_indexer = torch.randn(total_comp, index_dim, requires_grad=True)
        weights = torch.randn(total_q, index_heads, requires_grad=True)
        indexer_topk_idxs = torch.tensor([[0, 1], [1, 2], [2, 3]], dtype=torch.int32)
        compressed_kv = torch.randn(total_comp, head_dim)
        cu_seqlens_q = torch.tensor([0, total_q], dtype=torch.int32)
        cu_seqlens_k = torch.tensor([0, total_comp], dtype=torch.int32)
        q_causal_offsets = torch.tensor([1], dtype=torch.int32)
        q_padding_mask = torch.tensor([False, True, False])

        def fake_flash(q, kv, caller_topk, _softmax_scale, **kwargs):
            assert torch.equal(caller_topk, topk_idxs)
            assert kwargs["indexer_topk"] == topk
            return (
                torch.zeros_like(q),
                torch.zeros(total_q, num_heads),
                torch.zeros(total_q, num_heads),
            )

        monkeypatch.setattr(dk, "_dsa_fwd_flash_mla", fake_flash)
        fake_dsa = MagicMock()
        monkeypatch.setattr(dk, "_DSA", fake_dsa)

        def fake_sparse_attn_backward(q, kv, _out, _grad, _lse, sink, caller_topk, **kwargs):
            assert torch.equal(caller_topk, topk_idxs)
            return {"dq": torch.ones_like(q), "dkv": torch.ones_like(kv), "d_sink": torch.ones_like(sink)}

        fake_dsa.sparse_attention_backward_wrapper.side_effect = fake_sparse_attn_backward

        if sparse_loss:

            def fake_sparse_score(_q, _k, _w, caller_indexer_topk, **kwargs):
                assert torch.equal(
                    caller_indexer_topk.squeeze(0)[q_padding_mask],
                    torch.full((1, topk), -1, dtype=torch.int32),
                )
                return {"predict": torch.full((1, total_q, topk), 1.0 / topk)}

            def fake_target(_q, _k, _lse, caller_indexer_topk, *_args, **_kwargs):
                assert torch.equal(
                    caller_indexer_topk[q_padding_mask],
                    torch.full((1, topk), -1, dtype=torch.int32),
                )
                return torch.full((total_q, topk), 1.0 / topk)

            fake_dsa.sparse_indexer_score_recompute_wrapper.side_effect = fake_sparse_score
            fake_dsa.indexer_backward_wrapper.return_value = {
                "d_index_q": torch.full((1, total_q, index_heads, index_dim), 2.0),
                "d_index_k": torch.full((1, total_comp, index_dim), 3.0),
                "d_weights": torch.full((1, total_q, index_heads), 4.0),
            }
            monkeypatch.setattr(dk, "_compute_attn_target", fake_target)
        else:

            def assert_offsets(kwargs):
                assert kwargs["q_causal_offsets"] is q_causal_offsets

            def fake_dense_indexer(*_args, **kwargs):
                assert_offsets(kwargs)
                return torch.zeros(total_q, total_comp), torch.full(
                    (total_q,), math.log(total_comp)
                )

            def fake_dense_lse(*_args, **kwargs):
                assert_offsets(kwargs)
                return torch.zeros(total_q, num_heads)

            def fake_dense_attn_score(*_args, **kwargs):
                assert_offsets(kwargs)
                return torch.ones(total_q, total_comp), torch.full(
                    (total_q,), float(total_comp)
                )

            def fake_dense_backward(
                _q, _w, _k, attn_score, _attn_l1norm, index_score, index_lse, **kwargs
            ):
                assert_offsets(kwargs)
                assert torch.equal(attn_score[q_padding_mask], torch.zeros(1, total_comp))
                assert torch.equal(index_score[q_padding_mask], torch.zeros(1, total_comp))
                assert torch.equal(index_lse[q_padding_mask], torch.zeros(1))
                return {
                    "d_index_q": torch.full_like(q_indexer, 2.0),
                    "d_index_k": torch.full_like(k_indexer, 3.0),
                    "d_weights": torch.full_like(weights, 4.0),
                }

            monkeypatch.setattr(dk, "_compute_dense_indexer_score", fake_dense_indexer)
            monkeypatch.setattr(dk, "_compute_dense_attn_lse", fake_dense_lse)
            monkeypatch.setattr(dk, "_compute_dense_attn_score", fake_dense_attn_score)
            fake_dsa.dense_indexer_backward_wrapper.side_effect = fake_dense_backward
        ...
        output, indexer_loss = FusedIndexerSparseAttnFromTopkFunc.apply(
            ...
            1,
            total_comp,
            (cu_seqlens_q, cu_seqlens_k, q_causal_offsets),
            q_padding_mask,
        )
        (output.sum() + indexer_loss).backward()
        ...
        torch.testing.assert_close(attn_sink.grad, torch.ones_like(attn_sink))
        torch.testing.assert_close(q_indexer.grad[~q_padding_mask], torch.full_like(q_indexer.grad[~q_padding_mask], 2.0))
        torch.testing.assert_close(q_indexer.grad[q_padding_mask], torch.zeros_like(q_indexer.grad[q_padding_mask]))
        torch.testing.assert_close(k_indexer.grad, torch.full_like(k_indexer, 3.0))
        torch.testing.assert_close(weights.grad[~q_padding_mask], torch.full_like(weights.grad[~q_padding_mask], 4.0))
        torch.testing.assert_close(weights.grad[q_padding_mask], torch.zeros_like(weights.grad[q_padding_mask]))
...
class TestPublicApi:
    ...
    def test_public_surface(self):
        ...
        assert issubclass(FusedIndexerSparseAttnFromTopkFunc, torch.autograd.Function)
