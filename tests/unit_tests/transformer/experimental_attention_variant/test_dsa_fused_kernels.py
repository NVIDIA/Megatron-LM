...
from megatron.core.transformer.experimental_attention_variant.dsa import DSAFunction
...
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("topk", [2048, 4096])
def test_sparse_mla_fwd(topk, S=4096, H=128, HKV=1, DQK=576, DV=512, dtype=torch.bfloat16):
...
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("topk", [2048, 4096])
def test_sparse_mla_bwd(topk, S=4096, H=128, HKV=1, DQK=576, DV=512, dtype=torch.bfloat16):
    torch.manual_seed(1234)
    torch.cuda.manual_seed(1234)
...
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("topk", [2048, 4096])
def test_indexer_topk_reducesum(topk, S=4096, H=64, D=128, dtype=torch.bfloat16):
    torch.manual_seed(1234)
    torch.cuda.manual_seed(1234)
...
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("topk", [2048, 4096])
def test_indexer_bwd(topk, S=4096, H=64, D=128, dtype=torch.bfloat16):
    torch.manual_seed(1234)
    torch.cuda.manual_seed(1234)
...
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("topk", [2048, 4096])
def test_sparse_mla_topk_reducesum(topk, S=4096, H=128, DV=512, D_tail=64, dtype=torch.bfloat16):
    torch.manual_seed(1234)
    torch.cuda.manual_seed(1234)
