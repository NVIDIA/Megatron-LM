...
def test_config_accepts_multi_layer_hybrid_mtp_ratio_list():
    config = _make_config(num_layers=2, mtp_num_layers=1, csa_compress_ratios=[0, 4, 128, 0])
    assert config.csa_compress_ratios == [0, 4, 128, 0]


def test_hybrid_dsv4_stack_spec_assigns_symbol_ratios():
    ...
    assert "compress_ratio" not in submodules.dsa_layer.submodules.self_attention.params
...
def test_config_accepts_static_context_parallelism():
    with patch(
        "megatron.core.transformer.transformer_config.is_te_min_version", return_value=True
    ):
        config = _make_config(
...
@pytest.mark.parametrize(
    ("config_kwargs", "match"),
    [
        ({"context_parallel_size": 2}, "sequence_packing_scheduler"),
        (
            {"context_parallel_size": 2, "sequence_packing_scheduler": "dp_balanced"},
            "cp_partition_mode='contiguous'",
        ),
        ({"hybrid_context_parallel": True}, "dynamic per-microbatch context parallelism"),
        ({"cp_partition_mode": "interleaved"}, "Unsupported cp_partition_mode"),
    ],
    ids=["missing_scheduler", "zigzag", "dynamic_cp", "invalid_partition"],
)
def test_config_rejects_unsupported_context_parallelism(config_kwargs, match):
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"qkv_format": "thd", "local_cp_size": 2, "cp_partition_mode": "contiguous"},
            "per-microbatch context-parallel groups",
        ),
        ({"qkv_format": "sbhd", "cp_partition_mode": "contiguous"}, "qkv_format='thd'"),
        ({"qkv_format": "thd", "cp_partition_mode": "zigzag"}, "contiguous CP partition"),
    ],
    ids=["local_cp", "non_thd", "zigzag"],
)
def test_forward_rejects_unsupported_context_parallel_inputs(packed_kwargs, match):
    from megatron.core.transformer.experimental_attention_variant.deepseek_v4_hybrid_attention import (
        DSv4HybridAttention,
    )

    cp_group = SimpleNamespace(size=lambda: 2)
    attention = SimpleNamespace(pg_collection=SimpleNamespace(cp=cp_group))

    with pytest.raises(ValueError, match=match):
        DSv4HybridAttention.forward(
            attention,
            hidden_states=None,
            attention_mask=None,
            packed_seq_params=PackedSeqParams(**packed_kwargs),
        )
