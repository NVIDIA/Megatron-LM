...
class TestDSv4HybridAttentionTHDCP:
    ...
    def test_thd_cp_matches_full_reference_forward_backward(
        self, layer_number, sparse_loss, apply_rope_fusion, monkeypatch
    ):
        ...
        project_weights_calls = 0
        if layer_number == 2:
            original_project_weights = cp_attn.core_attention.indexer._project_weights

            def tracked_project_weights(x):
                nonlocal project_weights_calls
                project_weights_calls += 1
                return original_project_weights(x)

            monkeypatch.setattr(
                cp_attn.core_attention.indexer, "_project_weights", tracked_project_weights
            )
        ...
        if layer_number == 2:
            assert project_weights_calls == 1
        ref_out, _ = ref_attn(
    ...
    def test_cp_mla_up_proj_recompute_matches_eager(self, apply_rope_fusion):
        """Selective MLA recompute must retain the configured CP group."""
        ...
        eager_attn = _build_attention(eager_config, layer_number=2, pg_collection=self.pg).cuda()
        recompute_attn = _build_attention(
            recompute_config, layer_number=2, pg_collection=self.pg
        ).cuda()
        eager_attn.train()
        ...
        mode = f"cp:rope_fused={apply_rope_fusion}"
