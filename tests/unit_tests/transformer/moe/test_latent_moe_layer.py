...
class TestLatentMoELayer:
    ...
    def test_latent_moe_layer(
        ...
        assert moe_layer.shared_experts.linear_fc1.weight.shape[1] == config.hidden_size, (
            "Shared expert computation has to happen in hidden dimension."
        )
        assert moe_layer.shared_experts.linear_fc2.weight.shape[0] == config.hidden_size, (
            "Shared expert computation has to happen in hidden dimension."
        )
        if grouped_gemm:
            for i in range(num_moe_experts):
                ...
                assert fc1_weight.shape[1] == config.moe_latent_size, (
                    f"Shape mismatch for expert {i} {fc1_weight.shape=}"
                )
                assert fc2_weight.shape[0] == config.moe_latent_size, (
                    f"Shape mismatch for expert {i} {fc2_weight.shape=}"
                )
        else:
            for i in range(num_moe_experts):
                expert = moe_layer.experts.local_experts[i]
                assert expert.linear_fc1.weight.shape[1] == config.moe_latent_size, (
                    f"Shape mismatch for expert {i} {fc1_weight.shape=}"
                )
                assert expert.linear_fc2.weight.shape[0] == config.moe_latent_size, (
                    f"Shape mismatch for expert {i} {fc2_weight.shape=}"
                )
        assert moe_layer.router.weight.shape[1] == config.hidden_size, (
