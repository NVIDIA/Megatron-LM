...
class MoEModelTestContainer:
    ...
    def dispatcher_router_padding_for_fp8_test(self):
        ...
        1. First with moe_router_padding_for_quant=False
        2. Then with moe_router_padding_for_quant=True

        We verify that:
        1. The results are the same in both cases
        2. The number of tokens received by each expert is padded to a multiple of 16
        """
        # First run with moe_router_padding_for_quant = False
        moe_layer = self.new_moe_layer(moe_router_padding_for_quant=False)
        ...
        # Run with moe_router_padding_for_quant = True
        moe_layer_2 = self.new_moe_layer(moe_router_padding_for_quant=True, fp8="hybrid")
        moe_layer_2.load_state_dict(moe_layer.state_dict())
