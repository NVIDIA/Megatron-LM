...
from megatron.core.device_utils import get_current_device, get_xla_model, set_manual_seed
...
def model_provider(
    ...
    **config_kwargs,
    ...
    print(f"use_te: {use_te} layer_spec_fn: {layer_spec_fn}")
    model = GPTModel(
        config=config,
...
class TestGPTModel:
    ...
    def test_upcycling_TE(self, tp_ep, granularity, grouped_gemm, swiglu, squared_relu):
        ...
        data = list(range(args.seq_length))
        input_ids = torch.tensor(data, dtype=torch.int64).repeat((args.micro_batch_size, 1)).to(get_current_device())
        position_ids = (
            torch.tensor(data, dtype=torch.int64).repeat((args.micro_batch_size, 1)).to(get_current_device())
        )
        attention_mask = torch.ones(
            (args.micro_batch_size, 1, args.seq_length, args.seq_length), dtype=bool
        ).to(get_current_device())
