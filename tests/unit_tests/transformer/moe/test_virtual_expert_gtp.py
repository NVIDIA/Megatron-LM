...
def test_virtual_expert_gtp_persistent_wgrads_match_recycled_scratch(monkeypatch, mxfp8):
    ...
    try:
        with monkeypatch.context() as patch:
            ...
            assert not any(hasattr(p, "_gtp_eager_wgrad_ring_slot") for p in experts)
        ...
        for seed in (21, 22, 21, 22):
            ...
            pointers = [p._gtp_eager_wgrad_ring_slot.tensor.data_ptr() for p in experts]
            if slots is None:
                slots = pointers
        ...
        try:
            ...
            with monkeypatch.context() as patch:
                ...
                for step in range(first_step, 3):
                    ...
                    if virtual:
                        assert tables and all(p._gtp_eager_wgrad_ring_slot is not None for p in experts)
                    history.append(
                        (
