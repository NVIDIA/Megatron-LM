...
def test_contiguous_roll_matches_whole_pack_and_gradients(cp_size):
    ...
    try:
        ...
        for _ in range(3):
            value = roll_contiguous(value, -1, group, packed)
            padding = roll_contiguous(padding, -1, group, packed, True)
