...
def test_cuda_graph_determine_first_last_layer_logic(
    ...
    hidden_states = torch.ones((sequence_length, micro_batch_size, hidden_size)).to(get_current_device())
    attention_mask = torch.ones((1, 1, sequence_length, sequence_length), dtype=bool).to(get_current_device())
    for m in model:
        _ = m(
            input_ids=None,
...
class TestLLaVACudaGraph:
    ...
    def test_llava_cudagraph_is_last_layer_logic(self):
        ...
        # Move model to CUDA
        self.llava_model.to(get_current_device())
        ...
        images = torch.ones((num_images, 3, 336, 336), dtype=torch.float32).to(get_current_device())

        # Create text input with image tokens
        input_ids = torch.randint(0, 1000, (batch_size, seq_length), dtype=torch.long).to(get_current_device())
        # Insert image token (using default image token index)
        input_ids[0, 5] = self.llava_model.image_token_index

        position_ids = torch.arange(seq_length).unsqueeze(0).expand(batch_size, -1).to(get_current_device())
        attention_mask = None

        # Create labels and loss mask for training
        labels = torch.randint(0, 1000, (batch_size, seq_length), dtype=torch.long).to(get_current_device())
        loss_mask = torch.ones((batch_size, seq_length), dtype=torch.float32).to(get_current_device())

        # Create num_image_tiles
        num_image_tiles = torch.ones(num_images, dtype=torch.int).to(get_current_device())
...
class TestParallelMambaBlockCudagraphs:
    ...
    def test_gpu_cudagraph(self):
        try:
            ...
            parallel_mamba_block.to(get_current_device())
