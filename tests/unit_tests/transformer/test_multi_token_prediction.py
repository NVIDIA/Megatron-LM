...
class TestMultiTokenPredictionLayer:
    ...
    def test_recompute_forward_propagates_rolled_padding_mask(self, monkeypatch):
        """Test full recompute forwards rolled padding_mask to transformer path."""
        torch.manual_seed(_SEED)
        config, mtp_block_spec = self._create_config_and_mtp_block_spec(tp=1, cp=1)
        ...
        with pytest.warns(UserWarning, match="block.*not supported for MTP"):
            _, _, _, returned_padding_mask = mtp_layer.forward(
                input_ids=input_ids,
                position_ids=position_ids,
                hidden_states=hidden_states,
