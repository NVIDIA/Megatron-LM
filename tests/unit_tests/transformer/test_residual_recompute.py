...
class _ConfigNorm(nn.LayerNorm):
    ...
    def forward(self, hidden_states):
        assert hidden_states.dtype == self.output_dtype
...
class _RecordingMambaLayer(WideResidualMambaLayer):
    def forward(
        self,
        hidden_states,
        *args,
...
def _ordinary_mamba_spec() -> ModuleSpec:
    spec = _mamba_spec()
    spec.module = MambaLayer
    return spec
...
class TestWideResidualHybridConstruction:
    ...
    def test_static_specs_name_explicit_wide_layer_classes(self, wide_stack_spec):
        ...
        for name in (
            ...
            layer_spec = getattr(wide, name)
            if layer_spec is not IdentityOp:
                assert layer_spec.module is WideResidualTransformerLayer

    def test_ordinary_mamba_layer_rejects_wide_config(self):
        config = _wide_recompute_config(num_layers=1)
        with pytest.raises(ValueError, match="WideResidualMambaLayer"):
            build_module(_ordinary_mamba_spec(), config=config, layer_number=1)
    ...
    def test_hybrid_mamba_cp_layout_finalizes_before_residual_replay(self, monkeypatch):
        ...
        class _RecordingCPLayoutManager:
            def build_forward_state(
                self, packed_seq_params, packed_seq_params_by_layout=None, thd_plan=None
            ):
                assert packed_seq_params is boundary_packed_seq_params
                assert packed_seq_params_by_layout is None
                assert thd_plan is None
