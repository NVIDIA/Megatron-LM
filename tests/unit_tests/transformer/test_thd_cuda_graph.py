...
class TestDynamicMicrobatchSlots:
    ...
    @pytest.mark.internal
    def test_dynamic_cp_parent_communicator_warmup_scope(self):
        ...
        assert helper._should_warm_dynamic_cp_parent_p2p()
        assert not helper._should_warm_dynamic_cp_parent_collective()

        helper.config.cuda_graph_modules = [CudaGraphModule.moe_router]
        assert not helper._should_warm_dynamic_cp_parent_p2p()
        assert helper._should_warm_dynamic_cp_parent_collective()

        helper.config.cuda_graph_modules = [CudaGraphModule.attn]
        helper.config.cp_comm_type = "all_gather"
        assert not helper._should_warm_dynamic_cp_parent_p2p()

    @pytest.mark.internal
    def test_dynamic_cp_parent_communicator_warmup_dispatch(self):
        from megatron.core.transformer.cuda_graphs import TECudaGraphHelper

        helper = TECudaGraphHelper.__new__(TECudaGraphHelper)
        parent_group = object()
        capture_contexts = [(2, object())]
        calls = []
        helper.dp_cp_group = parent_group
        helper._should_warm_dynamic_cp_parent_collective = lambda: True
        helper._should_warm_dynamic_cp_parent_p2p = lambda: True
        helper._warm_dynamic_cp_parent_collective = lambda group: calls.append(
            ("collective", group)
        )
        helper._warm_dynamic_cp_parent_p2p = lambda contexts, group: calls.append(
            ("p2p", contexts, group)
        )

        helper._warm_dynamic_cp_parent_communicators(capture_contexts)

        assert calls == [("collective", parent_group), ("p2p", capture_contexts, parent_group)]
