...
class TestParallelTransformerLayer:
    ...
    def test_te_cuda_graph_omits_absent_attention_mask(self):
        ...
        layer = TransformerLayer(
            config, get_gpt_layer_with_transformer_engine_submodules()
        )
    ...
    def test_te_cuda_graph_keeps_configured_attention_mask(self):
        ...
        assert static_inputs["attention_mask"].shape == (2, 1, 32, 32)

        hidden_states = torch.ones((32, 2, config.hidden_size), device="cuda")
        _, cudagraph_kwargs = layer._get_te_cuda_graph_replay_args(
            hidden_states, attention_mask=None
        )
        assert cudagraph_kwargs["attention_mask"].shape == (2, 1, 32, 32)
    ...
    def test_te_cuda_graph_warns_for_omitted_padding_mask(self, monkeypatch):
        config = TransformerConfig(
            num_layers=2,
            hidden_size=12,
            num_attention_heads=4,
        ...
        monkeypatch.setattr(
            ...
            lambda _logger, _level, message: warnings.append(message),
        )
        static_inputs = layer.get_layer_static_inputs(
            seq_length=32, micro_batch_size=1
        )
        assert "attention_mask" not in static_inputs
        assert any("attn_mask_type=padding" in message for message in warnings)
