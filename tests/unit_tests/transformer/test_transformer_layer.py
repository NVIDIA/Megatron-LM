...
from megatron.core.tensor_parallel.random import MHCRecomputeManager, model_parallel_cuda_manual_seed
...
class TestTransformerLayerWithHyperConnectionRecompute:
    ...
    def test_multiple_layers_chain_with_recompute(self):
        ...
        MHCRecomputeManager, simulating TransformerBlock behavior.
        """
        hidden_size = 64
        num_streams = 4
        seq_len = 8
...
class TestMHCRecomputeMemorySaving:
    ...
    def _run_forward_backward(
        ...
        When use_recompute=True, a new MHCRecomputeManager is created every
        ...
        manager = MHCRecomputeManager() if use_recompute else None
...
class TestMHCWithCudaGraph:
    ...
    def test_cuda_graph_fwd_bwd_with_hyper_connection_and_recompute(self):
        """CUDA graph capture+replay for fwd+bwd with mHC and MHCRecomputeManager.

        When a MHCRecomputeManager is used, additional CheckpointWithoutOutput
        ...
        capture_mgr = MHCRecomputeManager()
    ...
    def test_mcore_cudagraph_manager_with_mhc_recompute_manager(self):
        ...
        iterates over all kwargs to check supported types. MHCRecomputeManager
