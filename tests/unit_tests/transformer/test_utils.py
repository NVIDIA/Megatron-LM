...
from megatron.core.transformer.utils import set_model_to_sequence_parallel, is_layer_window_attention
...
class TestIsLayerWindowAttention:
    ...
    def test_no_window_size(self):
        ...
        for layer_number in [1, 2, 3, 4]:
            assert is_layer_window_attention(config, layer_number) == False
    ...
    def test_window_size_with_no_skip_freq(self):
        ...
        for layer_number in [1, 2, 3, 4]:
            assert is_layer_window_attention(config, layer_number) == True
    ...
    def test_integer_skip_frequency(self):
        ...
        for layer_number, expected in expected_results.items():
            result = is_layer_window_attention(config, layer_number)
    ...
    def test_invalid_skip_frequency_type(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            is_layer_window_attention(config, 1)
    ...
    def test_edge_cases(self):
        ...
        assert is_layer_window_attention(config, 1) == True
        ...
        assert is_layer_window_attention(config, 100) == False
        # Layer 99 should use window attention (99 % 10 != 0)
        assert is_layer_window_attention(config, 99) == True
    ...
    def test_window_size_different_formats(self):
        ...
        try:
            ...
            result = is_layer_window_attention(config, 1)
    ...
    def test_layer_number_edge_cases(self):
        ...
        assert is_layer_window_attention(config, 0) == False
        
        # Test negative layer numbers
        # -1 % 2 != 0 in Python, so should return True
        assert is_layer_window_attention(config, -1) == True
        assert is_layer_window_attention(config, -2) == False  # -2 % 2 == 0
        ...
        scenarios = [
            {
                ...
                "expected": {
                    1: True, 2: True, 3: True, 4: False,  # First group
                    5: True, 6: True, 7: True, 8: False,  # Second group
                    9: True, 10: True, 11: True, 12: False  # Third group
                }
            },
            {
                "config": TransformerConfig(
...
class TestIsLayerWindowAttentionIntegration:
    ...
    def test_with_transformer_config_validation(self):
        ...
        for layer in range(1, 9):
            result = is_layer_window_attention(config, layer)
