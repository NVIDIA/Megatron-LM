...
"""
Script to build a json file with the sequences per dataset to use
with the --per-dataset-sequences-path. Accepts the same arguments
as the training script.

Usage:
python3 tools/build_sequences_per_dataset.py \
    --per-split-data-args-path my-training-dataset-blend.json \
    --per-dataset-sequences-path \
    my-training-dataset-blend-sequences-per-dataset.json
...
def build_sequences_per_dataset(args):
    ...
    for file_prefix in file_prefixes:
        # NOTE(asolergi-nv): For every file prefix, read index file and get the number of sequences and documents  # noqa: E501
...
if __name__ == "__main__":
    ...
    print(
        f"Done! Saving --path-to-sequences-per-dataset file to {args.path_to_sequences_per_dataset_json}"  # noqa: E501
    )
