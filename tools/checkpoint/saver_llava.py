...
python tools/checkpoint/convert.py \
    ...
    --load-dir <load_path> \
    --save-dir <save_path> --megatron-path . \
    --target-tensor-parallel-size 1 --true-vocab-size 151680
"""
...
class MegatronCheckpointSaverLLaVA(MegatronCheckpointSaverBase):
    """Orchestrates saving a LLaVA Megatron checkpoint using
    parameters received on a multiprocessing queue.
    ...
    def _load_checkpoint_args(self, margs):
        if hasattr(self.md, 'checkpoint_args'):
            # These are arguments that we are either changing, or cause problems for validation if they are set  # noqa: E501
            ...
            for arg, value in vars(self.md.checkpoint_args).items():
                ...
                if getattr(margs, arg) != value:
                    print(
                        f"Overwriting default {arg} value {getattr(margs, arg)} with value from checkpoint {value}."  # noqa: E501
                    )
    ...
    def _maybe_parse_additional_megatron_args(self, margs):
        ...
        margs.use_te = getattr(
            self.md.checkpoint_args, "use_te", margs.transformer_impl == "transformer_engine"
        )
        ...
        margs.tokenizer_prompt_format = getattr(
            self.md.checkpoint_args, "tokenizer_prompt_format", "dummy"
        )
        margs.disable_vision_class_token = getattr(
            self.md.checkpoint_args, "disable_vision_class_token", False
        )
        ...
        margs.allow_missing_vision_projection_checkpoint = getattr(
            self.md.checkpoint_args, "allow_missing_vision_projection_checkpoint", False
        )
    ...
    def receive_vision_backbone(self, schema):
        ...
        if self.md.vision_model_type in ("radio", "radio-g"):
            embedder_weight = chunk_weight(
                vit_embeddings_msg["embedder weight"],
                "column",
                self.args.target_tensor_parallel_size,
                self.args.target_expert_parallel_size,
            )
            if self.md.vision_model_type == "radio-g":
                embedder_bias = chunk_bias(
                    vit_embeddings_msg["embedder bias"],
                    "column",
                    self.args.target_tensor_parallel_size,
                    self.args.target_expert_parallel_size,
                )
        ...
        for ep_rank in range(self.args.target_expert_parallel_size):
            for tp_rank in range(self.args.target_tensor_parallel_size):
                ...
                if self.md.vision_model_type in ("internvit", "clip", "siglip"):
                    ...
                    model.vision_model.position_embeddings.weight.data.copy_(
                        vit_embeddings_msg["position embeddings"]
                    )
                ...
                if self.md.vision_model_type in ("radio", "radio-g"):
                    ...
                    model.vision_model.position_embeddings.data.copy_(
                        vit_embeddings_msg["position embeddings"]
                    )
                ...
                if self.md.vision_model_type in ("siglip", "radio-g"):
                    model.vision_model.ln_post.weight.data.copy_(
                        vit_embeddings_msg["ln post weight"]
                    )
        ...
        for layer_id in range(schema.get_num_layers(self.get_local_model(pp_rank, 0, 0))):
            ...
            if self.md.vision_model_type == "internvit":
                ...
                k_norm_weight = chunk_bias(
                    msg.pop("k norm weight"), "column", self.args.target_tensor_parallel_size
                )
                q_norm_weight = chunk_bias(
                    msg.pop("q norm weight"), "column", self.args.target_tensor_parallel_size
                )
            if self.md.vision_norm_has_bias and self.md.vision_model_type == "internvit":
                k_norm_bias = chunk_bias(
                    msg.pop("k norm bias"), "column", self.args.target_tensor_parallel_size
                )
                q_norm_bias = chunk_bias(
                    msg.pop("q norm bias"), "column", self.args.target_tensor_parallel_size
                )
            ...
            for tp_rank in range(self.args.target_tensor_parallel_size):
                params_dict = {
                    ...
                    "self_attn_norm_bias": input_norm_bias
                    if self.md.vision_norm_has_bias
                    else None,
                ...
                if self.md.vision_model_type == "radio-g":
                    params_dict.update({"ls1": ls1, "ls2": ls2})
                if self.md.vision_model_type == "internvit":
                    params_dict.update(
                        {
    ...
    def receive_vision_projection(self):
        ...
        vision_projection_l0_weight = chunk_weight(
            vision_projection_msg.pop("vision projection l0 weight"),
            "column",
            self.args.target_tensor_parallel_size,
        )
        vision_projection_l1_weight = chunk_weight(
            vision_projection_msg.pop("vision projection l1 weight"),
            "row",
            self.args.target_tensor_parallel_size,
        )
        ...
        if "vision projection norm weight" in vision_projection_msg:
            vision_projection_norm_weight = vision_projection_msg.pop(
                "vision projection norm weight"
            )
        ...
        if self.md.vision_projection_linear_bias:
            vision_projection_l0_bias = chunk_bias(
                vision_projection_msg.pop("vision projection l0 bias"),
                "column",
                self.args.target_tensor_parallel_size,
            )
        ...
        for tp_rank in range(self.args.target_tensor_parallel_size):
            ...
            if self.md.vision_projection_linear_bias:
                ...
                model.vision_projection.encoder.linear_fc2.bias.data.copy_(
                    vision_projection_l1_bias
                )
