...
def get_core_transformer_block_key(model_key):
    return {"GPT": "decoder", "BERT": "encoder"}[model_key]
        ...
        super().__init__(
            {
                ...
                "output_layer": {"weight": f"{prefix}output_layer.weight"},
                "pooler": {
...
class CoreMoETESchema(CoreSchema):
    def __init__(
        self, model_type, num_experts, expert_model_parallel_size, prefix, extra_layer_schema
    ):
        ...
        super().__init__(
            ...
            layer_schema={
                ...
                **{
                    f"mlp_fc1_weight.{expert_idx}": f"mlp.experts.local_experts.{expert_idx}.linear_fc1.weight"  # noqa: E501
                    for expert_idx in range(num_local_experts)
                },
                **{
                    f"mlp_fc2_weight.{expert_idx}": f"mlp.experts.local_experts.{expert_idx}.linear_fc2.weight"  # noqa: E501
                    for expert_idx in range(num_local_experts)
                },
            }
            | extra_layer_schema,
...
def get_model_schema(
    ...
    if num_experts is not None and num_experts > 0:
        ...
        return CoreMoETESchema(
            model_type, num_experts, expert_model_parallel_size, prefix, extra_layer_schema
        )
    return {"local": CoreLocalSchema, "transformer_engine": CoreTESchema}[transformer_impl](
        model_type, prefix, extra_layer_schema
    )
