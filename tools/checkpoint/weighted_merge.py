...
class _WeightedMergeDirectOutputSavePlanner(SavePlanner):
    ...
    def __init__(
        ...
        *,
        ...
        self.extra_state_source_dir = (
            extra_state_source_dir
            if extra_state_source_dir is not None
            else resolved_input_dirs[extra_state_source_index]
        )
        self.load_time = 0.0
        self.accumulation_time = 0.0
        self._write_spec_by_index = {
            (spec.sharded_key, spec.global_offsets, spec.chunk_shape): spec for spec in write_specs
        }
        self._byte_spec_by_fqn = {spec.fqn: spec for spec in self.byte_specs}
        self._plan = SavePlan([])

    def set_up_planner(
        self, state_dict: ShardedStateDict, storage_meta: Any = None, is_coordinator: bool = False
    ) -> None:
    ...
    def create_local_plan(self) -> SavePlan:
        items = [self._write_item(spec, index) for index, spec in enumerate(self.write_specs)]
    ...
    def _byte_write_item(spec: _DcpMetadataByteSpec) -> WriteItem:
        return WriteItem(
            index=MetadataIndex(spec.fqn), type=WriteItemType.BYTE_IO, tensor_data=None
        )

    def _resolve_merged_chunk(self, spec: _DirectDcpWriteSpec) -> torch.Tensor:
        load_leaf = _metadata_same_layout_chunk_leaf(
            fqn=spec.sharded_key,
            dtype=spec.source_dtype,
            global_shape=spec.global_shape,
            global_offsets=spec.global_offsets,
        ...
        for checkpoint_dir, weight in zip(self.resolved_input_dirs, self.weights):
            ...
            loaded_by_path = _load_tensor_path_group_fast(
                checkpoint_dir, path_leaves, self.load_strategies[checkpoint_dir]
            )
    ...
    def _resolve_extra_state(self, spec: _DirectDcpWriteSpec) -> torch.Tensor:
        ...
        if tensor is None:
            raise WeightedMergeError(
                ...
                f"'{_path_label(spec.path)}' loaded as {type(value).__name__}."
            )
    ...
    def _resolve_byte_object(self, write_item: WriteItem) -> io.BytesIO:
        ...
        loaded = _load_tensor_path_group_fast(
            ...
            self.load_strategies[self.extra_state_source_dir],
        )
        self.load_time += time.perf_counter() - load_start
...
def _print_rank_0(*args: Any, **kwargs: Any) -> None:
    if _is_rank_0():
        print(*args, **kwargs)
...
def _parse_schedule_style(style: str) -> tuple[str, str | None]:
...
def _get_valid_styles() -> list[str]:
...
def _schedule_to_merge_coefficients(
...
def _apply_modifier(
    coefficients: list[float], modifier: str | None, seed: int | None = 0
) -> list[float]:
...
def checkpoint_coefficients(
    checkpoints: list[int], schedule: str, seed: int | None = 0
) -> dict[int, float]:
    """Return ``iteration -> coefficient`` for the checkpoints in input order."""

    base_schedule, modifier = _parse_schedule_style(schedule)
    ...
    coefficients = _schedule_to_merge_coefficients(SCHEDULES[base_schedule], len(checkpoints))
    coefficients = _apply_modifier(coefficients, modifier, seed)
...
def _validate_weights(weights: Iterable[float]) -> list[float]:
    weights = [float(weight) for weight in weights]
    if any(not math.isfinite(weight) for weight in weights):
        raise WeightedMergeError(f"Weights must be finite, got {weights}.")
    return weights
...
def _warn_manual_weight_policy(weights: Iterable[float], *, normalize: bool) -> None:
    for message in _manual_weight_warnings(weights, normalize=normalize):
        _print_rank_0(message, flush=True)


def _parse_weighted_inputs(specs: Iterable[str]) -> tuple[list[Path], list[float]]:
...
def _discover_checkpoint_iterations(checkpoint_root: str | Path) -> list[int]:
...
def _filter_checkpoints_by_interval(
    checkpoints: list[int], min_iteration_interval: int | None
) -> list[int]:
...
def _select_checkpoints_in_window(
    checkpoint_root: str | Path,
    *,
    start_iteration: int | None,
    end_iteration: int,
    min_iteration_interval: int | None = None,
) -> list[int]:
    """Select sorted checkpoint iterations for range merging."""
    ...
    available = _discover_checkpoint_iterations(checkpoint_root)
    ...
    selected = _filter_checkpoints_by_interval(selected, min_iteration_interval)
...
def _checkpoint_paths_for_iterations(
    checkpoint_root: str | Path, iterations: Iterable[int]
) -> list[Path]:
    root = Path(checkpoint_root)
    return [root / _iteration_dir_name(iteration) for iteration in iterations]


def _validate_min_checkpoints(num_checkpoints: int, min_checkpoints: int | None) -> None:
...
def _read_latest_checkpointed_iteration(path: Path) -> int | str:
...
def _resolve_checkpoint_dir(path: str | Path) -> Path:
    ...
    if (checkpoint / LATEST_CHECKPOINTED_ITERATION).exists():
        ...
        if latest == "release":
            ...
            resolved = checkpoint / _iteration_dir_name(latest)
...
def _output_checkpoint_dir(output_root: str | Path, output_iteration: int | None) -> Path:
    ...
    expected_name = _iteration_dir_name(output_iteration)
...
def _write_latest_checkpointed_iteration(checkpoint_dir: str | Path, iteration: int) -> None:
    ...
    temporary_tracker = None
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL
    if hasattr(os, "O_NOFOLLOW"):
        flags |= os.O_NOFOLLOW
    data = f"{iteration}\n".encode("utf-8")
    last_exists_error = None
    for _attempt in range(16):
        candidate = tracker.with_name(f".{tracker.name}.tmp-{uuid.uuid4().hex}")
        fd: int | None = None
        try:
            ...
            try:
                ...
                _print_rank_0(
                    f"WARNING: Could not fsync latest checkpoint marker temporary file "
                    f"{candidate}: {exc}",
                    flush=True,
                )
            os.close(fd)
            fd = None
            temporary_tracker = candidate
            break
        except FileExistsError as exc:
            last_exists_error = exc
            continue
        except Exception:
            if fd is not None:
                os.close(fd)
            try:
                ...
                pass
            raise
    if temporary_tracker is None:
        raise WeightedMergeError(
            f"Could not create a unique temporary {LATEST_CHECKPOINTED_ITERATION} marker "
            f"under {tracker.parent}."
        ) from last_exists_error
    try:
        os.replace(temporary_tracker, tracker)
    except Exception:
        try:
            temporary_tracker.unlink()
        except FileNotFoundError:
            pass
        raise
...
def _directory_size_for_accounting(path: str | Path, byte_accounting: str) -> int:
    if byte_accounting == "none":
        return 0
    if byte_accounting == "rank0" and not _is_rank_0():
        return 0
...
def _distributed_memory_peaks(host_peak_bytes: int) -> tuple[int, int, int, int]:
    ...
    local_record = {"rank": rank, "host_peak_bytes": int(host_peak_bytes)}
    if world_size == 1:
        return rank, world_size, rank, int(host_peak_bytes)

    records: list[dict[str, int] | None] = [None for _ in range(world_size)]
    dist.all_gather_object(records, local_record)
    valid_records = [record for record in records if record is not None]
    host_record = max(
        valid_records, key=lambda record: (int(record["host_peak_bytes"]), -int(record["rank"]))
    )
    return (rank, world_size, int(host_record["rank"]), int(host_record["host_peak_bytes"]))


def _distributed_max_seconds(seconds: float) -> float:
...
def _single_path_state_dict(path: tuple[str | int, ...], leaf: Any) -> ShardedStateDict:
    if not path:
...
def _add_merge_provenance(
    ...
    *,
    ...
    common_state_source_path: Path | None = None,
    extra_state_source_path: Path | None = None,
) -> None:
    common_state["weighted_merge_provenance"] = {
        ...
        "extra_state_source_path": str(
            extra_state_source_path
            if extra_state_source_path is not None
            else input_dirs[extra_state_source_index]
        ),
        ...
        "common_state_source_path": (
            str(common_state_source_path.resolve())
            if common_state_source_path is not None
            else None
        ),
        "strict": strict.value,
...
def _prepare_temporary_output_dir(output_dir: Path) -> Path:
    if output_dir.exists():
        raise WeightedMergeError(
            f"Output directory already exists: {output_dir}. "
            "Remove the existing checkpoint explicitly or write to a new output path/iteration."
        )
...
def _best_effort_fsync_path(path: Path, description: str) -> None:
    ...
    try:
        ...
        _print_rank_0(f"WARNING: Could not open {description} {path} for fsync: {exc}", flush=True)
        return
    try:
        os.fsync(fd)
    except OSError as exc:
        _print_rank_0(f"WARNING: Could not fsync {description} {path}: {exc}", flush=True)
    finally:
        os.close(fd)
...
def _publish_temporary_output_dir(temporary_dir: Path, output_dir: Path) -> None:
    def publish() -> None:
        _require_publishable_checkpoint_dir(temporary_dir)
        _best_effort_fsync_checkpoint_metadata(temporary_dir)
        if output_dir.exists():
            raise WeightedMergeError(
                f"Output directory already exists at publish time: {output_dir}. "
                "Remove it explicitly or write to a new output path/iteration."
            )
...
def _prepare_common_state(
    common_state: dict[str, Any], output_iteration: int | None
) -> dict[str, Any]:
...
def _load_output_common_state(
    resolved_input_dirs: list[Path],
    output_iteration: int | None,
    common_state_checkpoint: str | Path | None = None,
) -> tuple[dict[str, Any], Path]:
    if common_state_checkpoint is not None:
        checkpoint_dir = _resolve_checkpoint_dir(common_state_checkpoint)
        common_state = dist_checkpointing.load_common_state_dict(str(checkpoint_dir))
        common_iteration = _common_state_iteration(common_state)
        if output_iteration is not None and common_iteration not in (None, output_iteration):
            raise WeightedMergeError(
                f"Explicit common-state checkpoint iteration {common_iteration} does not "
                f"match output iteration {output_iteration}."
            )
        return common_state, checkpoint_dir
    ...
    if output_iteration is None:
        return common_states[0], resolved_input_dirs[0]

    input_iterations = [_common_state_iteration(common_state) for common_state in common_states]
    for common_state, input_iteration, checkpoint_dir in zip(
        common_states, input_iterations, resolved_input_dirs
    ):
...
def _metadata_same_layout_is_model_key(
    fqn: str, model_key_prefixes: tuple[str, ...], *, include_default_model_roots: bool = True
) -> bool:
...
def _metadata_same_layout_tensor_dtype(
    ...
    if not isinstance(dtype, torch.dtype):
        raise WeightedMergeError(f"DCP metadata for '{fqn}' does not expose a torch dtype.")
    return dtype
    ...
    *,
    ...
    for fqn, metadata_entry in metadata.state_dict_metadata.items():
        ...
        is_model_key = _metadata_same_layout_is_model_key(
            fqn, model_key_prefixes, include_default_model_roots=include_default_model_roots
        )
    ...
    return _DcpMetadataSnapshot(
        tensor_metadata=tensor_metadata, byte_extra_state_keys=tuple(sorted(byte_extra_state_keys))
    )
...
def _validate_metadata_same_layout(
    *,
    ...
    first_layouts = {
        fqn: _metadata_same_layout_tensor_layout(fqn, tensor_metadata_by_checkpoint[0][fqn])
    ...
    for fqn in first_keys:
        ...
        for checkpoint_dir, tensor_metadata in zip(
            ...
            if actual.dtype != expected.dtype:
                raise WeightedMergeError(
                    f"Dtype mismatch for '{fqn}' in metadata-same-layout: "
                    f"expected {expected.dtype}, got {actual.dtype} in {checkpoint_dir}."
                )

    for fqn in first_keys:
        if _metadata_same_layout_is_extra_state_key(fqn):
            continue
        for checkpoint_dir, tensor_metadata in zip(
...
def _estimate_merge_work_bytes(
    shape: Iterable[int], *, source_dtype: torch.dtype, target_dtype: torch.dtype, input_count: int
) -> int:
...
def _rank_balanced_assignments(
    candidates: list[tuple[int, int, _DirectDcpWriteSpec]], *, world_size: int
    ...
    rank_heap = [(0, 0, rank) for rank in range(world_size)]
    ...
    for cost_bytes, _stable_index, spec in sorted(
        ...
        _current_bytes, _current_chunks, target_rank = heapq.heappop(rank_heap)
        assigned_specs[target_rank].append(spec)
        rank_bytes[target_rank] += int(cost_bytes)
        rank_chunks[target_rank] += 1
        heapq.heappush(rank_heap, (rank_bytes[target_rank], rank_chunks[target_rank], target_rank))
...
def _build_metadata_same_layout_write_specs(
    *,
    ...
    for fqn in selected_keys:
        ...
        is_extra_state = _metadata_same_layout_is_extra_state_key(fqn)
        if is_extra_state:
            extra_state_keys += 1
        ...
        for global_offsets, chunk_shape in layout.chunks:
            spec = _DirectDcpWriteSpec(
                path=path,
                sharded_key=fqn,
            ...
            if balance_rank_work:
                ...
                assigned_rank = stable_index % world_size
    ...
    if balance_rank_work:
        assigned_specs, assigned_bytes, assigned_chunks = _rank_balanced_assignments(
            candidate_specs, world_size=world_size
        )
    ...
    for byte_index, fqn in enumerate(byte_extra_state_keys):
        ...
        byte_specs.append(_DcpMetadataByteSpec(fqn=fqn, path=(fqn,), template_leaf=template_leaf))
...
def merge_same_layout_dcp_metadata_checkpoints(
    ...
    *,
    ...
    weights = _normalize_weights(weights) if normalize else _validate_weights(weights)
    output_dir = _output_checkpoint_dir(output_root, output_iteration)
    ...
    tensor_metadata_by_checkpoint = [snapshot.tensor_metadata for snapshot in metadata_snapshots]
    ...
    base_common_state, common_state_source_path = _load_output_common_state(
        resolved_input_dirs, output_iteration, common_state_checkpoint
    )
    extra_state_source_path = (
        common_state_source_path
        if common_state_checkpoint is not None
    ...
    _add_merge_provenance(
        ...
        include_default_model_roots=include_default_model_roots,
        ignore_non_model_state=ignore_non_model_state,
        common_state_source_path=common_state_source_path,
        extra_state_source_path=extra_state_source_path,
    )
    ...
    load_strategies = {
        ...
        for checkpoint_dir in {*resolved_input_dirs, extra_state_source_path}
    }
    planner = _WeightedMergeDirectOutputSavePlanner(
        ...
        extra_state_source_index=extra_state_source_index,
        extra_state_source_dir=extra_state_source_path,
    )
    ...
    byte_accounting_start = time.perf_counter()
    bytes_read = sum(
    ...
    try:
        torch_dcp.save(
            {}, storage_writer=writer, planner=planner, no_dist=_direct_dcp_save_uses_no_dist()
        )
    except (Exception, CheckpointException) as exc:
        rank_suffix = (
            f" on rank {dist.get_rank()}" if dist.is_available() and dist.is_initialized() else ""
        )
        raise WeightedMergeError(
            f"Metadata same-layout DCP save failed{rank_suffix}: " f"{type(exc).__name__}: {exc}"
        ) from exc
    ...
    _publish_temporary_output_dir(temporary_output_dir, output_dir)
    if dist.is_initialized():
        dist.barrier()
    if output_iteration is not None and write_latest:
        _run_rank0_filesystem_op(
            f"Writing {LATEST_CHECKPOINTED_ITERATION}",
            lambda: _write_latest_checkpointed_iteration(output_dir, output_iteration),
        )
    if dist.is_initialized():
        dist.barrier()
    ...
    timings = MergeTimings(
        discovery=discovery_time,
        byte_accounting=_distributed_max_seconds(byte_accounting_time),
        load=_distributed_max_seconds(planner.load_time),
        accumulation=_distributed_max_seconds(planner.accumulation_time),
        save=_distributed_max_seconds(save_time),
        total=_distributed_max_seconds(time.perf_counter() - total_start),
    )
    return MergeResult(
        output_dir=output_dir,
...
def _add_merge_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    group.add_argument(
        ...
        required=True,
        help=(
            "Manual mode: PATH:WEIGHT entries. Range mode: one checkpoint root "
    ...
    group.add_argument(
        "--merge-style",
        choices=_get_valid_styles(),
        default="linear",
        help="Coefficient schedule for range mode.",
    )
    group.add_argument(
        "--coefficient-seed",
        type=int,
    ...
    group.add_argument(
        ...
        help=(
            ...
            "and requires matching input dtypes."
        ),
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        help=(
            ...
            "latest_checkpointed_iteration.txt. Defaults to --end-checkpoint in range mode."
        ),
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Optional checkpoint directory that supplies common.pt and model _extra_state. "
            "Use this when merge inputs intentionally contain only distributed model shards."
        ),
    )
    group.add_argument(
        "--merge-model-prefix",
...
) -> tuple[list[Path], list[float], int | None, str | None]:
    use_selection_mode = args.start_checkpoint is not None
    output_iteration = args.output_iteration

    if use_selection_mode:
        if len(args.merge_inputs) != 1:
            raise WeightedMergeError("Range mode expects exactly one --merge-inputs root.")
        if args.end_checkpoint is None:
            raise WeightedMergeError("--end-checkpoint is required for range mode.")

        checkpoint_root = Path(args.merge_inputs[0])
        selected_iterations = _select_checkpoints_in_window(
            checkpoint_root,
        ...
        input_paths = _checkpoint_paths_for_iterations(checkpoint_root, selected_iterations)
        ...
        input_paths, weights = _parse_weighted_inputs(args.merge_inputs)
        merge_style = None
        _warn_manual_weight_policy(weights, normalize=args.normalize)

    _validate_min_checkpoints(len(input_paths), args.min_checkpoints)
    return input_paths, weights, output_iteration, merge_style


def _parse_metadata_same_layout_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Merge same-layout torch_dist checkpoints using public DCP "
            "metadata without Megatron model/template initialization."
        )
    )
    _add_merge_args(parser)
    parser.add_argument(
        "--ckpt-format", choices=("torch_dist",), default="torch_dist", help=argparse.SUPPRESS
    )
...
def _run_metadata_same_layout_cli(args: argparse.Namespace) -> MergeResult:
    ...
    return merge_same_layout_dcp_metadata_checkpoints(
        ...
        common_state_checkpoint=args.common_state_checkpoint,
...
def _print_merge_result(result: MergeResult) -> None:
    ...
    _print_rank_0(
        ...
        f"byte_accounting={result.timings.byte_accounting:.2f}s, "
    ...
    print(
        f"Memory rank={result.rank}: host_peak={_format_bytes(result.host_peak_bytes)}", flush=True
    )
    _print_rank_0(
