...
// Specialization for AccessSize=2
template <>
__device__ __forceinline__ void store_global<2>(void* dst, const void* src)
{
    const uint16_t* data = reinterpret_cast<const uint16_t*>(src);
#ifdef PTX_AVAILABLE
    asm volatile("st.global.wb.u16 [%0], %1;\n"
                 :
                 : "l"(dst), "h"(*data)); // Using "h" constraint for 16-bit values
#else
    uint16_t* dst_cast = reinterpret_cast<uint16_t*>(dst);
    *dst_cast = *data;
#endif
}
